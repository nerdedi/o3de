...
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.read_android_settings",
    return_value={},
)
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.resolve_adb_tool",
    return_value=pathlib.Path("Foo"),
)
def test_Initialize(mock_resolve_adb_tool, mock_read_android_settings):
    ...
    inst = android_deployment.AndroidDeployment(
...
def test_read_android_settings(tmpdir):
    ...
    game_project_json_file = tmpdir.join(
        f"dev_root/{game_name}/Platform/Android/android_project.json"
    )
    game_project_json_file.write(
        f'{{"android_settings": {{"package_name": "{package_name}" }} }}'
    )

    result = android_deployment.AndroidDeployment.read_android_settings(
        pathlib.Path(tmpdir.join("dev_root").realpath()), game_name
    )
...
def test_resolve_adb_tool(tmpdir):
    ...
    result = android_deployment.AndroidDeployment.resolve_adb_tool(
        pathlib.Path(tmpdir.join(sdk_path).realpath())
    )
...
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.adb_call",
    return_value="PASS",
)
...
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.adb_shell",
    return_value="output",
)
...
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.adb_ls",
...
def test_check_known_android_paths_fail(mock_adb_ls):
    with (
        ...
        assert not result
        assert mock_adb_ls.call_count == len(
            android_deployment.KNOWN_ANDROID_EXTERNAL_STORAGE_PATHS
        )
...
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.check_known_android_paths",
    return_value="PATH",
)
def test_detect_device_storage_path_no_external_storage_env(
    mock_check_known_android_paths, mock_adb_shell
):
    with (
        patch.object(
            android_deployment.AndroidDeployment,
...
def test_detect_device_storage_path_invalid_external_storage_env(
    mock_check_known_android_paths, mock_adb_shell
):
    with (
        patch.object(
...
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.adb_shell",
    return_value="EXTERNAL_STORAGE=/foo/bar",
)
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.adb_ls",
    return_value=(True, "foo.bar"),
)
def test_detect_device_storage_path_valid_external_storage_env(
...
def test_detect_device_storage_path_real_path():
    ...
    with (
        ...
        patch.object(
            android_deployment.AndroidDeployment, "adb_shell", wraps=_mock_adb_shell
        ),
        patch.object(
            android_deployment.AndroidDeployment, "adb_ls", wraps=_mock_adb_ls
        ),
        patch.object(pathlib.Path, "glob", return_value=["foo.bar"]),
    ):
        inst = android_deployment.AndroidDeployment(
...
@patch(
    "cmake.Tools.Platform.Android.android_deployment.AndroidDeployment.adb_shell",
    return_value="2020-04-30 09:20:00.0000",
)
def test_get_device_file_timestamp_success(mock_adb_shell):
    with (
        ...
        result = inst.get_device_file_timestamp(
            remote_file_path=remote_path, device_id=TEST_DEVICE_ID
        )

        assert result == time.mktime(
            time.strptime("2020-04-30 09:20:00.0000", "%Y-%m-%d %H:%M:%S.%f")
        )

        mock_adb_shell.assert_called_once_with(
            command=f"cat {remote_path}", device_id=TEST_DEVICE_ID
        )
...
def test_update_device_file_timestamp(tmpdir):
    tmpdir.join(
        f"{TEST_DEV_ROOT}/{TEST_BUILD_DIR}/app/src/assets/deploy.timestamp"
    ).ensure()
    ...
    with (
        ...
        patch.object(
            android_deployment.AndroidDeployment, "adb_call", return_value=""
        ) as mock_adb_call,
    ):
        inst = android_deployment.AndroidDeployment(
            dev_root=mock_dev_root,
            build_dir=TEST_BUILD_DIR,
            configuration="profile",
            game_name=TEST_GAME_NAME,
            asset_mode=TEST_ASSET_MODE,
        ...
        remote_path = "/foo/bar/timestamp.txt"
        inst.update_device_file_timestamp(
            relative_assets_path=remote_path, device_id=TEST_DEVICE_ID
        )
...
def test_execute_success(
    tmpdir, test_config, test_package_name, test_device_storage_path
):
    mock_dev_root = tmpdir.join(TEST_DEV_ROOT).realpath()

    tmpdir.join(
        f"{TEST_DEV_ROOT}/{TEST_BUILD_DIR}/app/build/outputs/apk/{test_config}/app-{test_config}.apk"
    ).ensure()
    expected_apk_path = str(
        tmpdir.join(
            f"{TEST_DEV_ROOT}/{TEST_BUILD_DIR}/app/build/outputs/apk/{test_config}/app-{test_config}.apk"
        ).realpath()
    )
    ...
    expected_asset_path = str(
        tmpdir.join(f"{TEST_DEV_ROOT}/{TEST_BUILD_DIR}/app/src/assets").realpath()
    )

    tmpdir.join(
        f"{TEST_DEV_ROOT}/{TEST_BUILD_DIR}/app/src/main/assets/Registry/dummy.txt"
    ).ensure()
    expected_registry_path = str(
        tmpdir.join(
            f"{TEST_DEV_ROOT}/{TEST_BUILD_DIR}/app/src/main/assets/Registry"
        ).realpath()
    )

    expected_storage_registry_path = (
        f"{test_device_storage_path}/Android/data/{test_package_name}/files/Registry"
    )
        ...
        if arg_list == "kill-server":
            ...
            if match_arg_list(arg_list, ["install", "-r", expected_apk_path]):
                ...
                [
                    ...
                    f"{test_device_storage_path}/Android/data/{test_package_name}/files",
                ],
            ):
                return "SUCCESS"
            elif match_arg_list(
                arg_list,
                ["push", expected_registry_path, expected_storage_registry_path],
            ):
                return "SUCCESS"
            elif match_arg_list(
                arg_list,
                ["shell", "cmd", "package", "list", "packages", test_package_name],
            ):
                return "SUCCESS"
            elif match_arg_list(
                arg_list,
                [
                    "shell",
                    f"mkdir {test_device_storage_path}/Android/data/{test_package_name}",
                ],
            ):
                return "SUCCESS"
            elif match_arg_list(
                ...
                [
                    "shell",
                    f"mkdir {test_device_storage_path}/Android/data/{test_package_name}/files",
                ],
            ):
                return "SUCCESS"
            elif match_arg_list(
    ...
    with (
        patch.object(
            android_deployment.AndroidDeployment,
            "get_target_android_devices",
            return_value=[TEST_DEVICE_ID],
        ),
        patch.object(
            android_deployment.AndroidDeployment,
            "detect_device_storage_path",
            return_value=test_device_storage_path,
        ),
        patch.object(
            android_deployment.AndroidDeployment,
            "get_device_file_timestamp",
            return_value=None,
        ),
        patch.object(
            android_deployment.AndroidDeployment, "update_device_file_timestamp"
        ) as mock_update_device_file_timestamp,
        patch.object(
            android_deployment.AndroidDeployment,
            "read_android_settings",
            return_value={"package_name": test_package_name},
        ),
        patch.object(
            android_deployment.AndroidDeployment,
            "resolve_adb_tool",
        ...
        patch.object(
            android_deployment.AndroidDeployment, "adb_call", wraps=_mock_adb_call
        ) as mock_adb_call,
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "game1", "profile", "org.o3de.foo", "/data/fool_storage", "android"
        ),
        pytest.param("game1", "debug", "org.o3de.foo", "/data/fool_storage", "android"),
        pytest.param(
            "game2", "profile", "org.o3de.bar", "/data/fool_storage", "android"
        ),
...
def test_execute_clean_deploy_success(
    tmpdir,
    test_game_name,
    test_config,
    test_package_name,
    test_device_storage_path,
    test_asset_type,
):
    mock_dev_root = tmpdir.join(TEST_DEV_ROOT).realpath()
    ...
    tmpdir.join(
        f"{TEST_DEV_ROOT}/{TEST_BUILD_DIR}/app/src/assets/Registry/dummy.txt"
    ).ensure()
    expected_asset_path = str(
        tmpdir.join(f"{TEST_DEV_ROOT}/{TEST_BUILD_DIR}/app/src/assets").realpath()
    )
    ...
    def _mock_adb_call(arg_list, device_id=None):
        ...
        if arg_list == "kill-server":
            ...
            if match_arg_list(arg_list, ["install", "-r", expected_apk_path]):
                ...
                arg_list, ["push", f"{expected_asset_path}/.", expected_storage_path]
            ):
                return "SUCCESS"
            elif match_arg_list(
                arg_list,
    ...
    with (
        patch.object(
            ...
            "get_target_android_devices",
            return_value=[TEST_DEVICE_ID],
        ) as mock_get_target_android_devices,
        patch.object(
            android_deployment.AndroidDeployment,
            "detect_device_storage_path",
            return_value=test_device_storage_path,
        ) as mock_detect_device_storage_path,
        patch.object(
            android_deployment.AndroidDeployment,
            "get_device_file_timestamp",
            return_value=None,
        ) as mock_get_device_file_timestamp,
        patch.object(
            android_deployment.AndroidDeployment, "update_device_file_timestamp"
        ) as mock_update_device_file_timestamp,
        patch.object(
...
def test_execute_incremental_deploy_success(
    tmpdir, test_config, test_package_name, test_device_storage_path
):
    mock_dev_root = tmpdir.join(TEST_DEV_ROOT).realpath()
    ...
    with (
        ...
        patch.object(
            android_deployment.AndroidDeployment,
            "should_copy_file",
            wraps=_mock_should_copy_file,
        ),
        patch.object(
            pathlib.Path,
            "glob",
            return_value=[pathlib.Path("foo.bar"), pathlib.Path("no.bar")],
        ),
    ):
        inst = android_deployment.AndroidDeployment(
            dev_root=mock_dev_root,
