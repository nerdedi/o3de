...
def verify_gradle(override_gradle_path=None):
    ...
    """
    return common.verify_tool(
        override_tool_path=override_gradle_path,
        tool_name="gradle",
...
def verify_cmake(override_cmake_path=None):
    ...
    """
    return common.verify_tool(
        override_tool_path=override_cmake_path,
        tool_name="cmake",
...
def verify_ninja(override_ninja_path=None):
    ...
    """
    return common.verify_tool(
        override_tool_path=override_ninja_path,
        tool_name="ninja",
        tool_filename="ninja",
...
def build_optional_signing_profile(store_file, store_password, key_alias, key_password):
    ...
    if not any([store_file, store_password, key_alias, key_password]):
        return
    return android_support.AndroidSigningConfig(
...
MIN_ANDROID_SDK_PLATFORM = (
    28  # The minimum platform/api level that is supported for the SDK Platform
)
...
DEFAULT_3RD_PARTY_PATH = pathlib.Path(
    manifest_json.get(
...
def main(args):
    ...
    parser.add_argument(
        ANDROID_SDK_ARGUMENT_NAME, help="The path to the android SDK", required=True
    )
    ...
    parser.add_argument(
        ANDROID_SDK_PLATFORM_ARGUMENT_NAME,
    ...
    # Override arguments
    parser.add_argument(
        ANDROID_SDK_COMMAND_LINE_TOOLS_VER,
        default="latest",
    ...
    parser.add_argument(
        ANDROID_SDK_PREFERRED_TOOL_VER,
        help="The android SDK build tools version.",
        required=False,
    )

    parser.add_argument(
        ANDROID_NDK_PLATFORM_ARGUMENT_NAME,
        help="The android NDK version",
        required=False,
    )

    parser.add_argument(
        GRADLE_ARGUMENT_NAME,
    ...
    parser.add_argument(
        ANDROID_GRADLE_PLUGIN_ARGUMENT_NAME,
    ...
    # Asset Options
    parser.add_argument(
        INCLUDE_APK_ASSETS_ARGUMENT_NAME,
        action="store_true",
    ...
    parser.add_argument(
        "--enable-unity-build", action="store_true", help="Enable unity build"
    )
    ...
    gradle_version, override_gradle_path = verify_gradle(
        override_gradle_path=parsed_args.get_argument(GRADLE_ARGUMENT_NAME)
    )
    logging.info("Detected Gradle version %s", str(gradle_version))

    # Verify the cmake requirements
    cmake_version, override_cmake_path = verify_cmake(
        override_cmake_path=parsed_args.get_argument(CMAKE_ARGUMENT_NAME)
    )
    logging.info("Detected CMake version %s", str(cmake_version))

    # Verify the ninja requirements
    ninja_version, override_ninja_path = verify_ninja(
        override_ninja_path=parsed_args.get_argument(NINJA_ARGUMENT_NAME)
    )
    ...
    android_sdk_platform_version = parsed_args.get_argument(
        ANDROID_SDK_PLATFORM_ARGUMENT_NAME
    )

    # Get the gradle plugin details and validate against the current environment
    android_gradle_plugin_version = parsed_args.get_argument(
        ANDROID_GRADLE_PLUGIN_ARGUMENT_NAME
    )
    android_gradle_plugin = android_support.AndroidGradlePluginInfo(
        android_gradle_plugin_version
    )
    logging.info(
        f"Generating Android Gradle Plugin version {android_gradle_plugin_version} based project"
    )

    if gradle_version < android_gradle_plugin.min_gradle_version:
        raise common.LmbrCmdError(
            f"The current version of gradle ({gradle_version}) does not satisfy the minimum version "
    ...
    if cmake_version < android_gradle_plugin.min_cmake_version:
        raise common.LmbrCmdError(
            f"The current version of cmake ({cmake_version}) does not satisfy the minimum version "
    ...
    if (
        android_gradle_plugin.max_cmake_version
        and cmake_version > android_gradle_plugin.max_cmake_version
    ):
        raise common.LmbrCmdError(
            f"The current version of cmake ({cmake_version}) exceeds the maximum version "
    ...
    android_sdk = android_support.AndroidSDKResolver(
        android_sdk_path=parsed_args.get_argument(ANDROID_SDK_ARGUMENT_NAME),
        command_line_tools_version=parsed_args.get_argument(
            ANDROID_SDK_COMMAND_LINE_TOOLS_VER
        ),
    )
    ...
    if android_sdk_platform_version < 0:
        ...
        installed_android_sdk_platforms = android_sdk.is_package_installed(
            "platforms;*"
        )
        if installed_android_sdk_platforms:
            ...
            for installed_android_sdk_platform in installed_android_sdk_platforms:
                platform_number_match = re.match(
                    r"platforms;android-([0-9]*)", installed_android_sdk_platform.path
                )
        ...
        if android_sdk_platform_version < MIN_ANDROID_SDK_PLATFORM:
            raise common.LmbrCmdError(
                f"Invalid argument for {ANDROID_SDK_PLATFORM_ARGUMENT_NAME} ({android_sdk_platform_version}). Must be greater than the minimum value supported {MIN_ANDROID_SDK_PLATFORM}."
            )
    ...
    if android_native_api_level < 0:
        ...
        if android_native_api_level < MIN_NATIVE_API_LEVEL:
            raise common.LmbrCmdError(
                f"Invalid argument for {ANDROID_NATIVE_API_LEVEL} ({android_native_api_level}). Must be greater than the minimum value supported {MIN_NATIVE_API_LEVEL}."
            )
    ...
    build_tools_version = (
        parsed_args.get_argument(ANDROID_SDK_PREFERRED_TOOL_VER)
        or android_gradle_plugin.default_sdk_build_tools_version
    )
    build_tools_package_name = f"build-tools;{build_tools_version}"
    build_tools_package = android_sdk.install_package(
    ...
    android_ndk_version = (
        parsed_args.get_argument(ANDROID_NDK_PLATFORM_ARGUMENT_NAME)
        or android_gradle_plugin.default_ndk_version
    )
    android_ndk_package_name = f"ndk;{android_ndk_version}"
    android_ndk_package = android_sdk.install_package(
        package_install_path=android_ndk_package_name, package_description="Android NDK"
    )

    # Verify the engine root path and project path
    verified_project_path, verified_engine_root = common.verify_project_and_engine_root(
        project_root=parsed_args.project_path, engine_root=parsed_args.engine_root
    )
    ...
    if not third_party_path.is_dir():
        raise common.LmbrCmdError(
            f"Invalid --third-party-path '{parsed_args.third_party_path}'.",
            common.ERROR_CODE_INVALID_PARAMETER,
        )

    build_dir = parsed_args.build_dir

    signing_config = build_optional_signing_profile(
        store_file=parsed_args.get_argument(SIGNING_PROFILE_STORE_FILE_ARGUMENT_NAME),
        store_password=parsed_args.get_argument(
            SIGNING_PROFILE_STORE_PASSWORD_ARGUMENT_NAME
        ),
        key_alias=parsed_args.get_argument(SIGNING_PROFILE_KEY_ALIAS_ARGUMENT_NAME),
        key_password=parsed_args.get_argument(
            SIGNING_PROFILE_KEY_PASSWORD_ARGUMENT_NAME
        ),
    )
    ...
    generator = android_support.AndroidProjectGenerator(
        engine_root=verified_engine_root,
        build_dir=build_dir,
