...
ROOT_DEV_PATH = os.path.realpath(
...
ANDROID_GRADLE_PLUGIN_COMPATIBILITY_MAP = {
    "4.2.2": {
        "min_gradle_version": "6.7.1",
        "sdk_build": "30.0.3",
        "default_ndk": "21.4.7075529",
        "min_cmake_version": "3.20",
    },
    "7.3.1": {
        "min_gradle_version": "7.5.1",
        "sdk_build": "33.0.0",
        ...
        try:
            if is_test:
                ...
                project_properties_path = (
                    ...
                    / "Platform"
                    / "Android"
                    / "android_project.json"
                )
                assert project_properties_path.is_file(), (
                    f"Missing required android settings file {project_properties_path.resolve()}"
                )
                project_properties_content = project_properties_path.read_text(
                ...
                assert project_properties_path.is_file(), (
                    f"Missing required project settings file {project_properties_path.resolve()}"
                )
                project_properties_content = project_properties_path.read_text(
                    encoding=common.DEFAULT_TEXT_READ_ENCODING,
                    errors=common.ENCODING_ERROR_HANDLINGS,
                ...
                android_project_properties_path = (
                    project_path / "Platform" / "Android" / "android_project.json"
                )
                if android_project_properties_path.is_file():
                    android_project_properties_content = (
                        android_project_properties_path.read_text(
                            encoding=common.DEFAULT_TEXT_READ_ENCODING,
                            errors=common.ENCODING_ERROR_HANDLINGS,
                        )
                    )
                    android_project_json = json.loads(
                        android_project_properties_content
                    )
            ...
            project_activity = (
                f"{TEST_RUNNER_PROJECT}Activity"
                if is_test
                else f"{project_name}Activity"
            )
            ...
            self.internal_dict = {
                ...
                "ANDROID_APP_NAME": TEST_RUNNER_PROJECT
                if is_test
                else product_name,  # external facing name
                "ANDROID_PROJECT_NAME": TEST_RUNNER_PROJECT
                if is_test
                else project_name,  # internal facing name
                "ANDROID_PROJECT_ACTIVITY": project_activity,
                "ANDROID_LAUNCHER_NAME": TEST_RUNNER_PROJECT
                if is_test
                else ANDROID_LAUNCHER_NAME_PATTERN.format(project_name=project_name),
                "ANDROID_CONFIG_CHANGES": multi_window_options[
                    "ANDROID_CONFIG_CHANGES"
                ],
                "ANDROID_APP_PUBLIC_KEY": android_settings.get(
                    "app_public_key", "NoKey"
                ),
            ...
            raise common.LmbrCmdError(
                f"Missing key from android project settings for project at {project_path}:'{e}' "
            )
...
class AndroidSigningConfig(object):
    ...
    def __init__(self, store_file, store_password, key_alias, key_password):
        if not store_file:
            raise common.LmbrCmdError(
                "Keystore file not supplied for signing configuration",
                common.ERROR_CODE_INVALID_PARAMETER,
            )
        if not os.path.isfile(store_file):
            raise common.LmbrCmdError(
                f"Missing/Invalid keystore file {store_file} for signing config",
                common.ERROR_CODE_INVALID_PARAMETER,
            )
        ...
        if not store_password:
            raise common.LmbrCmdError(
                "Keystore password not supplied for signing configuration",
                common.ERROR_CODE_INVALID_PARAMETER,
            )
        ...
        if not key_alias:
            raise common.LmbrCmdError(
                "Signing key alias not supplied for signing configuration",
                common.ERROR_CODE_INVALID_PARAMETER,
            )
        ...
        if not key_password:
            raise common.LmbrCmdError(
                "Signing key password not supplied for signing configuration",
                common.ERROR_CODE_INVALID_PARAMETER,
            )
...
class AndroidProjectGenerator(object):
    ...
    def __init__(
        ...
        self.android_project_builder_path = (
            self.engine_root / "Code/Tools/Android/ProjectBuilder"
        )
    ...
    def create_file_from_project_template(
        ...
        if not dst_file.exists() or self.overwrite_existing:
            default_local_properties_content = common.load_template_file(
    ...
    def prepare_gradle_wrapper(self):
        ...
        if proc_result.returncode != 0:
            raise common.LmbrCmdError(
                "Gradle was unable to generate a gradle wrapper for this project (code {}): {}".format(
                    proc_result.returncode, proc_result.stderr or ""
                ),
    ...
    def create_default_local_properties(self):
        ...
        """
        template_android_sdk_path = common.normalize_path_for_settings(
            self.android_sdk_path, True
        )
        if self.override_cmake_path:
            ...
            template_cmake_path = common.normalize_path_for_settings(
                pathlib.Path(self.override_cmake_path).parent.parent, True
            )
        else:
    ...
    def patch_and_transfer_android_libs(self):
        ...
        android_libraries_substitution_table = {
            "ANDROID_SDK_HOME": common.normalize_path_for_settings(
                self.android_sdk_path, False
            ),
            "ANDROID_SDK_VERSION": f"android-{self.android_sdk_platform}",
        }
        android_libraries_template_json_path = (
            self.android_project_builder_path / ANDROID_LIBRARIES_JSON_FILE
        )
        ...
        for libName, value in android_libraries_json.items():
            ...
            if not src_dir:
                raise common.LmbrCmdError(
                    "[ERROR] Failed to find library - {} - in path(s) [{}]. Please download the "
                    "library from the Android SDK Manager and run this command again.".format(
                        libName,
                        ", ".join(
                            string.Template(path).substitute(self.env)
                            for path in value["srcDir"]
                        ),
                    )
                )
            ...
            if "patches" in value:
                lib_to_patch = self._Library(
                    libName, src_dir, self.overwrite_existing, self.signing_config
                )
        ...
        for lib in libs_to_patch:
            lib.process_patch_lib(
                android_project_builder_path=self.android_project_builder_path,
    ...
    def create_lumberyard_app(self, project_dependencies):
        ...
        template_third_party_path = common.normalize_path_for_settings(
            self.third_party_path
        )
        template_ndk_path = common.normalize_path_for_settings(
            os.path.join(self.android_sdk_path, self.android_ndk.location)
        )
        template_unity_build = 1 if self.unity_build_enabled else 0

        native_build_path = (
            pathlib.Path(self.native_build_path).resolve().as_posix()
            if self.native_build_path
            else "."
        )
        ...
        absolute_cmakelist_path = (
            (engine_root_as_path / "CMakeLists.txt").resolve().as_posix()
        )
        absolute_azandroid_path = (
            (engine_root_as_path / "Code/Framework/AzAndroid/java").resolve().as_posix()
        )
        ...
        gradle_build_env["NATIVE_CMAKE_SECTION_ANDROID"] = (
            NATIVE_CMAKE_SECTION_ANDROID_FORMAT.format(
                cmake_version=str(self.cmake_version),
        ...
        for native_config in BUILD_CONFIGURATIONS:
            ...
            if self.vulkan_validation_path:
                cmake_argument_list.append(
                    f'"-DLY_ANDROID_VULKAN_VALIDATION_PATH={pathlib.PurePath(self.vulkan_validation_path).as_posix()}"'
                )

            if not self.is_test_project:
                cmake_argument_list.append(
                    f'"-DLY_PROJECTS={pathlib.PurePath(self.project_path).as_posix()}"'
                )
            else:
            ...
            if self.override_ninja_path:
                cmake_argument_list.append(
                    f'"-DCMAKE_MAKE_PROGRAM={common.normalize_path_for_settings(self.override_ninja_path)}"'
                )
            ...
            if self.extra_cmake_configure_args:
                cmake_argument_list.extend(
                    map(json.dumps, self.extra_cmake_configure_args)
                )
            ...
            gradle_build_env[f"NATIVE_CMAKE_SECTION_{native_config_upper}_CONFIG"] = (
                NATIVE_CMAKE_SECTION_BUILD_TYPE_CONFIG_FORMAT_STR.format(
                    ...
                    if project_name and not self.is_test_project
                    else 'targets "TEST_SUITE_main"',
                )
            )
            ...
            if project_name:
                ...
                ] = CUSTOM_GRADLE_COPY_NATIVE_CONFIG_FORMAT_STR.format(
            ...
            if self.is_test_project:
                ...
                ] = CUSTOM_GRADLE_COPY_NATIVE_CONFIG_BUILD_ARTIFACTS_FORMAT_STR.format(
                    config=native_config,
                    config_lower=native_config_lower,
                ...
                gradle_build_env[
                    ...
                    working_dir=common.normalize_path_for_settings(
                        self.engine_root / "cmake/Tools"
                    ),
                    python_full_path=common.normalize_path_for_settings(
                        self.engine_root / "python" / PYTHON_SCRIPT
                    ),
                    asset_type=self.asset_type,
                    project_path=self.project_path.as_posix(),
                    asset_mode=self.asset_mode if native_config != "Release" else "PAK",
                    asset_layout_folder=(self.build_dir / layout_folder)
                    .resolve()
                    .as_posix(),
                    config=native_config,
                )
                ...
                gradle_build_env[
                    f"CUSTOM_APPLY_ASSET_LAYOUT_{native_config_upper}_TASK"
                ] += CUSTOM_GRADLE_COPY_REGISTRY_FOLDER_FORMAT_STR.format(
                    config=native_config,
                    config_lower=native_config_lower,
                    asset_layout_folder=(self.build_dir / layout_folder)
                    .resolve()
                ...
                gradle_build_env[
                    f"CUSTOM_APPLY_ASSET_LAYOUT_{native_config_upper}_TASK"
                ] += CUSTOM_GRADLE_COPY_REGISTRY_FOLDER_DEPENDENCY_FORMAT_STR.format(
                    config=native_config
                )
...
"""
    ...
    def apply_android_builder_rules(self, az_android_dst_path, az_android_package_env):
        ...
        android_builder_json_path = (
            self.android_project_builder_path / "android_builder.json"
        )
        android_builder_json_content = common.load_template_file(
        ...
        def _copy(src_file, dst_path, dst_is_directory):
            ...
            if puremagic.what(resolved_src) in (
                ...
                project_activity_for_game_content = common.load_template_file(
    ...
    def construct_source_resource_path(self, source_path):
        ...
        raise common.LmbrCmdError(
            f'Unable to locate resources folder for project at path "{self.project_path}"'
        )
    ...
    def resolve_icon_overrides(self, az_android_dst_path, az_android_package_env):
        ...
        if default_icon is not None:
            ...
            logging.debug(
                f"No default icon override specified for project_at path {self.project_path}"
            )
        ...
        for resolution in ANDROID_RESOLUTION_SETTINGS:
            ...
            if icon_source is default_icon:
                ...
                # if only the resolution is unspecified, remove the resolution specific version from the project
                    ...
                    logging.debug(
                        f'Default icon being used for "{resolution}" in {self.project_path}',
                        resolution,
                    )
            ...
            for resolution in valid_resolutions:
                ...
                if splash_img_source is default_splash_img:
                    ...
                    if splash_img_source is None:
                        ...
                        # if only the resolution is unspecified, remove the resolution specific version from the project
                        logging.debug(
                            f'Default splash screen being used for "{orientation_key}-{resolution}" in {self.project_path}'
                        )
                        common.remove_dir_path(target_directory)
                    continue
    ...
    class _Library:
        ...
        def process_patch_lib(self, android_project_builder_path, dest_root):
            ...
            build_gradle_content = common.load_template_file(
...
def resolve_adb_tool(android_sdk_path):
    ...
    for executable_path_ext in common.PLATFORM_EXECUTABLE_EXTENSIONS:
        check_adb_target = (
            android_sdk_path / "platform-tools" / f"adb{executable_path_ext}"
        )
    ...
    if not file_found:
        raise common.LmbrCmdError(
            f"Invalid Android SDK path '{str(android_sdk_path)}': Unable to locate 'adb'."
        )
...
class AdbTool(common.CommandLineExec):
    ...
    def get_connected_device_serial_ids(self):
        ...
        device_result_lines = [
            device_result_line.strip()
            for device_result_line in devices_result.split("\n")
            if device_result_line
        ]
    ...
    def connect(self, device_filter=None):
        ...
        if device_filter:
            ...
            if not device_matched:
                raise common.LmbrCmdError(
                    f"Invalid device serial {device_filter}. The current connected device serial ids are : {','.join(connected_serial_ids)}"
                )
...
class AndroidGradlePluginInfo(object):
    def __init__(self, android_gradle_plugin_version):
        if (
            ...
            not in ANDROID_GRADLE_PLUGIN_COMPATIBILITY_MAP.keys()
        ):
            raise common.LmbrCmdError(
                f"Android Gradle Plugin version {android_gradle_plugin_version} is not supported. "
        ...
        self.max_cmake_version = (
            None
            if max_cmake_version_number is None
            else Version(max_cmake_version_number)
        )
...
class AndroidSDKResolver(object):
    ...
    def __init__(self, android_sdk_path, command_line_tools_version):
        ...
        if not self.android_sdk_path:
            raise common.LmbrCmdError(
                "Android SDK path not set or it was not passed into the command to generate the android project"
            )
        if not os.path.isdir(self.android_sdk_path):
            raise common.LmbrCmdError(
                f"Android SDK path {self.android_sdk_path} is not valid"
            )
        ...
        if tools_path.exists():
            ...
            if not tools_path.exists():
                raise common.LmbrCmdError(
                    f"The desired version of the Android 'cmdline-tools' ({command_line_tools_version}) is not detected"
                )
        else:
        ...
        if not self.sdk_manager_path.is_file():
            raise common.LmbrCmdError(
                f"Android SDK path {self.android_sdk_path} is not valid or complete. Missing {self.sdk_manager_path}"
            )
    ...
    def call_sdk_manager(self, arguments, action):
        result_code, result_stdout, result_stderr = self.sdk_manager.exec(
            arguments, capture_stdout=True, suppress_stderr=False
        )
        if result_code != 0:
            ...
            if "java.lang.UnsupportedClassVersionError" in result_stderr:
                err_msg = (
                    "The current installed java runtime is not compatible with the sdkmanager. Make sure that "
    ...
    def refresh_sdk_installation(self):
        ...
        def _factory_installed_package(package_map, item_components):
            package_map[item_components[0]] = AndroidSDKResolver.InstalledPackage(
                item_components
            )

        def _factory_available_package(package_map, item_components):
            package_map[item_components[0]] = AndroidSDKResolver.AvailablePackage(
                item_components
            )

        def _factory_available_update(package_map, item_components):
            package_map[item_components[0]] = AndroidSDKResolver.AvailableUpdate(
                item_components
            )
    ...
    def is_package_available(self, search_package_path):
        ...
        package_detail_result_list = []
        for (
            available_package_path,
            available_package_details,
        ) in self.available_packages.items():
    ...
    def install_package(self, package_install_path, package_description):
        ...
        self.call_sdk_manager(
            ["--install", available_package_to_install.path],
            f"installing package {available_package_to_install.path}",
        )
