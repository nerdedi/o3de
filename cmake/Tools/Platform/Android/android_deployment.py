...
class AndroidDeployment(object):
    ...
    def __init__(
        ...
        if self.embedded_assets:
            if self.deployment_type == AndroidDeployment.DEPLOY_ASSETS_ONLY:
                raise common.LmbrCmdError(
                    f"Deployment type {deployment_type} set but the assets are embedded in the APK. To deploy assets, build the APK again."
                )
        ...
        if not self.is_test_project:
            if asset_mode == "PAK":
                self.local_asset_path = (
                    self.dev_root / "Pak" / f"{game_name.lower()}_{asset_type}_paks"
                )
            else:
            ...
            assert game_name is not None, "'game_name' is required"
            self.game_name = game_name

            assert asset_mode is not None, "'asset_mode' is required"
            self.asset_mode = asset_mode

            assert asset_type is not None, "'asset_type' is required"
            self.asset_type = asset_type

            self.files_in_asset_path = list(self.local_asset_path.glob("**/*"))

            self.android_settings = AndroidDeployment.read_android_settings(
                self.dev_root, game_name
            )
        else:
            ...
            if asset_mode:
                logging.warning(
                    f"'asset_mode' argument '{asset_mode}' ignored for unit test deployment."
                )
            if asset_type:
                logging.warning(
                    f"'asset_type' argument '{asset_type}' ignored for unit test deployment."
                )
        ...
        self.apk_path = (
            ...
            / "apk"
            / configuration
            / f"app-{configuration}.apk"
        )

        self.android_device_filter = (
            [
                android_device.strip()
                for android_device in android_device_filter.split(",")
            ]
            if android_device_filter
            else []
        )

        self.adb_path = AndroidDeployment.resolve_adb_tool(
            pathlib.Path(android_sdk_path)
        )
    ...
    def read_android_settings(dev_root, game_name):
        ...
        game_folder_project_properties_path = (
            game_folder / "Platform" / "Android" / "android_project.json"
        )
        game_project_properties_content = game_folder_project_properties_path.resolve(
            strict=True
        ).read_text(
    ...
    def resolve_adb_tool(android_sdk_path):
        ...
        if adb_target:
            ...
            if not check_adb_target.exists():
                raise common.LmbrCmdError(
                    f"Invalid Android SDK path '{str(android_sdk_path)}': Unable to locate '{adb_target}'."
                )
    ...
    def adb_call(self, arg_list, device_id=None):
        ...
        try:
            output = subprocess.check_output(
                subprocess.list2cmdline(call_arguments),
                shell=True,
                stderr=subprocess.PIPE,
            ).decode(common.DEFAULT_TEXT_READ_ENCODING, common.ENCODING_ERROR_HANDLINGS)
            logging.debug(f"adb output:\n{output}")
            return output
        except subprocess.CalledProcessError as err:
            std_out = err.stdout.decode(
                common.DEFAULT_TEXT_READ_ENCODING, common.ENCODING_ERROR_HANDLINGS
            )
            std_err = err.stderr.decode(
                common.DEFAULT_TEXT_READ_ENCODING, common.ENCODING_ERROR_HANDLINGS
            )
            logging.debug(
                f"adb returned non-zero.\noutput:\n{std_out}\nerror:\n{std_err}\n"
            )
    ...
    def adb_ls(self, path, device_id, args=None):
        ...
        """
        error_messages = ["No such file or directory", "Permission denied"]
        ...
        raw_output = self.adb_shell(
            command=" ".join(shell_command), device_id=device_id
        )

        if not raw_output:
            return False, None

        if raw_output is None or any(
            [error for error in error_messages if error in raw_output]
        ):
            status = False
        else:
            status = True
    ...
    def get_target_android_devices(self):
        ...
        for device_output in device_output_list:
            if any(x in device_output for x in ["List", "*"]):
                logging.debug(
                    f"Skipping the following line as it has 'List', '*' or 'emulator' in it: {device_output}"
                )
                continue
            ...
            if device_serial:
                if "unauthorized" in device_output.lower():
                    logging.warning(
                        f"Device {device_serial[0]} is not authorized for development access. Please reconnect the device and check for a confirmation dialog."
                    )
                elif (
                    device_serial[0] in self.android_device_filter
                    or not self.android_device_filter
                ):
    ...
    def check_known_android_paths(self, device_id):
        ...
        for path in KNOWN_ANDROID_EXTERNAL_STORAGE_PATHS:
            ...
            result, output = self.adb_ls(path=path, args=None, device_id=device_id)
            if result:
                return path[:-1]
    ...
    def detect_device_storage_path(self, device_id):
        ...
        external_storage = self.adb_shell(
            command="set | grep EXTERNAL_STORAGE", device_id=device_id
        )
        if not external_storage:
            logging.debug(
                f"Unable to get 'EXTERNAL_STORAGE' environment from device '{device_id}'. Falling back to known android paths."
            )
        ...
        if len(storage_path_key_value) != 2:
            logging.debug(
                f"The value for 'EXTERNAL_STORAGE' environment from device '{device_id}' does not represent a valid key-value pair: {storage_path_key_value}. Falling back to known android paths"
            )
            return self.check_known_android_paths(device_id)
        # Check the existence and permissions issue of the storage path
        storage_path = storage_path_key_value[1].strip()
        is_external_valid, _ = self.adb_ls(path=storage_path, device_id=device_id)
        if is_external_valid:
            return storage_path

        # The set external path has an issue, attempt to determine its real path through an adb shell call
        logging.debug(
            f"The path specified in EXTERNAL_STORAGE seems to have permission issues, attempting to resolve with realpath for device {device_id}."
        )
        real_path = self.adb_shell(
            command=f"realpath {storage_path}", device_id=device_id
        )
        if not real_path:
            logging.debug(
                f"Unable to determine the real path '{storage_path}' (from EXTERNAL_STORAGE) for {self.game_name} on device {device_id}. Falling back to known android paths"
            )
            return self.check_known_android_paths(device_id)
        real_path = real_path.strip()
        is_external_valid, _ = self.adb_ls(path=real_path, device_id=device_id)
        if is_external_valid:
            return real_path

        logging.debug(
            f"Unable to validate the resolved EXTERNAL_STORAGE environment variable path for device {device_id}."
        )
        return self.check_known_android_paths(device_id)
    ...
    def get_device_file_timestamp(self, remote_file_path, device_id):
        ...
        """
        try:
            timestamp_string = self.adb_shell(
                command=f"cat {remote_file_path}", device_id=device_id
            ).strip()
    ...
    def update_device_file_timestamp(self, relative_assets_path, device_id):
        ...
        local_timestamp_file_path = (
            self.local_asset_path / ANDROID_TARGET_TIMESTAMP_FILENAME
        )
        local_timestamp_file_path.write_text(timestamp_str)
        target_timestamp_file_path = (
            f"{relative_assets_path}/{ANDROID_TARGET_TIMESTAMP_FILENAME}"
        )

        self.adb_call(
            arg_list=[
                "push",
                str(local_timestamp_file_path),
                target_timestamp_file_path,
            ],
            device_id=device_id,
        )
    ...
    def check_package_installed(self, package_name, target_device):
        ...
        """
        output_result = self.adb_call(
            ["shell", "cmd", "package", "list", "packages", package_name], target_device
        )
    ...
    def install_apk_to_device(self, target_device):
        ...
        if self.is_test_project:
            ...
            android_package_name = self.get_android_project_settings(
                key_name="package_name", default_value="org.o3de.sdk"
            )

        if self.clean_deploy and self.check_package_installed(
            android_package_name, target_device
        ):
            logging.info(
                f"Device '{target_device}': Uninstalling pre-existing APK for {self.game_name} ..."
            )
            self.adb_call(
                arg_list=["uninstall", android_package_name], device_id=target_device
            )

        logging.info(
            f"Device '{target_device}': Installing APK for {self.game_name} ..."
        )
        self.adb_call(
            arg_list=["install", "-t", "-r", str(self.apk_path.resolve())],
            device_id=target_device,
        )

    def path_exists_on_device(self, path, device_id):
        try:
            result, _ = self.adb_ls(path=path, args=None, device_id=device_id)
            return result
        except (common.LmbrCmdError, AttributeError):
            return False
    ...
    def install_assets_to_device(self, detected_storage, target_device):
        ...
        target_timestamp = self.get_device_file_timestamp(
            remote_file_path=device_timestamp_file, device_id=target_device
        )

        if self.clean_deploy:
            logging.info(
                f"Device '{target_device}': Cleaning target assets before deployment..."
            )
            self.adb_shell(command=f"rm -rf {output_target}", device_id=target_device)
        ...
        if self.clean_deploy or not target_timestamp:
            logging.info(
                f"Device '{target_device}': Pushing {len(self.files_in_asset_path)} files from {str(self.local_asset_path)} to device {output_target} ..."
            )
            try:
                # '/.' is necessary in the source path to not copy folder 'assets' to destination, but its content.
                self.adb_call(
                    arg_list=["push", f"{str(self.local_asset_path)}/.", output_target],
                    device_id=target_device,
                )
            except common.LmbrCmdError as err:
                # Something went wrong, clean up before leaving
                self.adb_shell(
                    command=f"rm -rf {output_target}", device_id=target_device
                )
                raise err
            ...
            if len(files_to_copy) > 0:
                logging.info(
                    f"Copying {len(files_to_copy)} assets to device {target_device}"
                )
            ...
            for src_path in files_to_copy:
                ...
                target_path = f"{output_target}/{relative_path}"
                self.adb_call(
                    arg_list=["push", str(src_path), target_path],
                    device_id=target_device,
                )

        self.update_device_file_timestamp(
            relative_assets_path=output_target, device_id=target_device
        )
    ...
    def execute(self):
        ...
        if self.is_test_project:
            if not self.apk_path.is_file():
                raise common.LmbrCmdError(
                    f"Missing apk for {android_support.TEST_RUNNER_PROJECT} ({str(self.apk_path)}). Make sure it is built and is set as a signed APK."
                )
        else:
            if self.deployment_type in (
                AndroidDeployment.DEPLOY_APK_ONLY,
                AndroidDeployment.DEPLOY_BOTH,
            ):
                if not self.apk_path.is_file():
                    raise common.LmbrCmdError(
                        f"Missing apk for game {self.game_name} ({str(self.apk_path)}). Make sure it is built and is set as a signed APK."
                    )

            if self.deployment_type in (
                AndroidDeployment.DEPLOY_ASSETS_ONLY,
                AndroidDeployment.DEPLOY_BOTH,
            ):
                if not self.local_asset_path.is_dir():
                    raise common.LmbrCmdError(
                        f"Missing {self.asset_type} assets for game {self.game_name} ."
                    )
        ...
        try:
            ...
            if not target_devices:
                raise common.LmbrCmdError(
                    "No connected and eligible android devices found"
                )
            ...
            for target_device in target_devices:
                ...
                if not detected_storage:
                    logging.warning(
                        f"Unable to resolve storage path for device '{target_device}'. Skipping."
                    )
                    continue
                ...
                if self.is_test_project:
                    ...
                    if self.deployment_type in (
                        ...
                        if not self.check_package_installed(
                            ...
                            raise common.LmbrCmdError(
                                f"Unable to locate APK for {self.game_name} on device '{target_device}'. Make sure it is installed "
                                f"first before installing the assets."
                            )

                        self.install_assets_to_device(
