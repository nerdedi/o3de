...
def test_copy_asset_files_to_layout_success():
    ...
    try:
        ...
        test_denylist_file = ["assetprocessorplatformconfig.setreg"]
        ...
        test_skip_source_folders = ["src_folder"]
        # Destination 'folders' will be skipped
        test_skip_dest_is_folder = ["fake_dst_folder"]
        # Skip files that are the same in the destination
        test_dest_same_as_src = ["dst_same_as_src"]
        # COPY files that are the same in the destination
        test_dest_diff_as_src = ["dst_diff_as_src"]
        # COPY files that are not in the destination
        test_src_not_in_dst = ["system_goodplatform_pc", "good_src_1", "good_src_2"]
        ...
        layout_tool.copy_asset_files_to_layout(
            project_asset_folder=test_game_asset_folder,
...
def test_construct_and_validate_cache_game_asset_folder_success(
    tmpdir, project_path, asset_type, warn_on_missing, expected_result
):
    ...
    try:
        result = layout_tool.construct_and_validate_cache_project_asset_folder(
        ...
        if result == None:
            assert warn_on_missing, (
                "Expecting a warn_on_missing==True if None is returned"
            )
...
def test_sync_layout_vfs_success(
    tmpdir, existing_temp_vfs_folder, existing_gems_link, existing_game_link
):
    ...
    try:
        ...
        cache_game_folder_gems = os.path.join(
            cache_game_folder, test_asset_type, "gems"
        )
        ...
        test_layout_folder = str(
            tmpdir.join("{}/vfs/foo".format(tmp_folder_subfolder)).realpath()
        )
        test_layout_config_folder = str(
            tmpdir.join("{}/vfs/foo/config".format(tmp_folder_subfolder)).realpath()
        )
        ...
        mock_layout_tool_create_link_validation = {
            os.path.normcase(cache_game_folder_gems): os.path.normcase(
                layout_target_gems_realpath
            ),
            os.path.normcase(test_layout_folder): os.path.normcase(
                layout_target_game_realpath
            ),
        }
        ...
        def _mock_layout_tool_create_link(src, dst, copy):
            ...
            assert check_src in mock_layout_tool_create_link_validation, (
                "Unexpected create link call to {}->{}".format(src, dst)
            )
            assert mock_layout_tool_create_link_validation[check_src] == check_dst, (
                "Assertion on create linked failed: {}->{}".format(src, dst)
            )

        layout_tool.create_link = _mock_layout_tool_create_link

        def _mock_copy_asset_files_to_layout(
            project_path, project_asset_folder, target_platform, layout_target
        ):
            # Validate the correct call to copy asset files
            assert target_platform == target_platform
            assert os.path.normcase(layout_target) == os.path.normcase(
                layout_target_root_realpath
            )
...
def test_sync_layout_non_vfs_success(
    tmpdir, mode, existing_game_link, existing_gems_link, test_override_pak_folder
):
    old_rmdir = os.rmdir
    old_copy_asset_files_to_layout = layout_tool.copy_asset_files_to_layout
    ...
    try:
        ...
        test_layout_target_gems_realpath = os.path.join(
            test_layout_target_realpath, "gems"
        )
        test_layout_target_game_realpath = os.path.join(
            test_layout_target_realpath,
        )
        ...
        if mode == "PAK":
            ...
            if test_override_pak_folder:
                test_game_asset_folder = os.path.join(
                    engine_root_realpath,
                    test_override_pak_folder,
            ...
            mock_layout_tool_create_link_validation[
                os.path.normcase(cache_game_folder_game_realpath)
            ] = os.path.normcase(test_layout_target_game_realpath)
            ...
            if existing_gems_link:
                ...
                expected_rmdir_paths.add(
                    os.path.normcase(test_layout_target_gems_realpath)
                )
                os.rmdir = _mock_os_rmdir

            test_game_asset_folder = os.path.join(
                cache_game_folder_realpath, test_asset_type
            )
            cache_game_folder_gems_realpath = os.path.join(
                cache_game_folder_realpath, test_asset_type, "gems"
            )
            cache_game_folder_game_realpath = os.path.join(
                cache_game_folder_realpath, test_asset_type
            )
            mock_layout_tool_create_link_validation[
                os.path.normcase(cache_game_folder_gems_realpath)
            ] = os.path.normcase(test_layout_target_gems_realpath)
            mock_layout_tool_create_link_validation[
                os.path.normcase(cache_game_folder_game_realpath)
            ] = os.path.normcase(test_layout_target_game_realpath)
        ...
        def _mock_copy_asset_files_to_layout(
            ...
            assert os.path.normcase(project_asset_folder) == os.path.normcase(
                test_game_asset_folder
            )
        ...
        layout_tool.sync_layout_non_vfs(
            mode=mode,
            target_platform=test_target_platform,
            project_path=test_project_path,
            asset_type=test_asset_type,
