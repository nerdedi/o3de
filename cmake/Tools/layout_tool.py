...
def verify_layout(layout_dir, platform_name, project_path, asset_mode, asset_type):
    ...
    def _validate_remote_ap(input_remote_ip, input_remote_connect, remote_on_check):
        if remote_on_check is None:
            ...
            if input_remote_connect == "1" and input_remote_ip == LOCAL_HOST:
                return _warn(
                    "'bootstrap.setreg' is configured to connect to Asset Processor remotely, but the 'remote_ip' "
            ...
            if remote_on_check:
                ...
                if input_remote_ip == LOCAL_HOST:
                    return _warn(
                        f"'bootstrap.setreg' is not configured for a remote Asset Processor connection (remote_ip={input_remote_ip})"
                    )
                if input_remote_connect != "1":
                    return _warn(
                        f"'bootstrap.setreg' is not configured for a remote Asset Processor connection ({platform_name}_connect_to_remote={input_remote_connect}"
                    )
            else:
    ...
    bootstrap_values = common.get_bootstrap_values(
        str(bootstrap_path),
        [
    ...
    if bootstrap_values:
        remote_ip = (
            bootstrap_values.get(f"{platform_name_lower}_remote_ip")
            or bootstrap_values.get("remote_ip")
            or LOCAL_HOST
        )
        remote_connect = (
            bootstrap_values.get(f"{platform_name_lower}_connect_to_remote") or "0"
        )

        # Validate that the asset type for the platform matches the one set for the build
        bootstrap_asset_type = bootstrap_values.get(
            f"{platform_name_lower}_assets"
        ) or bootstrap_values.get("assets")
        if not bootstrap_asset_type:
            warning_count += _warn(
                "'bootstrap.setreg' is missing specifications for asset type."
            )
        elif bootstrap_asset_type != asset_type:
            warning_count += _warn(
                f"The asset type specified in bootstrap.setreg ({bootstrap_asset_type}) does not match the asset type specified for this deployment({asset_type})."
            )
        ...
        if not project_asset_path.is_dir():
            warning_count += _warn(
                f"Asset folder for project {project_name} is missing from the deployment layout."
            )
        else:
            ...
            if asset_mode == ASSET_MODE_PAK:
                ...
                remote_file_system = (
                    bootstrap_values.get(f"{platform_name_lower}_remote_filesystem")
                    or "0"
                )
                if not remote_file_system != "1":
                    warning_count += _warn(
                        "Remote file system is not configured in bootstrap.setreg for VFS mode."
                    )
                else:
                    warning_count += _validate_remote_ap(
                        remote_ip, remote_connect, True
                    )
...
def copy_asset_files_to_layout(project_asset_folder, target_platform, layout_target):
    ...
    for src_file in src_asset_contents:
        ...
        if os.path.isdir(abs_dst):
            ...
            logging.warning(
                "Skipping layout copying of file '%s' because the target '%s' refers to a directory",
                src_file,
                abs_dst,
            )
            continue
        ...
        if not os.path.exists(abs_dst):
            ...
            if src_hash == dst_hash:
                logging.debug(
                    "Skipping layout copy of '%s', fingerprints of source and destination matches (%s)",
                    src_file,
                    src_hash,
                )
                continue
        ...
        try:
            ...
            try:
                ...
                raise common.LmbrCmdError(
                    f"Error trying remove directory {link}: {shutil_error}",
                    shutil_error.errno,
                )
...
def create_link(src: pathlib.Path, tgt: pathlib.Path, copy):
    ...
    if copy:
        ...
        try:
            ...
            raise common.LmbrCmdError(
                f"Error trying to create {link_type} {src} => {tgt} : {e}", e.errno
            )


def construct_and_validate_cache_project_asset_folder(
    project_path, asset_type, warn_on_missing_project_cache
):
    """
    ...
    if (
        not os.path.isdir(cache_project_folder_root)
        and not warn_on_missing_project_cache
    ):
        raise common.LmbrCmdError(
...
def sync_layout_vfs(
    ...
    logging.debug(
        f'Syncing VFS layout for project at path "{project_path}" to layout path "{layout_target}"'
    )

    project_asset_folder = construct_and_validate_cache_project_asset_folder(
    ...
    if not os.path.isdir(vfs_asset_source):
        raise common.LmbrCmdError(
            "Cache folder for the project '{}' missing 'config' folder".format(
                project_path
            ),
            common.ERROR_CODE_ERROR_DIRECTORY,
        )
    ...
    temp_vfs_layout_project_config_path = os.path.join(
        temp_vfs_layout_project_path, "config"
    )
    ...
    root_assets = [
        "engine.json",
        "bootstrap.client.debug.setreg",
        "bootstrap.client.profile.setreg",
        "bootstrap.client.release.setreg",
        "bootstrap.server.debug.setreg",
        "bootstrap.server.profile.setreg",
        "bootstrap.server.release.setreg",
        "bootstrap.unified.debug.setreg",
        "bootstrap.unified.profile.setreg",
        "bootstrap.unified.release.setreg",
    ]
    for root_asset in root_assets:
        logging.debug(
            "Copying %s -> %s",
            os.path.join(project_asset_folder, root_asset),
            layout_target,
        )
...
def sync_layout_non_vfs(
    ...
    if not project_name:
        raise common.LmbrCmdError(
            f"Project at path {project_path} does not have a valid project.json"
        )
    ...
    if mode == ASSET_MODE_PAK:
        ...
        if override_pak_folder:
            ...
            project_asset_folder = os.path.join(
                project_path,
                override_pak_folder or PAK_FOLDER_NAME,
                target_pak_folder_name,
            )
        if not os.path.isdir(project_asset_folder):
            if warning_on_missing_assets:
                logging.warning(
                    f'Pak folder for the project at path "{project_path}" is missing'
                    f' (expected at "{project_asset_folder}"). Skipping layout sync'
                )
                return
            else:
                raise common.LmbrCmdError(
                    f'Pak folder for the project at path "{project_path}" is missing (expected at'
    ...
    copy_asset_files_to_layout(
        project_asset_folder=project_asset_folder,
...
def main(args):
    parser = argparse.ArgumentParser(
        description="Synchronize a project's assets to a layout folder"
    )
    ...
    parser.add_argument(
        "-p", "--platform", help="Target platform for the layout.", required=True
    )
    parser.add_argument(
        "-a",
        "--asset-type",
    ...
    if input_asset_mode not in ALL_ASSET_MODES:
        raise common.LmbrCmdError(
            "Invalid asset mode '{}'. Must be one of : '{}'.".format(
                input_asset_mode, ",".join(ALL_ASSET_MODES)
            ),
    ...
    logging.info(
        "Starting (%s) Asset Synchronization in %s mode and project %s",
        parsed_args.asset_type,
        input_asset_mode,
        parsed_args.project_path,
    )
    start_time = timeit.default_timer()
    ASSET_SYNC_MODE_FUNCTION[input_asset_mode](
    ...
    if os.path.isdir(parsed_args.layout_root) and not os.path.exists(
    ...
    if not os.path.isdir(parsed_args.layout_root):
        ...
        if parsed_args.create_layout_root:
            try:
                ...
                raise common.LmbrCmdError(
                    "Unable to create layout folder '{}': {}".format(
                        e, parsed_args.layout_root
                    ),
                    common.ERROR_CODE_ERROR_DIRECTORY,
                )
        else:
            raise common.LmbrCmdError(
                "Invalid layout folder (--layout-root): '{}'".format(
                    parsed_args.layout_root
                ),
                common.ERROR_CODE_ERROR_DIRECTORY,
            )
    ...
    if parsed_args.verify:
        warnings = verify_layout(
            layout_dir=os.path.normpath(parsed_args.layout_root),
        ...
        if warnings > 0:
            if parsed_args.fail_on_warning:
                raise common.LmbrCmdError(
                    f"Layout verification failed: {warnings} warnings."
                )
