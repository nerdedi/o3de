...
DEFAULT_TEXT_READ_ENCODING = (
    "UTF-8"  # The default encoding to use when reading from a text file
)
...
DEFAULT_PAK_ROOT = (
    "Pak"  # The default Pak root folder under engine root where the game paks are built
)
...
def get_bootstrap_values(bootstrap_dir, keys_to_extract):
    ...
    with open(bootstrap_file, "r") as f:
        try:
            ...
            for search_key in keys_to_extract:
                try:
                    search_result = json_data["Amazon"]["AzCore"]["Bootstrap"][
                        search_key
                    ]
                except KeyError as e:
                    logging.warning(
                        f"Bootstrap.setreg cannot find /Amazon/AzCore/Bootstrap/{search_key}: {str(e)}"
                    )
                else:
...
def validate_ap_config_asset_type_enabled(engine_root, bootstrap_asset_type):
    ...
    ap_config_file = os.path.join(
        engine_root, "Registry", "AssetProcessorPlatformConfig.setreg"
    )
    if not os.path.isfile(ap_config_file):
        raise LmbrCmdError(
            "Missing required asset processor configuration file at '{}'".format(
                engine_root
            ),
...
def file_fingerprint(path, deep_check=False):
    ...
    if os.path.isdir(path):
        raise LmbrCmdError(
            "Cannot fingerprint '{}' because its a directory".format(path),
...
def load_template_file(template_file_path, template_env):
    ...
    """
    try:
        template_file_content = template_file_path.resolve(strict=True).read_text(
            encoding=DEFAULT_TEXT_READ_ENCODING, errors=ENCODING_ERROR_HANDLINGS
        )
        # Filter out all lines that start with '###' before replacement
        filtered_template_file_content = str(
            re.sub("###.*", "", template_file_content)
        ).strip()

        return string.Template(filtered_template_file_content).substitute(template_env)
    except FileNotFoundError:
        raise FileNotFoundError(
            f"Invalid file path. Cannot find template file located at {str(template_file_path)}"
        )
...
if platform.system() == "Windows":
    ...
    PLATFORM_EXECUTABLE_EXTENSIONS.extend(
        [pathext.lower() for pathext in path_extensions_str.split(";")]
    )
    ...
    try:
        ...
        if override_tool_path:
            ...
            if isinstance(override_tool_path, str):
                ...
                raise LmbrCmdError(
                    f"Invalid {tool_name} path argument. '{override_tool_path}' must be a string or Path",
            ...
            if not file_found:
                raise LmbrCmdError(
                    f"Invalid {tool_name} path argument. '{override_tool_path}' is not a valid {tool_name} path",
                    ERROR_CODE_INVALID_PARAMETER,
                )
        ...
        version_output = subprocess.check_output(
            [tool_source, tool_version_argument],
            shell=(platform.system() == "Windows"),
            stderr=subprocess.PIPE,
        ).decode(DEFAULT_TEXT_READ_ENCODING, ENCODING_ERROR_HANDLINGS)
        ...
        result_version_str = re.sub(
            r"[^\.0-9]", "", str(version_match.group(1)).strip()
        )
        result_version = Version(result_version_str)

        if min_version and result_version < min_version:
            raise LmbrCmdError(
                f"The {tool_desc} does not meet the minimum version of {tool_name} required ({str(min_version)}).",
                ERROR_CODE_ENVIRONMENT_ERROR,
            )
        elif max_version and result_version > max_version:
            raise LmbrCmdError(
                f"The {tool_desc} exceeds maximum version of {tool_name} supported ({str(max_version)}).",
                ERROR_CODE_ENVIRONMENT_ERROR,
            )
        ...
        error_msg = e.output.decode(
            DEFAULT_TEXT_READ_ENCODING, ENCODING_ERROR_HANDLINGS
        )
        raise LmbrCmdError(
            f"{tool_name} cannot be resolved or there was a problem determining its version number. "
...
def verify_project_and_engine_root(project_root, engine_root):
    ...
    if not engine_root_path.exists():
        raise LmbrCmdError(
            f"Invalid engine root path ({engine_root})", ERROR_CODE_INVALID_PARAMETER
        )
    # Sanity check: engine.json
    engine_json_path = engine_root_path / ENGINE_ROOT_CHECK_FILE
    if not engine_json_path.exists():
        raise LmbrCmdError(
            f"Invalid engine root path ({engine_root}). Missing {ENGINE_ROOT_CHECK_FILE}",
            ERROR_CODE_INVALID_PARAMETER,
        )
    ...
    if project_root is None:
        ...
        if not project_path_project_properties.is_file():
            raise LmbrCmdError(
                f'Invalid project at path "{project_path}". It is missing the project.json file',
                ERROR_CODE_INVALID_PARAMETER,
            )
        project_name = read_project_name_from_project_json(project_path)
        if not project_name:
            raise LmbrCmdError(
                f'Invalid project at path "{project_path}". Its project.json does not contains a "project_name" key',
                ERROR_CODE_INVALID_PARAMETER,
            )
...
def remove_dir_path(path):
    ...
    if not path.is_dir() and path.is_file():
        ...
        raise RuntimeError(
            "Cannot perform 'remove_dir_path' on file {path}. It must be a directory."
        )
    if path.exists():
        files_to_delete = []
...
class PlatformSettings(object):
    ...
    def __init__(self, build_dir):
        ...
        if not platform_last_file.exists():
            raise LmbrCmdError(
                f"Invalid build directory {build_dir}. Missing 'platform.settings'."
            )
        ...
        if platform_name in config.sections():
            ...
            for platform_item_key, platform_item_value in platform_items:
                ...
                if platform_item_key in (
                    "asset_deploy_mode",
                    "asset_deploy_type",
                    "projects",
                ):
                    logging.warning(
                        f"Reserved key '{platform_item_key}' found in platform section of {str(platform_last_file)}. Ignoring"
                    )
                    continue
...
def validate_build_dir_and_config(build_dir_name, configuration):
    ...
    if not build_config_dir.is_dir():
        raise LmbrCmdError(
            f"Output path for build configuration {configuration} not found. Make sure that it was built."
        )
...
def validate_deployment_arguments(build_dir_name, configuration, project_path):
    ...
    build_dir, build_config_dir = validate_build_dir_and_config(
    ...
    if not project_path:
        if not platform_settings.projects:
            raise LmbrCmdError(
                "Missing required game project argument. Unable to determine a default one."
            )
        internal_project_path = pathlib.PurePath(
            platform_settings.projects[0]
        ).resolve()
        logging.info(
            f"Using project_path '{internal_project_path}' as the game project"
        )
    else:
        if project_path not in platform_settings.projects:
            raise LmbrCmdError(
                f"Game project {project_path} not valid. Was not configured for build directory {build_dir_name}."
            )

    return (
        build_dir,
        project_path,
        platform_settings.asset_deploy_mode,
        platform_settings.asset_deploy_type,
        platform_settings.override_pak_root or DEFAULT_PAK_ROOT,
    )
    ...
    def exec(self, arguments, capture_stdout=False, suppress_stderr=False, cwd=None):
        ...
        try:
            ...
            result = subprocess.run(
                ...
                if not capture_stdout and suppress_stderr
                else None,
                encoding="utf-8",
                errors="ignore",
                cwd=cwd,
            ...
            raise LmbrCmdError(
                f"Error trying to call '{self.executable_path}': {str(err)}"
            )
...
def sync_platform_layout(
    platform_name, project_path, asset_mode, asset_type, layout_root
):
    """
    ...
    """
    layout_tool.ASSET_SYNC_MODE_FUNCTION[asset_mode](
...
def get_cmake_dependency_modules(build_dir_path, target, module_type):
    ...
    try:
        cmake_dep_path = (
            build_dir_path / "Registry" / f"cmake_dependencies.{target.lower()}.setreg"
        )
...
def get_test_module_registry(build_dir_path):
    ...
    try:
        ...
        raise LmbrCmdError(
            f"Unit test registry not found ('{str(unit_test_module_path)}')"
        )
