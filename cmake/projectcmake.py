...
def processGemDependencies(gem_paths):
    ...
    for gem_path in gem_paths:
        ...
        if not has_editor_gem and not has_game_gem:
            print(
                f"gem.json for gem located at {gem_path} does not define a Game or Editor Module and is not an asset only gem"
            )
            sys.exit(1)
...
def generateCMakeFilesForProjectGemDependencies(
    toolTime_dependencies, runTime_dependencies, project_code_path
):
...
def main():
    parser = argparse.ArgumentParser(
        description="This script creates basic CMakeLists.txt and .cmake files for a waf based LY project",
    ...
    for input_path in args.path_to_projects:
        ...
        if not gems_list:
            print("Invalid Gems list found in gems.json for project {}").format(
                project_name
            )
            sys.exit(1)
        ...
        if not project_gem_json_dict:
            print(
                f"Could not load gem.json within the Gem folder for project {project_name}"
            )
            sys.exit(1)
        ...
        gemcmake.addSubdirectoryToCMakeLists(
            os.path.join(project_path, "CMakeLists.txt"), "Gem"
        )
        gemcmake.addSubdirectoryToCMakeLists(
            os.path.join(project_gem_path, "CMakeLists.txt"), "Code"
        )
        ...
        gemcmake.generateCMakeFilesForGem(
            ...
            project_gem_uuid,
            project_gem_version,
            getProjectGemCMakeListsTemplate,
        )

        # Generate our tooltime and runtime .cmake files
        generateCMakeFilesForProjectGemDependencies(
            toolTime_dependencies, runTime_dependencies, project_gem_code_path
        )
