...
def PrintError(*objs):
    ...
    if errorCount > MAX_ERRORS:
        print(
            "Maximum errors exceeded (%d) please check the tty for errors" % MAX_ERRORS,
            file=sys.stderr,
        )
        sys.exit(1)
...
def PrintUnhandledExcptionInfo():
    print(
        "An unexpected error occurred, please report the error you encountered and include your build output",
        file=sys.stderr,
    )
...
def CamelToHuman(string):
    return string[0].upper() + re.sub(
        r"((?<=[a-z])[A-Z]|(?<!\A)[A-Z](?=[a-z]))", r" \1", string[1:]
    )
...
def CreateAZHashValue64(btyes):
    ...
    hashStr = hash.hexdigest()
    return (
        "AZ::HashValue64{ 0x" + hashStr[0:16] + " }"
    )  # grab the first 64-bits of a sha256; any 64-bits of a sha256 are just as secure as any other 64.
...
def ComputeOutputPath(inputFiles, projectDir, outputDir):
    commonInputPath = os.path.commonpath(
        inputFiles
    )  # If we've globbed many source files, this finds the common path
    if os.path.isfile(
        commonInputPath
    ):  # If the commonInputPath resolves to an actual file, slice off the filename
        commonInputPath = os.path.dirname(commonInputPath)
    commonPath = os.path.commonpath(
        [commonInputPath, projectDir]
    )  # Finds the common path between the data source files and our project directory (//depot/dev/Code/Framework/AzCore/)
    inputRelativePath = os.path.relpath(
        commonInputPath, commonPath
    )  # Computes the relative path for the project source directory (Code/Framework/AzCore/AutoGen/)
    return os.path.join(
        outputDir, inputRelativePath
    )  # Returns a suitable output directory (//depot/dev/Generated/Code/Framework/AzCore/AutoGen/)


def ProcessTemplateConversion(
    autogenConfig, dataInputSet, dataInputFiles, templateFile, outputFile, templateCache
):
    ...
    try:
        ...
        for dataInputFile in sorted(dataInputFiles):
            try:
                ...
                PrintError(
                    "%s(%s) : %s"
                    % (fileinput.filename(), str(fileinput.filelineno()), e.message)
                )
        ...
        try:
            ...
            if (
                outputExtension == ".xml"
                or outputExtension == ".xhtml"
                or outputExtension == ".xsd"
            ):
                ...
                compareFD.write(
                    "<!-- Template Source {0};\n * XML Sources {1}-->\n".format(
                        templateFile, ", ".join(dataInputFiles)
                    )
                )
                compareFD.write("\n")
                ...
                outputExtension == ".h"
                or outputExtension == ".hpp"
                or outputExtension == ".inl"
                or outputExtension == ".c"
                or outputExtension == ".cpp"
            ):
                compareFD.write("/*\n")
                compareFD.write(
                    " * Copyright (c) Contributors to the Open 3D Engine Project.\n"
            ...
            PrintError(
                "%s(1) : error TemplateNotFound %s"
                % (os.path.abspath(templateFile), e.message)
            )
        except IOError as e:
            PrintError(
                "%s(%s) : error I/O(%s) accessing %s : %s"
                % (
                    fileinput.filename(),
        ...
        PrintError(
            "%s(%s) : error Template processing error: %s"
            % (os.path.abspath(e.filename), e.lineno, e.message)
        )
        ...
        PrintError(
            "%s(1) : error Template processing error: %s with %s"
            % (
                os.path.abspath(templateFile),
                e.message,
                ", ".join(
                    [
                        os.path.basename(dataInputFile)
                        for dataInputFile in dataInputFiles
                    ]
                ),
            )
        )
    ...
    try:
        if os.path.isfile(outputFile):
            with open(outputFile, "r+") as currentFile:
                ...
                if currentFileStringData == compareFD.getvalue():
                    ...
                    with open(outputFile, "w+") as currentFile:
                        ...
                        if autogenConfig.verbose == True:
                            print(
                                f"Generated {outputFile} with template {templateFile} and inputs, ".join(
                                    dataInputFiles
                                )
                            )
                        else:
            ...
            with open(outputFile, "w+") as outputFD:
                ...
                if autogenConfig.verbose == True:
                    print(
                        f"Generated {outputFile} using template {templateFile} and inputs, ".join(
                            dataInputFiles
                        )
                    )
                else:
        ...
        PrintError(
            "%s(%s) : error Processing: %s"
            % (fileinput.filename(), str(fileinput.filelineno()), line)
        )
...
def ProcessExpansionRule(
    ...
    try:
        ...
        if templateFile is None:
            print(
                "No matching template file found for %s, template may be missing from your _files.cmake"
                % expansionRuleSet[1]
            )
            return
        ...
        if os.path.isfile(testSingle):
            ...
            outputFileAbsolute = outputFile.replace(
                "$path",
                ComputeOutputPath(
                    dataInputFiles, autogenConfig.projectDir, autogenConfig.outputDir
                ),
            )
            outputFileAbsolute = outputFileAbsolute.replace(
                "$fileprefix",
                os.path.splitext(os.path.basename(testSingle))[0].split(".")[0],
            )
            outputFileAbsolute = outputFileAbsolute.replace(
                "$file", os.path.splitext(os.path.basename(testSingle))[0]
            )
            outputFileAbsolute = SanitizePath(outputFileAbsolute)
            ProcessTemplateConversion(
                autogenConfig,
                dataInputSet,
                dataInputFiles,
                templateFile,
            ...
            if "$fileprefix" in outputFile or "$file" in outputFile:
                ...
                for filename in fnmatch.filter(sourceFiles, inputFiles):
                    ...
                    outputFileAbsolute = outputFile.replace(
                        ...
                        ComputeOutputPath(
                            dataInputFiles,
                            autogenConfig.projectDir,
                            autogenConfig.outputDir,
                        ),
                    )
                    outputFileAbsolute = outputFileAbsolute.replace(
                        "$fileprefix",
                        os.path.splitext(os.path.basename(filename))[0].split(".")[0],
                    )
                    outputFileAbsolute = outputFileAbsolute.replace(
                ...
                dataInputFiles = [
                    ...
                    for file in fnmatch.filter(sourceFiles, inputFiles)
                ]
                if "$path" in outputFile:
                    ...
                    outputFileAbsolute = os.path.join(
                        autogenConfig.outputDir, outputFile
                    )
                outputFileAbsolute = SanitizePath(outputFileAbsolute)
                ProcessTemplateConversion(
                    autogenConfig,
        ...
        PrintError(
            "%s : error I/O(%s) accessing %s : %s"
            % (expansionRule, e.errno, e.filename, e.strerror)
        )
    except:
...
def PruneNonGeneratedFiles(
    autogenConfig: AutoGenConfig, outputFiles: list[pathlib.PurePath]
):
    """
    ...
    for outputDir in generatedOutputDirs:
        ...
        if outputDir.is_dir():
            for genFile in outputDir.iterdir():
                if genFile.is_file() and genFile not in outputFiles:
                    filesToRemove.append(genFile)
        if filesToRemove:
            logger.info(
                f'The following files will be pruned from the generated output directory "{outputDir}":\n'
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "inputFilePath",
        help="input file which contains autogen required files to run azcg expansion rules against",
    )
    parser.add_argument(
        "expansionRules",
        help="set of azcg expansion rules for matching data files to template files",
    )
    parser.add_argument(
        "-n",
        "--dryrun",
        action="store_true",
        help="does not execute autogen, only outputs the set of files that autogen would generate",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="output only the set of files that would be generated by an expansion run",
    )
    parser.add_argument(
        "-p",
        "--pythonPaths",
    ...
    autogenConfig = AutoGenConfig(
        SanitizeTargetName(args.targetName),
    ...
    dataInputSet = {}
    outputFiles = []
    autoGenResult = ExecuteExpansionRules(
        autogenConfig, dataInputSet, outputFiles, args.prune
    )
