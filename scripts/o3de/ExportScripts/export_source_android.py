...
def export_source_android_project(
    ...
    if is_installer_sdk:
        ...
        if build_config != exp.BUILD_CONFIG_RELEASE:
            logger.warning(
                "Only release packages are supported for Android in the O3DE SDK."
            )
    ...
    parsed_android_configure_command = android_arg_parser.parse_args(
    ...
    eutil.process_level_names(
        ctx, preprocessed_seedfile_paths, level_names, ASSET_PLATFORM
    )
    ...
    eutil.build_and_bundle_assets(
        ...
        ctx.project_path / "AssetBundling",
        max_bundle_size,
        logger,
    )
    ...
    android_sdk_home = pathlib.Path(
        android_project_config.get_value(key=android_support.SETTINGS_SDK_ROOT.key)
    )

    parsed_android_generate_command = android_arg_parser.parse_args(
        [
            "android-generate",
            "-p",
            str(ctx.project_name),
            "-B",
            str(target_android_project_path),
            "--asset-mode",
            asset_pack_mode,
        ]
    )
    ...
    gradle_build_command = [
        target_android_project_path / f"gradlew{android_support.GRADLE_EXTENSION}",
        f"assemble{build_config.title()}",
    ]
    has_errors = exp.process_command(
        gradle_build_command, cwd=target_android_project_path
    )
    if has_errors:
    ...
    if deploy_to_device:
        ...
        try:
            ...
            match_signing_config = re.compile(
                "(signingConfigs\\s).*(storeFile\\s).*(storePassword\\s).*(keyPassword\\s).*(keyAlias\\s).*",
                re.DOTALL,
            )
            signing_config_verified = (
                match_signing_config.search(build_gradle_content) is not None
            )
            if signing_config_verified:
                android_support.deloy_to_android_device(
                ...
                logger.error(
                    "Unable to verify signing configuration in build.gradle. Aborting deployment..."
                )
...
def export_source_android_parse_args(
    ...
    parser.add_argument(
        "--deploy-to-android",
        action="store_true",
        help="At completion of build, deploy to a connected android device.",
    )

    default_android_build_path = pathlib.Path(
        export_config.get_value(
            key=exp.SETTINGS_DEFAULT_ANDROID_BUILD_PATH.key,
    ...
    asset_mode = export_config.get_value(
        exp.SETTINGS_ANDROID_ASSET_MODE.key, default=exp.ASSET_MODE_LOOSE
    )
    parser.add_argument(
        "--asset-mode",
        type=str,
...
def export_source_android_run_command(
    o3de_context: exp.O3DEScriptExportContext,
    args,
    ...
    option_android_deploy = export_config.get_parsed_boolean_option(
        parsed_args=args,
    ...
    if not target_android_project_path.is_absolute():
        target_android_project_path = (
            o3de_context.project_path / target_android_project_path
        )
...
if "o3de_context" in globals():
    ...
    try:
        ...
        o3de_logger.debug(
            f"No project detected at {os.getcwd()}, using default settings from global config."
        )
        project_name = None
        project_path = None
