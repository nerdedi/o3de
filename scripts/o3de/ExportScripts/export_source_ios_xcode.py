...
def export_ios_xcode_project(
    ...
    expected_appicon_path = (
        ctx.project_path
        / f"Resources/Platform/iOS/Images.xcassets/{ctx.project_name}AppIcon.appiconset"
    )
    legacy_appicon_path = (
        ctx.project_path / "Resources/Platform/iOS/Images.xcassets/AppIcon.appiconset"
    )
    if not expected_appicon_path.exists():
        if legacy_appicon_path.exists():
            logger.error(
                f"The Legacy AppIcon resource path exists here: {legacy_appicon_path}\n Please rename it to the following:{expected_appicon_path}"
            )
        raise exp.ExportProjectError(
            f"Could not find expected AppIcon path for this project! {str(expected_appicon_path)}"
        )
    ...
    exp.process_command(
        [
            "cmake",
            "-B",
            ios_build_folder_str,
            "-G",
            "Xcode",
            f"-DCMAKE_TOOLCHAIN_FILE={str(cmake_toolchain_path)}",
    ...
    logger.info(
        f"Xcode project file should be generated now. Please check {ios_build_folder_str}"
    )

    xcode_build_common_args = [
        "xcodebuild",
        "-project",
        str(target_ios_project_path / f"{ctx.project_name}.xcodeproj"),
        "-scheme",
        f"{ctx.project_name}.GameLauncher",
        "-configuration",
        build_config,
        "-sdk",
        "iphoneos",
    ...
    exp.process_command(
        [
            ...
            "-r",
            str(
                target_ios_project_path
                / "bin"
                / build_config
                / f"{ctx.project_name}.GameLauncher.app"
            ),
            str(payload_path / f"{ctx.project_name}.GameLauncher.app"),
        ],
        cwd=ctx.project_path,
    )
    ...
    ipa_path = (
        ...
        / "bin"
        / build_config
        / f"{ctx.project_name}.GameLauncher.ipa"
    )
...
def export_source_ios_parse_args(
    ...
    default_ios_path = export_config.get_value(
        exp.SETTINGS_DEFAULT_IOS_BUILD_PATH.key,
        exp.SETTINGS_DEFAULT_IOS_BUILD_PATH.default,
    )
    parser.add_argument(
        "-ibp",
        "--ios-build-path",
        type=pathlib.Path,
        default=pathlib.Path(default_ios_path),
...
def export_source_ios_run_command(
