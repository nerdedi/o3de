...
def export_standalone_project(
    ctx: exp.O3DEScriptExportContext,
    ...
    if is_installer_sdk:
        if build_config == exp.BUILD_CONFIG_DEBUG:
            raise exp.ExportProjectError(
                "Exporting debug projects not supported with the O3DE SDK."
            )
        ...
        if build_config == exp.BUILD_CONFIG_RELEASE:
            ...
            if not exp.has_monolithic_artifacts(ctx):
                logger.error(
                    "No monolithic artifacts are detected in the engine installation."
                )
                raise exp.ExportProjectError(
                    "Trying to build monolithic without libraries."
                )

            logger.info("Preparing monolithic build for export.")
            monolithic_build = True

        else:
            logger.info("Preparing non-monolithic build for export.")
    ...
    validated_seedslist_paths = exp.validate_project_artifact_paths(
        project_path=ctx.project_path, artifact_paths=seedlist_paths
    )

    # Preprocess the seed file paths in case there are any wildcard patterns to use
    preprocessed_seedfile_paths = exp.preprocess_seed_path_list(
        project_path=ctx.project_path, paths=seedfile_paths
    )

    # Convert level names into seed files that the asset bundler can utilize for packaging
    eutil.process_level_names(
        ctx, preprocessed_seedfile_paths, level_names, selected_platform
    )
    ...
    tools_build_path = eutil.handle_tools(
        ctx,
        should_build_tools,
        is_installer_sdk,
        tool_config,
    ...
    # Generate the bundle
    expected_bundles_path = eutil.build_and_bundle_assets(
        ctx,
    ...
    export_layouts = []
    if should_build_game_launcher:
        export_layouts.append(
            exp.ExportLayoutConfig(
                output_path=output_path / f"{ctx.project_name}GamePackage",
                project_file_patterns=project_file_patterns_to_copy
                + game_project_file_patterns_to_copy,
                ignore_file_patterns=[
                    f"*.ServerLauncher{exp.EXECUTABLE_EXTENSION}",
    ...
    if should_build_server_launcher:
        export_layouts.append(
            exp.ExportLayoutConfig(
                output_path=output_path / f"{ctx.project_name}ServerPackage",
                project_file_patterns=project_file_patterns_to_copy
    ...
    if should_build_unified_launcher:
        export_layouts.append(
            exp.ExportLayoutConfig(
                output_path=output_path / f"{ctx.project_name}UnifiedPackage",
                project_file_patterns=project_file_patterns_to_copy
                + game_project_file_patterns_to_copy
                + server_project_file_patterns_to_copy,
                ignore_file_patterns=[
    ...
    if should_build_headless_server_launcher:
        export_layouts.append(
            exp.ExportLayoutConfig(
                output_path=output_path / f"{ctx.project_name}HeadlessServerPackage",
                project_file_patterns=project_file_patterns_to_copy
                + server_project_file_patterns_to_copy,
                ignore_file_patterns=[
                    f"*.ServerLauncher{exp.EXECUTABLE_EXTENSION}",
    ...
    for export_layout in export_layouts:
        exp.setup_launcher_layout_directory(
            ...
            build_config=build_config,
            bundles_to_copy=[
                expected_bundles_path / f"game_{selected_platform}.pak",
...
def export_standalone_parse_args(
    o3de_context: exp.O3DEScriptExportContext, export_config: command_utils.O3DEConfig
):
    ...
    default_output_path = pathlib.Path(
        export_config.get_value(
            key=exp.SETTINGS_DEFAULT_OUTPUT_PATH.key,
            default=exp.SETTINGS_DEFAULT_OUTPUT_PATH.default,
        )
    )
    if not default_output_path.is_absolute():
        default_output_path = (
            pathlib.Path(o3de_context.project_path) / default_output_path
        )
    ...
    default_archive_output_format = export_config.get_value(
        key=exp.SETTINGS_ARCHIVE_OUTPUT_FORMAT.key,
    ...
    parser.add_argument(
        ...
        choices=[
            exp.ARCHIVE_FORMAT_NONE,
            exp.ARCHIVE_FORMAT_ZIP,
            exp.ARCHIVE_FORMAT_GZIP,
            exp.ARCHIVE_FORMAT_BZ2,
            exp.ARCHIVE_FORMAT_XZ,
        ],
        default=default_archive_output_format,
    )

    export_config.add_multi_part_argument(
        argument=["-gpfp", "--game-project-file-pattern-to-copy"],
        parser=parser,
    ...
    export_config.add_multi_part_argument(
        argument=["-spfp", "--server-project-file-pattern-to-copy"],
        parser=parser,
    ...
    export_config.add_multi_part_argument(
        argument=["-pfp", "--project-file-pattern-to-copy"],
        parser=parser,
    ...
    default_launcher_build_path = export_config.get_value(
        exp.SETTINGS_DEFAULT_LAUNCHER_TOOLS_PATH.key,
        exp.SETTINGS_DEFAULT_LAUNCHER_TOOLS_PATH.default,
    )
    parser.add_argument(
        "-lbp",
        "--launcher-build-path",
        type=pathlib.Path,
        default=pathlib.Path(default_launcher_build_path),
    ...
    export_config.add_boolean_argument(
        parser=parser,
    ...
    default_asset_bundling_path = export_config.get_value(
        exp.SETTINGS_DEFAULT_ASSET_BUNDLING_PATH.key,
        exp.SETTINGS_DEFAULT_ASSET_BUNDLING_PATH.default,
    )
    parser.add_argument(
        "-abp",
        "--asset-bundling-path",
        type=pathlib.Path,
        default=pathlib.Path(default_asset_bundling_path),
    ...
    parser.add_argument(
        ...
        "--platform",
        type=str,
        default=exp.get_default_asset_platform(),
        choices=["pc", "linux", "mac"],
    )
...
def export_standalone_run_command(
    o3de_context, args, export_config: command_utils.O3DEConfig, o3de_logger
):
    option_build_assets = export_config.get_parsed_boolean_option(
        parsed_args=args,
    ...
    option_build_tools = export_config.get_parsed_boolean_option(
        parsed_args=args,
    ...
    fail_on_asset_errors = export_config.get_parsed_boolean_option(
        parsed_args=args,
    ...
    option_build_game_launcher = export_config.get_parsed_boolean_option(
        parsed_args=args,
        key=exp.SETTINGS_OPTION_BUILD_GAME_LAUNCHER.key,
    ...
    option_build_server_launcher = export_config.get_parsed_boolean_option(
        parsed_args=args,
        key=exp.SETTINGS_OPTION_BUILD_SERVER_LAUNCHER.key,
    ...
    option_build_headless_server_launcher = export_config.get_parsed_boolean_option(
        parsed_args=args,
        key=exp.SETTINGS_OPTION_BUILD_HEADLESS_SERVER_LAUNCHER.key,
    ...
    option_build_unified_launcher = export_config.get_parsed_boolean_option(
        parsed_args=args,
        key=exp.SETTINGS_OPTION_BUILD_UNIFIED_SERVER_LAUNCHER.key,
    ...
    option_build_engine_centric = export_config.get_parsed_boolean_option(
        parsed_args=args,
    ...
    option_allow_registry_overrrides = export_config.get_parsed_boolean_option(
        parsed_args=args,
        key=exp.SETTINGS_OPTION_ALLOW_REGISTRY_OVERRIDES.key,
    ...
    option_build_monolithically = export_config.get_parsed_boolean_option(
        parsed_args=args,
        key=exp.SETTINGS_OPTION_BUILD_MONOLITHICALLY.key,
    ...
    option_kill_prior_processes = export_config.get_parsed_boolean_option(
        parsed_args=args,
...
if "o3de_context" in globals():
    ...
    try:
        if o3de_context is None:
            ...
            export_config = exp.get_export_project_config(
                project_path=o3de_context.project_path
            )
    except command_utils.O3DEConfigError:
        o3de_logger.debug(
            f"No project detected at {os.getcwd()}, getting default settings from global config instead."
        )
        project_name = None
    ...
    o3de_logger.info("Finished exporting.")
    sys.exit(0)
