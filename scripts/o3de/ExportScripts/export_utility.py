...
def process_level_names(ctx, seedfile_paths, level_names, platform):
    ...
    for level in level_names:
        new_level_path = (
            ctx.project_path
            / f"Cache/{platform}/levels"
            / level.lower()
            / (level.lower() + ".spawnable")
        )
...
def compute_tools_build_path(
    ...
    if is_installer_sdk:
        tools_build_path = (
            ctx.engine_path
            / "bin"
            / exp.get_platform_installer_folder_name(selected_platform)
            / tool_config
            / "Default"
        )
...
def handle_tools(
    ...
    tools_build_path = compute_tools_build_path(
        ctx,
        is_installer_sdk,
        selected_platform,
        tool_config,
...
def build_and_bundle_assets(
    ...
    if should_build_all_assets:
        asset_processor_path = exp.get_asset_processor_batch_path(
            tools_build_path=tools_build_path,
    ...
    expected_bundles_path = exp.bundle_assets(
        ctx=ctx,
        selected_platforms=asset_platforms,
...
def create_common_export_params_and_config(parser, export_config):
    parser.add_argument(
        exp.CUSTOM_SCRIPT_HELP_ARGUMENT,
        default=False,
        action="store_true",
        help="Show this help message and exit.",
    )

    default_project_build_config = export_config.get_value(
        key=exp.SETTINGS_PROJECT_BUILD_CONFIG.key,
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        default=default_project_build_config,
        choices=[
            exp.BUILD_CONFIG_RELEASE,
            exp.BUILD_CONFIG_PROFILE,
            exp.BUILD_CONFIG_DEBUG,
        ],
        help="The CMake build configuration to use when building project binaries.",
    )

    default_tool_build_config = export_config.get_value(
        key=exp.SETTINGS_TOOL_BUILD_CONFIG.key,
    ...
    parser.add_argument(
        ...
        "--tool-config",
        type=str,
        default=default_tool_build_config,
        choices=[
            exp.BUILD_CONFIG_RELEASE,
            exp.BUILD_CONFIG_PROFILE,
            exp.BUILD_CONFIG_DEBUG,
    ...
    default_tools_build_path = export_config.get_value(
        exp.SETTINGS_DEFAULT_BUILD_TOOLS_PATH.key,
        exp.SETTINGS_DEFAULT_BUILD_TOOLS_PATH.default,
    )
    parser.add_argument(
        "-tbp",
        "--tools-build-path",
        type=pathlib.Path,
        default=pathlib.Path(default_tools_build_path),
    ...
    default_max_size = export_config.get_value(
        exp.SETTINGS_MAX_BUNDLE_SIZE.key, exp.SETTINGS_MAX_BUNDLE_SIZE.default
    )
    parser.add_argument(
        "-maxsize",
        "--max-bundle-size",
        ...
        help="Specify the maximum size of a given asset bundle in MB.",
    )
    ...
    parser.add_argument(
        ...
        "--quiet",
        action="store_true",
        help="Suppresses logging information unless an error occurs.",
    )
