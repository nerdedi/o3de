...
from o3de import command_utils, utils, export_project as exp
...
DEFAULT_READ_ENCODING = (
    "utf-8"  # The default encoding to use when reading from a text file or stream
)
...
def register_setting(
...
) -> command_utils.SettingsDescription:
    """
    ...
    new_setting = command_utils.SettingsDescription(
        key=key,
...
#
SETTINGS_SDK_ROOT = register_setting(
    key="sdk.root", description="The root path of the android sdk on this system."
)

SETTINGS_PLATFORM_SDK_API = register_setting(
    key="platform.sdk.api",
    description="The android platform API level (ref https://developer.android.com/tools/releases/platforms)",
    default="31",
)

SETTINGS_NDK_VERSION = register_setting(
    key="ndk.version",
...
SETTINGS_GRADLE_PLUGIN_VERSION = register_setting(
    key="android.gradle.plugin",
...
SETTINGS_GRADLE_HOME = register_setting(
    key="gradle.home",
...
SETTINGS_GRADLE_JVM_ARGS = register_setting(
    key="gradle.jvmargs",
...
SETTINGS_CMAKE_HOME = register_setting(
    key="cmake.home",
...
SETTINGS_SIGNING_STORE_FILE = register_setting(
    key="signconfig.store.file",
...
SETTINGS_SIGNING_STORE_PASSWORD = register_setting(
    key="signconfig.store.password",
    description="The password for the key store file",
    is_password=True,
)

SETTINGS_SIGNING_KEY_ALIAS = register_setting(
    key="signconfig.key.alias",
    description="The key alias withing the key store that idfentifies the signing key",
)

SETTINGS_SIGNING_KEY_PASSWORD = register_setting(
    key="signconfig.key.password",
...
SETTINGS_ASSET_MODE = register_setting(
    key="asset.mode",
...
SETTINGS_EXTRA_CMAKE_ARGS = register_setting(
    key="extra.cmake.args",
...
def get_android_config(project_path: Path or None) -> command_utils.O3DEConfig:
    ...
    """
    return command_utils.O3DEConfig(
...
def deloy_to_android_device(
    ...
    apk_file = (
        ...
        / f"app/build/outputs/apk/{build_config}/app-{build_config}.apk"
    )

    has_error = exp.process_command([adb_exec_path, "install", "-t", "-r", apk_file])
    if has_error:
        raise AndroidToolError(
            f"Installing APK '{apk_file}' has failed for the android device."
        )
...
class AndroidGradlePluginRequirements(object):
    ...
    def validate_java_version(self, java_version: str) -> None:
        ...
        if not java_version_check >= self._jdk_version:
            raise AndroidToolError(
                f"The installed version of java ({java_version_check}) does not meet the minimum version of ({self._jdk_version}) "
    ...
    def validate_gradle_version(self, gradle_version: str) -> None:
        ...
        if not gradle_version_check >= self._gradle_ver:
            raise AndroidToolError(
                f"The installed version of gradle ({gradle_version_check}) does not meet the minimum version of ({self._gradle_ver}) "
                f"which is required by the Android Gradle Plugin version ({self._agp_version}). Refer to the android gradle plugin "
                f"release notes at {self._release_notes_url}"
            )
...
ANDROID_GRADLE_PLUGIN_COMPATIBILITY_MAP = {
    "8.10": AndroidGradlePluginRequirements(
        agp_version="8.10",
        gradle_version="8.11",
    ...
    "8.1": AndroidGradlePluginRequirements(
        agp_version="8.1",
        gradle_version="8.0",
    ...
    "8.0": AndroidGradlePluginRequirements(
        agp_version="8.0",
        gradle_version="8.0",
    ...
    "7.4": AndroidGradlePluginRequirements(
        agp_version="7.4",
        gradle_version="7.5",
        sdk_build_tools_version="30.0.3",
        jdk_version="11",
    ...
    "7.3": AndroidGradlePluginRequirements(
        agp_version="7.3",
        gradle_version="7.4",
        sdk_build_tools_version="30.0.3",
        jdk_version="11",
    ...
    "7.2": AndroidGradlePluginRequirements(
        agp_version="7.2",
    ...
    "7.1": AndroidGradlePluginRequirements(
        agp_version="7.1",
        gradle_version="7.2",
        sdk_build_tools_version="30.0.3",
        jdk_version="11",
    ...
    "7.0": AndroidGradlePluginRequirements(
        agp_version="7.0",
    ...
    "4.2": AndroidGradlePluginRequirements(
        agp_version="4.2",
...
MAX_ANDROID_GRADLE_PLUGIN_VER = Version(
    sorted(list(ANDROID_GRADLE_PLUGIN_COMPATIBILITY_MAP.keys()), reverse=True)[0]
)


def get_android_gradle_plugin_requirements(
    requested_agp_version: str,
) -> AndroidGradlePluginRequirements:
    """
    ...
    if Version(lookup_version_str) > MAX_ANDROID_GRADLE_PLUGIN_VER:
        raise AndroidToolError(
            f"Android gradle plugin version {requested_agp_version} is newer than the latest version supported ({MAX_ANDROID_GRADLE_PLUGIN_VER})."
        )

    matched_agp_requirements = ANDROID_GRADLE_PLUGIN_COMPATIBILITY_MAP.get(
        lookup_version_str
    )
    if not matched_agp_requirements:
        raise AndroidToolError(
            f"Unrecognized/unsupported android gradle plugin version {requested_agp_version} specified. Supported Versions are "
...
class AndroidSDKManager(object):
    ...
    class BasePackage(object):
        ...
        def __init__(self, components: List):
            ...
            self._version = Version(
                components[1].strip().replace(" ", ".")
            )  # Fix for versions that have spaces between the version number and potential non-numeric versioning (PEP-0440)
    ...
    class InstalledPackage(BasePackage):
        ...
        def __init__(self, installed_package_components):
            ...
            assert len(installed_package_components) == 4, (
                "4 sections expected for installed package components (path, version, description, location)"
            )
    ...
    class AvailablePackage(BasePackage):
        ...
        def __init__(self, available_package_components):
            ...
            assert len(available_package_components) == 3, (
                "3 sections expected for installed package components (path, version, description)"
            )
    ...
    class AvailableUpdate(BasePackage):
        ...
        def __init__(self, available_update_components):
            ...
            assert len(available_update_components) == 3, (
                "3 sections expected for installed package components (path, version, available)"
            )
    ...
    def __init__(
        self, current_java_version: str, android_settings: command_utils.O3DEConfig
    ):
        """
        ...
        self._android_command_line_tools_sdkmanager_path, self.android_sdk_path = (
            AndroidSDKManager.validate_android_sdk_environment(
    ...
    @staticmethod
    def validate_android_sdk_environment(
        current_java_version: str, android_settings: command_utils.O3DEConfig
    ) -> Path:
        """
        ...
        if not android_sdk_root:
            raise AndroidToolError(
                f"The android sdk path was not set. Set the value of '{SETTINGS_SDK_ROOT}' to the path where the android sdk base is located."
            )
        android_sdk_root_path = Path(android_sdk_root)
        if not android_sdk_root_path.is_dir():
            raise AndroidToolError(
                f"The android sdk path '{SETTINGS_SDK_ROOT}' is set to an invalid path '{android_sdk_root_path}'. Folder does not exist."
            )
        ...
        android_sdk_command_line_tools_root = (
            android_sdk_root_path / "cmdline-tools" / "latest"
        )
        android_sdk_command_line_tool = (
            android_sdk_command_line_tools_root
            / "bin"
            / f"sdkmanager{SDKMANAGER_EXTENSION}"
        )
        if not android_sdk_command_line_tool.is_file():
            raise AndroidToolError(
                f"Unable to locate the android sdk command line tool from the android sdk root set at '{android_sdk_root_path}'. "
        ...
        logging.debug(
            "Validating tool version exec: (%s)", subprocess.list2cmdline(command_arg)
        )
        ...
        if result.returncode != 0:
            ...
            if match:
                ...
                cmdline_tool_java_version = JAVA_CLASS_FILE_MAP.get(
                    java_file_version.split(".")[0], None
                )
                if cmdline_tool_java_version:
                    raise AndroidToolError(
                        f"The android SDK command line tool requires java version {cmdline_tool_java_version} but the current version of java is {current_java_version}"
                    )
                else:
                    raise AndroidToolError(
                        f"The android SDK command line tool requires java that supports class version {java_file_version}, but the current version of java is {current_java_version}"
                    )
            elif (
                re.search(
                ...
                raise AndroidToolError(
                    f"The android SDK command line tool at {android_sdk_command_line_tool} is not located under a valid Android SDK root.\n"
                ...
                raise AndroidToolError(
                    f"An error occurred attempt to run the android command line tool:\n{result.stdout or result.stderr}"
                )
        else:
    ...
    def get_package_list(
        self, search_package_path: str, packageCategory: PackageCategory
    ) -> List:
        """
    ...
    def refresh_sdk_installation(self):
        ...
        def _factory_installed_package(package_map, item_components):
            package_map[item_components[0]] = AndroidSDKManager.InstalledPackage(
                item_components
            )

        def _factory_available_package(package_map, item_components):
            package_map[item_components[0]] = AndroidSDKManager.AvailablePackage(
                item_components
            )

        def _factory_available_update(package_map, item_components):
            package_map[item_components[0]] = AndroidSDKManager.AvailableUpdate(
                item_components
            )
        ...
        if result.returncode != 0:
            raise AndroidToolError(
                f"Error calling sdkmanager (arguments: '--list' error : {result.stderr or result.stdout}"
            )
    ...
    def install_package(self, package_install_path: str, package_description: str):
        ...
        package_result_list = self.get_package_list(
            package_install_path, AndroidSDKManager.PackageCategory.INSTALLED
        )
        if package_result_list:
            installed_package_detail = package_result_list[0]
            logger.info(
                f"{installed_package_detail.description} (version {installed_package_detail.version}) Detected"
            )
            return installed_package_detail

        # Make sure the package name is available
        package_result_list = self.get_package_list(
            package_install_path, AndroidSDKManager.PackageCategory.AVAILABLE
        )
        if not package_result_list:
            raise AndroidToolError(
                f"Invalid Android SDK Package {package_description}: Bad package path {package_install_path}"
            )
        ...
        available_package_to_install = package_result_list[
            0
        ]  # For multiple hits, resolve to the first item which will be the latest version
        ...
        if package_result_list:
            installed_package_detail = package_result_list[0]
            logger.info(
                f"{installed_package_detail.description} (version {installed_package_detail.version}) Installed"
            )
            return installed_package_detail
        else:
            raise AndroidToolError(
                f"Unable to verify package at {available_package_to_install.path}"
            )

    LICENSE_NOT_ACCEPTED_REGEX = re.compile(r"(^\d of \d.*$)", re.MULTILINE)

    LICENSE_ACCEPTED_REGEX = re.compile(
        r"(^All SDK package licenses accepted\.$)", re.MULTILINE
    )
    ...
    def check_licenses(self):
        ...
        result = subprocess.run(
            f"echo 'Y' | \"{self._android_command_line_tools_sdkmanager_path}\" --licenses",
            shell=True,
            capture_output=True,
            encoding=DEFAULT_READ_ENCODING,
            errors=ENCODING_ERROR_HANDLINGS,
        ...
        license_not_accepted_match = (
            AndroidSDKManager.LICENSE_NOT_ACCEPTED_REGEX.search(
                result.stdout or result.stderr
            )
        )
        if license_not_accepted_match:
            raise AndroidToolError(
                f"{license_not_accepted_match.group(1)}\n"
                f"Please run '{self._android_command_line_tools_sdkmanager_path} --licenses' and follow the instructions.\n"
            )
        license_accepted_match = AndroidSDKManager.LICENSE_ACCEPTED_REGEX.search(
            result.stdout or result.stderr
        )
        if license_accepted_match:
            ...
            raise AndroidToolError(
                "Unable to determine the Android SDK Package license state. \n"
...
def read_android_settings_for_project(project_path: Path) -> Tuple[dict, dict]:
    ...
    if not project_path.is_dir():
        raise AndroidToolError(
            f"Invalid project path {project_path}. Path does not exist or is a file."
        )
    ...
    if not project_json_path.is_file():
        raise AndroidToolError(
            f"Invalid project path {project_path}. Path does not contain 'project.json."
        )
    project_json_content = project_json_path.read_text(
        encoding=DEFAULT_READ_ENCODING, errors=ENCODING_ERROR_HANDLINGS
    )
    ...
    android_project_json_file = (
        project_path / "Platform" / "Android" / "android_project.json"
    )
    if android_project_json_file.is_file():
        android_project_json_content = android_project_json_file.read_text(
            encoding=DEFAULT_READ_ENCODING, errors=ENCODING_ERROR_HANDLINGS
        )
        ...
        if android_settings is None:
            raise AndroidToolError(
                f"Missing android settings in file {android_project_json_file}"
            )
        ...
        if android_settings is None:
            raise AndroidToolError(
                f"Missing android settings file {android_project_json_file} and cannot located legacy "
...
class AndroidSigningConfig(object):
    ...
    def __init__(
        self, store_file: Path, store_password: str, key_alias: str, key_password: str
    ):
        """
    ...
    def to_template_string(self, tabs) -> str:
        ...
        return (
            f"{tab_prefix}storeFile file('{self._store_file.as_posix()}')\n"
...
JAVA_VERSION_REGEX = re.compile(
    r".*(\w)\s(version)\s*\"?(?P<version>[\d\_\.]+)", re.MULTILINE
)
...
def validate_java_environment() -> str:
    ...
    if java_home:
        ...
        if not os.path.isfile(java_exe_path):
            raise AndroidToolError(
                f"JAVA_HOME is set to an invalid location ({java_home}). There is no java executable located in that path."
            )
    else:
        java_exe_working_dir = None

    result = subprocess.run(
        [f"java{EXE_EXTENSION}", "-version"],
        shell=(platform.system() == "Windows"),
        capture_output=True,
        encoding=DEFAULT_READ_ENCODING,
        errors=ENCODING_ERROR_HANDLINGS,
    ...
    if result.returncode != 0:
        if java_exe_working_dir:
            raise AndroidToolError(
                f"Unable to determine java version from {java_exe_working_dir} ({result.stderr or result.stdout})"
            )
        else:
            raise AndroidToolError(
                f"Unable to locate java. Either set it in the PATH environment or set JAVA_HOME to a valid java installation. ({result.stderr or result.stdout})"
            )

    java_version_match = JAVA_VERSION_REGEX.search(result.stdout or result.stderr)
    if java_version_match is None:
        raise AndroidToolError("Unable to determine java version")
...
def validate_build_tool(
    tool_name: str,
    tool_command: str,
    tool_config_key: str or None,
    tool_environment_var: str,
    tool_config_sub_path: str or None,
    ...
    if tool_config_key:
        ...
        if config_home:
            tool_home_and_src_list.append(
                (config_home, f"configration {tool_config_key}")
            )
    if tool_environment_var:
        env_home = os.getenv(tool_environment_var)
        if env_home:
            tool_home_and_src_list.append(
                (env_home, f"environment variable {tool_environment_var}")
            )
    ...
    for tool_home, tool_home_src in tool_home_and_src_list:
        if tool_home is not None:
            tool_test_command = os.path.join(
                tool_home, tool_config_sub_path, tool_command
            )
        else:
        ...
        try:
            result = subprocess.run(
                [tool_test_command, tool_version_arg],
                shell=(platform.system() == "Windows"),
                capture_output=True,
                encoding=DEFAULT_READ_ENCODING,
                errors=ENCODING_ERROR_HANDLINGS,
            ...
            if result.returncode == 0:
                if tool_home:
                    tool_full_path = (
                        Path(tool_home) / tool_config_sub_path / tool_command
                    )
                else:
                ...
                logger.warning(
                    f"Unable to resolve tool {tool_name} from {tool_home_src}"
                )
    ...
    if not tool_located:
        error_msgs = [
            f"Unable to resolve {tool_name}. Make sure its installed and in the PATH environment"
        ]
        if tool_config_key:
    ...
    if tool_version_match is None:
        raise AndroidToolError(
            f"Unable to determine {tool_name} version: {result.stdout or result.stderr}"
        )
...
class AndroidProjectManifestEnvironment(object):
    ...
    def __init__(
        ...
        android_settings: dict,
        android_gradle_plugin_version: Version,
        ...
        if not project_name:
            raise AndroidToolError(
                f"Invalid project settings for project at '{project_path}'. Missing required 'project_name' key"
            )
        ...
        if not package_name:
            raise AndroidToolError(
                f"Invalid android settings for project at '{project_path}'. Missing required 'package_name' key in the android settings."
            )
        ...
        self.internal_dict = {
            ...
            "ANDROID_LAUNCHER_NAME": ANDROID_LAUNCHER_NAME_PATTERN.format(
            ...
            "ANDROID_APP_OBFUSCATOR_SALT": android_settings.get(
                "app_obfuscator_salt", ""
            ),
            ...
            "ANDROID_ENABLE_KEEP_SCREEN_ON": android_settings.get(
                "enable_keep_screen_on", "false"
            ),
            "ANDROID_DISABLE_IMMERSIVE_MODE": android_settings.get(
                "disable_immersive_mode", "false"
            ),
    ...
    def process_android_multi_window_options(game_project_android_settings):
        ...
        def is_number_option_valid(value, name):
            if value:
                if isinstance(value, int):
                    ...
                    logging.warning(
                        '[WARN] Invalid value for property "%s", expected whole number',
                        name,
                    )
            return False
        ...
        multi_window_settings = game_project_android_settings.get(
            "multi_window_options", None
        )
        if not multi_window_settings:
            ...
            multi_window_options["ORIENTATION"] = ORIENTATION_MAPPING.get(
                requested_orientation, ORIENTATION_ALL
            )
        ...
        if samsung_dex_options:
            launch_in_fullscreen = samsung_dex_options.get(
                "launch_in_fullscreen", False
            )
            ...
            if launch_width and launch_height:
                ...
                if keep_alive in (True, False):
                    multi_window_options["SAMSUNG_DEX_KEEP_ALIVE"] = (
                        f"<meta-data "
        ...
        multi_window_options["ANDROID_CONFIG_CHANGES"] = "|".join(
            DEFAULT_CONFIG_CHANGES + ["density"]
        )

        # if targeting above the min API level the default value for this attribute is true so we need to explicitly disable it
        multi_window_options["ANDROID_MULTI_WINDOW"] = (
            f'android:resizeableActivity="{str(multi_window_enabled).lower()}"'
        )
...
class AndroidProjectGenerator(object):
    ...
    def __init__(
        ...
        self._android_project_builder_path = (
            self._engine_root / "Code/Tools/Android/ProjectBuilder"
        )
    ...
    def execute(self):
        ...
        if self._asset_mode == ASSET_MODE_PAK:
            ...
            if not src_pak_file_full_path.is_dir():
                logger.warning(
                    f"Android PAK files are expected at location {src_pak_file_full_path}, but the folder doesnt exist. Make sure "
                ...
                if pak_count == 0:
                    logger.warning(
                        f"Android PAK files are expected at location {src_pak_file_full_path}, but none was detected. Make sure "
                        "to create release bundles (PAK files) before building and deploying to an android device. Refer to "
                        "https://www.docs.o3de.org/docs/user-guide/packaging/asset-bundler/bundle-assets-for-release/ for more "
                        "information."
                    )
        ...
        self.create_file_from_project_template(
            src_template_file="root.build.gradle.in",
        ...
        logger.info(
            f"Android project scripts written to '{self._build_dir.absolute()}'."
        )

    def create_file_from_project_template(
        self, src_template_file, template_env, dst_file
    ):
        """
        ...
        if not dst_file.exists() or self._overwrite_existing:
            default_local_properties_content = utils.load_template_file(
                template_file_path=src_template_file_path, template_env=template_env
            )

            dst_file.write_text(
                default_local_properties_content,
    ...
    def prepare_gradle_wrapper(self):
        ...
        proc_result = subprocess.run(
            gradle_wrapper_cmd, shell=(platform.system() == "Windows")
        )
        if proc_result.returncode != 0:
            raise AndroidToolError(
                f"Gradle was unable to generate a gradle wrapper for this project (code {proc_result.returncode}): {proc_result.stderr or ''}"
            )
    ...
    def create_platform_settings(self):
        ...
        """
        platform_settings_content = PLATFORM_SETTINGS_FORMAT.format(
    ...
    def create_default_local_properties(self):
        ...
        local_properties_env = {
            ...
            "CMAKE_DIR_LINE": f"cmake.dir={template_cmake_path}"
            if template_cmake_path
            else "",
        }

        self.create_file_from_project_template(
            src_template_file="local.properties.in",
    ...
    def patch_and_transfer_android_libs(self):
        ...
        android_libraries_template_json_path = (
            self._android_project_builder_path / ANDROID_LIBRARIES_JSON_FILE
        )

        android_libraries_template_json_content = (
            android_libraries_template_json_path.resolve(strict=True).read_text(
                encoding=DEFAULT_READ_ENCODING, errors=ENCODING_ERROR_HANDLINGS
            )
        )

        android_libraries_json_content = string.Template(
            android_libraries_template_json_content
        ...
        for libName, value in android_libraries_json.items():
            ...
            for path in value["srcDir"]:
                path = string.Template(path).substitute(
                    self._android_replacement_map_env
                )
            ...
            if not src_dir:
                failed_paths = ", ".join(
                    string.Template(path).substitute(self._android_replacement_map_env)
                    for path in value["srcDir"]
                )
                raise AndroidToolError(
                    f"Failed to find library - {libName} - in path(s) [{failed_paths}]. Please download the "
                    "library from the Android SDK Manager and run this command again."
                )

            if "patches" in value:
                lib_to_patch = self._Library(
                    name=libName,
                    path=src_dir,
                    ...
                    for change in patch["changes"]:
                        ...
                        for oldLine in old_lines[:-1]:
                            change = self._Change(
                                line_num,
                                oldLine,
                                (new_lines.pop() if new_lines else None),
                            )
        ...
        for lib in libs_to_patch:
            lib.process_patch_lib(
                android_project_builder_path=self._android_project_builder_path,
    ...
    def create_lumberyard_app(self, project_dependencies):
        ...
        gradle_project_dependencies = [
            f"    api project(path: ':{project_dependency}')"
        ...
        template_ndk_path = (
            (self._android_sdk_path / self._android_ndk.location).resolve().as_posix()
        )
        ...
        absolute_cmakelist_path = (
            (self._engine_root / "CMakeLists.txt").resolve().as_posix()
        )
        absolute_azandroid_path = (
            (self._engine_root / "Code/Framework/AzAndroid/java").resolve().as_posix()
        )

        gradle_build_env["TARGET_TYPE"] = "application"
        gradle_build_env["PROJECT_DEPENDENCIES"] = (
            PROJECT_DEPENDENCIES_VALUE_FORMAT.format(
                dependencies="\n".join(gradle_project_dependencies)
            )
        )
        gradle_build_env["NATIVE_CMAKE_SECTION_ANDROID"] = (
            NATIVE_CMAKE_SECTION_ANDROID_FORMAT.format(
                cmake_version=str(self._cmake_version),
                native_build_path=native_build_path,
                absolute_cmakelist_path=absolute_cmakelist_path,
            )
        )
        gradle_build_env["NATIVE_CMAKE_SECTION_DEFAULT_CONFIG"] = (
            NATIVE_CMAKE_SECTION_DEFAULT_CONFIG_NDK_FORMAT_STR.format(abi=ANDROID_ARCH)
        )

        gradle_build_env["OVERRIDE_JAVA_SOURCESET"] = (
            OVERRIDE_JAVA_SOURCESET_STR.format(
        ...
        for native_config in BUILD_CONFIGURATIONS:
            ...
            if self._ninja_path:
                cmake_argument_list.append(
                    f'"-DCMAKE_MAKE_PROGRAM={self._ninja_path.as_posix()}"'
                )
            ...
            if self._extra_cmake_configure_args:
                ...
                extra_cmake_configure_arg_list = [
                    f'"{arg}"'
                    for arg in re.split(
                        r"""\s(?=(?:[^'"]|'[^']*'|"[^"]*")*$)""", extra_cmake_args
                    )
                ]
            ...
            gradle_build_env[f"NATIVE_CMAKE_SECTION_{native_config_upper}_CONFIG"] = (
                NATIVE_CMAKE_SECTION_BUILD_TYPE_CONFIG_FORMAT_STR.format(
            ...
            gradle_build_env[
                f"CUSTOM_GRADLE_COPY_NATIVE_{native_config_upper}_LIB_TASK"
            ] = ""
            ...
            sync_layout_command_line_source = [
                f"{python_full_path.resolve().as_posix()}",
                "android_post_build.py",
                az_android_dst_path.resolve().as_posix(),  # android_app_root
                "--project-root",
                self._project_path.as_posix(),
                "--gradle-version",
                self._gradle_version,
                "--asset-mode",
                self._asset_mode,
                "--asset-bundle-folder",
                self._src_pak_file_path,
            ]

            sync_layout_command_line = ",".join(
                [f"'{arg}'" for arg in sync_layout_command_line_source]
            )

            gradle_build_env[
                f"CUSTOM_APPLY_ASSET_LAYOUT_{native_config_upper}_TASK"
            ] = CUSTOM_APPLY_ASSET_LAYOUT_TASK_FORMAT_STR.format(
                working_dir=(self._engine_root / "cmake/Tools/Platform/Android")
                .resolve()
                .as_posix(),
            ...
            gradle_build_env[f"SIGNING_{native_config_upper}_CONFIG"] = (
                f"signingConfig signingConfigs.{native_config_lower}"
...
"""
        ...
        self.create_file_from_project_template(
            src_template_file="build.gradle.in",
        ...
        az_android_package_env = AndroidProjectManifestEnvironment(
            project_path=self._project_path,
        ...
        self.create_file_from_project_template(
            src_template_file=ANDROID_MANIFEST_FILE,
        ...
        self.apply_android_builder_rules(
            az_android_dst_path=az_android_dst_path,
            az_android_package_env=az_android_package_env,
        )

        self.resolve_icon_overrides(
            az_android_dst_path=az_android_dst_path,
            az_android_package_env=az_android_package_env,
        )

        self.resolve_splash_overrides(
            az_android_dst_path=az_android_dst_path,
            az_android_package_env=az_android_package_env,
        )
    ...
    def write_settings_gradle(self, project_list):
        ...
        settings_gradle_lines = [
            f"include ':{project_name}'" for project_name in project_list
        ]
        settings_gradle_content = "\n".join(settings_gradle_lines)
        settings_gradle_file = self._build_dir / "settings.gradle"
        settings_gradle_file.write_text(
            settings_gradle_content,
            encoding=DEFAULT_READ_ENCODING,
            errors=ENCODING_ERROR_HANDLINGS,
        )
        logger.info(
            "Generated settings.gradle -> %s", str(settings_gradle_file.resolve())
        )
        ...
        grade_properties_env = {"GRADLE_JVM_ARGS": self._gradle_custom_jvm_args}
        gradle_properties_file = self._build_dir / "gradle.properties"
        self.create_file_from_project_template(
            src_template_file="gradle.properties.in",
            template_env=grade_properties_env,
            dst_file=gradle_properties_file,
        )
        logger.info(
            "Generated gradle.properties -> %s", str(gradle_properties_file.resolve())
        )
    ...
    def apply_android_builder_rules(self, az_android_dst_path, az_android_package_env):
        ...
        android_builder_json_path = (
            self._android_project_builder_path / "android_builder.json"
        )
        android_builder_json_content = utils.load_template_file(
            template_file_path=android_builder_json_path,
            template_env=az_android_package_env,
        )
        ...
        def _copy(src_file, dst_path, dst_is_directory):
            ...
            if puremagic.what(resolved_src) in (
                "rgb",
                "gif",
                "pbm",
                "ppm",
                ...
                logging.debug(
                    "Copy Binary file %s -> %s",
                    str(src_path.resolve(strict=True)),
                ...
                project_activity_for_game_content = utils.load_template_file(
                    template_file_path=src_path, template_env=az_android_package_env
                )
                dst_file.write_text(project_activity_for_game_content)
                logging.debug(
                    "Copy/Update file %s -> %s",
                    str(src_path.resolve(strict=True)),
                    str(dst_path.resolve()),
                )
        ...
        def _process_dict(node, dst_path):
            ...
            assert isinstance(node, dict), (
                f"Node for {android_builder_json_path} expected to be a dictionary"
            )
            ...
            for key, value in node.items():
                if isinstance(value, str):
                    ...
                    for item in value:
                        assert isinstance(node, dict), (
                            f"Unexpected type found in '{android_builder_json_path}'.  Only lists of strings are supported"
                        )
                    ...
                    assert False, (
                        f"Unexpected type '{type(value)}' found in '{android_builder_json_path}'. Only str, list, and dict is supported"
                    )
    ...
    def construct_source_resource_path(self, source_path):
        ...
        raise AndroidToolError(
            f'Unable to locate resources folder for project at path "{self._project_path}"'
        )
    ...
    def resolve_icon_overrides(self, az_android_dst_path, az_android_package_env):
        ...
        if default_icon is not None:
            ...
            shutil.copyfile(
                src_default_icon_file.resolve(), dst_default_icon_file.resolve()
            )
            os.chmod(dst_default_icon_file.resolve(), stat.S_IWRITE | stat.S_IREAD)
        else:
            logging.debug(
                f"No default icon override specified for project_at path {self._project_path}"
            )
        ...
        for resolution in ANDROID_RESOLUTION_SETTINGS:
            ...
            if icon_source is default_icon:
                ...
                if icon_source is None:
                    warnings.append(
                        f'No icon override found for "{resolution}".  Either supply one for "{resolution}" or a '
                ...
                # if only the resolution is unspecified, remove the resolution specific version from the project
                    ...
                    logging.debug(
                        f'Default icon being used for "{resolution}" in {self._project_path}',
                        resolution,
                    )
    ...
    def resolve_splash_overrides(self, az_android_dst_path, az_android_package_env):
        ...
        for orientation_flag, orientation_key in ORIENTATION_FLAG_TO_KEY_MAP.items():
            ...
            if (orientation & orientation_flag) == 0:
                unused_override_warning = (
                    f'Splash screen overrides specified for "{orientation_key}" when desired orientation '
            ...
            if default_splash_img is not None:
                ...
                src_default_splash_img_file = self.construct_source_resource_path(
                    default_splash_img
                )
                ...
                dst_default_splash_img_file = (
                    dst_default_splash_img_dir / APP_SPLASH_NAME
                )

                shutil.copyfile(
                    src_default_splash_img_file.resolve(),
                    dst_default_splash_img_file.resolve(),
                )
                os.chmod(
                    dst_default_splash_img_file.resolve(), stat.S_IWRITE | stat.S_IREAD
                )
            else:
                logging.debug(
                    'No default splash screen override specified for "%s" orientation in %s',
                    orientation_key,
                    self._project_path,
                )
            ...
            for resolution in valid_resolutions:
                target_directory = (
                    dst_resource_path / f"{orientation_path_prefix}-{resolution}"
                )

                # get the current resolution splash image override
                splash_img_source = oriented_splashes.get(
                    resolution, default_splash_img
                )
                if splash_img_source is default_splash_img:
                    ...
                    if splash_img_source is None:
                        ...
                        warnings.append(
                            f'No splash screen override found for "{section}".  Either supply one for "{resolution}" '
                        ...
                        # if only the resolution is unspecified, remove the resolution specific version from the project
                        logging.debug(
                            f'Default splash screen being used for "{orientation_key}-{resolution}" in {self._project_path}'
                        )
                        utils.remove_dir_path(target_directory)
                    continue
                src_splash_img_file = self.construct_source_resource_path(
                    splash_img_source
                )
                dst_splash_img_file = target_directory / APP_SPLASH_NAME

                shutil.copyfile(
                    src_splash_img_file.resolve(), dst_splash_img_file.resolve()
                )
    ...
    def clear_unused_assets(az_android_dst_path, az_android_package_env):
        ...
        clear_folders.extend(
            [
                f"{path_prefix}-{resolution}"
                for resolution in ANDROID_RESOLUTION_SETTINGS
    ...
    class _Library:
        ...
        def read_package_namespace(self):
            ...
            if not library_android_manifest_file.is_file():
                raise AndroidToolError(
                    f"Missing expected 'AndroidManifest.xml' file from the anroid library package {self.name}"
                )

            library_android_manifest = library_android_manifest_file.read_text(
                encoding=DEFAULT_READ_ENCODING, errors=ENCODING_ERROR_HANDLINGS
            )
            match_package_name = re.search(
                r'[\s]*package="([a-zA-Z\.]+)"', library_android_manifest, re.MULTILINE
            )
            if not match_package_name:
                raise AndroidToolError(
                    f"Error reading 'AndroidManifest.xml' file from the anroid library package {self.name}. Unable to locate 'package' attribute."
                )
        ...
        def process_patch_lib(self, android_project_builder_path, dest_root):
            ...
            if self.build_dependencies:
                gradle_dependencies.extend(
                    [
                        f"    api '{build_dependency}'"
                        for build_dependency in self.build_dependencies
                    ]
                )
            if self.dependencies:
                gradle_dependencies.extend(
                    [
                        f"    api project(path: ':{dependency}')"
            ...
            if self._gradle_plugin_version >= Version("7.0"):
                build_gradle_env["PROJECT_NAMESPACE_OPTION"] = (
                    f'namespace "{name_space}"'
                    if self._gradle_plugin_version >= Version("7.0")
                    else ""
                )

            build_gradle_content = utils.load_template_file(
                template_file_path=android_project_builder_path / "build.gradle.in",
            ...
            if not dest_gradle_script_file.exists() or self.overwrite_existing:
                dest_gradle_script_file.write_text(
            ...
            # copying
            library_copy_subfolder_pairs = [("res", "res"), ("src", "java")]
            ...
            for file in self.patch_files:
                ...
                with open(output_file_path.resolve(), "w") as outFile:
                    for replace in file.changes:
                        lines[replace.line] = str.replace(
                            lines[replace.line],
                            replace.old,
                            (replace.new if replace.new else ""),
                            1,
                        )
