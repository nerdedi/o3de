...
def load_o3de_manifest(manifest_path: pathlib.Path = None) -> dict:
    ...
    if not manifest_path:
        ...
        if not manifest_path.is_file():
            logger.info(
                f"A manifest path of None has been supplied and the {manifest_path} does not exist."
    ...
    with manifest_path.open("r") as f:
        try:
            ...
            logger.error(
                f'Manifest json failed to load at path "{manifest_path}": {str(e)}'
            )
...
def get_manifest_engines() -> list:
    ...
    return list(
        map(
            lambda engine_object: engine_object.get("path", "")
...
def get_manifest_external_subdirectories() -> list:
    json_data = load_o3de_manifest()
    return (
        json_data["external_subdirectories"]
        if "external_subdirectories" in json_data
        else []
    )
...
def get_engine_projects(engine_path: pathlib.Path = None) -> list:
    ...
    if engine_object:
        return (
            list(
                map(
                    lambda rel_path: (pathlib.Path(engine_path) / rel_path).as_posix(),
                    engine_object["projects"],
                )
            )
            if "projects" in engine_object
            else []
        )
    return []
...
def get_engine_gems(
    ...
    recurse: bool = False,
    gems_json_data_by_path: dict = None,
) -> list:
    """
    ...
    """
    engine_gem_paths = get_gems_from_external_subdirectories(
        get_engine_external_subdirectories(engine_path)
    )
    if recurse:
        for path in engine_gem_paths:
            engine_gem_paths.extend(
                get_gem_external_subdirectories(path, list(), gems_json_data_by_path)
            )
...
def get_engine_external_subdirectories(engine_path: pathlib.Path = None) -> list:
    ...
    if engine_object:
        return (
            ...
            if "external_subdirectories" in engine_object
            else []
        )
    return []
...
def is_sdk_engine(
    engine_name: str = None, engine_path: str or pathlib.Path = None
) -> bool:
    """
    ...
    if engine_path:
        logger.warning(
            "Failed to retrieve engine json data for the engine at "
            f'"{engine_path}". Treating engine as source engine.'
        )
    elif engine_name:
        logger.warning(
            "Failed to retrieve engine json data for registered engine "
            f'"{engine_name}". Treating engine as source engine.'
        )
    else:
        logger.warning(
            "Failed to retrieve engine json data as neither an engine name nor path were given."
...
def get_project_gems(
    ...
    """
    project_gem_paths = get_gems_from_external_subdirectories(
        get_project_external_subdirectories(project_path)
    )
    if recurse:
        for path in project_gem_paths:
            project_gem_paths.extend(
                get_gem_external_subdirectories(path, list(), gems_json_data_by_path)
            )
...
def get_enabled_gem_cmake_file(
    ...
    if not project_name and not project_path:
        logger.error("Must supply either a Project Name or Project Path.")
        return None
    ...
    if platform == "Common":
        ...
        for (
            possible_project_enable_gem_filename_path
        ) in possible_project_enable_gem_filename_paths:
        ...
        possible_project_platform_enable_gem_filename_paths = [
            pathlib.Path(
                project_path / "Gem/Platform" / platform / enable_gem_filename
            ),
            pathlib.Path(
                project_path / "Gem/Code/Platform" / platform / enable_gem_filename
            ),
            pathlib.Path(
                project_path / "Code/Platform" / platform / enable_gem_filename
            ),
        ]
        for (
            possible_project_platform_enable_gem_filename_path
        ) in possible_project_platform_enable_gem_filename_paths:
    ...
    with cmake_file.open("r") as s:
        ...
        for line in s:
            ...
            if in_gem_list:
                ...
                gem_name_list = list(
                    map(lambda gem_name: gem_name.strip('"'), line.split())
                )
...
def get_project_enabled_gems(
    project_path: pathlib.Path, include_dependencies: bool = True
) -> dict or None:
    """
    ...
    if not project_json_data:
        logger.error(
            f"Failed to get project json data for the project at '{project_path}'"
        )
        return None
    ...
    gem_names_with_optional_gems = utils.get_gem_names_set(
        active_gem_names, include_optional=True
    )
    ...
    if not engine_path:
        ...
        if not engine_path:
            logger.error(
                "Failed to find an engine path for the project at "
        ...
        if project_path not in FALLBACK_ENGINE_PROJECT_PATHS_WARNINGS:
            ...
            logger.warning(
                "Failed to determine the correct engine for the project at "
    ...
    all_gems_json_data = get_gems_json_data_by_name(
    ...
    for gem_name_with_specifier in gem_names_with_optional_gems:
        gem_name_only, _ = utils.get_object_name_and_optional_version_specifier(
            gem_name_with_specifier
        )
        gem_names_with_version_specifiers[gem_name_only] = gem_name_with_specifier

    # Try to resolve with optional gems
    resolved_gems, errors = compatibility.resolve_gem_dependencies(
        gem_names_with_optional_gems,
        all_gems_json_data,
        engine_json_data,
    ...
    if errors:
        ...
        gem_names_without_optional = utils.get_gem_names_set(
            active_gem_names, include_optional=False
        )
        resolved_gems, errors = compatibility.resolve_gem_dependencies(
            gem_names_without_optional,
            all_gems_json_data,
            engine_json_data,
    ...
    if not errors:
        for _, gem in resolved_gems.items():
            ...
            gem_name_with_specifier = gem_names_with_version_specifiers.get(
                gem_name, gem_name
            )
            if gem_name_with_specifier in result or include_dependencies:
                result[gem_name_with_specifier] = (
                    gem.gem_json_data["path"].as_posix()
                    if gem.gem_json_data["path"]
                    else None
                )
    else:
...
def get_project_external_subdirectories(project_path: pathlib.Path) -> list:
    ...
    if project_object:
        return (
            list(
                map(
                    lambda rel_path: (pathlib.Path(project_path) / rel_path).as_posix(),
                    project_object["external_subdirectories"],
                )
            )
            if "external_subdirectories" in project_object
            else []
        )
    return []


def get_project_engine_path(
    project_path: pathlib.Path,
    project_json_data: dict = None,
    user_project_json_data: dict = None,
    engines_json_data: dict = None,
) -> pathlib.Path or None:
    """
    ...
    """
    engine_path = compatibility.get_most_compatible_project_engine_path(
        project_path, project_json_data, user_project_json_data, engines_json_data
    )
    if engine_path:
        return engine_path
    ...
    engine_path = utils.find_ancestor_dir_containing_file(
        pathlib.PurePath("engine.json"), resolved_project_path
    )
    if engine_path:
        projects = get_engine_projects(engine_path)
        for engine_project_path in projects:
            if resolved_project_path.samefile(
                pathlib.Path(engine_project_path).resolve()
            ):
                return engine_path
...
def get_project_templates(project_path: pathlib.Path) -> list:
    ...
    if project_object:
        return (
            ...
            if "templates" in project_object
            else []
        )
    return []
...
def get_gem_gems(gem_path: pathlib.Path) -> list:
    return get_gems_from_external_subdirectories(
        get_gem_external_subdirectories(gem_path, list(), dict())
    )


def get_gem_external_subdirectories(
    gem_path: pathlib.Path, visited_gem_paths: list, gems_json_data_by_path: dict = None
) -> list:
    """
    ...
    if gem_path in visited_gem_paths:
        logger.warning(
            f'A cycle has been detected when visiting external subdirectories at gem path "{gem_path}". The visited paths are: {visited_gem_paths}'
        )
        return []
    ...
    if gem_object:
        external_subdirectories = (
            list(
                map(
                    lambda rel_path: (pathlib.Path(gem_path) / rel_path)
                    .resolve()
                    .as_posix(),
                    gem_object["external_subdirectories"],
                )
            )
            if "external_subdirectories" in gem_object
            else []
        )
        ...
        for external_subdirectory in external_subdirectories:
            ...
            if gem_json_path.is_file():
                external_subdirectories.extend(
                    get_gem_external_subdirectories(
                        external_subdirectory, visited_gem_paths, gems_json_data_by_path
                    )
                )
...
def get_gem_templates(gem_path: pathlib.Path) -> list:
    ...
    if gem_object:
        return (
            list(
                map(
                    lambda rel_path: (pathlib.Path(gem_path) / rel_path).as_posix(),
                    gem_object["templates"],
                )
            )
            if "templates" in gem_object
            else []
        )
    return []
...
def get_all_gems(project_path: pathlib.Path = None) -> list:
    return get_gems_from_external_subdirectories(
        get_all_external_subdirectories(
            project_path=project_path, gems_json_data_by_path=dict()
        )
    )


def add_dependency_gem_names(
    gem_name: str, gems_json_data_by_name: dict, all_gem_names: set
):
    """
    ...
    if gem_json_data:
        ...
        for dependency_gem_name in dependencies:
            if dependency_gem_name not in all_gem_names:
                ...
                add_dependency_gem_names(
                    dependency_gem_name, gems_json_data_by_name, all_gem_names
                )


def remove_non_dependency_gem_json_data(
    gem_names: list, gems_json_data_by_name: dict
) -> None:
    """
    ...
    gem_names_to_remove = [
        gem_name
        for gem_name in gems_json_data_by_name
        if gem_name not in gem_names_to_keep
    ]
    for gem_name in gem_names_to_remove:
        del gems_json_data_by_name[gem_name]


def get_gems_json_data_by_path(
    engine_path: pathlib.Path = None,
    ...
    external_subdirectories = (
        list() if not external_subdirectories else external_subdirectories
    )
...
def get_gems_json_data_by_name(
    engine_path: pathlib.Path = None,
    project_path: pathlib.Path = None,
    include_manifest_gems: bool = False,
    include_engine_gems: bool = False,
    ...
    """
    all_gems_json_data = get_gems_json_data_by_path(
        engine_path,
        project_path,
    ...
    #     ],
    # }
    utils.replace_dict_keys_with_value_key(
        all_gems_json_data,
        value_key="gem_name",
        replaced_key_name="path",
        place_values_in_list=True,
    )

    return all_gems_json_data


def get_all_external_subdirectories(
    engine_path: pathlib.Path = None,
    project_path: pathlib.Path = None,
    gems_json_data_by_path: dict = None,
) -> list:
    ...
    if project_path:
        external_subdirectories_data.extend(
            get_project_external_subdirectories(project_path)
        )
    ...
    for gem_path in gem_paths:
        external_subdirectories_data.extend(
            get_gem_external_subdirectories(gem_path, list(), gems_json_data_by_path)
        )
...
def get_templates_for_generic_creation(project_path: pathlib.Path = None) -> list:
    ...
    for template_path in get_all_templates(project_path):
        ...
        gem_json_path = template_path / "Template" / "gem.json"
        project_json_path = template_path / "Template" / "project.json"
        if not validation.valid_o3de_gem_json(
            gem_json_path
        ) and not validation.valid_o3de_project_json(project_json_path):
...
def get_json_file_path(
    object_typename: str, object_path: str or pathlib.Path
) -> pathlib.Path:
...
def get_json_data_file(
    object_json: pathlib.Path, object_typename: str, object_validator: callable
) -> dict or None:
    ...
    if not object_json or not object_json.is_file():
        logger.error(
            f"Invalid {object_typename} json {object_json} supplied or file missing."
        )
        return None

    if not object_validator or not object_validator(object_json):
        logger.error(
            f"{object_typename} json {object_json} is not valid or could not be validated."
        )
        return None
...
def get_json_data(
    object_typename: str, object_path: str or pathlib.Path, object_validator: callable
) -> dict or None:
...
def get_project_json_data(
    ...
    if pathlib.Path(project_path).is_file():
        if user:
            ...
            return get_json_data_file(
                project_path, "project", validation.valid_o3de_project_json
            )
    elif user:
        ...
        if not user_project_json_path.exists():
            ...
            # skip validation because a user project.json is only for overrides and can be empty
            return (
                get_json_data("project", user_project_folder, validation.always_valid)
                or {}
            )
    else:
        return get_json_data(
            "project", project_path, validation.valid_o3de_project_json
        )


def get_gem_json_data(
    gem_name: str = None,
...
def get_template_json_data(
    ...
    if template_name and not template_path:
        template_path = get_registered(
            template_name=template_name, project_path=project_path
        )
    ...
    if pathlib.Path(template_path).is_file():
        return get_json_data_file(
            template_path, "template", validation.valid_o3de_template_json
        )
    else:
        return get_json_data(
            "template", template_path, validation.valid_o3de_template_json
        )
...
def get_restricted_json_data(
    ...
    if restricted_name and not restricted_path:
        restricted_path = get_registered(
            restricted_name=restricted_name, project_path=project_path
        )

    return get_json_data(
        "restricted", restricted_path, validation.valid_o3de_restricted_json
    )
...
def get_repo_path(
    repo_uri: str, cache_folder: str or pathlib.Path = None
) -> pathlib.Path:
...
def get_most_compatible_gem(
    gem_name: str, gem_json_data_by_name: dict or None
) -> pathlib.Path or None:
    """
    ...
    gem_name, version_specifier = utils.get_object_name_and_optional_version_specifier(
        gem_name
    )
    if gem_name not in gem_json_data_by_name:
        return None
    ...
    for gem_json_data in gem_json_data_by_name.get(gem_name, {}):
        if version_specifier:
            ...
            if compatibility.has_compatible_version(
                [gem_name_with_version_specifier], gem_name, candidate_version
            ):
...
def get_most_compatible_object(
    object_name: str, name_key: str, objects: list
) -> dict or None:
    """
    ...
    """
    most_compatible_version = Version("0.0.0")
    object_name, version_specifier = (
        utils.get_object_name_and_optional_version_specifier(object_name)
    )
    ...
    for json_data in objects:
        ...
        if version_specifier:
            if compatibility.has_compatible_version(
                [object_name + version_specifier], candidate_name, candidate_version
            ):
...
def get_most_compatible_object_path(
    object_name: str,
    object_typename: str,
...
def get_registered(
    ...
    if isinstance(engine_name, str):
        return get_most_compatible_object_path(
            engine_name,
            "engine",
            validation.valid_o3de_engine_json,
            "engine_name",
            get_manifest_engines(),
        )
        ...
        return get_most_compatible_object_path(
            gem_name, "gem", validation.valid_o3de_gem_json, "gem_name", gems
        )
        ...
        if default_folder == "engines":
            if "default_engines_folder" in json_data:
                default_engines_folder = pathlib.Path(
                    json_data["default_engines_folder"]
                )
            else:
                default_engines_folder = pathlib.Path(
                    get_default_o3de_manifest_json_data().get(
                        "default_engines_folder", None
                    )
                )
            ...
            if "default_projects_folder" in json_data:
                ...
                default_projects_folder = pathlib.Path(
                    get_default_o3de_manifest_json_data().get(
                        "default_projects_folder", None
                    )
                )
            return (
                default_projects_folder.resolve() if default_projects_folder else None
            )
            ...
            if "default_templates_folder" in json_data:
                ...
                default_templates_folder = pathlib.Path(
                    get_default_o3de_manifest_json_data().get(
                        "default_templates_folder", None
                    )
                )
            return (
                default_templates_folder.resolve() if default_templates_folder else None
            )
            ...
            if "default_restricted_folder" in json_data:
                ...
                default_restricted_folder = pathlib.Path(
                    get_default_o3de_manifest_json_data().get(
                        "default_restricted_folder", None
                    )
                )
            return (
                default_restricted_folder.resolve()
                if default_restricted_folder
                else None
            )
