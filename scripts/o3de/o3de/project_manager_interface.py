...
def register_project(project_path: pathlib.Path, force: bool) -> int:
    ...
    if result == 0:
        result = project_properties.edit_project_props(
...
def get_enabled_gems(project_path: str, include_dependencies: bool = True) -> list:
    ...
    """
    return manifest.get_project_enabled_gems(
        project_path=project_path, include_dependencies=include_dependencies
    )
...
def get_all_project_infos() -> list:
    ...
    for project_path in project_paths:
        ...
        if not project_json_data:
            continue
        user_project_json_data = manifest.get_project_json_data(
            project_path=project_path, user=True
        )
        if user_project_json_data:
            project_json_data.update(user_project_json_data)

        project_json_data["path"] = project_path
        project_json_data["engine_path"] = manifest.get_project_engine_path(
...
def get_project_engine_incompatible_objects(
    ...
    return compatibility.get_project_engine_incompatible_objects(
        project_path=project_path, engine_path=engine_path
    )


def get_incompatible_project_gems(
    gem_paths: list, gem_names: list, project_path: str
) -> set() or int:
    """
    ...
    if incompatible_objects:
        logger.error(
            "The following dependency issues were found:\n\n  ".join(
...
def add_gems_to_project(
    gem_paths: list, gem_names: list, project_path: str, force: bool = False
) -> int:
    """
    ...
    for index in range(len(gem_paths)):
        result = (
            enable_gem.enable_gem_in_project(
                gem_name=gem_names[index],
...
def get_all_gem_infos(project_path: pathlib.Path or None) -> list:
    ...
    if (
        project_path
        and utils.find_ancestor_dir_containing_file("template.json", project_path)
        is None
    ):
    ...
    all_gem_json_data = manifest.get_gems_json_data_by_path(
    ...
    engine_gem_paths = [
        ...
        for path in manifest.get_engine_gems(engine_path, recurse, all_gem_json_data)
    ]
    if project_path:
        project_gem_paths = [
            pathlib.PurePath(path)
            for path in manifest.get_project_gems(
                project_path, recurse, all_gem_json_data
            )
        ]
    ...
    all_gem_json_data_list = [
        gem_json_data
        for gem_versions in all_gem_json_data.values()
        for gem_json_data in gem_versions
    ]
    ...
    for i, gem_json_data in enumerate(all_gem_json_data_list):
        ...
        if gem_json_data["path"] in engine_gem_paths:
            ...
            incompatible_engine = compatibility.get_incompatible_objects_for_engine(
                gem_json_data, engine_json_data
            )
            if incompatible_engine:
                all_gem_json_data_list[i]["incompatible_engine"] = incompatible_engine

            incompatible_gem_dependencies = (
                compatibility.get_incompatible_gem_dependencies(
                    gem_json_data, all_gem_json_data
                )
            )
            if incompatible_gem_dependencies:
                all_gem_json_data_list[i]["incompatible_gem_dependencies"] = (
                    incompatible_gem_dependencies
                )
...
def get_gem_infos_from_all_repos(
    project_path: pathlib.Path = None, enabled_only: bool = True
) -> list:
    """
    ...
    for i, gem_json_data in enumerate(remote_gem_json_data_list):
        ...
        if incompatible_engine:
            remote_gem_json_data_list[i]["incompatible_engine_dependencies"] = (
                incompatible_engine
            )

        incompatible_gem_dependencies = compatibility.get_incompatible_gem_dependencies(
            gem_json_data, all_gem_json_data
        )
        if incompatible_gem_dependencies:
            remote_gem_json_data_list[i]["incompatible_gem_dependencies"] = (
                incompatible_gem_dependencies
            )
