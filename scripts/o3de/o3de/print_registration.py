...
def get_project_path(project_path: pathlib.Path, project_name: str) -> pathlib.Path:
    if not project_name and not project_path:
        logger.error("Must either specify a Project path or Project Name.")
        return None
    ...
    if not project_path:
        logger.error(
            f"Unable to locate project path from the registered manifest json files:"
...
def print_this_engine(verbose: int = 0) -> int:
    ...
    if verbose > 0:
        return print_manifest_json_data(
            [this_engine_path],
            "This Engine",
            manifest.get_engine_json_data,
            "engine_path",
        )
    return 0
...
def print_engines(verbose: int = 0) -> int:
    ...
    if verbose > 0:
        return print_manifest_json_data(
            engines_data, "Engine Jsons", manifest.get_engine_json_data, "engine_path"
        )
    return 0
...
def print_projects(verbose: int = 0) -> int:
    ...
    if verbose > 0:
        return print_manifest_json_data(
            projects_data,
            "Project Jsons",
            manifest.get_project_json_data,
            "project_path",
        )
    return 0
...
def print_gems(verbose: int = 0) -> int:
    ...
    if verbose > 0:
        return print_manifest_json_data(
            gems_data, "Gem Jsons", manifest.get_gem_json_data, "gem_path"
        )
    return 0
...
def print_templates(verbose: int) -> int:
    ...
    if verbose > 0:
        return print_manifest_json_data(
            templates_data,
            "Template Jsons",
            manifest.get_template_json_data,
            "template_path",
        )
    return 0
...
def print_restricted(verbose: int) -> int:
    ...
    if verbose > 0:
        return print_manifest_json_data(
            restricted_data,
            "Restricted Jsons",
            manifest.get_restricted_json_data,
            "restricted_path",
        )
    return 0
...
def print_engine_gems(verbose: int) -> int:
    ...
    if verbose > 0:
        return print_manifest_json_data(
            engine_gems_data, "Gem Jsons", manifest.get_gem_json_data, "gem_path"
        )
    return 0
...
def print_project_gems(
    verbose: int, project_path: pathlib.Path, project_name: str
) -> int:
    ...
    if verbose > 0:
        return print_manifest_json_data(
            project_gems_data, "Gems Jsons", manifest.get_gem_json_data, "gem_path"
        )
    return 0


def print_project_external_subdirectories(
    verbose: int, project_path: pathlib.Path, project_name: str
) -> int:
    project_path = get_project_path(project_path, project_name)
    if not project_path:
...
def print_all_gems(
    verbose: int, project_path: pathlib.Path = None, project_name: str = None
) -> int:
    ...
    # all projects
    project_path = (
        get_project_path(project_path, project_name)
    ...
    if verbose > 0:
        return print_manifest_json_data(
            all_gems, "Gem Jsons", manifest.get_gem_json_data, "gem_path"
        )
    return 0


def print_all_external_subdirectories(
    verbose: int, project_path: pathlib.Path = None, project_name: str = None
) -> int:
...
def print_manifest_json_data(
    uri_json_data: list,
    print_prefix: str,
    get_json_func: callable,
    get_json_data_kw: str,
) -> int:
    print("\n")
    print(f"{print_prefix}================================================")
    for manifest_uri in uri_json_data:
        # if it's not local it should be in the cache
        manifest_json_path, parsed_uri = repo.get_cache_file_uri(
            pathlib.Path(manifest_uri).as_posix()
        )
...
def _run_register_show(args: argparse) -> int:
    if args.this_engine:
        ...
        return print_project_external_subdirectories(
            args.verbose, args.project_path, args.project_name
        )
    elif args.project_templates:
        return print_project_templates(
            args.verbose, args.project_path, args.project_name
        )
    elif args.project_engine_name:
        return print_project_engine_name(
            args.verbose, args.project_path, args.project_name
        )
...
def add_parser_args(parser):
    ...
    group.add_argument(
        "-ees",
        "--engine-external-subdirectories",
        action="store_true",
        required=False,
        default=False,
    ...
    project_group.add_argument(
        "-pp", "--project-path", type=pathlib.Path, help="The path to a project."
    )
    project_group.add_argument(
        "-pn", "--project-name", type=str, help="The name of a project."
    )
