...
cpp_file_ext = {".cpp", ".h", ".hpp", ".hxx", ".inl"}

expect_license_info_ext = {".cpp", ".h", ".hpp", ".inl", ".py", ".cmake"}

restricted_platforms = {"Provo", "Salem", "Jasper", "Paris", "Xenia", "Lancaster"}

O3DE_LICENSE_TEXT = """'# {BEGIN_LICENSE}
# Copyright (c) Contributors to the Open 3D Engine Project.
# For complete copyright and license terms please see the LICENSE at the root of this distribution.
#
# SPDX-License-Identifier: Apache-2.0 OR MIT
...
def _remove_license_text_markers(source_data: str):
    ...
    while license_block_start_index != -1:
        ...
        license_block_end_index = source_data.find(
            end_marker, license_block_start_index
        )
        ...
        source_data = (
            source_data[:pre_license_block_end]
    ...
    for create_directory in json_data["createDirectories"]:
        ...
        if not keep_restricted_in_instance and "Platform" in new_dir.parts:
            try:
                ...
                found_platform = re.search(pattern, new_dir.as_posix()).group(
                    "Platform"
                )
    ...
    for copy_file in json_data["copyFiles"]:
        ...
        if not keep_restricted_in_instance and "Platform" in out_file.parts:
            try:
                # the name of the Platform should follow the '/Platform/'
                pattern = r"/Platform/(?P<Platform>[^/:*?\"<>|\r\n]+/?)"
                found_platform = re.search(pattern, out_file.as_posix()).group(
                    "Platform"
                )
                found_platform = found_platform.replace("/", "")
                if found_platform in restricted_platforms:
...
def _execute_restricted_template_json(
    ...
    for copy_file in template_json_data["copyFiles"]:
        ...
        if not keep_restricted_in_instance and "Platform" in new_file.parts:
            try:
                # the name of the Platform should follow the '/Platform/'
                pattern = r"/Platform/(?P<Platform>[^/:*?\"<>|\r\n]+/?)"
                found_platform = re.search(pattern, new_file.as_posix()).group(
                    "Platform"
                )
            except Exception:
                pass
    ...
    if "createDirectories" in json_data:
        for create_directory in json_data["createDirectories"]:
            ...
            if keep_restricted_in_instance:
                ...
                pal_dir = create_directory["dir"].replace(
    ...
    if "copyFiles" in json_data:
        for copy_file in json_data["copyFiles"]:
            ...
            if template_restricted_path:
                pal_file = copy_file["file"].replace(
                    f"Platform/{restricted_platform}/", ""
                )
                in_file = (
...
def _instantiate_template(
    ...
    logger.info(
        f"Instantiating template: `{template_name}` into `{destination_path}`..."
    )
    _execute_template_json(
        template_json_data,
        destination_path,
        template_path,
        replacements,
    ...
    for restricted_platform in restricted_platforms:
        restricted_json_data = {}
        if template_restricted_path:
            template_restricted_platform = (
                template_restricted_path / restricted_platform
            )
            template_restricted_platform_path_rel = (
                template_restricted_platform
                / template_restricted_platform_relative_path
            )
        ...
        _execute_restricted_template_json(
            template_json_data,
            restricted_json_data,
            restricted_platform,
            destination_name,
...
def create_template(
    ...
    if not template_path.is_absolute():
        ...
        logger.info(
            f"Template path empty. Using default templates folder {template_path}"
        )
    if not force and template_path.is_dir() and len(list(template_path.iterdir())):
        logger.error(
            f"Template path {template_path} already exists; use --force to overwrite existing contents."
        )
        return 1
    ...
    if template_name.lower() in TEMPLATE_RESERVED_WORDS:
        logger.error(
            f'Template name cannot be "{template_name}" as this might cause issues with compilation.  Please try another name.'
        )
        return 1
    ...
    if source_restricted_name and not source_restricted_path:
        source_restricted_path = manifest.get_registered(
            restricted_name=source_restricted_name
        )
    ...
    if source_restricted_path:
        if not source_restricted_path.is_dir():
            logger.error(
                f"Source restricted path {source_restricted_path} is not a folder."
            )
            return 1
    ...
    if template_restricted_name and not template_restricted_path:
        template_restricted_path = manifest.get_registered(
            restricted_name=template_restricted_name
        )
        ...
        if template_restricted_path.is_dir():
            ...
            with open(restricted_json, "r") as s:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to read restricted_name from {restricted_json}"
                    )
                    return 1
        else:
    ...
    def _transform_into_template(
        s_data: object, prefer_sanitized_name: bool = False
    ) -> Tuple[bool, str]:
        """
        ...
        def swap_sanitized_name_and_normal():
            (
                replacements[sanitized_name_index - 1],
    ...
    def _transform_restricted_into_copyfiles_and_createdirs(
        root_abs: pathlib.Path, path_abs: pathlib.Path = None
    ) -> None:
        """
        ...
        for entry in entries:
            ...
            if template_restricted_path:
                ...
                for component in destination_entry_rel.parts:
                    if first:
                        first = False
                        result = (
                            pathlib.Path(component) / "Platform" / restricted_platform
                        )
                    else:
                ...
                destination_entry_rel = result.as_posix()
                destination_entry_abs = (
                    template_path / "Template" / destination_entry_rel
                )
            ...
            if os.path.isfile(entry_abs):
                ...
                if ext in binary_file_ext:
                    ...
                    try:
                        ...
                        with open(entry_abs, "r") as s:
                            source_data = s.read()
                            templated, source_data = _transform_into_template(
                                source_data, _is_cpp_file(entry_abs)
                            )
                            ...
                            if keep_license_text and ext in expect_license_info_ext:
                                if (
                                    "Copyright (c)" not in source_data
                                    or "{BEGIN_LICENSE}" not in source_data
                                ):
                                    logger.warning(
                                        f"Un-templated License header in {entry_abs}"
                                    )
                ...
                if keep_restricted_in_template:
                    copy_files.append(
                        {"file": destination_entry_rel, "isTemplated": templated}
                    )
                else:
                    restricted_platform_entries[restricted_platform][
                        "copyFiles"
                    ].append({"file": destination_entry_rel, "isTemplated": templated})
            else:
                if keep_restricted_in_template:
                    ...
                    restricted_platform_entries[restricted_platform][
                        "createDirs"
                    ].append({"dir": destination_entry_rel})
                _transform_restricted_into_copyfiles_and_createdirs(root_abs, entry_abs)

    def _transform_dir_into_copyfiles_and_createdirs(
        root_abs: pathlib.Path, path_abs: pathlib.Path = None
    ) -> None:
        """
        Internal function recursively called to transform any paths files into copyfiles and create dirs relative to
        ...
        for entry in entries:
            ...
            if not keep_restricted_in_template and "Platform" in entry_abs.parts:
                ...
                try:
                    # the name of the Platform should follow the '/Platform/'
                    pattern = r"/Platform/(?P<Platform>[^/:*?\"<>|\r\n]+/?)"
                    found_platform = re.search(pattern, entry_abs.as_posix()).group(
                        "Platform"
                    )
                    found_platform = found_platform.replace("/", "")
                except Exception:
                ...
                if (
                    os.path.isfile(entry_abs)
                    and found_platform not in restricted_platform_entries
                ):
                    found_platform = ""
                ...
                if (
                    found_platform not in restricted_platform_entries
                    and found_platform in restricted_platforms
                ):
                    restricted_platform_entries.update(
                        {found_platform: {"copyFiles": [], "createDirs": []}}
                    )
            ...
            if platform and found_platform in restricted_platforms:
                ...
                if not template_restricted_path:
                    logger.warning(
                        "Restricted platform file found!!! {destination_entry_rel}, {found_platform}"
                    )
                    continue
        ...
        for restricted_platform in os.listdir(source_restricted_path):
            ...
            if os.path.isdir(restricted_platform_src_path_abs):
                if restricted_platform not in restricted_platform_entries:
                    restricted_platform_entries.update(
                        {restricted_platform: {"copyFiles": [], "createDirs": []}}
                    )
    ...
    if not keep_restricted_in_template and template_restricted_path:
        ...
        if template_restricted_platform_relative_path != "":
            json_data.update(
                {
                    "template_restricted_platform_relative_path": template_restricted_platform_relative_path.as_posix()
                }
            )
    json_data.update({"copyFiles": copy_files})
    json_data.update({"createDirectories": create_dirs})
    ...
    json_name = (
        template_path / source_restricted_platform_relative_path / "template.json"
    )
    ...
    if (
        not keep_restricted_in_template
        and not template_restricted_path
        and len(restricted_platform_entries)
    ):
        logger.info(
            "Restricted platform files found!!! and no template restricted path was found..."
        )
    ...
    if not keep_restricted_in_template and template_restricted_path:
        ...
        for restricted_platform in restricted_platform_entries:
            restricted_template_path = (
                template_restricted_path
                / restricted_platform
                / template_restricted_platform_relative_path
            )
            # sort
            restricted_platform_entries[restricted_platform]["copyFiles"].sort(
                key=lambda x: x["file"]
            )
            restricted_platform_entries[restricted_platform]["createDirs"].sort(
                key=lambda x: x["dir"]
            )
            ...
            json_data.update(
                {
                    "copyFiles": restricted_platform_entries[restricted_platform][
                        "copyFiles"
                    ]
                }
            )
            json_data.update(
                {
                    "createDirectories": restricted_platform_entries[
                        restricted_platform
                    ]["createDirs"]
                }
            )
        ...
        if not no_register:
            if register.register(restricted_path=template_restricted_path):
                logger.error(
                    f"Failed to register the restricted {template_restricted_path}."
                )
                return 1
...
def create_from_template(
    ...
    if template_name and template_path:
        logger.error(
            "Template Name and Template Path provided, these are mutually exclusive."
        )
        return 1

    if destination_restricted_name and destination_restricted_path:
        logger.error(
            "Destination Restricted Name and Destination Restricted Path provided, these are mutually"
            " exclusive."
        )
        return 1
    ...
    if template_restricted_name and template_restricted_path:
        logger.error(
            "Template Restricted Name and Template Restricted Path provided, these are mutually exclusive."
        )
        return 1

    # need either the template name or path
    if not template_path and not template_name:
        logger.error("Template Name or Template Path must be specified.")
        return 1
        ...
        for replace_index in range(0, len(replace), 2):
            if replace_index + 1 < len(replace):
                replacement_pairs.append(
                    f" {replace[replace_index]} -> {replace[replace_index + 1]}"
                )
            else:
                replacement_pairs.append(
                    f" {replace[replace_index]} is missing replacement"
                )
        logger.error(
            "Invalid replacement argument pairs.  "
    ...
    if not template_path:
        logger.error(
            f"Could not find the template path using name {template_name}.\n"
    ...
    try:
        ...
        logger.error(
            f'Could not read "template_name" from template json {template_json}: {str(e)}.'
        )
        return 1
    ...
    if not template_restricted_name and not template_restricted_path:
        try:
            ...
            logger.info('The template does not specify a "restricted_name".')
        else:
    ...
    if template_restricted_name or template_restricted_path:
        ...
        if template_restricted_name:
            ...
            try:
                ...
                # the template json doesn't have a 'restricted_name' element warn and use it
                logger.info(
                    f'The template does not specify a "restricted_name".'
        ...
        if template_restricted_path and not os.path.isdir(template_restricted_path):
            logger.error(
                f"Template restricted path {template_restricted_path} does not exist."
            )
            return 1
        ...
        if template_restricted_platform_relative_path:
            ...
            try:
                ...
                logger.info(
                    f'The template does not specify a "restricted_platform_relative_path".'
                ...
                if (
                    template_restricted_platform_relative_path
                    != template_json_restricted_platform_relative_path
                ):
                    logger.error(
                        f"The supplied --template-restricted-platform-relative-path"
    ...
    if not force and os.path.isdir(destination_path):
        logger.error(
            f"Destination path {destination_path} already exists; use --force to overwrite existing contents."
        )
        return 1
    else:
    ...
    if destination_name in restricted_platforms:
        logger.error(
            f"Destination path cannot be a restricted name. {destination_name}"
        )
        return 1

    # destination restricted name
    if destination_restricted_name:
        destination_restricted_path = manifest.get_registered(
            restricted_name=destination_restricted_name
        )

    # destination restricted path
    elif destination_restricted_path:
        if not os.path.isabs(destination_restricted_path):
            restricted_default_path = manifest.get_registered(
                default_folder="restricted"
            )
            new_destination_restricted_path = (
                restricted_default_path / "Templates" / destination_restricted_path
            )
            logger.info(
                f"{destination_restricted_path} is not a full path, making it relative"
        ...
        new_destination_restricted_path = (
            restricted_default_path / "Templates" / destination_name
        )
        logger.info(
            f"--destination-restricted-path is not specified, using default restricted path"
    ...
    if _instantiate_template(
        ...
        logger.error("Instantiation of the template has failed.")
        return 1
        ...
        if destination_restricted_path:
            ...
            if os.path.isdir(destination_restricted_path):
                ...
                if not os.path.isfile(restricted_json):
                    with open(restricted_json, "w") as s:
                        restricted_json_data = {}
                        restricted_json_data.update(
                            {"restricted_name": destination_name}
                        )
                        s.write(json.dumps(restricted_json_data, indent=4) + "\n")
                ...
                if not no_register:
                    if register.register(restricted_path=destination_restricted_path):
                        logger.error(
                            f"Failed to register the restricted {destination_restricted_path}."
                        )
                        return 1

    logger.warning(
        "Instantiation successful. NOTE: This is a generic instantiation of the template. If this"
        " was a template of an o3de object like a project, gem, template, etc. then the create-project"
        " or create-gem command can be used to register the object type via its project.json or gem.json, etc."
        " Create from template is meant only to instance a template of a non o3de object."
    )
...
def create_project(
    ...
    if project_restricted_name and project_restricted_path:
        logger.error(
            "Project Restricted Name and Project Restricted Path provided, these are mutually exclusive."
        )
        return 1
    ...
    if not os.path.isabs(project_path):
        ...
        logger.info(
            f"Project Path {project_path} is not a full path, we must assume its relative"
    ...
    if project_name.lower() in TEMPLATE_RESERVED_WORDS:
        logger.error(
            f'Project name cannot be "{project_name}" as this might cause issues with compilation.  Please try another name.'
        )
        return 1
    ...
    if project_restricted_name and not project_restricted_path:
        gem_restricted_path = manifest.get_registered(
            restricted_name=project_restricted_name
        )
        if not gem_restricted_path:
            logger.error(
                f"Project Restricted Name {project_restricted_name} cannot be found."
            )
            return 1
    ...
    if project_restricted_path:
        if not os.path.isabs(project_restricted_path):
            logger.error(
                f"Project Restricted Path {project_restricted_path} is not an absolute path."
            )
            return 1
    # neither put it in the default restricted projects
    else:
        project_restricted_path = (
            manifest.get_o3de_restricted_folder() / "Projects" / project_name
        )
    ...
    if editor_system_component_class_id:
        if (
            "{" not in editor_system_component_class_id
            or "-" not in editor_system_component_class_id
        ):
            logger.error(
            ...
            return 1
        replacements.append(
            ("${EditorSysCompClassId}", editor_system_component_class_id)
        )
    else:
        replacements.append(
            ...
            if os.path.isdir(project_restricted_path):
                ...
                with open(restricted_json, "r") as s:
                    try:
                        ...
                        logger.error(
                            f"Failed to load restricted json {restricted_json}."
                        )
                        return 1
                ...
                try:
                    ...
                    logger.error(
                        f'Failed to read "restricted_name" from restricted json {restricted_json}.'
                    )
                    return 1
                ...
                if not no_register:
                    if register.register(restricted_path=project_restricted_path):
                        logger.error(
                            f"Failed to register the restricted {project_restricted_path}."
                        )
                        return 1
...
def create_gem(
    ...
    if gem_restricted_name and gem_restricted_path:
        logger.error(
            "Gem Restricted Name and Gem Restricted Path provided, these are mutually exclusive."
        )
        return 1
    ...
    if not os.path.isabs(gem_path):
        ...
        logger.info(
            f"Gem Path {gem_path} is not a full path, we must assume its relative"
    ...
    if gem_name.lower() in TEMPLATE_RESERVED_WORDS:
        logger.error(
            f'Project name cannot be "{gem_name}" as this might cause issues with compilation.  Please try another name.'
        )
        return 1
    ...
    if gem_restricted_name and not gem_restricted_path:
        gem_restricted_path = manifest.get_registered(
            restricted_name=gem_restricted_name
        )
        if not gem_restricted_path:
    ...
    if gem_restricted_path:
        if not os.path.isabs(gem_restricted_path):
            logger.error(
                f"Gem Restricted Path {gem_restricted_path} is not an absolute path."
            )
            return 1
    ...
    replacements.append(
        ("${IconPath}", pathlib.PurePath(icon_path).as_posix() if icon_path else "")
    )
    replacements.append(("${Requirements}", requirements if requirements else ""))
    replacements.append(
        ("${DocumentationURL}", documentation_url if documentation_url else "")
    )
            ...
            if os.path.isdir(gem_restricted_path):
                ...
                if not no_register:
                    if register.register(restricted_path=gem_restricted_path):
                        logger.error(
                            f"Failed to register the restricted {gem_restricted_path}."
                        )
                        return 1
...
def create_repo(
    ...
    if not force and repo_json_path.is_file():
        logger.error(
            f"{repo_json_path} already exists. If you want to edit the repo.json properties, use edit-repo-properties command."
            "Use --force if you want to override the current repo.json file"
        )
        return 1
    elif repo_path.is_file():
        logger.error(
            f"{repo_path} already exists and is a file instead of a directory."
        )
        return 1
    ...
    replacements.append(
        (
            "${AdditionalInfo}",
            additional_info if additional_info else "${AdditionalInfo}",
        )
    )
    ...
    return (
        register.register(
            repo_uri=repo_path.as_posix(), force_register_with_o3de_manifest=True
        )
        if not no_register
        else 0
    )
...
def add_args(subparsers) -> None:
    ...
    create_template_subparser.add_argument(
        "-sp",
        "--source-path",
        type=pathlib.Path,
        required=True,
    ...
    create_template_subparser.add_argument(
        "-srprp",
        "--source-restricted-platform-relative-path",
        type=pathlib.Path,
        required=False,
        default=None,
    ...
    create_template_subparser.add_argument(
        "-trprp",
        "--template-restricted-platform-relative-path",
        type=pathlib.Path,
        required=False,
        default=None,
    ...
    create_from_template_subparser.add_argument(
        "-dp",
        "--destination-path",
        type=pathlib.Path,
        required=True,
    ...
    create_from_template_subparser.add_argument(
        "-drprp",
        "--destination-restricted-platform-relative-path",
        type=pathlib.Path,
        required=False,
        default=None,
    ...
    create_project_subparser.add_argument(
        "-prprp",
        "--project-restricted-platform-relative-path",
        type=pathlib.Path,
        required=False,
        default=None,
    ...
    create_project_subparser.add_argument(
        "--editor-system-component-class-id",
        type=uuid.UUID,
        required=False,
    ...
    create_gem_subparser.add_argument(
        "-gp",
        "--gem-path",
        type=pathlib.Path,
        required=True,
    ...
    create_gem_subparser.add_argument(
        "-grprp",
        "--gem-restricted-platform-relative-path",
        type=pathlib.Path,
        required=False,
        default=None,
    ...
    create_repo_subparser.add_argument(
        "-rp",
        "--repo-path",
        type=pathlib.Path,
        required=True,
...
if __name__ == "__main__":
    ...
    subparsers = the_parser.add_subparsers(
        help="To get help on a sub-command:\nengine_template.py <sub-command> -h",
        title="Sub-Commands",
        dest="command",
        required=True,
    )
