...
O3DE_SCRIPT_PATH = (
    f"{ENGINE_PATH}{os.sep}scripts{os.sep}o3de{android_support.O3DE_SCRIPT_EXTENSION}"
)
...
VALIDATION_WARNING_SIGNCONFIG_INCOMPLETE = (
    "Signing configuration settings is incomplete."
)
VALIDATION_MISSING_PASSWORD = "Signing configuration password not set."

ValidatedEnv = collections.namedtuple(
    "ValidatedEnv",
    [
        "java_version",
        "gradle_home",
...
def validate_android_config(android_config: command_utils.O3DEConfig) -> None:
    ...
    android_gradle_plugin_ver = android_config.get_value(
        android_support.SETTINGS_GRADLE_PLUGIN_VERSION.key
    )
    if not android_gradle_plugin_ver:
        raise android_support.AndroidToolError(
            f"Missing '{android_support.SETTINGS_GRADLE_PLUGIN_VERSION.key}' from the android settings"
        )

    android_gradle_requirements = (
        android_support.get_android_gradle_plugin_requirements(
            android_gradle_plugin_ver
        )
    )
    logger.info(
        f"Validating settings for requested version {android_gradle_requirements.version} of the Android Gradle Plugin."
    )
    ...
    has_signing_config_store_file = (
        len(
            android_config.get_value(
                android_support.SETTINGS_SIGNING_STORE_FILE.key, default=""
            )
        )
        > 0
    )
    has_signing_config_store_password = (
        len(
            android_config.get_value(
                android_support.SETTINGS_SIGNING_STORE_PASSWORD.key, default=""
            )
        )
        > 0
    )
    has_signing_config_key_alias = (
        len(
            android_config.get_value(
                android_support.SETTINGS_SIGNING_KEY_ALIAS.key, default=""
            )
        )
        > 0
    )
    has_signing_config_key_password = (
        len(
            android_config.get_value(
                android_support.SETTINGS_SIGNING_KEY_PASSWORD.key, default=""
            )
        )
        > 0
    )
    ...
    if (
        not has_signing_config_store_file
        and not has_signing_config_store_password
        and not has_signing_config_key_alias
        and not has_signing_config_key_password
    ):
        ...
        print(
            f"{android_support.SETTINGS_SIGNING_STORE_FILE}' is set, but '{android_support.SETTINGS_SIGNING_KEY_ALIAS}' is not. "
        ...
        logger.warning(VALIDATION_WARNING_SIGNCONFIG_INCOMPLETE)
        print(
            f" The setting for '{android_support.SETTINGS_SIGNING_KEY_ALIAS}' is set, but '{android_support.SETTINGS_SIGNING_STORE_FILE}' "
    ...
    if has_signing_config_store_file:
        signing_config_store_file = pathlib.Path(
            android_config.get_value(android_support.SETTINGS_SIGNING_STORE_FILE.key)
        )
        if not signing_config_store_file.is_file():
            raise android_support.AndroidToolError(
                f"The signing config key store file '{signing_config_store_file}' is not a valid file."
            )
    ...
    if has_signing_config_store_file and has_signing_config_key_alias:
        ...
        if not has_signing_config_store_password or not has_signing_config_key_password:
            ...
            if not has_signing_config_store_password:
                missing_passwords.append(
                    f"store password ({android_support.SETTINGS_SIGNING_STORE_PASSWORD})"
                )
            if not has_signing_config_key_password:
                missing_passwords.append(
                    f"key password ({android_support.SETTINGS_SIGNING_KEY_PASSWORD})"
                )

            print(
                "The signing configuration is set but missing the following settings:\n"
            )
...
def get_android_config_from_args(args: argparse) -> (command_utils.O3DEConfig, str):
    ...
    if is_global:
        ...
        if project_name:
            ...
            android_config = android_support.get_android_config(
...
def configure_android_options(args: argparse) -> int:
    ...
    try:
        ...
        if args.clear_value:
            android_config.set_config_value(
...
def generate_android_project(args: argparse) -> int:
    ...
    try:
        ...
        if not resolved_project_path:
            raise android_support.AndroidToolError(
                f"Project '{project_name}' is not registered with O3DE."
            )
        project_settings, android_settings = (
            android_support.read_android_settings_for_project(resolved_project_path)
        )
        ...
        android_env.sdk_manager.install_package(
            package_install_path="extras;google;market_apk_expansion",
            package_description="Google APK Expansion Library",
        )

        android_env.sdk_manager.install_package(
            package_install_path="extras;google;market_licensing",
        ...
        if args.platform_sdk_api_level:
            ...
            android_platform_sdk_api_level = android_config.get_value(
                android_support.SETTINGS_PLATFORM_SDK_API.key
            )
            if not android_platform_sdk_api_level:
                raise android_support.AndroidToolError(
                    f"The Android Platform SDK API Level was not specified in either the command argument (--platform-sdk-api-level) "
        ...
        platform_sdk_package = android_env.sdk_manager.install_package(
            package_install_path=platform_package_name,
            package_description=f"Android SDK Platform {android_platform_sdk_api_level}",
        )
        logger.info(
            f"Selected Android Platform API Level : {android_platform_sdk_api_level}"
        )
        ...
        if args.ndk_version:
            ...
            android_ndk_version = android_config.get_value(
                android_support.SETTINGS_NDK_VERSION.key
            )
            if not android_ndk_version:
                raise android_support.AndroidToolError(
                    f"The Android NDK version was not specified in either the command argument (--ndk-version) "
                    f"nor the android settings ({android_support.SETTINGS_NDK_VERSION.key})."
                )

        ndk_package_name = f"ndk;{android_ndk_version}"
        ndk_package = android_env.sdk_manager.install_package(
            package_install_path=ndk_package_name,
        ...
        android_env.sdk_manager.install_package(
            package_install_path=build_tools_package_name,
        ...
        if getattr(args, "strip_debug", False):
            ...
            strip_debug = android_config.get_boolean_value(
                android_support.SETTINGS_STRIP_DEBUG.key
            )

        logger.info(
            f"Debug symbol stripping {'enabled' if strip_debug else 'disabled'}."
        )
        ...
        if getattr(args, "oculus_project", False):
            ...
            is_oculus_project = android_config.get_boolean_value(
                android_support.SETTINGS_OCULUS_PROJECT.key
            )
        if is_oculus_project:
            logger.info("Oculus flags enabled.")
        ...
        src_bundle_pak_subfolder = android_config.get_value(
            android_support.SETTINGS_ASSET_BUNDLE_SUBPATH.key,
        ...
        if signconfig_store_file and signconfig_key_alias:
            ...
            if not signconfig_store_file_path.is_file():
                raise android_support.AndroidToolError(
                    f"Invalid signing configuration store file '{signconfig_store_file}' specified. File does not exist."
                )

            stored_signconfig_store_password = android_config.get_value(
                android_support.SETTINGS_SIGNING_STORE_PASSWORD.key
            )
            if stored_signconfig_store_password:
                signconfig_store_password = stored_signconfig_store_password
            else:
                signconfig_store_password = prompt_validated_password(
                    f"key store for {signconfig_store_file}"
                )

            stored_signconfig_key_password = android_config.get_value(
                android_support.SETTINGS_SIGNING_KEY_PASSWORD.key
            )
            if stored_signconfig_key_password:
                signconfig_key_password = stored_signconfig_key_password
            else:
                signconfig_key_password = prompt_validated_password(
                    f"key alias for {signconfig_key_alias}"
                )

            signing_config = android_support.AndroidSigningConfig(
                store_file=signconfig_store_file_path,
            ...
            raise android_support.AndroidToolError(
                f"Missing required '{SIGNCONFIG_ARG_KEY_ALIAS}' argument to accompany the provided '{SIGNCONFIG_ARG_STORE_FILE}' ({signconfig_store_file})"
            )
        elif not signconfig_store_file and signconfig_key_alias:
            raise android_support.AndroidToolError(
                f"Missing required '{SIGNCONFIG_ARG_STORE_FILE}' argument to accompany the provided '{SIGNCONFIG_ARG_KEY_ALIAS}' ({signconfig_store_file})"
            )
        else:
            signing_config = None
        ...
        if signing_config is not None:
            ...
            logger.warning(
                "No signing configuration enabled. This output APK will not be signed until a signing configuration is added."
            )

        apg = android_support.AndroidProjectGenerator(
...
def add_args(subparsers) -> None:
    ...
    try:
        ...
        logger.debug(
            f"No project detected at {os.getcwd()}, default settings from global config."
        )
        project_name = None
        android_config = android_support.get_android_config(project_path=None)
    ...
    epilog_lines = [
        "Configure the default values that control the android helper scripts for O3DE.",
    ...
    android_configure_subparser = subparsers.add_parser(
        O3DE_COMMAND_CONFIGURE,
        help="Configure the Android platform settings for generating, building, and deploying Android projects.",
        epilog=epilog,
        formatter_class=argparse.RawTextHelpFormatter,
    )

    android_configure_subparser.add_argument(
        "--global",
        default=False,
        action="store_true",
    ...
    android_configure_subparser.add_argument(
        "--debug",
        help="Enable debug level logging for this script.",
        action="store_true",
    )
    ...
    #
    android_generate_subparser = subparsers.add_parser(
        O3DE_COMMAND_GENERATE, help="Generate an Android/Gradle project."
    )
    android_generate_subparser.add_argument(
        "--settings-asset-bundle-path",
        type=str,
        default=android_support.SETTINGS_ASSET_BUNDLE_SUBPATH.key,
    ...
    platform_sdk_api_level = android_config.get_value(
        android_support.SETTINGS_PLATFORM_SDK_API.key
    )
    if platform_sdk_api_level:
        android_generate_subparser.add_argument(
            "--platform-sdk-api-level",
            type=str,
    ...
    signconfig_store_file = android_config.get_value(
        android_support.SETTINGS_SIGNING_STORE_FILE.key
    )
    if signconfig_store_file:
        android_generate_subparser.add_argument(
            SIGNCONFIG_ARG_STORE_FILE,
            type=str,
    ...
    signconfig_key_alias = android_config.get_value(
        android_support.SETTINGS_SIGNING_KEY_ALIAS.key
    )
    if signconfig_key_alias:
        android_generate_subparser.add_argument(
            SIGNCONFIG_ARG_KEY_ALIAS,
            type=str,
    ...
    # Asset Mode
    asset_mode = android_config.get_value(
        android_support.SETTINGS_ASSET_MODE.key,
        default=android_support.ASSET_MODE_LOOSE,
    )
    android_generate_subparser.add_argument(
        "--asset-mode",
        type=str,
        help=f"The mode of asset deployment to use.  Default: {asset_mode}",
        choices=android_support.ASSET_MODES,
        default=asset_mode,
    )

    # Extra CMake configure args
    extra_cmake_args = android_config.get_value(
        android_support.SETTINGS_EXTRA_CMAKE_ARGS.key, default=""
    )
    android_generate_subparser.add_argument(
        "--extra-cmake-args",
        type=str,
    ...
    custom_jvm_args = android_config.get_value(
        android_support.SETTINGS_GRADLE_JVM_ARGS.key, default=""
    )
    android_generate_subparser.add_argument(
        "--custom-jvm-args",
        type=str,
    ...
    if strip_debug:
        android_generate_subparser.add_argument(
            "--no-strip-debug",
            help="Don't strip the debug symbols from the built binaries.",
            action="store_true",
        )
    else:
        android_generate_subparser.add_argument(
            "--strip-debug",
            help="Strip the debug symbols from the built binaries.",
            action="store_true",
        )

    # Flag indicating this is an oculus project
    is_oculus_project = android_config.get_value(
        android_support.SETTINGS_OCULUS_PROJECT.key
    )
    if is_oculus_project:
        android_generate_subparser.add_argument(
            "--no-oculus-project",
            help="Turn off the flag that marks the project as an Oculus project.",
            action="store_true",
        )
    else:
        android_generate_subparser.add_argument(
