...
PROJECT_ENGINE_PRESET_RELATIVE_PATH = pathlib.PurePath(
...
def update_cmake_presets_for_project(
    preset_path: pathlib.PurePath,
    engine_name: str = "",
    engine_version: str = "",
    engine_path: pathlib.PurePath or None = None,
) -> UpdatePresetResult:
    """
    ...
    """
    if not engine_path:
        engine_with_specifier = (
            f"{engine_name}=={engine_version}" if engine_version else engine_name
        )
        engine_path = manifest.get_registered(engine_name=engine_with_specifier)
        if not engine_path:
            logger.error(
                f"Engine with identifier {engine_with_specifier} is not registered.\n"
    ...
    try:
        with preset_path.open("r") as preset_fp:
            try:
                ...
                logger.warning(
                    f'Cannot parse JSON data from cmake-presets file at path "{preset_path}".\n'
    ...
    if preset_json:
        ...
        if engine_cmake_presets_path in map(
            lambda preset_json_include: pathlib.PurePath(preset_json_include),
            preset_include_list,
        ):
        ...
        try:
            preset_json = json.loads(
                string.Template(TEMPLATE_CMAKE_PRESETS_INCLUDE_JSON).safe_substitute(
                    CMakePresetsInclude=engine_cmake_presets_path.as_posix()
                )
            )
        except json.JSONDecodeError:
            logger.error(
                f"Failed to substitute engine path {engine_path} into project CMake Presets template"
            )
            return UpdatePresetResult.Error
...
def remove_gem_dependency(cmake_file: pathlib.Path, gem_name: str) -> int:
    """
    ...
    with cmake_file.open("r") as s:
        ...
        for line in s:
            ...
            if in_gem_list:
                ...
                gem_name_list = list(
                    map(lambda gem_name: gem_name.strip('"'), parsed_line.split())
                )
...
def resolve_gem_dependency_paths(
    ...
    if not engine_path and not project_path:
        logger.error(
            "project path or engine path are required to resolve dependencies"
        )
        return 1
    ...
    if not engine_path:
        ...
        if not engine_path:
            ...
            if not engine_path:
                logger.error(
                    "Failed to find a valid engine path for the project at "
    ...
    if project_path:
        ...
        if not project_json_data:
            logger.error(
                "Failed to retrieve project json data for the project at "
                f'"{project_path}" which is required to resolve gem dependencies.'
            )
            return 1
        active_gem_names = project_json_data.get("gem_names", [])
        enabled_gems_file = manifest.get_enabled_gem_cmake_file(
    ...
    if not gem_names_with_optional_gems:
        logger.info(
            "No gem names were found to use as input to resolve gem dependencies."
        )
    ...
    all_gems_json_data = manifest.get_gems_json_data_by_name(
        ...
        external_subdirectories=external_subdirectories.split(";")
        if isinstance(external_subdirectories, str)
        else external_subdirectories,
    )

    # First try to resolve with optional gems
    results, errors = compatibility.resolve_gem_dependencies(
    ...
    if errors:
        logger.warning(
            "Failed to resolve dependencies with optional gems, trying without optional gems."
        )
    ...
    gem_paths = sorted(
        f"{gem.gem_json_data['gem_name'].strip()};{gem.gem_json_data['path'].resolve().as_posix()}"
        for _, gem in results.items()
    )
...
def _update_project_presets_to_include_engine_presets(args: argparse) -> int:
    ...
    return (
        0
        if update_cmake_presets_for_project(
            preset_path=preset_path,
            engine_name=args.engine_name,
            engine_version=args.engine_version,
            engine_path=args.engine_path,
        )
        != UpdatePresetResult.Error
        else 1
    )
...
def add_args(subparsers) -> None:
    ...
    update_cmake_presets_for_project_parser = subparsers.add_parser(
        "update-cmake-presets-for-project"
    )
    project_group = (
        update_cmake_presets_for_project_parser.add_mutually_exclusive_group(
            required=True
        )
    )
    project_group.add_argument(
        "--project-path", "-pp", type=pathlib.Path, help="The path to a project."
    )
    project_group.add_argument(
        "--project-name", "-pn", type=str, help="The name of a project."
    )

    engine_group = update_cmake_presets_for_project_parser.add_argument_group(
        "engine identifiers"
    )
    ...
    engine_path_group.add_argument(
        "--engine-path", "-ep", type=pathlib.Path, help="The path to the engine."
    )
    engine_path_group.add_argument(
        "--engine-name",
        "-en",
    ...
    update_cmake_presets_for_project_parser.set_defaults(
        func=_update_project_presets_to_include_engine_presets
    )
