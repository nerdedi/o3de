...
def get_cache_file_uri(uri: str):
    ...
    cache_file = manifest.get_o3de_cache_folder() / str(
        uri_sha256.hexdigest() + ".json"
    )
...
def repo_uri_enabled(repo_uri: str) -> bool:
    ...
    repo_json_data = manifest.get_json_data_file(
        repo_json_cache_file, "repo", validation.valid_o3de_repo_json
    )
    if repo_json_data:
...
def download_repo_manifest(
    manifest_uri: str, force_overwrite: bool = True
...
def download_object_manifests(
    repo_data: dict, download_missing_files_only: bool = False
):
...
def validate_remote_repo(
    repo_uri: str, validate_contained_objects: bool = False
) -> bool:
    ...
    if not validation.valid_o3de_repo_json(cache_file):
        logger.error(
            f"Repository JSON {cache_file} could not be loaded or is missing required values"
        )
        return False
        ...
        if repo_schema_version == REPO_IMPLICIT_SCHEMA_VERSION:
            ...
            for gem_json_data in cached_gems_json_data:
                if not validation.valid_o3de_gem_json_data(gem_json_data):
                    logger.error(
                        f"Invalid gem JSON - {gem_json_data} is missing required values"
                    )
                    return False
            ...
            for project_json_data in cached_project_json_data:
                if not validation.valid_o3de_project_json_data(project_json_data):
                    logger.error(
                        f"Invalid project JSON - {project_json_data} is missing required values"
                    )
                    return False
            cached_template_json_data = get_template_json_data_from_cached_repo(
                repo_uri
            )
            for template_json_data in cached_template_json_data:
                if not validation.valid_o3de_template_json_data(template_json_data):
                    logger.error(
                        f"Invalid template JSON - {template_json_data} is missing required values"
                    )
                    return False
            ...
            for gem_json in gem_list:
                if not validation.valid_o3de_gem_json_data(gem_json):
                    logger.error(
                        f"Invalid gem JSON - {gem_json} is missing required values"
                    )
                    return False
                ...
                if "versions_data" in gem_json:
                    ...
                    for version in versions_data:
                        if not all(key in version for key in ["version"]):
                            logger.error(
                                "Invalid gem JSON - {gem_json} The version field must be defined for each entry in the versions_data field"
                            )
                            return False
                        ...
                        download_origin_defined = (
                            download_source_uri_defined and not origin_uri_defined
                        ) or (not download_source_uri_defined and origin_uri_defined)

                        if not (source_control_uri_defined or download_origin_defined):
                            logger.error(
                                f"Invalid gem JSON - {gem_json} At least one of source_control_uri or download_source_uri must be defined"
                            )
                            return False
            ...
            for project_json in project_list:
                if not validation.valid_o3de_project_json_data(project_json):
                    logger.error(
                        f"Invalid project JSON - {project_json} is missing required values"
                    )
                    return False
            ...
            for template_json in template_list:
                if not validation.valid_o3de_template_json_data(template_json):
                    logger.error(
                        f"Invalid template JSON - {template_json} is missing required values"
                    )
                    return False
...
def process_add_o3de_repo(
    file_name: str or pathlib.Path,
    repo_set: set,
    download_missing_files_only: bool = False,
) -> int:
    file_name = pathlib.Path(file_name).resolve()
    if not validation.valid_o3de_repo_json(file_name):
        logger.error(
            f"Repository JSON {file_name} could not be loaded or is missing required values"
        )
        return 1
    ...
    for repo in repo_list:
        if repo not in repo_set:
            ...
            return process_add_o3de_repo(
                cache_file, repo_set, download_missing_files_only
            )
    return 0


def get_object_versions_json_data(
    remote_object_list: list,
    required_json_key: str = None,
    required_json_value: str = None,
) -> list:
    """
    ...
    for remote_object_json_data in remote_object_list:
        if (
            required_json_key
            and remote_object_json_data.get(required_json_key, "")
            != required_json_value
        ):
            continue
        ...
        if versions_data:
            ...
            for version_json_data in versions_data:
                if remote_object_json_data.get("version") == version_json_data.get(
                    "version"
                ):
                    version_found = True
                object_json_data_list.append(
                    remote_object_json_data | version_json_data
                )
...
def get_object_json_data_from_cached_repo(
    repo_uri: str,
    repo_key: str,
    object_typename: str,
    object_validator,
    ...
    if not file_name.is_file():
        logger.info(
            f"Could not find cached repository json file for {repo_uri}, attempting to download"
        )
        ...
        if repo_schema_version == REPO_IMPLICIT_SCHEMA_VERSION:
            ...
            for o3de_object_uris, manifest_json in repo_objects:
                for o3de_object_uri in o3de_object_uris:
                    ...
                    cache_object_json_filepath, _ = get_cache_file_uri(
                        manifest_json_uri
                    )

                    if not cache_object_json_filepath.is_file():
                        # attempt to download the missing file
                        cache_object_json_filepath = download_repo_manifest(
                            manifest_json_uri
                        )
                        if not cache_object_json_filepath:
                            logger.warning(
                                f"Could not download the missing cached {repo_key} json file {cache_object_json_filepath} from {manifest_json_uri} in repo {repo_uri}"
                            )
                            continue

                    json_data = manifest.get_json_data_file(
                        cache_object_json_filepath, object_typename, object_validator
                    )
            ...
            o3de_object_json_data.extend(
                get_object_versions_json_data(repo_data.get(repo_key, []))
            )

    return o3de_object_json_data


def get_gem_json_data_from_cached_repo(
    repo_uri: str, enabled_only: bool = True
) -> list:
    return get_object_json_data_from_cached_repo(
        repo_uri, "gems", "gem", validation.valid_o3de_gem_json, enabled_only
    )
...
def get_gem_json_data_from_all_cached_repos(enabled_only: bool = True) -> list:
    ...
    for repo_uri in manifest.get_manifest_repos():
        gems_json_data.extend(
            get_gem_json_data_from_cached_repo(repo_uri, enabled_only)
        )

    return gems_json_data


def get_project_json_data_from_cached_repo(
    repo_uri: str, enabled_only: bool = True
) -> list:
    return get_object_json_data_from_cached_repo(
        repo_uri,
...
def get_project_json_data_from_all_cached_repos(enabled_only: bool = True) -> list:
    ...
    for repo_uri in manifest.get_manifest_repos():
        projects_json_data.extend(
            get_project_json_data_from_cached_repo(repo_uri, enabled_only)
        )

    return projects_json_data


def get_template_json_data_from_cached_repo(
    repo_uri: str, enabled_only: bool = True
) -> list:
    return get_object_json_data_from_cached_repo(
        repo_uri,
...
def get_template_json_data_from_all_cached_repos(enabled_only: bool = True) -> list:
    ...
    for repo_uri in manifest.get_manifest_repos():
        templates_json_data.extend(
            get_template_json_data_from_cached_repo(repo_uri, enabled_only)
        )
...
def refresh_repo(
    repo_uri: str, repo_set: set = None, download_missing_files_only: bool = False
) -> int:
...
def search_repo(
    ...
    if repo_schema_version == REPO_IMPLICIT_SCHEMA_VERSION:
        if isinstance(engine_name, str):
            o3de_object = search_o3de_manifest_for_object(
                manifest_json_data, "engines", "engine.json", "engine_name", engine_name
            )
            ...
            o3de_object = search_o3de_manifest_for_object(
                manifest_json_data, "gems", "gem.json", "gem_name", gem_name
            )
        ...
        if isinstance(engine_name, str):
            o3de_object = search_o3de_repo_for_object(
                manifest_json_data, "engines_data", "engine_name", engine_name
            )
        elif isinstance(project_name, str):
            o3de_object = search_o3de_repo_for_object(
                manifest_json_data, "projects_data", "project_name", project_name
            )
        elif isinstance(gem_name, str):
            o3de_object = search_o3de_repo_for_object(
                manifest_json_data, "gems_data", "gem_name", gem_name
            )
        elif isinstance(template_name, str):
            o3de_object = search_o3de_repo_for_object(
                manifest_json_data, "templates_data", "template_name", template_name
            )
        elif isinstance(restricted_name, str):
            o3de_object = search_o3de_repo_for_object(
                manifest_json_data,
    ...
    search_func = lambda manifest_json_data: search_repo(
        manifest_json_data, engine_name, project_name, gem_name, template_name
    )
    return search_o3de_object(manifest_json, o3de_object_uris, search_func)


def search_o3de_repo_for_object(
    repo_json_data: dict,
    ...
    target_name_without_version_specifier, _ = (
        utils.get_object_name_and_optional_version_specifier(target_name)
    )

    # merge all versioned data into a list of candidates
    versioned_candidates = get_object_versions_json_data(
        remote_candidates, target_json_key, target_name_without_version_specifier
    )

    return manifest.get_most_compatible_object(
        object_name=target_name, name_key=target_json_key, objects=versioned_candidates
    )


def search_o3de_manifest_for_object(
    manifest_json_data: dict,
    manifest_attribute: str,
    target_manifest_json: str,
    target_json_key: str,
    target_name: str,
):
    ...
    return manifest.get_most_compatible_object(
        object_name=target_name, name_key=target_json_key, objects=candidates
    )
...
def set_repo_enabled(repo_uri: str, enabled: bool) -> int:
    ...
    if not repo_json_data:
        ...
        if not repo_json_data:
            logger.error(
                f"Repository JSON {repo_json_cache_file} could not be loaded or is missing required values"
            )
