...
def merge_json_data(json_path: pathlib.Path, json_data: dict) -> dict:
    ...
    if json_path.exists():
        ...
        if "version" not in merged_json_data:
...
def create_remote_object_archive(
    src_data_path: pathlib.Path,
    ...
    if not force and zip_path.exists():
        logger.error(
            f"{zip_path} already exists.  Use --force command to overwrite the existing zip or "
            "provide a new location to save your archive."
        )
        return {}
    if not upload_git_release_tag and not download_prefix:
        logger.error(
            "You need to provide a --upload-git-release-tag if you want to upload to a Git provider like GitHub.\n"
    ...
    if upload_git_release_tag and download_prefix:
        logger.error(
            "When a Git release tag is provided, no download prefix should be provided "
    ...
    if upload_git_release_tag and not download_prefix:
        ...
        if git_provider:
            ...
            owner_repo = (
                repo_uri.geturl().split("github.com/")[1].rstrip(".git").split("/")
            )
            owner = owner_repo[0]
            repo = owner_repo[1]
        else:
        ...
        json_data = merge_json_data(
            json_data_path,
            {"repo_uri": repo_uri, "download_source_uri": download_prefix},
        )

        logging.info(f"Creating '{download_prefix}'")

        shutil.make_archive(
            releases_path / pathlib.Path(archive_filename).stem, "zip", src_data_path
        )
        ...
        if (
            git_provider.upload_release(
                repo_uri, zip_path, archive_filename, upload_git_release_tag
            )
            == 1
        ):
...
def get_repo_props(path: pathlib.Path) -> dict or None:
    repo_json = manifest.get_json_data_file(
        path, "Repo", validation.valid_o3de_repo_json
    )
    if not isinstance(repo_json, dict):
        logger.error(
            f"Could not retrieve repo.json file from {path} or the repo.json file is invalid."
        )
        return None
    return repo_json
...
def _edit_objects(
    ...
    if add_objects or replace_objects:
        if replace_objects:
            repo_objects_data = []
            paths = (
                replace_objects.split()
                if isinstance(replace_objects, str)
                else replace_objects
            )
        else:
        ...
        for object_path in paths:
            ...
            if json_data:
                ...
                if release_archive_path:
                    ...
                    json_data = create_remote_object_archive(
                        object_path,
                ...
                index = _find_index(
                    repo_objects_data,
                    f"{object_typename}_name",
                    json_data[f"{object_typename}_name"],
                )
                ...
                if index != -1:
                    ...
                    if not version_data:
                        ...
                        logger.warning(
                            f"No changes were found between the object JSON data in repo.json and the object data being added from {object_path}"
                        )
                        continue
                    ...
                    if (
                        "download_source_uri" not in json_data
                        and "source_control_uri" not in json_data
                    ):
                        logger.warning(
                            "No download_source_uri found and sorce_control_uri found!"
                        )
                    if "download_source_uri" in json_data:
                        version_data["download_source_uri"] = json_data[
                            "download_source_uri"
                        ]
                    if "source_control_uri" in json_data:
                        version_data["source_control_uri"] = json_data[
                            "source_control_uri"
                        ]
                    if "source_control_ref" in json_data:
                        version_data["source_control_ref"] = json_data[
                            "source_control_ref"
                        ]

                    version_index = _find_index(
                        repo_objects_data[index].get("versions_data", []),
                        "version",
                        json_data["version"],
                    )
                    if version_index != -1:
                        ...
                        if version_data:
                            repo_objects_data[index]["versions_data"][version_index] = (
                                version_data
                            )
                    # if version is same, update field
                    elif (
                        repo_objects_data[index].get("version", "0.0.0")
                        ...
                        if "versions_data" not in repo_objects_data[index]:
                            ...
                            repo_objects_data[index]["versions_data"].append(
                                version_data
                            )
                else:
    ...
    if delete_objects:
        ...
        for removal_object in removal_list:
            ...
            object_name, version_specifier = (
                utils.get_object_name_and_optional_version_specifier(removal_object)
            )
            if not version_specifier:
                # Remove the JSON object(s) with the specified object_name value
                repo_objects_data = [
                    object_typename
                    for object_typename in repo_objects_data
                    if object_typename.get(object_key) != object_name
                ]
            else:
                index = _find_index(
                    repo_objects_data, f"{object_typename}_name", object_name
                )
                # if version exists
                if index > -1:
                    ...
                    if versions_data:
                        ...
                        versions_data = [
                            ...
                            if Version(data.get("version", "0.0.0"))
                            not in SpecifierSet(version_specifier)
                        ]
                    ...
                    if Version(
                        repo_objects_data[index].get("version", "0.0.0")
                    ) in SpecifierSet(version_specifier):
                        if versions_data:
...
def _auto_update_json(
    object_type: str or list, repo_path: pathlib.Path, repo_json: dict
):
    repo_directory = os.path.dirname(repo_path)
    expected_files = {"project.json": [], "gem.json": [], "template.json": []}
    for (
        directory,
        sub_directory,
        files,
    ) in os.walk(repo_directory):
    ...
    for object in objects:
        ...
        if object_name == "project":
            _edit_objects(
                ...
                repo_json,
                expected_files.get("project.json"),
            )
        if object_name == "template":
            _edit_objects(
                ...
                repo_json,
                expected_files.get("template.json"),
            )
    return 0


def print_repo_diff(repo_json: dict, repo_json_original: dict):
    ...
    for key in repo_json:
        ...
        if repo_json_original[key] != repo_json[key]:
            ...
            object_original = {
                object[f"{object_key}_name"]
                for object in repo_json_original.get(key, [])
            }
            object_dry_run = {
                object[f"{object_key}_name"] for object in repo_json.get(key, [])
            }
            # check for new objects
            objects_added = object_dry_run - object_original
            pretty_print_string.append(
                f"{object_key.capitalize()}s Added: {len(objects_added)}"
            )
            ...
            modified_objects = [
                ...
                if dry_run_repo[f"{object_key}_name"]
                == original_repo[f"{object_key}_name"]
                and any(field not in original_repo for field in dry_run_repo)
            ]
            if modified_objects:
                pretty_print_string.append(
                    f"{object_key.capitalize()}s Modified: {len(modified_objects)}"
                )
...
def edit_repo_props(
    ...
    if isinstance(repo_name, str):
        if not utils.validate_identifier(repo_name):
            logger.error(
                f'Repo name must be fewer than 64 characters, contain only alphanumeric, "_" or "-"'
...
def add_parser_args(parser):
    ...
    required_general_group.add_argument(
        "--repo-path",
        "-rp",
        type=pathlib.Path,
    ...
    modify_object_group = parser.add_argument_group(
        "Create Release",
        "Path arguments to use with the --add-objects or --replace-objects option",
    )
    modify_object_group.add_argument(
