...
class CLICommand(object):
    ...
    def run(self) -> int:
        ...
        try:
            with Popen(
                self.args, cwd=self.cwd, env=self.env, stdout=PIPE, stderr=STDOUT
            ) as process:
                self.logger.info(
                    f"Running process '{self.args[0]}' with PID({process.pid}): {self.args}"
                )
...
def load_and_execute_script(
    script_path: pathlib.Path or str, **context_variables
) -> int:
    """
    ...
    try:
        ...
        logger.error(
            f"Failed to run script '{script_path}'. Here is the stacktrace: ",
            exc_info=True,
        )
        return 1
...
def validate_version_specifier_list(version_specifiers: str or list) -> bool:
    version_specifier_list = (
        version_specifiers.split()
        if isinstance(version_specifiers, str)
        else version_specifiers
    )
    if not isinstance(version_specifier_list, list):
        logger.error(
            f"Version specifiers must be in the format <name><version specifiers>. e.g. name==1.2.3 \n {version_specifiers}"
        )
        return False
    ...
    if parsed_uri.scheme in ["http", "https", "ftp", "ftps"]:
        try:
            ...
            with urllib.request.urlopen(current_request) as s:
                ...
                if (
                    file_exists
                    and (
                        resume_position == download_file_size or download_file_size == 0
                    )
                    and not force_overwrite
                ):
                    logger.error(
                        f"File already downloaded to {download_path} and force_overwrite is not set."
                    )
                    return 1
                ...
                if file_exists and force_overwrite:
                    try:
                        ...
                        logger.error(
                            f"Could not remove existing download path {download_path}."
                        )
                        return 1
                ...
                def print_progress(downloaded, total_size):
                    ...
                    if total_size == 0 or downloaded > total_size:
                        print(
                            f"Downloading {object_name if object_name else parsed_uri.geturl()} - {downloaded} bytes"
                        )
                    else:
                        if downloaded == total_size:
                            end_ch = "\n"
                        print(
                            f"Downloading {object_name if object_name else parsed_uri.geturl()} - {downloaded} of {total_size} bytes - {(downloaded / total_size) * 100:.2f}%",
                            end=end_ch,
                        )
                ...
                def download_progress(downloaded_bytes):
                    if download_progress_callback:
                        return download_progress_callback(
                            int(downloaded_bytes), download_file_size
                        )
                    return False
        ...
        if isinstance(download_path, pathlib.PureWindowsPath):
            ...
            if parsed_uri_path.startswith("/"):
                ...
                logger.warning(
                    f"The provided path URI '{parsed_uri_path}' may be missing a '/', "
        ...
        if not origin_file.is_file():
            logger.error(
                f"Failed to find local file '{origin_file}' based on URI '{parsed_uri}'"
            )
            return 1
...
def download_zip_file(
    ...
    if not zipfile.is_zipfile(download_zip_path):
        logger.error(
            f"File zip {download_zip_path} is invalid. Try re-downloading the file."
        )
...
def get_project_path_from_file(
    target_file_path: pathlib.Path, supplied_project_path: pathlib.Path = None
) -> pathlib.Path or None:
    """
    ...
    if not project_path:
        project_path = find_ancestor_dir_containing_file(
            pathlib.PurePath("project.json"), target_file_path
        )

    if not project_path or not valid.valid_o3de_project_json(
...
def get_gem_names_set(gems: list, include_optional: bool = True) -> set:
    ...
    return set(
        [
            gem["name"] if isinstance(gem, dict) else gem
            for gem in gems
            if should_include_gem(gem)
        ]
    )
...
def add_or_replace_object_names(object_names: set, new_object_names: list) -> list:
    ...
    for object_name_with_specifier in object_names:
        object_name, _ = get_object_name_and_optional_version_specifier(
            object_name_with_specifier
        )
...
def update_keys_and_values_in_dict(
    existing_values: dict,
    new_values: list or str,
    remove_values: list or str,
    ...
    if remove_values:
        remove_values = (
            remove_values.split() if isinstance(remove_values, str) else remove_values
        )
...
def update_values_in_key_list(
    ...
    if remove_values:
        ...
        existing_values = list(
            filter(lambda value: value not in remove_values, existing_values)
        )
...
def get_object_name_and_version_specifier(input: str) -> (str, str) or None:
    ...
    if not match:
        raise InvalidVersionSpecifierException(
            f"Invalid name and/or version specifier {input}, expected <name><version specifiers> e.g. o3de==1.2.3"
        )

    if not match.group("object_name"):
        raise InvalidObjectNameException(
            f"Invalid or missing name {input}, expected <name><version specifiers> e.g. o3de==1.2.3"
        )
...
def replace_dict_keys_with_value_key(
    input: dict,
    value_key: str,
    replaced_key_name: str = None,
    place_values_in_list: bool = False,
):
    """
...
def safe_kill_processes(
    *processes: List[Popen], process_logger: logging.Logger = None
) -> None:
    """
    ...
    def on_terminate(proc) -> None:
        try:
            process_logger.info(
                f"process '{proc.args[0]}' with PID({proc.pid}) terminated with exit code {proc.returncode}"
            )
        except Exception:  # purposefully broad
            process_logger.error(
                "Exception encountered with termination request, with stacktrace:",
                exc_info=True,
            )
    ...
    for proc in processes:
        try:
            process_logger.info(
                f"Terminating process '{proc.args[0]}' with PID({proc.pid})"
            )
            proc.kill()
        except Exception:  # purposefully broad
            process_logger.error(
...
def load_template_file(
    ...
    encoding_error_action: str = "ignore",
) -> str:
    """
    ...
    try:
        template_file_content = template_file_path.resolve(strict=True).read_text(
            encoding=read_encoding, errors=encoding_error_action
        )
