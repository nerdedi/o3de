...
def edit_gem_props(
    ...
    if not gem_path and not gem_name:
        logger.error(
            "Either a gem path or a gem name must be supplied to lookup gem.json"
        )
        return 1
    if not gem_path:
    ...
    if not gem_path:
        logger.error(
            f"Error unable locate gem path: No gem with name {gem_name} is registered in any manifest"
        )
        return 1
    ...
    if new_name:
        if not utils.validate_identifier(new_name):
            logger.error(
                f'Engine name must be fewer than 64 characters, contain only alphanumeric, "_" or "-"'
    ...
    if new_tags or remove_tags or replace_tags != None:
        update_key_dict["user_tags"] = utils.update_values_in_key_list(
            gem_json_data.get("user_tags", []), new_tags, remove_tags, replace_tags
        )

    if new_platforms or remove_platforms or replace_platforms != None:
        update_key_dict["platforms"] = utils.update_values_in_key_list(
            gem_json_data.get("platforms", []),
            new_platforms,
            remove_platforms,
            replace_platforms,
        )
    ...
    if (
        ...
        if (
            ...
            or (
                ...
                and not utils.validate_version_specifier_list(remove_compatible_engines)
            )
            or (
        ...
        update_key_dict["compatible_engines"] = utils.update_values_in_key_list(
            gem_json_data.get("compatible_engines", []),
            new_compatible_engines,
            remove_compatible_engines,
            replace_compatible_engines,
        )
    ...
    if (
        ...
        or remove_engine_api_dependencies
        ...
        update_key_dict["engine_api_dependencies"] = utils.update_values_in_key_list(
            gem_json_data.get("engine_api_dependencies", []),
            new_engine_api_dependencies,
            remove_engine_api_dependencies,
            replace_engine_api_dependencies,
        )
    ...
    return (
        0
        if manifest.save_o3de_manifest(
            gem_json_data, pathlib.Path(gem_path) / "gem.json"
        )
        else 1
    )
...
def add_parser_args(parser):
    ...
    group.add_argument(
        "-gn", "--gem-name", type=str, required=False, help="The name of the gem."
    )
    group = parser.add_argument_group(
        "properties", "arguments for modifying individual gem properties."
    )
    group.add_argument(
        "-gnn",
        "--gem-new-name",
    ...
    group.add_argument(
        "-dav",
        "--remove-engine-api-dependencies",
        type=str,
        nargs="*",
        required=False,
