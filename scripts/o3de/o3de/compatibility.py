...
def get_most_compatible_project_engine_path(
    ...
    if not project_json_data:
        logger.error(
            f"Failed to load project.json data from {project_path}. "
    ...
    if not isinstance(user_project_json_data, dict):
        ...
        if user_project_json_path.is_file():
            user_project_json_data = manifest.get_json_data_file(
                user_project_json_path, "project", validation.always_valid
            )
    ...
    for engine_path, engine_json_data in engines_json_data.items():
        ...
        if has_compatible_version([project_engine], engine_name, engine_version):
            ...
            if pathlib.PurePath(project_path).is_relative_to(
                pathlib.PurePath(engine_path)
            ):
...
def get_project_engine_incompatible_objects(
    project_path: pathlib.Path, engine_path: pathlib.Path = None
) -> set:
    """
    ...
    if not engine_json_data:
        logger.error(
            f"Failed to load engine.json data needed for compatibility check from {engine_path}. "
            "Please verify the path is correct, the file exists and is formatted correctly."
        )
    ...
    if not project_json_data:
        logger.error(
            f"Failed to load project.json data needed for compatibility check from {project_path}. "
            "Please verify the path is correct, the file exists and is formatted correctly."
        )
        return set("project.json (missing)")

    incompatible_objects = get_incompatible_objects_for_engine(
        project_json_data, engine_json_data
    )
    ...
    all_gems_json_data = manifest.get_gems_json_data_by_name(
        engine_path, project_path, include_manifest_gems=True
    )

    # Dependency resolution takes into account gem and engine requirements so if
    # it succeeds, all is well
    _, errors = resolve_gem_dependencies(
        active_gem_names, all_gems_json_data, engine_json_data
    )
    if errors:
...
def get_incompatible_gem_dependencies(
    gem_json_data: dict, all_gems_json_data: dict
) -> set:
    """
    ...
    return get_incompatible_gem_version_specifiers(
        gem_json_data, all_gems_json_data, checked_specifiers=set()
    )


def get_gems_project_incompatible_objects(
    gem_paths: list, gem_names: list, project_path: pathlib.Path
) -> set():
    """
    ...
    if not engine_path:
        logger.warning(
            f"Project at path {project_path} is not registered to an engine and compatibility cannot be checked."
        )
        return set()
    ...
    if not project_json_data:
        logger.error(
            f"Failed to load project.json data from {project_path} needed for checking compatibility"
        )
        return set("project.json (missing)")
    ...
    if not engine_json_data:
        logger.error(
            "Failed to load engine.json data based on the engine field in project.json or detect the engine from the current folder"
        )
        return set("engine.json (missing)")
    ...
    all_gems_json_data = manifest.get_gems_json_data_by_name(
        engine_path,
        project_path,
        external_subdirectories=gem_paths,
...
def get_gem_engine_incompatible_objects(
    gem_json_data: dict, engine_json_data: dict, all_gems_json_data: dict
) -> set:
    """
    ...
    """
    incompatible_objects = get_incompatible_objects_for_engine(
        gem_json_data, engine_json_data
    )
    return incompatible_objects.union(
        get_incompatible_gem_dependencies(gem_json_data, all_gems_json_data)
    )


def get_incompatible_objects_for_engine(
    object_json_data: dict, engine_json_data: dict
) -> set:
    """
    ...
    if compatible_engines:
        if engine_version and has_compatible_version(
            compatible_engines, engine_name, engine_version
        ):
            return set()
        elif not engine_version and has_compatible_name(
            compatible_engines, engine_name
        ):
            # assume an engine with no version is compatible
            return set()

        # object is not known to be compatible with this engine
        incompatible_objects.add(
            f"{engine_name} {engine_version} does not match any version specifiers in the list of compatible engines: {compatible_engines}"
        )
    ...
    if engine_api_version_specifiers:
        ...
        for api_version_specifier in engine_api_version_specifiers:
            api_name, _ = utils.get_object_name_and_optional_version_specifier(
                api_version_specifier
            )
            engine_api_version = engine_api_versions.get(api_name, "")
            if not has_compatible_version(
                [api_version_specifier], api_name, engine_api_version
            ):
                if engine_api_version:
                    incompatible_apis.add(
                        f"The engine '{api_name}' version '{engine_api_version}' doesn't satisfy requirement '{api_version_specifier}'"
                    )
                else:
                    incompatible_apis.add(
                        f"The engine doesn't satisfy the requirement '{api_version_specifier}'"
                    )
...
def get_incompatible_gem_version_specifiers(
    gem_json_data: dict, all_gems_json_data: dict, checked_specifiers: set
) -> set:
    """
    ...
    for gem_version_specifier in gem_version_specifier_list:
        ...
        gem_name, version_specifier = (
            utils.get_object_name_and_optional_version_specifier(gem_version_specifier)
        )
        if gem_name not in all_gems_json_data:
            incompatible_gem_version_specifiers.add(
                f"{gem_json_data['gem_name']} is missing the dependency {gem_version_specifier}"
            )
            continue
        ...
        for candidate_gem_json_data in all_gems_json_data[gem_name]:
            if version_specifier:
                ...
                if gem_version and not has_compatible_version(
                    [gem_version_specifier], gem_name, gem_version
                ):
                    if not found_candidate:
                        all_candidate_incompatible_gem_version_specifiers.add(
                            f"{gem_json_data['gem_name']} depends on {gem_version_specifier} but {gem_name} version {gem_version} was found"
                        )
                    continue
                else:
            ...
            candidate_incompatible_gem_version_specifiers = (
                get_incompatible_gem_version_specifiers(
                    candidate_gem_json_data, all_gems_json_data, checked_specifiers
                )
            )
            if candidate_incompatible_gem_version_specifiers:
                all_candidate_incompatible_gem_version_specifiers.update(
                    candidate_incompatible_gem_version_specifiers
                )
            else:
                all_candidate_incompatible_gem_version_specifiers = set()
        ...
        incompatible_gem_version_specifiers.update(
            all_candidate_incompatible_gem_version_specifiers
        )

    return incompatible_gem_version_specifiers


def has_compatible_name(
    name_and_version_specifier_list: list, object_name: str
) -> bool:
    """
...
def has_compatible_version(
    name_and_version_specifier_list: list, object_name: str, object_version: str
) -> bool:
    """
    ...
    try:
        ...
        logger.warning(
            f"Failed to parse version specifier {object_version}, please verify it is PEP 440 compatible."
        )
        return False
    ...
    for name_and_version_specifier in name_and_version_specifier_list:
        ...
        try:
            name, version_specifier = utils.get_object_name_and_version_specifier(
                name_and_version_specifier
            )
            if name == object_name and version in SpecifierSet(version_specifier):
                return True
        except (
            utils.InvalidObjectNameException,
            utils.InvalidVersionSpecifierException,
        ):
...
class GemRequirement(namedtuple("GemRequirement", ["name", "specifier"])):
    def __repr__(self):
        return (
            f"<GemRequirement({self.name}{self.specifier if self.specifier else ''})>"
        )
...
class EngineRequirement(namedtuple("EngineRequirement", ["gem_json_data"])):
    ...
    def failure_reason(self, engine_json_data):
        ...
        incompatible_engine_objects = get_incompatible_objects_for_engine(
            self.gem_json_data, engine_json_data
        )
        incompatible_engine_objects = [
            f"{gem_name} is incompatible because: {error}"
            for error in incompatible_engine_objects
        ]
        return "\n".join(incompatible_engine_objects)


class EngineCandidate(
    namedtuple("Candidate", ["name", "version", "requirements", "engine_json_data"])
):
    def __repr__(self):
...
class GemCandidate(
    namedtuple("Candidate", ["name", "version", "requirements", "gem_json_data"])
):
    def __repr__(self):
...
class GemDependencyProvider(AbstractProvider):
    ...
    def is_satisfied_by(self, requirement, candidate):
        if isinstance(candidate, EngineCandidate) and isinstance(
            requirement, EngineRequirement
        ):
            incompatible_engine_objects = get_incompatible_objects_for_engine(
                requirement.gem_json_data, candidate.engine_json_data
            )
        ...
        elif isinstance(candidate, GemCandidate) and isinstance(
            ...
            return (
                ...
                and (
                    requirement.specifier is None
                    or candidate.version in requirement.specifier
                )
            )
        else:
...
def resolve_gem_dependencies(
    gem_names: list,
    all_gem_json_data: dict,
    engine_json_data: dict,
    ...
    candidates = [
        EngineCandidate(
            engine_json_data.get("engine_name"),
            engine_json_data.get("version", "0.0.0"),
            [],
            engine_json_data,
        )
    ]
    ...
    for gem_name, gem_versions_json_data in all_gem_json_data.items():
        for gem_json_data in gem_versions_json_data:
            ...
            gem_dependency_names = utils.get_gem_names_set(
                gem_dependencies, include_optional=include_optional
            )
            for gem_dependency in gem_dependency_names:
                dep_name, dep_version_specifier = (
                    utils.get_object_name_and_optional_version_specifier(gem_dependency)
                )
            ...
            candidate = GemCandidate(
                gem_name, Version(gem_version), requirements, gem_json_data
            )
    ...
    for gem_name in gem_names:
        dep_name, dep_version_specifier = (
            utils.get_object_name_and_optional_version_specifier(gem_name)
        )
        if dep_version_specifier:
            dep_version_specifier = SpecifierSet(dep_version_specifier)
        ...
        for gem_spec in gem_spec_list:
            ...
            if provided_unique_service:
                provided_unique_service_gem_map.setdefault(
                    provided_unique_service, []
                ).append(gem_spec.get("display_name", gem_name))
    ...
    try:
        ...
        errors.add(
            f"An error exists in the dependency resolver provider resulting in an inconsistent candidate {e.candidate} with criterion {e.criterion}"
        )
        ...
        for cause in e.causes:
            ...
            if isinstance(cause.requirement, EngineRequirement):
                ...
                reason += cause.requirement.failure_reason(
                    cause.parent if cause.parent else "The project"
                )
    ...
    for unique_service, gem_provider_list in provided_unique_service_gem_map.items():
        if len(gem_provider_list) > 1:
            errors.add(
                f"Multiple gems enabled ({', '.join(gem_provider_list)}) that provide the "
