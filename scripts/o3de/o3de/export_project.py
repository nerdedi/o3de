...
CURRENT_PLATFORM_NAME_WITH_CASE = (
    platform.system()
)  # used to find the Installer binaries folder.
CURRENT_PLATFORM = CURRENT_PLATFORM_NAME_WITH_CASE.lower()

ALL_AVAILABLE_ARCHIVE_FORMATS = ["none"] + [
    name for name, description in shutil.get_archive_formats()
]
...
class O3DEScriptExportContext(object):
    ...
    def __init__(
        ...
        cmake_additional_build_args: list = [],
    ) -> None:
        ...
        assert project_json_data, (
            f"Invalid project configuration file '{project_path}/project.json'. Invalid settings."
        )

        project_name = project_json_data.get("project_name")
        assert project_name, (
            f"Invalid project configuration file '{project_path}/project.json'. 'project_name' not found in the settings"
        )
...
def get_default_asset_platform():
    host_platform_to_asset_platform_map = {
        "windows": "pc",
        "linux": "linux",
        "darwin": "mac",
    }
...
def get_platform_installer_folder_name(selected_platform=None):
    ...
    host_platform_to_installer_name_map = {
        "pc": "Windows",
        "linux": "Linux",
        "mac": "Mac",
    }
...
def has_monolithic_artifacts(context: O3DEScriptExportContext):
    monolithic_artifacts = glob.glob(
        f"cmake/Platform/{get_platform_installer_folder_name()}/Monolithic/ConfigurationTypes_*.cmake",
...
def process_command(
    args: list, cwd: pathlib.Path = None, env: os._Environ = None
) -> int:
    """
    ...
    if len(args) == 0:
        logging.error(
            "function `process_command` must be supplied a non-empty list of arguments"
        )
        return 1
...
def execute_python_script(
    target_script_path: pathlib.Path or str,
    o3de_context: O3DEScriptExportContext = None,
    logger=None,
) -> int:
    """
    ...
    return utils.load_and_execute_script(
        target_script_path, o3de_context=o3de_context, o3de_logger=logger
    )
    ...
    for arg in arg_list:
        if not in_cca and not in_cba:
            if arg in ("-cca", "--cmake-configure-arg"):
                ...
                raise ExportProjectError(
                    "Invalid argument '/'. This argument marks terminator for the '-cca'  or '-cba' argument, but is not part of that argument"
                )
            else:
...
def _export_script(
    ...
    if export_script_path.suffix != ".py":
        logging.error(
            f"Invalid export script type for '{export_script_path}'. Please provide a file path to an existing python script with '.py' extension."
        )
        return 1
    ...
    if validated_export_script_path is None:
        logging.error(
            f"Invalid export script '{export_script_path}'. File does not exist."
        )
        return 1
    ...
    computed_project_path = utils.get_project_path_from_file(
        validated_export_script_path, project_path
    )
    if (
        not computed_project_path
        and project_path is not None
        and not project_path.is_absolute()
    ):
        ...
        computed_project_path = utils.get_project_path_from_file(
            validated_export_script_path, LOCAL_ENGINE_PATH / project_path
        )

    if not computed_project_path:
        if project_path:
            logging.error(
                f"Project path '{project_path}' is invalid: does not contain a project.json file."
            )
        else:
            logging.error(
                f"Unable to find project folder associated with file '{validated_export_script_path}'. Please specify using --project-path, or ensure the file is inside a project folder."
            )
        return 1

    export_process_args, cmake_configure_args, cmake_build_args = (
        extract_cmake_custom_args(passthru_args)
    )

    o3de_context = O3DEScriptExportContext(
        export_script_path=validated_export_script_path,
    ...
    return execute_python_script(
        validated_export_script_path, o3de_context, logger=logger
    )
...
def _ui_export(project_path: pathlib.Path) -> int:
    ...
    export_script = pathlib.Path(
        export_config.get_value("option.default.export.script")
    )
...
def _run_export_script(args: argparse, passthru_args: list) -> int:
    ...
    if args.configure:
        if tkinter_installed and tkinter_tix_installed:
            ...
            project_info = manifest.get_project_json_data(
                project_path=args.project_path
            )
            is_o3de_sdk = project_info.get("engine") == "o3de-sdk"
            export_project_ui.MainWindow(
                export_config, is_o3de_sdk
            ).configure_settings()
            return 0
        else:
            if not tkinter_installed:
                print(
                    "Unable to open the configure window. Required package 'tk' is not installed on this system."
                )
            else:
                print(
                    "Unable to open the configure window. Required package 'tix' is not installed on this system."
                )
            return 1
...
def get_project_export_config_from_args(
    args: argparse,
) -> (command_utils.O3DEConfig, str):
    """
    ...
    if is_global:
        if project:
            logger.warning(
                f"Both --global and --project ({project}) arguments were provided. The --project argument will "
        ...
        if project_name:
            logger.info(
                f"The execution of this command will be based on the currently detected project ({project_name})."
            )
            project_export_config = get_export_project_config(project_path=project_path)
        else:
            logger.info(
                "The execution of this command will be based on the global settings."
            )
            project_export_config = get_export_project_config(project_path=None)
    else:
        ...
        if project_path.is_dir():
            ...
            if not project_path:
                raise command_utils.O3DEConfigError(
                    f"Unable to resolve project named '{project_name}'. "
                    f"Make sure it is registered with O3DE."
                )

        logger.info(
            f"The execution of this command will be based on project ({project_name})."
        )
        project_export_config = get_export_project_config(project_path=project_path)
...
def configure_project_export_options(args: argparse) -> int:
    ...
    try:
        ...
        if args.clear_value:
            project_export_config.set_config_value(
                key=args.clear_value, value="", validate_value=False
            )
...
def add_parser_args(parser) -> None:
    parser.add_argument(
        ...
        type=pathlib.Path,
        required=False,
        help="An external Python script to run",
    )
    parser.add_argument(
        "-pp",
        "--project-path",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Configure the project export settings",
    )
    parser.add_argument(
        "-ll",
        "--log-level",
...
def add_args(subparsers) -> None:
    ...
    epilog_lines = [
        "Configure the default values that control the project export scripts for O3DE.",
    ...
    epilog = "\n".join(epilog_lines)
    export_configure_subparser = subparsers.add_parser(
    ...
    export_configure_subparser.add_argument(
        "--global",
        default=False,
        action="store_true",
...
def get_asset_processor_batch_path(
    ...
    if using_installer_sdk:
        asset_processor_batch_path = (
            tools_build_path / f"AssetProcessorBatch{EXECUTABLE_EXTENSION}"
        )
    else:
        asset_processor_batch_path = (
            tools_build_path
            / f"bin/{tool_config}/AssetProcessorBatch{EXECUTABLE_EXTENSION}"
        )
    if required and not asset_processor_batch_path.is_file():
        raise ExportProjectError(
            f"Missing the 'AssetProcessorBatch' tool, expected at '{asset_processor_batch_path}'"
        )
    return asset_processor_batch_path


def get_asset_bundler_batch_path(
    tools_build_path: pathlib.Path,
    using_installer_sdk: bool = False,
    tool_config: str = PREREQUISITE_TOOL_BUILD_CONFIG,
    required: bool = False,
    ...
    if using_installer_sdk:
        asset_bundler_batch_path = (
            tools_build_path / f"AssetBundlerBatch{EXECUTABLE_EXTENSION}"
        )
    else:
        asset_bundler_batch_path = (
            tools_build_path
            / f"bin/{tool_config}/AssetBundlerBatch{EXECUTABLE_EXTENSION}"
        )
    if required and not asset_bundler_batch_path.is_file():
        raise ExportProjectError(
            f"Missing the 'AssetBundlerBatch' tool, expected at '{asset_bundler_batch_path}'"
        )
    return asset_bundler_batch_path


def build_assets(
    ctx: O3DEScriptExportContext,
    tools_build_path: pathlib.Path,
    engine_centric: bool,
    ...
    asset_processor_batch_path = get_asset_processor_batch_path(
        tools_build_path, using_installer_sdk, tool_config=tool_config, required=True
    )
    if not asset_processor_batch_path.exists():
        raise ExportProjectError(
            "Missing AssetProcessorBatch. The pre-requisite tools must be built first."
        )
    ...
    if selected_platforms:
        cmake_build_assets_command.extend(
    ...
    if ret != 0:
        if fail_on_ap_errors:
            raise ExportProjectError(
                f"Error building assets for project {ctx.project_name}."
            )
        else:
            if logger:
...
def build_export_toolchain(
    ...
    if ret != 0:
        raise ExportProjectError(
            "Error generating the project for the pre-requisite tools."
        )
    ...
    if ret != 0:
        raise ExportProjectError(
            "Error building the project for the pre-requisite tools."
        )
    if logger:
        logger.info("Tool chain built successfully.")
...
def validate_export_toolchain(tools_build_path: pathlib.Path):
    ...
    build_tools = [
        get_asset_bundler_batch_path(tools_build_path),
    ...
    if len(tools_missing) > 0:
        raise ExportProjectError(
            f"Necessary Build Tools have not been created! The following are missing: {', '.join(tools_missing)}"
...
def build_game_targets(
    ...
    ) == LauncherType.UNIFIED
    should_build_headless_server_launcher = (
        launcher_types & LauncherType.HEADLESS_SERVER
    ) == LauncherType.HEADLESS_SERVER

    if not (
        should_build_server_launcher
        or should_build_game_launcher
        or should_build_unified_launcher
        or should_build_headless_server_launcher
    ):
        return
    ...
    if logger:
        logger.info(
            f"Generating {'monolithic' if monolithic_build else 'non-monolithic'} build folder for project {ctx.project_name}"
        )
    ret = process_command(cmake_configure_command)
    if ret != 0:
        raise ExportProjectError(
            f"Error generating projects for project {ctx.project_name}."
        )
...
def bundle_assets(
    ...
    asset_bundler_batch_path = get_asset_bundler_batch_path(
        tools_build_path, using_installer_sdk, tool_config=tool_config, required=True
    )
    ...
    for selected_platform in selected_platforms:
        ...
        engine_asset_list_path = (
            asset_list_path / f"engine_{selected_platform}.assetlist"
        )
        ...
        if ret != 0:
            raise RuntimeError(
                f"Error generating game assets lists for {game_asset_list_path}"
            )
        ...
        if ret != 0:
            raise RuntimeError(
                f"Error generating engine assets lists for {engine_asset_list_path}"
            )
        ...
        gen_game_bundle_command = [
            ...
            "--allowOverwrites",
            "--outputBundlePath",
            bundles_path / f"game_{selected_platform}.pak",
            "--assetListFile",
            game_asset_list_path,
        ]
        ret = process_command(
        ...
        if ret != 0:
            raise RuntimeError(
                f"Error generating game bundle for {bundles_path / f'game_{selected_platform}.pak'}"
            )

        gen_engine_bundle_command = [
            asset_bundler_batch_path,
            "bundles",
            "--maxSize",
            str(max_bundle_size),
            ...
            "--allowOverwrites",
            "--outputBundlePath",
            bundles_path / f"engine_{selected_platform}.pak",
            "--assetListFile",
            engine_asset_list_path,
        ]
        ret = process_command(
        ...
        if ret != 0:
            raise RuntimeError(
                f"Error generating engine bundle for {bundles_path / f'game_{selected_platform}.pak'}"
            )
...
def kill_existing_processes(project_name: str):
    o3de_process_names = [
        ...
        "editor",
        "assetprocessor",
        f"{project_name.lower()}.serverlauncher",
    ...
    for process in psutil.process_iter():
        ...
        if (
            os.path.splitext(process.name())[0].lower() in o3de_process_names
            and process.name() != O3DE_SCRIPT_NAME
        ):
...
def setup_launcher_layout_directory(
    project_path: pathlib.Path,
    project_name: str,
    asset_platform: str,
    ...
    ignore_file_patterns = (
        export_layout.ignore_file_patterns + ADDITIONAL_PLATFORM_IGNORE_FILES
    )
    ...
    for file in glob.glob(str(launcher_build_path / f"bin/{build_config}/*")):
        ...
        if not skip:
            if file_path.is_dir():
                shutil.copytree(
                    file, export_layout.output_path / file_path.name, dirs_exist_ok=True
                )
            else:
    ...
    for project_file_pattern in export_layout.project_file_patterns:
        for file in glob.glob(
            str(pathlib.PurePath(project_path / project_file_pattern))
        ):
            shutil.copy(file, export_layout.output_path)
    ...
    if build_config == "profile":
        ...
        setregpatch_file = (
            ...
            / "ExportScripts/IgnoreAssetProcessor.profile.setregpatch"
        )
    ...
    if archive_output_format != "none":
        if logger:
            logger.info(
                f"Archiving output directory {export_layout.output_path} (this may take a while)..."
            )
        shutil.make_archive(
            export_layout.output_path,
            archive_output_format,
            root_dir=export_layout.output_path,
        )


def preprocess_seed_path_list(
    project_path: pathlib.Path, paths: List[pathlib.Path]
) -> List[pathlib.Path]:
    """
...
def validate_project_artifact_paths(
    project_path: pathlib.Path,
    ...
    preprocessed_artifact_paths = preprocess_seed_path_list(
        project_path=project_path, paths=artifact_paths
    )
    ...
    for input_artifact_path in preprocessed_artifact_paths:
        ...
        if validated_artifact_path is None:
            raise ExportProjectError(
                f"Invalid {file_description} provided: '{input_artifact_path}' does not exist."
            )
        else:
...
SETTINGS_PROJECT_BUILD_CONFIG = register_setting(
    key="project.build.config",
...
SETTINGS_TOOL_BUILD_CONFIG = register_setting(
    key="tool.build.config",
...
SETTINGS_ARCHIVE_OUTPUT_FORMAT = register_setting(
    key="archive.output.format",
...
SETTINGS_OPTION_BUILD_ASSETS = register_setting(
    key="option.build.assets",
    description="The option to build all the assets for the bundle when running the export.",
    is_boolean=True,
    default="False",
)

SETTINGS_OPTION_FAIL_ON_ASSET_ERR = register_setting(
    key="option.fail.on.asset.errors",
    description="Option to fail the project export process on any failed asset during asset building.",
    is_boolean=True,
    default="False",
)

SETTINGS_SEED_LIST_PATHS = register_setting(
    key="seedlist.paths",
...
SETTINGS_SEED_FILE_PATHS = register_setting(
    key="seedfile.paths",
...
SETTINGS_DEFAULT_LEVEL_NAMES = register_setting(
    key="default.level.names",
...
SETTINGS_ADDITIONAL_GAME_PROJECT_FILE_PATTERN = register_setting(
    key="additional.game.project.file.pattern.to.copy",
...
SETTINGS_ADDITIONAL_SERVER_PROJECT_FILE_PATTERN = register_setting(
    key="additional.server.project.file.pattern.to.copy",
...
SETTINGS_ADDITIONAL_PROJECT_FILE_PATTERN = register_setting(
    key="additional.project.file.pattern.to.copy",
...
SETTINGS_OPTION_BUILD_TOOLS = register_setting(
    key="option.build.tools",
...
SETTINGS_DEFAULT_ANDROID_BUILD_PATH = register_setting(
    key="default.android.build.path",
...
SETTINGS_ANDROID_ASSET_MODE = register_setting(
    key="option.android.assets.mode",
...
SETTINGS_ANDROID_DEPLOY = register_setting(
    key="option.android.deploy",
...
SETTINGS_DEFAULT_IOS_BUILD_PATH = register_setting(
    key="default.ios.build.path",
    description="Designates where the iOS build files are generated.",
    default="build/game_ios",
)

SETTINGS_OPTION_ALLOW_REGISTRY_OVERRIDES = register_setting(
    key="option.allow.registry.overrides",
...
SETTINGS_DEFAULT_BUILD_TOOLS_PATH = register_setting(
    key="default.build.tools.path",
    description="Designates where the build files for the O3DE toolchain are generated.",
    default="build/tools",
)

SETTINGS_DEFAULT_LAUNCHER_TOOLS_PATH = register_setting(
    key="default.launcher.build.path",
    description="Designates where the launcher build files (Game/Server/Unified) are generated.",
    default="build/launcher",
)

SETTINGS_DEFAULT_ASSET_BUNDLING_PATH = register_setting(
    key="asset.bundling.path",
...
SETTINGS_MAX_BUNDLE_SIZE = register_setting(
    key="max.size",
    description="The maximum size of a given asset bundle.",
    default="2048",
    restricted_regex="(^[0-9]+$)",
    restricted_regex_description="Value must be a whole number.",
)

SETTINGS_OPTION_BUILD_GAME_LAUNCHER = register_setting(
    key="option.build.game.launcher",
    description="The option to build an exported game launcher.",
    is_boolean=True,
    default="True",
)

SETTINGS_OPTION_BUILD_SERVER_LAUNCHER = register_setting(
    key="option.build.server.launcher",
    description="The option to build an exported server launcher.",
    is_boolean=True,
    default="True",
)

SETTINGS_OPTION_BUILD_HEADLESS_SERVER_LAUNCHER = register_setting(
    key="option.build.headless.server.launcher",
    description="The option to build an exported headless server launcher.",
    is_boolean=True,
    default="False",
)

SETTINGS_OPTION_BUILD_UNIFIED_SERVER_LAUNCHER = register_setting(
    key="option.build.unified.launcher",
    description="The option to build an exported unified launcher.",
    is_boolean=True,
    default="True",
)

SETTINGS_OPTION_ENGINE_CENTRIC = register_setting(
    key="option.engine.centric",
    description="Option use the engine-centric work flow to export the project.",
    is_boolean=True,
    default="False",
)

SETTINGS_OPTION_BUILD_MONOLITHICALLY = register_setting(
    key="option.build.monolithic",
    description="The option to build the launchers monolithically vs non-monolithically.",
    is_boolean=True,
    default="True",
)

SETTINGS_OPTION_KILL_PRIOR_PROCESSES = register_setting(
    key="option.processes.kill",
    description="Before running the export process, this will kill all currently running O3DE processes.",
    is_boolean=True,
    default="False",
)

SETTINGS_DEFAULT_EXPORT_SCRIPT = register_setting(
    key="option.default.export.script",
    description="The default export script to execute.",
    default="export_source_built_project.py",
)

SETTINGS_DEFAULT_OUTPUT_PATH = register_setting(
    key="option.default.output.path",
    description="The default target export path.",
    default="build/export",
)


def get_export_project_config(
    project_path: pathlib.Path or None,
) -> command_utils.O3DEConfig:
    """
