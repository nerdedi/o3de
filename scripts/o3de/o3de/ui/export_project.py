...
class MainWindow(tkp.Tk):
    ...
    def add_simple_text_entry(
        self,
        parent: tk.Frame or tk.LabelFrame,
        config_key: str,
        label_text: str,
        ...
        assert config_key not in self.config_key_entry_map.keys(), (
            f"Duplicate config key {config_key}"
        )
        ...
        label = tk.Label(
            parent, text=label_text, anchor=tk.W, width=self.min_label_width
        )
        label.grid(
            column=0, sticky=tk.W, padx=self.field_padding, pady=self.field_padding
        )
        ...
        entry.grid(
            ...
            column=1,
            sticky=tk.EW,
            padx=self.field_padding,
            pady=self.field_padding,
        )

        self.config_key_entry_map[config_key] = (
            label_text,
            default_value,
            entry_var,
            settings,
    ...
    def add_dropdown_entry(
        ...
        assert default_value in choices, (
            f"{default_value} is not in the list of choices ({','.join(choices)})"
        )
    ...
    def add_labeled_checkbox(
        ...
        default_value = (
            1 if self.export_config.get_value(config_key).lower() == "true" else 0
        )
        settings = self.export_config.get_settings_description(config_key)
        tooltip_description = settings.description
        ...
        button = tk.Checkbutton(
            parent, text=label_text, variable=check_button_var, onvalue=1, offvalue=0
        )
    ...
    def add_multi_file_entry(
        self,
        parent: tk.Frame or tk.LabelFrame,
        config_key: str,
        label_text: str,
        multifile_file_select: bool = False,
        entry_read_only: bool = False,
        file_filters=None,
    ) -> None:
        """
        ...
        if multifile_file_select:
            button_add = tk.Button(
                ...
                text="...",
                width=3,
                command=lambda: MainWindow.open_multi_file_dialog(
                    parent=parent, str_var=entry_var, file_filters=file_filters
                ),
            )
        else:
            button_add = tk.Button(
                ...
                text="...",
                width=3,
                command=lambda: MainWindow.open_single_file_dialog(
                    parent=parent,
                    str_var=entry_var,
                    file_filters=file_filters,
                    starting_dir=EXPORT_SCRIPTS_PATH,
                ),
            )
        button_add.grid(
            row=sub_row_line,
    ...
    def add_multi_text_entry(
        ...
        button_add = tk.Button(
            ...
            text="...",
            width=3,
            command=lambda: MainWindow.open_multi_text_dialog(
                parent=parent, str_var=entry_var
            ),
        )
    ...
    def open_multi_file_dialog(parent, str_var: tk.StringVar, file_filters):
        ...
        """
        value = str_var.get()
        result = multiple_file_picker.Dialog(
            parent=parent, file_filters=file_filters, initial_list=value
        ).get_result()
        str_var.set(result)

    @staticmethod
    def open_single_file_dialog(
        parent, str_var: tk.StringVar, file_filters, starting_dir
    ):
        result = filedialog.askopenfilename(
            filetypes=file_filters, initialdir=starting_dir
        )
    ...
    def init_source_engine_build_options(self, parent):
        ...
        :param parent:  The parent widget grid container
        """
        tool_chain_build_options_frame = tk.LabelFrame(
            parent, text="Toolchain Build Options"
        )
        ...
        self.add_labeled_checkbox(
            parent=tool_chain_build_options_frame,
    ...
    def init_project_build_options(self, parent):
        ...
        :param parent:  The parent widget grid container
        """
        project_build_options_frame = tk.LabelFrame(
            parent, text="Project Build Options"
        )
        ...
        self.add_dropdown_entry(
            parent=project_build_options_frame,
    ...
    def init_asset_bundling_options(self, parent):
        ...
        :param parent:  The parent widget grid container
        """
        project_asset_bundling_options_frame = tk.LabelFrame(
            parent, text="Asset/Bundling Options"
        )
        ...
        self.add_labeled_checkbox(
            parent=project_asset_bundling_options_frame,
    ...
    def init_android_tab(self, parent):
        ...
        self.add_labeled_checkbox(
            parent=android_build_options_frame,
    ...
    def init_okay_cancel_buttons(self, parent):
        ...
        button_okay = tk.Button(
            okay_cancel_frame, text="Save", width=8, command=self.on_ok
        )
        button_okay.grid(column=0, sticky=tk.E)
        okay_row = button_okay.grid_info().get("row")

        button_cancel = tk.Button(
            okay_cancel_frame, text="Cancel", width=8, command=self.on_cancel
        )
    ...
    def on_ok(self):
        for key, item in self.config_key_entry_map.items():
            ...
            if previous_value != current_value:
                ...
                try:
                    self.export_config.set_config_value(
                        key=key, value=set_value, validate_value=True, show_log=False
                    )
                except Exception as e:
                    messagebox.showerror(
                        "Error", f"Input for {label_text} is not a valid value: {e}"
                    )
                    widget.focus_set()
                    return
