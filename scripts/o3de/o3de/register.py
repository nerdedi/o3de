...
def register_shipped_engine_o3de_objects(force: bool = False) -> int:
    ...
    error_code = register_all_engines_in_folder(
        manifest.get_registered(default_folder="engines"), force=force
    )
    if error_code:
        ret_val = error_code
    error_code = register_all_projects_in_folder(
        manifest.get_registered(default_folder="projects")
    )
    if error_code:
        ret_val = error_code
    error_code = register_all_gems_in_folder(
        manifest.get_registered(default_folder="gems")
    )
    if error_code:
        ret_val = error_code
    error_code = register_all_templates_in_folder(
        manifest.get_registered(default_folder="templates")
    )
    if error_code:
        ret_val = error_code
    error_code = register_all_restricted_in_folder(
        manifest.get_registered(default_folder="restricted")
    )
    if error_code:
        ret_val = error_code
    error_code = register_all_restricted_in_folder(
...
def register_all_in_folder(
    ...
    if not folder_path:
        logger.error("Folder path cannot be empty.")
        return 1

    folder_path = pathlib.Path(folder_path).resolve()
    if not folder_path.is_dir():
        logger.error("Folder path is not a dir.")
        return 1
    ...
    for project in sorted(projects_set, reverse=True):
        error_code = register(
            engine_path=engine_path, project_path=project, remove=remove
        )
        if error_code:
            ret_val = error_code
    ...
    for template in sorted(templates_set, reverse=True):
        error_code = register(
            engine_path=engine_path, template_path=template, remove=remove
        )
        if error_code:
            ret_val = error_code
    ...
    for restricted in sorted(restricted_set, reverse=True):
        error_code = register(
            engine_path=engine_path, restricted_path=restricted, remove=remove
        )
        if error_code:
            ret_val = error_code
...
def register_all_o3de_objects_of_type_in_folder(
    o3de_object_path: pathlib.Path,
    ...
    **register_kwargs,
    ...
    register_path_kwarg = (
        f"{o3de_object_type}_path"
        if o3de_object_type != "repo"
        else f"{o3de_object_type}_uri"
    )
    ...
    for o3de_object_type_root in sorted(o3de_object_type_set, reverse=True):
        error_code = register(
            **{register_path_kwarg: o3de_object_type_root},
            remove=remove,
            force=force,
            **register_kwargs,
        )
...
def register_all_engines_in_folder(
    engines_path: pathlib.Path, remove: bool = False, force: bool = False
) -> int:
    return register_all_o3de_objects_of_type_in_folder(
        engines_path, "engine", remove, force, None
    )


def register_all_projects_in_folder(
    ...
    return register_all_o3de_objects_of_type_in_folder(
        ...
        remove,
        force,
        stop_on_template_folders,
        engine_path=engine_path,
        dry_run=dry_run,
    )


def register_all_gems_in_folder(
    gems_path: pathlib.Path,
    remove: bool = False,
    engine_path: pathlib.Path = None,
...
def register_all_templates_in_folder(
...
def register_all_restricted_in_folder(
...
def register_all_repos_in_folder(
    ...
    return register_all_o3de_objects_of_type_in_folder(
        repos_path, "repo", remove, force, None, engine_path=engine_path
    )


def remove_engine_name_to_path(json_data: dict, engine_path: pathlib.Path) -> int:
    """
    ...
    """
    if engine_path.is_dir() and validation.valid_o3de_engine_json(
        pathlib.Path(engine_path).resolve() / "engine.json"
    ):
...
def add_engine_name_to_path(json_data: dict, engine_path: pathlib.Path, force: bool):
    ...
    if not engine_json_data:
        logger.error(
            f"Unable to retrieve json data from engine.json at path {engine_path.as_posix()}"
        )
        return 1
    engines_path_json = json_data.setdefault("engines_path", {})
    if "engine_name" not in engine_json_data:
        logger.error(
            f'engine.json at path {engine_path.as_posix()} is missing "engine_name" key'
        )
        return 1
    ...
    if (
        not force
        and engine_name in engines_path_json
        and pathlib.PurePath(engines_path_json[engine_name]) != engine_path
    ):
        logger.info("Engine successfully registered.")
        logger.warning(
...
def register_o3de_object_path(
    ...
    if remove:
        logger.info(
            f"Removing `{o3de_object_path}` from key `{o3de_object_key}` in the manifest."
        )
    else:
        logger.info(
            f"Adding `{o3de_object_path}` to key `{o3de_object_key}` in the manifest."
        )

    save_path = None

    if not o3de_object_path:
        logger.error("o3de object path cannot be empty.")
        return 1
    ...
    if engine_path and project_path:
        logger.error(
            f"Both a project path: {project_path} and engine path: {engine_path} has been supplied."
    ...
    if not dry_run:
        manifest_data[o3de_object_key] = list(
            filter(
                lambda p: pathlib.Path(p) not in paths_to_remove,
...
def register_engine_path(
    ...
    def transform_engine_dict_to_string(engine):
        return engine.get("path", "") if isinstance(engine, dict) else engine

    json_data["engines"] = list(map(transform_engine_dict_to_string, engine_list))

    result = register_o3de_object_path(
        json_data,
        engine_path,
        "engines",
        "engine.json",
        validation.valid_o3de_engine_json,
        remove,
    )
    if result != 0:
        return result
...
def register_external_subdirectory(
    ...
    if (
        not force_register_with_o3de_manifest
        and not gem_path
        and not project_path
        and not engine_path
    ):
        if not gem_path and external_subdir_path.parent != external_subdir_path:
            gem_path = utils.find_ancestor_dir_containing_file(
                pathlib.PurePath("gem.json"), external_subdir_path.parent
            )
        if not gem_path:
            project_path = utils.find_ancestor_dir_containing_file(
                pathlib.PurePath("project.json"), external_subdir_path
            )
        if not project_path:
            engine_path = utils.find_ancestor_dir_containing_file(
                pathlib.PurePath("engine.json"), external_subdir_path
            )
    return register_o3de_object_path(
        json_data,
        external_subdir_path,
        "external_subdirectories",
        "",
        None,
        remove,
        pathlib.Path(engine_path).resolve()
        if not force_register_with_o3de_manifest and engine_path
        else None,
        pathlib.Path(project_path).resolve()
        if not force_register_with_o3de_manifest and project_path
        else None,
        pathlib.Path(gem_path).resolve()
        if not force_register_with_o3de_manifest and gem_path
        else None,
    )


def register_gem_path(
    json_data: dict,
    ...
    if (
        ...
        if gem_path.parent != gem_path:
            ancestor_gem_path = utils.find_ancestor_dir_containing_file(
                pathlib.PurePath("gem.json"), gem_path.parent
            )
        if not ancestor_gem_path:
            project_path = utils.find_ancestor_dir_containing_file(
                pathlib.PurePath("project.json"), gem_path
            )
        if not project_path:
            engine_path = utils.find_ancestor_dir_containing_file(
                pathlib.PurePath("engine.json"), gem_path
            )

    if not remove and not force:
        gem_json_data = manifest.get_gem_json_data(
            gem_path=gem_path, project_path=project_path
        )
        if not gem_json_data:
            logger.error(
                f"Failed to load gem.json data needed for registration from {gem_path}"
            )
            return 1
        ...
        if project_path and manifest.get_project_engine_path(project_path):
            ...
            incompatible_objects = compatibility.get_gems_project_incompatible_objects(
                [gem_path], [gem_json_data["gem_name"]], project_path
            )
            if incompatible_objects:
                logger.error(
                    f"{gem_json_data['gem_name']} is not known to be compatible with the "
            ...
            if not engine_json_data:
                logger.error(
                    f"Failed to load engine.json data needed for registration from {engine_path}"
                )
                return 1
            ...
            engine_gems_json_data = manifest.get_gems_json_data_by_name(
                engine_path=engine_path, external_subdirectories=[gem_path]
            )

            incompatible_objects = compatibility.get_gem_engine_incompatible_objects(
                gem_json_data, engine_json_data, engine_gems_json_data
            )
            if incompatible_objects:
                logger.error(
                    f"{gem_json_data['gem_name']} is not known to be compatible with the "
    ...
    result = register_o3de_object_path(
        ...
        "external_subdirectories",
        "gem.json",
        validation.valid_o3de_gem_json,
        remove,
        pathlib.Path(engine_path).resolve()
        if not force_register_with_o3de_manifest and engine_path
        else None,
        ...
        pathlib.Path(ancestor_gem_path).resolve()
        if not force_register_with_o3de_manifest and ancestor_gem_path
        else None,
        dry_run=dry_run,
    )
    ...
    if result == 0 and dry_run:
        logger.info(
            f"Gem path {gem_path} was not registered because the --dry-run option was specified"
        )
...
def register_project_path(
    ...
    if not engine_path:
        engine_path = utils.find_ancestor_dir_containing_file(
            pathlib.PurePath("engine.json"), project_path
        )

    if not remove:
        engine_json_data = manifest.get_engine_json_data(
        ...
        if not project_json_data:
            logger.error(
                f"Failed to load project.json data needed for registration from {project_path}"
            )
            return 1

        if not force:
            incompatible_objects = (
                compatibility.get_project_engine_incompatible_objects(
                    project_path, engine_path
                )
            )
            if incompatible_objects:
                logger.error(
                    f"{project_json_data['project_name']} is not known to be compatible with the "
                    "following objects/APIs and requires the --force parameter to register:\n  "
                    + "\n  ".join(incompatible_objects)
                )
                return 1
    ...
    result = register_o3de_object_path(
        ...
        "projects",
        "project.json",
        validation.valid_o3de_project_json,
        remove,
    ...
    if not remove:
        ...
        try:
            update_project_json = (
                ...
                or project_json_data["engine_version"] != engine_json_data["version"]
            )
        except KeyError:
        ...
        if not dry_run:
            ...
            cmake.update_cmake_presets_for_project(
                preset_path=project_path / cmake.PROJECT_ENGINE_PRESET_RELATIVE_PATH,
    ...
    if dry_run:
        logger.info(
            "Project path was not registered because --dry-run option was specified"
        )
...
def register_template_path(
    ...
    if not project_path and not engine_path:
        project_path = utils.find_ancestor_dir_containing_file(
            pathlib.PurePath("project.json"), template_path
        )
        if not project_path:
            engine_path = utils.find_ancestor_dir_containing_file(
                pathlib.PurePath("engine.json"), template_path
            )
    return register_o3de_object_path(
        json_data,
        template_path,
        "templates",
        "template.json",
        validation.valid_o3de_template_json,
        remove,
        pathlib.Path(engine_path).resolve() if engine_path else None,
...
def register_restricted_path(
    ...
    if not project_path and not engine_path:
        project_path = utils.find_ancestor_dir_containing_file(
            pathlib.PurePath("project.json"), restricted_path
        )
        if not project_path:
            engine_path = utils.find_ancestor_dir_containing_file(
                pathlib.PurePath("engine.json"), restricted_path
            )
    return register_o3de_object_path(
        json_data,
        restricted_path,
        "restricted",
        "restricted.json",
        validation.valid_o3de_restricted_json,
        remove,
        pathlib.Path(engine_path).resolve() if engine_path else None,
        pathlib.Path(project_path).resolve() if project_path else None,
    )
...
def register_repo(json_data: dict, repo_uri: str, remove: bool = False) -> int:
...
def register_default_o3de_object_folder(
    json_data: dict, default_o3de_object_folder: pathlib.Path, o3de_object_key: str
) -> int:
    # make sure the path exists
    default_o3de_object_folder = pathlib.Path(default_o3de_object_folder).resolve()
    if not default_o3de_object_folder.is_dir():
        logger.error(
            f"Default o3de object folder {default_o3de_object_folder} does not exist."
        )
        return 1
...
def register_default_engines_folder(
    json_data: dict, default_engines_folder: pathlib.Path, remove: bool = False
) -> int:
    return register_default_o3de_object_folder(
        json_data,
...
def register_default_projects_folder(
    json_data: dict, default_projects_folder: pathlib.Path, remove: bool = False
) -> int:
    return register_default_o3de_object_folder(
        json_data,
...
def register_default_gems_folder(
    json_data: dict, default_gems_folder: pathlib.Path, remove: bool = False
) -> int:
    return register_default_o3de_object_folder(
        json_data,
...
def register_default_templates_folder(
    json_data: dict, default_templates_folder: pathlib.Path, remove: bool = False
) -> int:
    return register_default_o3de_object_folder(
        json_data,
...
def register_default_restricted_folder(
    json_data: dict, default_restricted_folder: pathlib.Path, remove: bool = False
) -> int:
    return register_default_o3de_object_folder(
        json_data,
...
def register_default_third_party_folder(
    json_data: dict, default_third_party_folder: pathlib.Path, remove: bool = False
) -> int:
    return register_default_o3de_object_folder(
        json_data,
        manifest.get_o3de_third_party_folder()
        if remove
        else default_third_party_folder,
...
def remove_invalid_o3de_projects(manifest_path: pathlib.Path = None) -> int:
    ...
    for project in json_data.get("projects", []):
        if not validation.valid_o3de_project_json(
            pathlib.Path(project).resolve() / "project.json", generate_uuid=True
        ):
...
def remove_invalid_o3de_objects() -> None:
    for engine_path in manifest.get_manifest_engines():
        if not validation.valid_o3de_engine_json(
            pathlib.Path(engine_path).resolve() / "engine.json"
        ):
    ...
    for external in manifest.get_manifest_external_subdirectories():
        ...
        if not external.is_dir():
            ...
            register(
                engine_path=engine_path, external_subdir_path=external, remove=True
            )

    for template in manifest.get_manifest_templates():
        if not validation.valid_o3de_template_json(
            pathlib.Path(template).resolve() / "template.json"
        ):
    ...
    for restricted in manifest.get_manifest_restricted():
        if not validation.valid_o3de_restricted_json(
            pathlib.Path(restricted).resolve() / "restricted.json"
        ):
    ...
    default_engines_folder = pathlib.Path(
        json_data.get("default_engines_folder", manifest.get_o3de_engines_folder())
    ).resolve()
    ...
    default_projects_folder = pathlib.Path(
        json_data.get("default_projects_folder", manifest.get_o3de_projects_folder())
    ).resolve()
    ...
    default_gems_folder = pathlib.Path(
        json_data.get("default_gems_folder", manifest.get_o3de_gems_folder())
    ).resolve()
    if not default_gems_folder.is_dir():
        ...
        logger.warning(
            f"Default gems folder {default_gems_folder} is invalid."
    ...
    default_templates_folder = pathlib.Path(
        json_data.get("default_templates_folder", manifest.get_o3de_templates_folder())
    ).resolve()
    ...
    default_third_party_folder = pathlib.Path(
        json_data.get(
            "default_third_party_folder", manifest.get_o3de_third_party_folder()
        )
    ).resolve()
...
def register(
    ...
    try:
        ...
        if not force:
            logger.error(
                "O3DE object registration has halted due to JSON Decode Error in manifest at path:"
    ...
    if isinstance(project_path, pathlib.PurePath):
        if not project_path:
            ...
            return 1
        result = result or register_project_path(
            json_data, project_path, remove, engine_path, force, dry_run
        )
    ...
    if isinstance(gem_path, pathlib.PurePath):
        if not gem_path:
            ...
            return 1
        result = result or register_gem_path(
            json_data,
            gem_path,
            remove,
            external_subdir_engine_path,
            external_subdir_project_path,
    ...
    if isinstance(external_subdir_path, pathlib.PurePath):
        if not external_subdir_path:
            logger.error("External Subdirectory path is None.")
            return 1
        result = result or register_external_subdirectory(
            json_data,
            external_subdir_path,
            remove,
            external_subdir_engine_path,
    ...
    if isinstance(template_path, pathlib.PurePath):
        if not template_path:
            ...
            return 1
        result = result or register_template_path(
            json_data, template_path, remove, project_path, engine_path
        )

    if isinstance(restricted_path, pathlib.PurePath):
        if not restricted_path:
            logger.error("Restricted path cannot be empty.")
            return 1
        result = result or register_restricted_path(
            json_data, restricted_path, remove, project_path, engine_path
        )

    if isinstance(repo_uri, str):
        if not repo_uri:
            logger.error("Repo URI cannot be empty.")
            return 1
        result = result or register_repo(json_data, repo_uri, remove)

    if isinstance(default_engines_folder, pathlib.PurePath):
        result = result or register_default_engines_folder(
            json_data, default_engines_folder, remove
        )

    if isinstance(default_projects_folder, pathlib.PurePath):
        result = result or register_default_projects_folder(
            json_data, default_projects_folder, remove
        )

    if isinstance(default_gems_folder, pathlib.PurePath):
        result = result or register_default_gems_folder(
            json_data, default_gems_folder, remove
        )

    if isinstance(default_templates_folder, pathlib.PurePath):
        result = result or register_default_templates_folder(
            json_data, default_templates_folder, remove
        )

    if isinstance(default_restricted_folder, pathlib.PurePath):
        result = result or register_default_restricted_folder(
            json_data, default_restricted_folder, remove
        )

    if isinstance(default_third_party_folder, pathlib.PurePath):
        result = result or register_default_third_party_folder(
            json_data, default_third_party_folder, remove
        )
...
def _run_register(args: argparse) -> int:
    ...
    if args.update:
        ...
        return register_all_engines_in_folder(
            args.all_engines_path, args.remove, args.force
        )
    elif args.all_projects_path:
        return register_all_projects_in_folder(
            args.all_projects_path, args.remove, force=args.force, dry_run=args.dry_run
        )
    elif args.all_gems_path:
        return register_all_gems_in_folder(
            args.all_gems_path, args.remove, force=args.force, dry_run=args.dry_run
        )
    elif args.all_templates_path:
        return register_all_templates_in_folder(
            args.all_templates_path, args.remove, force=args.force, dry_run=args.dry_run
        )
    elif args.all_restricted_path:
        return register_all_restricted_in_folder(
            args.all_restricted_path,
            args.remove,
            force=args.force,
            dry_run=args.dry_run,
        )
    elif args.all_repo_uri:
        return register_all_repos_in_folder(
            args.all_restricted_path, args.remove, args.force
        )
    else:
        return register(
...
def add_parser_args(parser):
    ...
    external_subdir_group = parser.add_argument_group(
        title="external-subdirectory",
    ...
    external_subdir_path_group.add_argument(
        "-esep",
        "--external-subdirectory-engine-path",
        type=pathlib.Path,
        help="If supplied, registers the external subdirectory with the engine.json at"
        " the engine-path location",
    )
    external_subdir_path_group.add_argument(
        "-espp", "--external-subdirectory-project-path", type=pathlib.Path
    )
    external_subdir_path_group.add_argument(
        "-esgp",
        "--external-subdirectory-gem-path",
        type=pathlib.Path,
        help="If supplied, registers the external subdirectory with the gem.json at the gem-path location",
    )
    external_subdir_path_group.add_argument(
        "-frwom",
        "--force-register-with-o3de-manifest",
        action="store_true",
        default=False,
...
def main():
    ...
    logger.info(
        "Success!" if ret == 0 else "Completed with issues: result {}".format(ret)
    )
