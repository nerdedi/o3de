...
def get_project_props(
    name: str = None, path: pathlib.Path = None, user: bool = False
) -> dict:
    proj_json = manifest.get_project_json_data(
        project_name=name, project_path=path, user=user
    )
...
def _edit_gem_names(
    ...
    if new_gem_names:
        add_list = (
            new_gem_names.split() if isinstance(new_gem_names, str) else new_gem_names
        )
        if is_optional_gem:
        ...
        def is_version_of_gem(candidate: str or dict, gem_name) -> bool:
            candidate_gem_name = (
                candidate if isinstance(candidate, str) else candidate.get("name")
            )
            candidate_gem_name_only, _ = (
                utils.get_object_name_and_optional_version_specifier(candidate_gem_name)
            )
        ...
        if proj_json.get("gem_names", []):
            for new_gem_name in add_list:
                new_gem_name = (
                    new_gem_name
                    if isinstance(new_gem_name, str)
                    else new_gem_name["name"]
                )
                ...
                proj_json["gem_names"] = [
                    gem
                    for gem in proj_json["gem_names"]
                    if not is_version_of_gem(gem, gem_name_only)
                ]
    ...
    if delete_gem_names:
        removal_list = (
            delete_gem_names.split()
            if isinstance(delete_gem_names, str)
            else delete_gem_names
        )
        if "gem_names" in proj_json:

            def in_list(gem: str or dict, remove_list: list) -> bool:
                gem_name_with_specifier = (
                    gem.get("name", "") if isinstance(gem, dict) else gem
                )
                gem_name, _ = utils.get_object_name_and_optional_version_specifier(
                    gem_name_with_specifier
                )
                return gem_name_with_specifier in remove_list or gem_name in remove_list

            proj_json["gem_names"] = [
                gem for gem in proj_json["gem_names"] if not in_list(gem, removal_list)
            ]

    if replace_gem_names:
        tag_list = (
            replace_gem_names.split()
            if isinstance(replace_gem_names, str)
            else replace_gem_names
        )
    ...
    proj_json["gem_names"] = sorted(
        proj_json["gem_names"],
        key=lambda gem_name: gem_name.get("name")
        if isinstance(gem_name, dict)
        else gem_name,
    )


def edit_project_props(
    proj_path: pathlib.Path = None,
    proj_name: str = None,
    new_name: str = None,
    new_id: str = None,
    new_origin: str = None,
    ...
    if isinstance(new_name, str):
        if (not user or (user and new_name)) and not utils.validate_identifier(
            new_name
        ):
            logger.error(
                f'Project name must be fewer than 64 characters, contain only alphanumeric, "_" or "-" characters, and start with a letter.  {new_name}'
            )
            return 1
    ...
    if isinstance(new_engine_path, pathlib.Path):
        if not user:
            logger.error(
                "Setting the engine_path in the shared project.json is not allowed to prevent adding local paths.  Run the command again with the --user argument to set the engine_path locally only."
            )
            return 1
        ...
        if new_engine_path and new_engine_path.name:
            ...
            engine_manifest_data = manifest.get_engine_json_data(
                engine_path=new_engine_path.resolve()
            )
            if not engine_manifest_data:
                logger.error(
                    f"Cannot load engine.json data at path {new_engine_path} ({engine_path_absolute.resolve()}), please verify an engine exists at the supplied location with a valid engine.json file."
                )
                return 1

        # if the path is empty use an empty string because as_posix() will return "."
        proj_json["engine_path"] = (
            new_engine_path.as_posix() if new_engine_path.name else ""
        )
    if isinstance(new_engine_finder_cmake_path, pathlib.Path):
        proj_json["engine_finder_cmake_path"] = (
            new_engine_finder_cmake_path.as_posix()
            if new_engine_finder_cmake_path.name
            else ""
        )

    if new_tags or delete_tags or replace_tags != None:
        proj_json["user_tags"] = utils.update_values_in_key_list(
            proj_json.get("user_tags", []), new_tags, delete_tags, replace_tags
        )
    ...
    if (
        ...
        or delete_compatible_engines
        or replace_compatible_engines != None
    ):
        if (
            (
                new_compatible_engines
                and not utils.validate_version_specifier_list(new_compatible_engines)
            )
            or (
                delete_compatible_engines
                and not utils.validate_version_specifier_list(delete_compatible_engines)
            )
            or (
                replace_compatible_engines
                and not utils.validate_version_specifier_list(
                    replace_compatible_engines
                )
            )
        ):
        ...
        proj_json["compatible_engines"] = utils.update_values_in_key_list(
            proj_json.get("compatible_engines", []),
            new_compatible_engines,
            delete_compatible_engines,
            replace_compatible_engines,
        )
    ...
    if (
        new_engine_api_dependencies
        or delete_engine_api_dependencies
        or replace_engine_api_dependencies != None
    ):
        if (
            (
                new_engine_api_dependencies
            ...
            or (
                delete_engine_api_dependencies
                and not utils.validate_version_specifier_list(
                    delete_engine_api_dependencies
                )
            )
        ...
        proj_json["engine_api_dependencies"] = utils.update_values_in_key_list(
            proj_json.get("engine_api_dependencies", []),
            new_engine_api_dependencies,
            delete_engine_api_dependencies,
            replace_engine_api_dependencies,
        )
    ...
    proj_json_path = (
        pathlib.Path(proj_path) if not user else pathlib.Path(proj_path) / "user"
    )
    return (
        0
        if manifest.save_o3de_manifest(proj_json, proj_json_path / "project.json")
        else 1
    )


def _edit_project_props(args: argparse) -> int:
    return edit_project_props(
...
def add_parser_args(parser):
    ...
    group = parser.add_argument_group(
        "properties", "arguments for modifying individual project properties."
    )
    group.add_argument(
        "-pv",
        "--project-version",
    ...
    group.add_argument(
        "-dav",
        "--delete-engine-api-dependencies",
        type=str,
        nargs="*",
        required=False,
    ...
    group.add_argument(
        "-rav",
        "--replace-engine-api-dependencies",
        type=str,
        nargs="*",
        required=False,
