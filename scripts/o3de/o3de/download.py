...
def unzip_manifest_json_data(
    download_zip_path: pathlib.Path, zip_file_name: str
) -> dict:
    json_data = {}
...
def validate_downloaded_zip_sha256(
    download_uri_json_data: dict, download_zip_path: pathlib.Path, manifest_json_name
) -> int:
    ...
    try:
        ...
        logger.warning(
            'SECURITY WARNING: The advertised o3de object you downloaded has no "sha256"!!! Be VERY careful!!!'
        ...
        with download_zip_path.open("rb") as f:
            ...
            if sha256A.lower() != sha256B.lower():
                logger.error(
                    f"SECURITY VIOLATION: Downloaded zip sha256 {sha256B} does not match"
                    f' the advertised "sha256":{sha256A} in the {manifest_json_name}.'
                )
                return 0

    unzipped_manifest_json_data = unzip_manifest_json_data(
        download_zip_path, manifest_json_name
    )
    ...
    if download_uri_json_data != unzipped_manifest_json_data:
        logger.error(
            f"SECURITY VIOLATION: Downloaded {manifest_json_name} contents do not match"
...
def get_downloadable(
    ...
    search_func = lambda manifest_json_data: repo.search_repo(
        manifest_json_data,
        engine_name,
        project_name,
        gem_name,
...
def download_o3de_object(
    object_name: str,
    default_folder_name: str,
    dest_path: str or pathlib.Path,
    object_type: str,
    downloadable_kwarg_key,
    skip_auto_register: bool,
    force_overwrite: bool,
    download_progress_callback=None,
    use_source_control: bool = False,
) -> int:
    object_name_without_version_specifier, version_specifier = (
        utils.get_object_name_and_optional_version_specifier(object_name)
    )
    download_path = (
        manifest.get_o3de_cache_folder()
        / default_folder_name
        / object_name_without_version_specifier
    )
    ...
    if use_source_control:
        ...
        if not origin_uri:
            logger.error(
                f"Tried to use source control to acquire {object_name} but the 'source_control_uri' is empty or missing."
            )
            return 1
    else:
        ...
        if not origin_uri:
            logger.error(
                f"Cannot download remote object {object_name} because neither the 'download_source_uri' or 'origin_uri' are set."
            )
            return 1
    ...
    if not object_version:
        logger.warning(
            f"The 'version' value for {object_name} was empty, using '0.0.0'"
        )
    ...
    if git_provider:
        ...
        clone_result = git_provider.clone_from_git(
            parsed_uri.geturl(), dest_path, force_overwrite, source_control_ref
        )
        if clone_result:
            if source_control_ref:
                logger.error(
                    f"Could not clone '{parsed_uri.geturl()}' reference '{source_control_ref}'. Please verify the repository uri and reference are correct and accessible."
                )
            else:
                logger.error(
                    f"Could not clone '{parsed_uri.geturl()}'. Please verify the repository uri and reference are correct and accessible."
                )
            return 1
    elif use_source_control:
        logger.error(
            f"Currently only git repositories are supported and the provided uri '{parsed_uri.geturl()}' does not have the '.git' extension "
        )
        return 1
    else:
        download_zip_result = utils.download_zip_file(
            parsed_uri,
            download_zip_path,
            force_overwrite,
            object_name,
        ...
        if not validate_downloaded_zip_sha256(
            downloadable_object_data, download_zip_path, f"{object_type}.json"
        ):
        ...
        if dest_path.exists():
            if not force_overwrite:
                ...
                try:
                    ...
                    logger.error(
                        f"Could not remove existing destination path {dest_path}."
                    )
                    return 1
        ...
        with zipfile.ZipFile(download_zip_path, "r") as zip_file_ref:
            try:
                ...
                logger.error(
                    f"Error unzipping {download_zip_path} to {dest_path}. Deleting {dest_path}."
                )
                logger.error(f"{e}")
                shutil.rmtree(dest_path)
                return 1
    ...
    if not skip_auto_register:
        ...
        if object_type == "gem":
            return register.register(
                gem_path=dest_path, force_register_with_o3de_manifest=True
            )
        elif object_type == "project":
            return register.register(
                project_path=dest_path, force_register_with_o3de_manifest=True
            )
        elif object_type == "template":
            return register.register(
                template_path=dest_path, force_register_with_o3de_manifest=True
            )
...
def is_o3de_object_update_available(
    object_name: str, downloadable_kwarg_key, local_last_updated: str
) -> bool:
    downloadable_object_data = get_downloadable(**{downloadable_kwarg_key: object_name})
    if not downloadable_object_data:
        logger.error(f"Downloadable o3de object {object_name} not found.")
    ...
    try:
        ...
        logger.warning(
            f"last_updated field has incorrect format for local copy of {downloadable_kwarg_key} {object_name}."
        )
    ...
    try:
        ...
        logger.error(
            f"last_updated field in incorrect format for repository copy of {downloadable_kwarg_key} {object_name}."
        )
        return False
...
def is_o3de_engine_update_available(engine_name: str, local_last_updated: str) -> bool:
    return is_o3de_object_update_available(
        engine_name, "engine_name", local_last_updated
    )


def is_o3de_project_update_available(
    project_name: str, local_last_updated: str
) -> bool:
    return is_o3de_object_update_available(
...
def is_o3de_template_update_available(
    template_name: str, local_last_updated: str
) -> bool:
    return is_o3de_object_update_available(
        template_name, "template_name", local_last_updated
    )


def is_o3de_restricted_update_available(
    restricted_name: str, local_last_updated: str
) -> bool:
    return is_o3de_object_update_available(
        restricted_name, "restricted_name", local_last_updated
    )
