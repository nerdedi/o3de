...
def test_get_android_gradle_plugin_requirements(agp_version, expected_gradle_version):
    result = android_support.get_android_gradle_plugin_requirements(
        requested_agp_version=agp_version
    )
...
def test_validate_java_environment_from_java_home(tmpdir):
    ...
    with (
        ...
        mock_os_getenv.return_value = tmpdir.join("java_home").realpath()
...
def test_validate_java_environment_from_invalid_java_home(tmpdir):
    ...
    with patch("os.getenv") as mock_os_getenv:
        mock_os_getenv.return_value = tmpdir.join("empty")
...
def test_validate_java_environment_from_java_home_error(tmpdir):
    ...
    test_java_home_val = tmpdir.join("java_home").realpath()
...
def test_android_sdk_environment_failed_settings(tmpdir, test_path):
    tmpdir.ensure("android_sdk_empty", dir=True)
    tmpdir.ensure(
        f"android_sdk/cmdline-tools/bin/sdkmanager{android_support.SDKMANAGER_EXTENSION}"
    )
    ...
    mock_settings.get_value.return_value = (
        tmpdir.join(test_path).realpath() if test_path is not None else None
    )

    # Test the method
    android_support.AndroidSDKManager.validate_android_sdk_environment(
        test_java_version, mock_settings
    )


def test_android_sdk_environment_failed_validation_test_bad_java_version(tmpdir):
    tmpdir.ensure(
        f"android_sdk/cmdline-tools/latest/bin/sdkmanager{android_support.SDKMANAGER_EXTENSION}"
    )
...
"""
    ...
    with patch("subprocess.run") as mock_subprocess_run:
        ...
        try:
            ...
            assert "command line tool requires java version" in str(err), (
                "Expected specific java error message"
            )
        else:
            assert False, "Error expected"


def test_android_sdk_environment_failed_validation_test_bad_android_sdk_cmd_tools_location(
    tmpdir,
):
    tmpdir.ensure(
        f"android_sdk/cmdline-tools/latest/bin/sdkmanager{android_support.SDKMANAGER_EXTENSION}"
    ...
    with patch("subprocess.run") as mock_subprocess_run:
        ...
        try:
            ...
            # Validate the error
            assert "not located under a valid Android SDK root" in str(err), (
                "Expected specific sdk command line error message"
            )
        else:
...
def test_android_sdk_environment_failed_validation_test_general_error(tmpdir):
    ...
    with patch("subprocess.run") as mock_subprocess_run:
        ...
        try:
            ...
            # Validate the error
            assert (
                "An error occurred attempt to run the android command line tool"
                in str(err)
            ), "Expected specific sdk command line error message"
        else:
            assert False, "Expected failure"
...
Available Updates:
  platforms;android-33                                                                     | 4             | Google APIs
    ...
    with (
        patch(
            "o3de.android_support.AndroidSDKManager.validate_android_sdk_environment"
        ) as mock_validate_android_sdk_environment,
        patch("subprocess.run") as mock_subprocess_run,
    ):
        ...
        result_installed = test.get_package_list(
            "*", android_support.AndroidSDKManager.PackageCategory.INSTALLED
        )
        ...
        result_available = test.get_package_list(
            "*", android_support.AndroidSDKManager.PackageCategory.AVAILABLE
        )
        ...
        result_updateable = test.get_package_list(
            "*", android_support.AndroidSDKManager.PackageCategory.UPDATABLE
        )
...
def test_validate_build_tool_from_config_key(
    test_version_query, test_version_result, test_version_regex, test_expected_version
):
    # Test Data
    tool_name = "test"
    ...
    tool_config_full_path = os.path.join(
        tool_config_value, tool_config_subpath, tool_cmd
    )
    ...
    # Mocks
    with (
        patch("o3de.command_utils.O3DEConfig") as mock_android_config,
        ...
        try:
            # Test the method
            result_location, result_version = android_support.validate_build_tool(
                tool_name=tool_name,
...
def test_validate_build_tool_from_env(
    test_version_query, test_version_result, test_version_regex, test_expected_version
):
    # Test Data
    tool_name = "test"
            ...
            **kwargs,
            ...
            if full_cmd_line_args[0] == tool_config_full_path:
                return subprocess.CompletedProcess(
                    args=full_cmd_line_args, returncode=1, stdout=""
                )
...
def test_validate_build_tool_from_path(
    test_version_query, test_version_result, test_version_regex, test_expected_version
):
    # Test Data
    tool_name = "test"
