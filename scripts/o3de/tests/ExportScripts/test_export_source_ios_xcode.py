...
@pytest.mark.parametrize(
    "skip_assets_flag, should_build_tools_flag, expect_get_asset_path, expect_process_command_count",
    [
...
def test_asset_and_build_combinations(
    tmp_path,
    skip_assets_flag,
    should_build_tools_flag,
    expect_get_asset_path,
    expect_process_command_count,
):
    ...
    with (
        patch("o3de.export_project.process_command") as mock_process_command,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ["/test/test.seedlist", "/test/test2.seedlist"],
            ["/test2/seed.seed", "/test2/seed2.seed"],
            ["main", "second", "THIRD"],
...
def test_export_standalone_multipart_args(
    ...
    with (
        ...
        patch(
            "o3de.export_project.get_default_asset_platform", return_value="mac"
        ...
        exp_ios.export_source_ios_run_command(
...
def test_export_standalone_exhaustive(
    tmpdir, dum_fail_asset_err, dum_build_tools, dum_build_assets
):
...
def test_export_standalone_single(
    ...
    with (
        ...
        for (
            ...
            [True, False], [True, False], [True, False], ["release", "profile"]
        ):
            tool_config = "profile"
...
def test_build_tool_combinations(tmp_path, should_build_tools_flag):
    ...
    with (
        patch("o3de.manifest.is_sdk_engine", return_value=False) as mock_is_sdk_engine,
        patch(
        ...
        for base_path in [
            ...
            exp_ios.export_ios_xcode_project(
                mock_ctx,
                test_output_path,
                [],
                [],
