...
def test_export_standalone_multipart_args(
    ...
    with (
        ...
        patch(
            "export_source_android.export_source_android_project"
        ) as mock_export_func,
    ):
        mock_get_o3de_folder.return_value = pathlib.Path(
            tmpdir.join(".o3de").realpath()
        ...
        expa.export_source_android_run_command(
...
def test_export_standalone_exhaustive(
    tmpdir, dum_fail_asset_err, dum_build_tools, dum_build_assets, dum_engine_centric
):
...
@pytest.mark.parametrize(
    "dum_fail_asset_err, dum_build_tools, dum_build_assets, dum_engine_centric", []
)
def test_export_standalone_single(
    ...
    with (
        ...
        patch(
            ...
            return_value=tmpdir / "assetproc",
        ),
        patch(
            "o3de.export_project.get_asset_bundler_batch_path",
            return_value=tmpdir / "assetbundles",
        ),
        patch(
...
def test_build_tool_combinations(
    tmp_path, use_sdk, should_build_tools_flag, has_monolithic
):
    ...
    mock_config.set_config_value(
        key=android_support.SETTINGS_SDK_ROOT.key,
        value=str(tmp_path / "android-sdk").replace("\\", "/"),
    ...
    with (
        ...
        try:
            for base_path in [
                ...
                expa.export_source_android_project(
                    mock_ctx,
                    test_output_path,
                    [],
                    [],
