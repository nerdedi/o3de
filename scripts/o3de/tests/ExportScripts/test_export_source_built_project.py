...
utils.prepend_to_system_path(
    pathlib.Path(__file__).parent.parent.parent / "ExportScripts"
)
from export_source_built_project import (
    export_standalone_project,
    export_standalone_parse_args,
    export_standalone_run_command,
)
...
def test_should_fail_immediately_for_installer_mono_build_with_no_artifacts(tmp_path):
    ...
    with (
        patch("o3de.manifest.is_sdk_engine", return_value=True) as mock_is_sdk_engine,
        patch(
            "o3de.export_project.has_monolithic_artifacts", return_value=False
        ) as mock_has_mono_artifacts,
...
@pytest.mark.parametrize(
    "is_engine_centric, use_sdk, should_build_tools_flag, has_monolithic, use_monolithic, expect_toolchain_build_called",
    [
...
def test_build_tools_combinations(
    tmp_path,
    is_engine_centric,
    use_sdk,
    ...
    test_o3de_base_path = (
        test_project_path if not is_engine_centric else test_engine_path
    )
    ...
    with (
        patch(
            "o3de.manifest.is_sdk_engine", return_value=use_sdk
        ) as mock_is_sdk_engine,
        patch(
            "o3de.export_project.has_monolithic_artifacts", return_value=has_monolithic
        ) as mock_has_mono_artifacts,
        patch(
            "o3de.export_project.get_platform_installer_folder_name",
            return_value="Windows",
        ) as mock_platform_folder_name,
        patch(
            "o3de.export_project.validate_project_artifact_paths", return_value=[]
        ) as mock_validate_project_artifacts,
        patch("o3de.export_project.kill_existing_processes") as mock_kill_processes,
        patch(
            "o3de.export_project.build_export_toolchain"
        ) as mock_build_export_toolchain,
        patch("o3de.export_project.build_game_targets") as mock_build_game_targets,
        patch(
            "o3de.export_project.get_asset_processor_batch_path"
        ) as mock_get_asset_processor_path,
        patch("o3de.export_project.build_assets") as mock_build_assets,
        patch(
            "o3de.export_project.get_asset_bundler_batch_path"
        ) as mock_get_asset_bundler_path,
        patch("o3de.export_project.bundle_assets") as mock_bundle_assets,
        patch(
            "o3de.export_project.setup_launcher_layout_directory"
        ) as mock_setup_launcher_layout_directory,
        ...
        for base_path in [
            None,
            test_o3de_base_path,
            test_absolute_base_path,
            test_relative_base_path,
        ]:
            if base_path:
            ...
            try:
                ...
                if expect_toolchain_build_called:
                    if not test_tools_build_path:
                        ...
                        test_tools_build_path = (
                            test_o3de_base_path / test_tools_build_path
                        )

                    mock_build_export_toolchain.assert_called_once_with(
                        ctx=mock_ctx,
                        tools_build_path=test_tools_build_path,
                        engine_centric=is_engine_centric,
                        tool_config="profile",
                ...
                assert not has_monolithic, (
                    "Error expected if we do not have monolithic binaries what requesting monolithic builds."
                )
...
@pytest.mark.parametrize(
    "is_engine_centric, use_sdk, has_monolithic, use_monolithic",
    [
...
def test_asset_bundler_combinations(
    tmp_path, is_engine_centric, use_sdk, has_monolithic, use_monolithic
):
    test_project_name = "TestProject"
    test_project_path = tmp_path / "project"
    test_engine_path = tmp_path / "engine"
    ...
    with (
        ...
        for should_build_tools, buildconf in zip(
            [True, False, True, False], ["profile", "profile", "release", "release"]
        ):
            for base_path in [
                None,
                test_o3de_base_path,
                ...
                test_tools_build_path = (
                    None if not base_path else (base_path / "build" / "tools")
                )
                test_asset_bundling_path = (
                    None if not base_path else (base_path / "build" / "asset_bundling")
                )
                ...
                try:
                    ...
                    selected_tools_build_path = (
                        test_tools_build_path if not use_sdk else test_tools_sdk_path
                    )
                    ...
                    if not selected_tools_build_path.is_absolute():
                        selected_tools_build_path = (
                            test_o3de_base_path / selected_tools_build_path
                        )

                    selected_asset_bundling_path = (
                        test_asset_bundling_path
                        if test_asset_bundling_path
                        else test_o3de_base_path / "build/asset_bundling"
                    )

                    if not selected_asset_bundling_path.is_absolute():
                        selected_asset_bundling_path = (
                            test_o3de_base_path / selected_asset_bundling_path
                        )

                    mock_get_asset_bundler_path.assert_not_called()

                    mock_bundle_assets.assert_called_once_with(
                        ctx=mock_ctx,
...
@pytest.mark.parametrize(
    "test_seedlists, test_seedfiles, test_levelnames",
    [
        pytest.param([], [], []),
        pytest.param(
            [pathlib.PurePath("C:\\test\\test.seedlist")],
            [pathlib.PurePath("C:\\test1\\test.seed")],
            [],
        ),
        pytest.param(
            [pathlib.PurePath("C:\\test\\test.seedlist")],
        ...
        pytest.param([pathlib.PurePath("C:\\test\\test.seedlist")], [], ["MAIN"]),
        pytest.param(
            [pathlib.PurePath("C:\\test\\test.seedlist")], [], ["main", "second"]
        ),
        pytest.param(
            [pathlib.PurePath("/test/test.seedlist")],
            [pathlib.PurePath("/test1/test.seed")],
            [],
        ),
        pytest.param(
            [pathlib.PurePath("/test/test.seedlist")],
...
def test_asset_bundler_seed_combinations(
    tmp_path, test_seedlists, test_seedfiles, test_levelnames
):
    test_project_name = "TestProject"
    test_project_path = tmp_path / "project"
    test_engine_path = tmp_path / "engine"
    ...
    with (
        patch("o3de.manifest.is_sdk_engine", return_value=True) as mock_is_sdk_engine,
        patch(
            "o3de.export_project.has_monolithic_artifacts", return_value=True
        ) as mock_has_mono_artifacts,
        patch(
            "o3de.export_project.get_platform_installer_folder_name",
            return_value="Windows",
        ...
        patch(
            "o3de.export_project.validate_project_artifact_paths",
            return_value=test_seedlists,
        ) as mock_validate_project_artifacts,
        patch("o3de.export_project.kill_existing_processes") as mock_kill_processes,
        patch(
            "o3de.export_project.build_export_toolchain"
        ...
        for ln in test_levelnames:
            combined_seedfiles.append(
                test_project_path
                / f"Cache/{mock_platform}/levels"
                / ln.lower()
                / (ln.lower() + ".spawnable")
            )
...
def test_asset_processor_combinations(
    tmp_path, is_engine_centric, use_sdk, has_monolithic, use_monolithic
):
    test_project_name = "TestProject"
    test_project_path = tmp_path / "project"
        ...
        for should_build_tools, buildconf in zip(
            ...
            for base_path in [
                ...
                try:
                    ...
                    mock_get_asset_processor_path.assert_called_once_with(
                        tools_build_path=selected_tools_build_path,
                        using_installer_sdk=use_sdk,
                        tool_config="profile",
                        required=True,
...
def test_build_game_targets_combinations(
    tmp_path, is_engine_centric, use_sdk, has_monolithic, use_monolithic
):
    test_project_name = "TestProject"
    test_project_path = tmp_path / "project"
        ...
        for should_build_tools, buildconf in zip(
            ...
            for base_path in [
                ...
                for launcher_indices in launcher_powerset_indices():
                    ...
                    test_tools_sdk_path = (
                        test_engine_path / "bin/Windows/profile/Default"
                    )

                    test_launcher_build_path = (
                        None if not base_path else base_path / "build/launcher"
                    )
                    ...
                    if not selected_launcher_build_path:
                        selected_launcher_build_path = (
                            test_o3de_base_path / "build/launcher"
                        )

                    if not selected_launcher_build_path.is_absolute():
                        selected_launcher_build_path = (
                            test_o3de_base_path / selected_launcher_build_path
                        )
                    ...
                    if check_launcher_type == 0:
                        ...
                        mock_build_game_targets.assert_called_once_with(
                            ctx=mock_ctx,
...
def test_setup_launcher_layout_directory(
    tmp_path, is_engine_centric, use_sdk, has_monolithic, use_monolithic
):
    setup_dir_cache = []
    ...
    with (
        ...
        patch(
            "o3de.export_project.bundle_assets", return_value=bundle_output_path
        ) as mock_bundle_assets,
        patch(
            "o3de.export_project.setup_launcher_layout_directory",
            side_effect=cache_params,
        ) as mock_setup_launcher_layout_directory,
        patch("logging.getLogger", return_value=mock_logger) as mock_get_logger,
    ):
        mock_ctx = create_autospec(O3DEScriptExportContext)
        ...
        launcher_stubs = [
            "GamePackage",
            "ServerPackage",
            "HeadlessServerPackage",
            "UnifiedPackage",
        ]

        launcher_map = {
            "GamePackage": LauncherType.GAME,
            ...
            for base_path in [
                ...
                for launcher_indices in launcher_powerset_indices():
                    ...
                    try:
                        ...
                        assert len(setup_dir_cache) == settings_count, (
                            check_launcher_type
                        )

                        assert (
                            mock_setup_launcher_layout_directory.call_count
                            == settings_count
                        )
                        ...
                        for argset in setup_dir_cache:
                            ...
                            assert (
                                test_bundles_to_copy[0]
                                == bundle_output_path / f"game_{mock_platform}.pak"
                            )
                            assert (
                                test_bundles_to_copy[1]
                                == bundle_output_path / f"engine_{mock_platform}.pak"
                            )
                            ...
                            for stub in launcher_stubs:
                                if (
                                    export_folder_name
                                    == f"{mock_ctx.project_name}{stub}"
                                    and launcher_map[stub] & check_launcher_type > 0
                                ):
...
from export_test_utility import (
    setup_local_export_config_test,
)
...
def test_export_standalone_parse_args_should_require_output(tmpdir):
    ...
    test_project_name, test_project_path, test_engine_path = (
        setup_local_export_config_test(tmpdir)
    )

    # this should run fine, note that all parameters are considered optional except for project-path now
    with (
        patch("o3de.manifest.get_o3de_folder") as mock_get_o3de_folder,
        patch(
            "o3de.export_project.get_default_asset_platform", return_value="pc"
        ) as mock_get_asset_platform,
    ):
        mock_get_o3de_folder.return_value = pathlib.Path(
            tmpdir.join(".o3de").realpath()
        )
...
def test_export_standalone_exhaustive(
    tmpdir,
    dum_fail_asset_err,
    dum_build_tools,
    dum_build_assets,
    dum_build_game,
    dum_build_server,
    dum_build_headless_server,
    dum_build_unified,
    dum_engine_centric,
    dum_monolithic,
    dum_reg_override,
):
    test_export_standalone_single(
        tmpdir,
        dum_fail_asset_err,
        dum_build_tools,
        dum_build_assets,
...
@pytest.mark.parametrize(
    "dum_fail_asset_err, dum_build_tools, dum_build_assets,dum_build_game, dum_build_server, dum_build_headless_server, dum_build_unified,dum_engine_centric, dum_monolithic, dum_reg_override",
    [],
)
def test_export_standalone_single(
    tmpdir,
    dum_fail_asset_err,
    dum_build_tools,
    dum_build_assets,
    ...
    test_project_name, test_project_path, test_engine_path = (
        setup_local_export_config_test(
            ...
            tool_config=dummy_tool_config,
            archive_format=dummy_archive_format,
            fail_asset_errors=dum_fail_asset_err,
            build_assets=dum_build_assets,
            build_tools=dum_build_tools,
            build_game_launcher=dum_build_game,
            build_server_launcher=dum_build_server,
            build_headless_server_launcher=dum_build_headless_server,
            build_unified_launcher=dum_build_unified,
            engine_centric=dum_engine_centric,
            monolithic=dum_monolithic,
            allow_reg_overrides=dum_reg_override,
        )
    )
    ...
    with (
        ...
        patch(
            "export_source_built_project.export_standalone_project"
        ) as mock_export_func,
    ):
        mock_get_o3de_folder.return_value = pathlib.Path(
            tmpdir.join(".o3de").realpath()
        ...
        for i in range(5):
            ...
            (
                use_game_build,
                use_server_build,
                use_headless_server_build,
                use_unified_build,
            ) = True, True, False, True
            for product_entry in product([True, False], [True, False]):
                ...
                if not check_build_config:
                    check_build_config = test_export_config.get_value(
                ...
                if not check_tool_config:
                    check_tool_config = test_export_config.get_value(
                        "tool.build.config"
                    )
                ...
                if not check_archive_format:
                    check_archive_format = test_export_config.get_value(
                ...
                export_standalone_run_command(
                    mock_ctx, args, test_export_config, mock_logger
                )
...
@pytest.mark.parametrize(
    "seedlists, seedfiles, levelnames, gamefile_patterns, serverfile_patterns, project_patterns",
    [
        pytest.param([], [], [], [], [], []),
        pytest.param(
            ["C:\\test\\test.seedlist"],
        ...
        pytest.param(
            ["C:\\test\\test.seedlist", "C:\\test\\test2.seedlist"],
            ["C:\\test2\\seed.seed", "C:\\test2\\seed2.seed"],
            ["main", "second", "THIRD"],
            ["game.cfg", "music/test*.mp3"],
            ["access_ctrls.xml", "server.cfg"],
            ["licensing/test*.txt", "dependency.graph"],
        ),
    ],
)
def test_export_standalone_multipart_args(
    tmpdir,
    seedlists,
    seedfiles,
    levelnames,
    ...
    with (
        ...
        mock_ctx.args = [
            ...
            str(tmpdir / "output"),
            "--max-bundle-size",
            str(mock_bundle_size),
            "-lbp",
            str(pathlib.PurePath("LauncherBuilds/Packages")),
            "-tbp",
            str(tmpdir / "tool_builds"),
            "-abp",
            str(tmpdir / "assets/bundling/paks"),
            "-pl",
            mock_platform,
        ]
        ...
        assert (
            "game_project_file_patterns_to_copy" in args
            and getattr(args, "game_project_file_patterns_to_copy") == gamefile_patterns
        )

        assert (
            "server_project_file_patterns_to_copy" in args
            and getattr(args, "server_project_file_patterns_to_copy")
            == serverfile_patterns
        )

        assert (
            "project_file_patterns_to_copy" in args
            and getattr(args, "project_file_patterns_to_copy") == project_patterns
        )

        assert (
            "seedlist_paths" in args
            and getattr(args, "seedlist_paths") == verify_seedlists
        )
        assert (
            "seedfile_paths" in args
            and getattr(args, "seedfile_paths") == verify_seedfiles
        )
