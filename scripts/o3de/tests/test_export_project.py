...
from o3de.export_project import (
    _export_script,
    process_command,
    setup_launcher_layout_directory,
    ...
    build_game_targets,
    validate_project_artifact_paths,
    LauncherType,
    extract_cmake_custom_args,
    get_default_asset_platform,
    get_platform_installer_folder_name,
...
@pytest.mark.parametrize(
    "input_script, args, should_pass_project_folder, project_folder_subpath, script_folder_subpath, output_filename, is_expecting_error, output_evaluation_func, expected_result",
    [
        # TEST_PYTHON_SCRIPT
        # successful cases
        pytest.param(
            ...
            ["456"],
            False,
            pathlib.PurePath("test_project"),
            pathlib.PurePath("test_project/ExportScripts"),
            pathlib.PurePath("test_output.txt"),
            False,
            check_for_o3de_context_arg,
            0,
        ),
        pytest.param(
            TEST_PYTHON_SCRIPT,
        ...
        pytest.param(
            ...
            True,
            pathlib.PurePath("test_project"),
            pathlib.PurePath("export_scripts"),
            pathlib.PurePath("test_output.txt"),
            False,
            check_for_o3de_context_arg,
            0,
...
def test_export_script(
    ...
    with patch(
        "o3de.manifest.get_project_engine_path", return_value=pathlib.Path
    ) as mock_get_project_engine_path:
        ...
        result = _export_script(
            test_script, project_folder if should_pass_project_folder else None, args
        )
...
@pytest.mark.parametrize(
    "engine_centric, fail_on_ap_errors",
    [pytest.param(False, False), pytest.param(False, True)],
)
def test_build_assets(tmp_path, engine_centric, fail_on_ap_errors):
    ...
    test_tools_build_path = (
        (test_project_path / "build" / "test")
        if engine_centric
        else (test_engine_path / "build" / "test")
    )
    ...
    for test_process_return_values in [0, 1]:
        with (
            patch(
                "o3de.export_project.process_command",
                return_value=test_process_return_values,
            ) as mock_process_command,
            patch(
                "o3de.export_project.get_asset_processor_batch_path",
                return_value=test_asset_processor_batch_path,
            ),
        ):
            ...
            mock_build_assets_process_input = mock_process_command.call_args_list[0][0][
                0
            ]
...
@pytest.mark.parametrize(
    "engine_centric, additional_cmake_configure_options, additional_build_args",
    [
        pytest.param(False, [], []),
        pytest.param(True, [], []),
...
def test_build_export_toolchain(
    tmp_path, engine_centric, additional_cmake_configure_options, additional_build_args
):
    test_project_name = "TestProject"
    test_project_path = tmp_path / "project"
    test_engine_path = tmp_path / "engine"
    ...
    with (
        patch(
            "o3de.export_project.process_command", return_value=0
        ) as mock_process_command,
        ...
        patch(
            "o3de.export_project.PREREQUISITE_TOOL_BUILD_CONFIG",
        ...
        expected_project_path = (
            test_engine_path if engine_centric else test_project_path
        )
        ...
        if engine_centric:
            expected_generate_args.extend([f"-DLY_PROJECTS={test_project_path}"])
...
@pytest.mark.parametrize(
    "focal_platform, focal_asset_platform, mock_platform",
    [
...
def test_platform_helpers_with_no_params(
    focal_platform, focal_asset_platform, mock_platform
):
...
@pytest.mark.parametrize(
    "build_config, build_game_launcher, build_server_launcher, build_unified_launcher, allow_registry_overrides, engine_centric, additional_build_args",
    [
...
def test_build_game_targets(
    tmp_path,
    build_config,
    build_game_launcher,
    build_server_launcher,
    build_unified_launcher,
    ...
    with (
        ...
        if (
            not build_game_launcher
            and not build_server_launcher
            and not build_unified_launcher
        ):
...
@pytest.mark.parametrize("asset_platform", [pytest.param("linux")])
def test_bundle_assets(tmp_path, asset_platform):
    ...
    test_build_tool_asset_bundler_path = (
        test_build_tool_path / "bin" / "profile" / "AssetBundler"
    )
    ...
    with (
        ...
        patch(
            ...
            return_value=test_build_tool_asset_bundler_path,
        ),
    ):
        mock_ctx = create_autospec(O3DEScriptExportContext)
        mock_ctx.project_path = test_project_path

        result = bundle_assets(
            ctx=mock_ctx,
        ...
        for arg_index, assetlist_type in [(0, "game"), (1, "engine")]:
            ...
            expected_assetlist_path = (
                test_custom_asset_base_path
                / "AssetLists"
                / f"{assetlist_type}_{asset_platform}.assetlist"
            )
        ...
        for arg_index, assetlist_type in [(2, "game"), (3, "engine")]:
            ...
            expected_output_bundle_path = (
                test_custom_asset_base_path
                / "Bundles"
                / f"{assetlist_type}_{asset_platform}.pak"
            )
...
@pytest.mark.parametrize(
    "build_config, asset_platform, project_files_to_copy, file_patterns_to_ignore, archive_format, project_name",
    [
        pytest.param(
            "profile",
            "pc",
            ["include*.cfg"],
            ["ExcludeMe.ServerLauncher"],
            "zip",
            "testproject",
        ),
        pytest.param(
            "profile",
            "linux",
            ["include_me1.cfg", "include_me2.cfg"],
            ["*.ServerLauncher"],
            "zip",
            "testproject",
        ),
...
def test_setup_launcher_layout_directory(
    ...
    test_include_mono_launcher_path = (
        test_mono_build_path_bin_dir / test_include_mono_launcher_name
    )
    ...
    test_exclude_mono_launcher_path = (
        test_mono_build_path_bin_dir / test_exclude_mono_launcher_name
    )
    ...
    result_bundle_file = (
        test_output_path / "Cache" / test_asset_platform / "test_bundle.pak"
    )
    assert result_bundle_file.is_file(), (
        f"Missing <output>/Cache/{test_asset_platform}/test_bundle.pak"
    )
    ...
    assert result_include_game_launcher.is_file(), (
        f"Missing <output>/{test_include_mono_launcher_name}"
    )

    result_exclude_server_launcher = test_output_path / test_exclude_mono_launcher_name
    assert not result_exclude_server_launcher.is_file(), (
        f"<output>/{test_exclude_mono_launcher_name} not excluded"
    )

    if test_archive_output_format != "none":
        result_archive_file = tmp_path / f"output.{test_archive_output_format}"
        assert result_archive_file.is_file(), (
            f"Missing <output>.{test_archive_output_format}"
        )

    setregpatch_file = (
        test_output_path / "Registry/IgnoreAssetProcessor.profile.setregpatch"
    )
...
@pytest.mark.parametrize(
    "project_path, create_files, check_abs_files, check_rel_files, expect_error",
    [
        pytest.param(
            "project",
            ["project/SeedLists/seed1", "project/SeedLists/seed2"],
            ["project/SeedLists/seed1", "project/SeedLists/seed2"],
            [],
            False,
        ),
        ...
        pytest.param(
            "project",
            ["project/SeedLists/seed1", "project/SeedLists/seed2"],
            ["project/SeedLists/seed1", "project/SeedLists/seed3"],
            [],
            True,
        ),
    ],
)
def test_validate_project_artifact_paths(
    tmp_path, project_path, create_files, check_abs_files, check_rel_files, expect_error
):
    ...
    try:
        validated_list = validate_project_artifact_paths(
            project_path=test_project_path, artifact_paths=artifact_list
        )
    except ExportProjectError:
...
@pytest.mark.parametrize(
    "input_args, expected_export_args, expected_cca, expected_cba",
    [
        pytest.param(
            [
                "-pp",
...
def test_extract_cmake_custom_args(
    input_args, expected_export_args, expected_cca, expected_cba
):
    result_export_args, result_cca_args, result_cba_args = extract_cmake_custom_args(
        input_args
    )
...
@pytest.mark.parametrize(
    "project_path, create_files, check_files, expected_result",
    [
        pytest.param(
            "project",
            ["project/SeedLists/seed1", "project/SeedLists/seed2"],
            ["SeedLists/seed1", "SeedLists/seed2"],
            ["SeedLists/seed1", "SeedLists/seed2"],
        ),
        pytest.param(
            "project",
        ...
        pytest.param(
            "project",
            ["project/SeedLists/seed1", "project/SeedLists/seed2"],
            ["SeedLists/seed1", "SeedLists/seed*"],
            ["SeedLists/seed1", "SeedLists/seed2"],
        ),
        pytest.param(
            "project",
            [
                "project/SeedLists/seed1",
                "project/SeedLists/seed2",
                "project/SeedLists/seed3",
            ],
            ["SeedLists/*1"],
            ["SeedLists/seed1"],
        ),
...
def test_preprocess_seed_path_list(
    tmp_path, project_path, create_files, check_files, expected_result
):
    test_project_path = tmp_path / project_path
    test_project_path.mkdir(parents=True, exist_ok=True)
    for create_file in create_files:
    ...
    result_path_list = preprocess_seed_path_list(
        project_path=test_project_path, paths=path_list
    )
