...
from o3de import manifest, download, git_utils
...
TEST_O3DE_REPO_FILE_NAME = (
    "3fb160cdfde8b32864335e71a9b7a0519591f3080d2a06e7ca10f830e0cb7a54.json"
)
...
TEST_O3DE_REPO_GEM_FILE_NAME = (
    "a765db91484f0d963d4ba5c98161074df7cd87caf1340e6bc7cebdce1807c994.json"
)
...
TEST_O3DE_LOCAL_REPO_GEM_FILE_NAME = (
    "8758b5acace49baf89ba5d36c1c214f10f8e47cd198096d1ae6b016b23b0833d.json"
)
...
class TestObjectDownload:
    ...
    @pytest.mark.parametrize(
        "test_name, manifest_data, gem_name, expected_result, expected_version, \
                             gem_data, zip_data,  \
        ...
        [
            ...
            pytest.param(
                "remote_gem_is_registered",
                TEST_O3DE_MANIFEST_JSON_PAYLOAD,
                "TestGem",
                0,
                None,
                TEST_O3DE_REPO_GEM_JSON_PAYLOAD,
                TEST_O3DE_ZIP_FILE_DATA,
                False,
                True,
                False,
                True,
            ...
            pytest.param(
                "gem_with_contents_in_subdir_registered",
                TEST_O3DE_MANIFEST_JSON_PAYLOAD,
                "TestGem",
                0,
                None,
            ...
            # versions
            pytest.param(
                "versioned_remote_gem_1.0.0_registered",
                TEST_O3DE_MANIFEST_JSON_PAYLOAD,
                "TestRemoteVersionedGem==1.0.0",
                0,
                "1.0.0",
                TEST_O3DE_REPO_GEM_JSON_PAYLOAD_VERSION_1_0_0,
                TEST_O3DE_ZIP_FILE_DATA,
            ...
            pytest.param(
                "versioned_remote_gem_2.0.0_registered",
                TEST_O3DE_MANIFEST_JSON_PAYLOAD,
                "TestRemoteVersionedGem==2.0.0",
                0,
                "2.0.0",
                TEST_O3DE_REPO_GEM_JSON_PAYLOAD_VERSION_2_0_0,
                TEST_O3DE_ZIP_FILE_DATA,
            ...
            pytest.param(
                "missing_versioned_remote_gem_fails",
                TEST_O3DE_MANIFEST_JSON_PAYLOAD,
                "TestRemoteVersionedGem==3.0.0",
                1,
                None,
                "",
                "",
    ...
    def test_download_gem(
        ...
        self.created_files.append(
            f"C:/Users/testuser/.o3de/cache/{TEST_O3DE_REPO_FILE_NAME}"
        )
        self.created_files.append(
            f"C:/Users/testuser/.o3de/cache/{TEST_O3DE_REPO_GEM_FILE_NAME}"
        )
        self.created_files.append(
            f"C:/Users/testuser/.o3de/cache/{TEST_O3DE_LOCAL_REPO_GEM_FILE_NAME}"
        )
        ...
        def save_o3de_manifest(
            manifest_data: dict, manifest_path: pathlib.Path = None
        ) -> bool:
        ...
        def replace_parent_with_subdir(
            parent_path: pathlib.Path, subdir_path: pathlib.Path
        ):
        ...
        def mocked_open(path, mode="", *args, **kwargs):
            ...
            if pathlib.Path(path).name == TEST_O3DE_REPO_FILE_NAME:
                ...
                pathlib.Path(path).name == TEST_O3DE_REPO_GEM_FILE_NAME
                or pathlib.Path(path).name == TEST_O3DE_LOCAL_REPO_GEM_FILE_NAME
                or pathlib.Path(path).name == "gem.json"
            ):
                file_data = gem_data
        ...
        def mocked_isfile(path):
            matches = [
                pathlib.Path(x).name
                for x in self.created_files
                if pathlib.Path(x).name == pathlib.Path(path).name
            ]
            if len(matches) != 0:
                return True
            else:
        ...
        with (
            ...
            patch(
                "o3de.utils.find_ancestor_dir_containing_file", return_value=None
            ) as _3,
            patch(
                "o3de.download.replace_parent_with_subdir",
                side_effect=replace_parent_with_subdir,
            ) as replace_parent_with_subdir_patch,
            ...
            patch(
                ...
                return_value=[pathlib.Path("subdir")] if contents_in_subdir else [],
            ) as iterdir_patch,
            ...
            result = download.download_gem(
                gem_name, "", skip_auto_register, force_overwrite, download_callback
            )
            assert result == expected_result

            if registration_expected:
                assert (
                    self.extracted_gem_path
                    in manifest.get_manifest_external_subdirectories()
                )
    ...
    @pytest.mark.parametrize(
        "test_name, gem_name, expected_ref, expected_registered_path, expected_result",
        [
            pytest.param(
                "clone_default_uses_latest",
                "TestRemoteVersionedGem",
                "release-2.0.0",
                pathlib.PurePath(
                    f"{TEST_DEFAULT_GEMS_FOLDER}/TestRemoteVersionedGem/2.0.0"
                ),
                0,
            ),
            pytest.param(
                "clone_1.0.0_gets_correct_ref",
                "TestRemoteVersionedGem==1.0.0",
                "release-1.0.0",
                pathlib.PurePath(
                    f"{TEST_DEFAULT_GEMS_FOLDER}/TestRemoteVersionedGem/1.0.0"
                ),
                0,
            ),
            pytest.param(
                "clone_2.0.0_gets_correct_ref",
                "TestRemoteVersionedGem==2.0.0",
                "release-2.0.0",
                pathlib.PurePath(
                    f"{TEST_DEFAULT_GEMS_FOLDER}/TestRemoteVersionedGem/2.0.0"
            ...
            pytest.param(
                "clone_3.0.0_fails", "TestRemoteVersionedGem==3.0.0", None, None, 1
            ),
        ],
    )
    def test_clone_gem(
        ...
        with (
            ...
            patch("pathlib.Path.resolve", new=self.resolve) as pathlib_is_resolve_mock,
            patch(
                "o3de.manifest.get_o3de_cache_folder",
                return_value=pathlib.Path("Cache"),
            ) as get_o3de_cache_folder_patch,
            patch("o3de.register.register", side_effect=register) as register_patch,
            patch(
                "o3de.utils.get_git_provider", side_effect=get_git_provider
            ) as git_git_provider_patch,
            patch(
                "o3de.utils.find_ancestor_dir_containing_file", return_value=None
            ) as _3,
            patch("pathlib.Path.mkdir") as _7,
            ...
            if expected_result == 0:
                assert (
                    pathlib.PurePath(self.registered_path) == expected_registered_path
                )
                assert self.source_control_ref == expected_ref

    @pytest.mark.parametrize(
        "update_function, object_name, object_data, existing_time, update_available",
        [
            # Repo gem is newer
            pytest.param(
                download.is_o3de_gem_update_available,
                "TestGem",
                TEST_O3DE_REPO_GEM_JSON_PAYLOAD,
                "2022-01-01 13:00:00",
                True,
            ...
            pytest.param(
                download.is_o3de_engine_update_available,
                "TestEngine",
                TEST_O3DE_REPO_ENGINE_JSON_PAYLOAD,
                "2021-12-01",
                False,
            ...
            pytest.param(
                download.is_o3de_project_update_available,
                "TestProject",
                TEST_O3DE_REPO_PROJECT_JSON_PAYLOAD,
                "",
                True,
            ...
            pytest.param(
                download.is_o3de_template_update_available,
                "TestTemplate",
                TEST_O3DE_REPO_TEMPLATE_JSON_PAYLOAD,
                "",
                False,
    ...
    def test_check_updates(
        self, update_function, object_name, object_data, existing_time, update_available
    ):
