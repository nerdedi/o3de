...
class TestEnableGemCommand:
    ...
    @pytest.mark.parametrize(
        "gem_path, project_path, gem_registered_with_project, gem_registered_with_engine,"
        "optional, expected_result",
        [
            pytest.param(
                pathlib.PurePath("TestProject/TestGem"),
                pathlib.PurePath("TestProject"),
                False,
                True,
                False,
                0,
    ...
    def test_enable_gem_registers_gem_name_with_project_json(
        self,
        gem_path,
        project_path,
        gem_registered_with_project,
        gem_registered_with_engine,
        optional,
        expected_result,
    ):
        def get_registered_path(
            project_name: str = None, gem_name: str = None, engine_name: str = None
        ) -> pathlib.Path:
            if project_name:
                return project_path
            elif gem_name:
        ...
        def save_o3de_manifest(
            new_project_data: dict, manifest_path: pathlib.Path = None
        ) -> bool:
        ...
        def get_engine_json_data(
            engine_name: str = None, engine_path: pathlib.Path = None
        ):
        ...
        with (
            patch("pathlib.Path.is_dir", return_value=True) as pathlib_is_dir_patch,
            ...
            patch(
                "o3de.manifest.save_o3de_manifest", side_effect=save_o3de_manifest
            ) as save_o3de_manifest_patch,
            patch(
                "o3de.manifest.get_gems_json_data_by_name",
                side_effect=get_gems_json_data_by_name,
            ) as get_gems_json_data_by_name_patch,
            patch(
                "o3de.manifest.get_registered", side_effect=get_registered_path
            ) as get_registered_patch,
            patch(
            ...
            patch(
                "o3de.manifest.get_project_gems", side_effect=get_project_gems
            ) as get_project_gems_patch,
            patch(
                "o3de.manifest.get_engine_json_data", side_effect=get_engine_json_data
            ) as get_engine_json_data_patch,
            patch(
                "o3de.manifest.get_engine_gems", side_effect=get_engine_gems
            ) as get_engine_gems_patch,
            patch(
            ...
            patch(
                "o3de.validation.valid_o3de_gem_json", return_value=True
            ) as valid_gem_json_patch,
        ):
            self.enable_gem.project_data.pop("gem_names", None)
            result = enable_gem.enable_gem_in_project(
                gem_path=gem_path, project_path=project_path, optional=optional
            )
    ...
    @pytest.mark.parametrize(
        "gem_registered_with_project, gem_registered_with_engine, "
        ...
        [
            ...
            pytest.param(
                ...
                False,
                "1.2.3",
                ["testgem1==1.2.3", "testgem2>1.0.0"],
                {"testgem1": {"version": "1.2.3"}, "testgem2": {"version": "2.0.0"}},
                False,
                False,
                ["o3de-test<=1.0.0"],
                [],
            ...
            pytest.param(
                ...
                ["testgem1==1.2.3"],
                {
                    "testgem1": {
                        "version": "1.2.3",
                        "dependencies": ["testgem2>1.0.0"],
                    },
                    "testgem2": {"version": "2.0.0"},
                },
                False,
                False,
                ["o3de-test<=1.0.0"],
            ...
            pytest.param(
                ...
                False,
                "1.2.3",
                ["testgem1", "testgem2~=2.0.0"],
                {"testgem1": {"version": "1.2.3"}, "testgem2": {"version": "2.0.1"}},
                False,
                False,
                ["o3de-test<=1.0.0"],
                [],
            ...
            pytest.param(
                ...
                "1.2.3",
                ["testgem1==1.2.3"],
                {"testgem1": {"version": "1.2.3", "dependencies": ["testgem2>1.0.0"]}},
                False,
                False,
                ["o3de-test<=1.0.0"],
                [],
            ...
            pytest.param(
                ...
                "1.2.3",
                ["testgem1==1.2.3", "testgem2>1.0.0"],
                {"testgem1": {"version": "1.0.0"}, "testgem2": {"version": "1.0.0"}},
                False,
                False,
                ["o3de-test~=1.0.0"],
                [],
    ...
    def test_enable_gem_checks_engine_compatibility(
        self,
        gem_registered_with_project,
        gem_registered_with_engine,
        gem_version,
        gem_dependencies,
        gem_json_data_by_name,
        dry_run,
        force,
        compatible_engines,
        ...
        def get_registered_path(
            engine_name: str = None, project_name: str = None, gem_name: str = None
        ) -> pathlib.Path:
            if project_name:
                return project_path
            elif gem_name:
        ...
        with (
            ...
            ) as get_enabled_gem_cmake_patch,
            patch("pathlib.Path.resolve", new=self.resolve) as pathlib_is_resolve_mock,
            patch(
                "o3de.validation.valid_o3de_gem_json", return_value=True
            ) as valid_gem_json_patch,
            ...
            result = enable_gem.enable_gem_in_project(
                ...
                force=force,
                dry_run=dry_run,
                optional=is_optional_gem,
            )
            assert result == expected_result

            if not result:
                gem_json = get_gem_json_data(
                ...
                gem = (
                    gem_name
                    if not is_optional_gem
                    else {"name": gem_name, "optional": is_optional_gem}
                )
                if not dry_run:
                    assert gem in project_json.get("gem_names", [])
                else:
    ...
    @pytest.mark.parametrize(
        "gem_name, gem_json_data_by_path, force,expected_result",
        [
            # when no version specifier used and gem exists expect it is found
            pytest.param(
                "GemA", {pathlib.PurePath("GemA"): {"gem_name": "GemA"}}, False, 0
            ),
            # when version specifier used and gem exists with version expect it is found
            pytest.param(
                "GemA==1.0",
                {pathlib.PurePath("GemA"): {"gem_name": "GemA", "version": "1.0.0"}},
                False,
                0,
            ),
            ...
            pytest.param(
                "GemA",
                {
                    pathlib.PurePath("GemA"): {
                        "gem_name": "GemA",
                        "version": "1.0.0",
                        "dependencies": ["GemB==3.0.0", "GemC==2.0.0"],
                    },
                    pathlib.PurePath("GemB"): {
                        "gem_name": "GemB",
                        "version": "3.0.0",
                        "dependencies": ["GemC==1.0.0"],
                    },
    ...
    def test_enable_gem_with_version_specifier_checks_compatibility(
        self, gem_name, gem_json_data_by_path, force, expected_result
    ):
        project_path = pathlib.PurePath("TestProject")
        engine_path = pathlib.PurePath("o3de")
        ...
        with (
            ...
            ) as get_manifest_engines_patch,
            patch(
                "o3de.manifest.get_engine_json_data", side_effect=get_engine_json_data
            ) as get_engine_json_data_patch,
            patch(
                "o3de.manifest.get_json_data", side_effect=get_json_data
            ...
            patch(
                "o3de.manifest.get_all_gems", side_effect=get_all_gems
            ...
            self.enable_gem.project_data.pop("gem_names", None)
            result = enable_gem.enable_gem_in_project(
                gem_name=gem_name, project_path=project_path, force=force
            )
            assert result == expected_result
