...
TEST_O3DE_REPO_FILENAME = (
    "c93a8a9235af9a27e63c4034ee3a0c26a284ced84c96d1de5dff9903daecc2f6.json"
)
...
TEST_O3DE_REPOA_FILENAME = (
    "ecc7945d5e982114942a8918b52cb37476c38903ff9fd0a1eb9977d3fa2f23b5.json"
)
...
TEST_O3DE_REPOB_FILENAME = (
    "fe04d87c744a0f41383122f3dd279b216376f94b4105d874b7856aa42f5e4112.json"
)
...
TEST_O3DE_REPO_JSON_VERSION_2_FILENAME = (
    "3b14717bafd5a3bd768d3d0791a44998c3bd0fb2bfa1a7e2ee8bb1a39b04d631.json"
)
...
TEST_O3DE_RECURSIVE_REPO_FILENAME = (
    "9265e40cf042c1e70b04f3886ed361c7b305fed5e6f5cb239b22d69ccff7cf09.json"
)
...
TEST_O3DE_REPO_PROJECT_FILE_NAME = (
    "233c6e449888b4dc1355b2bf668b91b53715888e6777a2791df0e7aec9d08989.json"
)
...
TEST_O3DE_REPO_TEMPLATE_FILE_NAME = (
    "7802eae005ca1c023e14611ed63182299bf87e760708b4dba8086a134e309f3a.json"
)
...
class TestRepos:
    ...
    @pytest.mark.parametrize(
        "repo_path, expected_manifest_file, expected_result",
        [
            pytest.param(
                "http://o3de.org", pathlib.PurePath("o3de_manifest.json"), True
            ),
            pytest.param(
                "http://o3de.org/incorrect",
                pathlib.PurePath("o3de_manifest.json"),
                False,
            ),
        ],
    )
    def test_get_repository_list(
        self, repo_path, expected_manifest_file, expected_result
    ):
        self.o3de_manifest_data = json.loads(TEST_O3DE_MANIFEST_JSON_PAYLOAD)
        self.o3de_manifest_data["repos"] = [
            "http://o3de.org",
            "http://removablerepo.com",
        ]
    ...
    @pytest.mark.parametrize(
        "repo_uri, expected_result, expected_in_repo, download_repo_data, created_file",
        [
            pytest.param(
                "http://o3derepo.org",
                0,
            ...
            pytest.param(
                "http://o3derepo.org", 1, False, TEST_O3DE_REPO_BROKEN_JSON_PAYLOAD, ""
            ),
            pytest.param(
                "http://o3derepo.org",
                0,
    ...
    def test_add_repository(
        ...
        with (
            ...
            matches = [
                ...
                if pathlib.Path(x).name == created_file
            ]
            assert (len(matches) != 0) == (created_file != "")

    @pytest.mark.parametrize(
        "repo_path, existing_repo, expected_result",
        [
    ...
    @pytest.mark.parametrize(
        "test_name, repo_paths, expected_gems_json_data, expected_projects_json_data, expected_templates_json_data",
        [
            pytest.param(
                "repoA loads repoA objects",
                ["http://o3de.org/repoA"],
                [TEST_O3DE_REPO_GEM_JSON_PAYLOAD],
                [TEST_O3DE_REPO_PROJECT_JSON_PAYLOAD],
                [TEST_O3DE_REPO_TEMPLATE_JSON_PAYLOAD],
            ...
            pytest.param(
                "repoA and repoB loads all objects",
                ["http://o3de.org/repoA", "http://o3de.org/repoB"],
                [
                    TEST_O3DE_REPO_GEM_JSON_PAYLOAD,
                    TEST_O3DE_REPO_GEM_JSON_PAYLOAD,
                    TEST_O3DE_REPO_GEM2_JSON_PAYLOAD,
    ...
    def test_get_object_json_data(
        self,
        test_name,
        repo_paths,
        expected_gems_json_data,
        expected_projects_json_data,
        expected_templates_json_data,
    ):
        self.o3de_manifest_data = json.loads(TEST_O3DE_MANIFEST_JSON_PAYLOAD)
        ...
        with (
            ...
            self.test_gem_cache_filename, _ = repo.get_cache_file_uri(
                "http://o3derepo.org/TestGem/gem.json"
            )
            self.test_gem2_cache_filename, _ = repo.get_cache_file_uri(
                "http://o3derepo.org/TestGem2/gem.json"
            )
            self.test_project_cache_filename, _ = repo.get_cache_file_uri(
                "http://o3derepo.org/TestProject/project.json"
            )
            self.test_project2_cache_filename, _ = repo.get_cache_file_uri(
                "http://o3derepo.org/TestProject2/project.json"
            )
            self.test_template_cache_filename, _ = repo.get_cache_file_uri(
                "http://o3derepo.org/TestTemplate/template.json"
            )
            self.test_template2_cache_filename, _ = repo.get_cache_file_uri(
                "http://o3derepo.org/TestTemplate2/template.json"
            )

            # Gems
            gems_json_data = repo.get_gem_json_data_from_all_cached_repos()
            expected_gems_json_list = [
                json.loads(data) for data in expected_gems_json_data
            ]
            assert all(
                json_data in expected_gems_json_list for json_data in gems_json_data
            )
            # Projects
            projects_json_data = repo.get_project_json_data_from_all_cached_repos()
            expected_projects_json_list = [
                json.loads(data) for data in expected_projects_json_data
            ]
            assert all(
                json_data in expected_projects_json_list
                for json_data in projects_json_data
            )
            # Templates
            templates_json_data = repo.get_template_json_data_from_all_cached_repos()
            expected_templates_json_list = [
                json.loads(data) for data in expected_templates_json_data
            ]
            assert all(
                json_data in expected_templates_json_list
                for json_data in templates_json_data
            )

    @pytest.mark.parametrize(
        "repo_uri, validate_objects",
        [
