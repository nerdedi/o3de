...
class TestEditGemProperties:
    @pytest.mark.parametrize(
        "gem_path, gem_name, gem_new_name, gem_display, gem_origin,\
        ...
        [
            pytest.param(
                pathlib.PurePath("D:/TestProject"),
                None,
                "TestGem2",
                "New Gem Name",
                "O3DE",
                "Code",
                "Gem that exercises Default Gem Template",
                "new_preview.png",
                "Do this extra thing",
                "https://o3de.org/docs/user-guide/gems/",
                "Apache 2.0",
                "https://www.apache.org/licenses/LICENSE-2.0",
                "https://github.com/o3de/o3de.git",
                "1.2.3",
                ["o3de>=1.0", "o3de-sdk~=1.0"],
                None,
                None,
                ["o3de-sdk==1.2.3", "o3de>=1.0", "o3de-sdk~=1.0"],
                ["Physics", "Rendering", "Scripting"],
                None,
                None,
                ["TestGem", "OtherTag", "Physics", "Rendering", "Scripting"],
                ["Windows", "MacOS", "Linux"],
                None,
                None,
                ["Windows", "MacOS", "Linux"],
    ...
    def test_edit_gem_properties(
        ...
        with (
            ...
            patch(
                "o3de.manifest.get_registered", side_effect=get_gem_path
            ) as get_registered_patch,
        ):
            result = gem_properties.edit_gem_props(
                gem_path,
                gem_name,
                gem_new_name,
                gem_display,
            ...
            if expected_result == 0:
                ...
                if gem_requirements:
                    assert (
                        self.gem_json.data.get("requirements", "") == gem_requirements
                    )
                if gem_documentation_url:
                    assert (
                        self.gem_json.data.get("documentation_url", "")
                        == gem_documentation_url
                    )
                if gem_license:
                ...
                assert set(self.gem_json.data.get("compatible_engines", [])) == set(
                    expected_compatible_engines
                )
                assert set(self.gem_json.data.get("user_tags", [])) == set(
                    expected_tags
                )
                assert set(self.gem_json.data.get("platforms", [])) == set(
                    expected_platforms
                )
                assert set(
                    self.gem_json.data.get("engine_api_dependencies", [])
