...
def test_register_engine_path(engine_path, engine_name, force, expected_result):
    ...
    with (
        patch(
            "o3de.manifest.load_o3de_manifest", side_effect=load_manifest_from_string
        ) as load_manifest_mock,
        patch(
            "o3de.manifest.save_o3de_manifest", side_effect=save_manifest_to_string
        ) as save_manifest_mock,
        patch(
            "o3de.manifest.get_engine_json_data", return_value=engine_json_data
        ) as engine_paths_mock,
        patch(
            "o3de.validation.valid_o3de_engine_json", return_value=True
        ) as valid_engine_mock,
...
class TestRegisterThisEngine:
    ...
    def test_register_this_engine(
        self, engine_path, engine_name, force, expected_result
    ):
        parser = argparse.ArgumentParser()
        ...
        with (
            patch(
                "o3de.manifest.load_o3de_manifest",
                side_effect=load_manifest_from_string,
            ) as load_manifest_mock,
            patch(
                "o3de.manifest.save_o3de_manifest", side_effect=save_manifest_to_string
            ) as save_manifest_mock,
            ...
            patch(
                "o3de.manifest.get_this_engine_path", return_value=engine_path
            ) as engine_paths_mock,
            patch(
                "o3de.validation.valid_o3de_engine_json", return_value=True
            ) as valid_engine_mock,
...
class TestRegisterGem:
    ...
    @pytest.mark.parametrize(
        "gem_path, expected_manifest_file, dry_run, force_o3de_manifest_register, expected_result",
        [
            pytest.param(
            ...
            pytest.param(
                project_path / "TestGem",
                pathlib.PurePath("project.json"),
                False,
                False,
                0,
            ),
            ...
            pytest.param(
                pathlib.PurePath("TestGem/TestSubGem"),
                pathlib.PurePath("gem.json"),
                False,
                False,
                0,
            ),
            ...
            pytest.param(
                engine_path / "TestGem", pathlib.PurePath("engine.json"), True, False, 0
            ),
            pytest.param(
                pathlib.PurePath("TestGem/TestSubGem"),
                pathlib.PurePath("gem.json"),
    ...
    def test_register_gem_auto_detects_manifest_update(
        self,
        gem_path,
        expected_manifest_file,
        dry_run,
        force_o3de_manifest_register,
        expected_result,
    ):
        def save_o3de_manifest(
            ...
            if (
                manifest_path
                == pathlib.Path(TestRegisterGem.ancestor_gem_path).resolve()
                / "gem.json"
            ):
                self.ancestor_gem_data = manifest_data
            if (
                manifest_path
                == pathlib.Path(TestRegisterGem.project_path).resolve() / "project.json"
            ):
                self.project_data = manifest_data
            elif (
                manifest_path
                == pathlib.Path(TestRegisterGem.engine_path).resolve() / "engine.json"
            ):
        ...
        def find_ancestor_dir(
            target_file_name: pathlib.PurePath, start_path: pathlib.Path
        ):
            try:
                if target_file_name == pathlib.PurePath(
                    "gem.json"
        ...
        with (
            ...
            patch(
                "o3de.manifest.get_gem_json_data",
                side_effect=TestRegisterGem.get_gem_json_data,
            ) as _5,
            patch(
                "o3de.utils.find_ancestor_dir_containing_file",
                side_effect=find_ancestor_dir,
            ) as _6,
...
class TestRegisterProject:
    ...
    def test_register_project(
        ...
        engine_version,
        engine_api_versions,
        registered_gem_versions,
        project_engine_name,
        project_engine_version,
        project_gems,
        project_compatible_engines,
        project_engine_api_dependencies,
        gem_compatible_engines,
        gem_engine_api_dependencies,
        force,
        dry_run,
        expected_result,
    ):
        def save_o3de_manifest(
            ...
            if (
                manifest_path
                == pathlib.Path(TestRegisterProject.project_path).resolve()
                / "project.json"
            ):
                self.project_data = manifest_data
            elif (
                manifest_path
                == pathlib.Path(TestRegisterProject.engine_path).resolve()
                / "engine.json"
            ):
                self.engine_data = manifest_data
            else:
        ...
        def get_project_json_data(
            ...
            if project_engine_api_dependencies != None:
                project_json_data["engine_api_dependencies"] = (
                    project_engine_api_dependencies
                )
        ...
        with (
            ...
            result = register.register(
                project_path=TestRegisterProject.project_path,
                force=force,
                dry_run=dry_run,
            )

            if dry_run:
                assert TestRegisterProject.project_path not in map(
                    lambda subdir: pathlib.PurePath(subdir),
                    self.o3de_manifest_data.get("external_subdirectories", []),
                )
