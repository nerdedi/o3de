...
class TestRemoveGemDependency:
    ...
    def test_remove_gem_dependency(
        self, enable_gems_cmake_data, gem_name, expected_set, expected_return
    ):
        ...
        with (
            ...
            patch(
                "pathlib.Path.open", side_effect=lambda mode: StringBufferIOWrapper()
            ) as pathlib_open_mock,
        ):
            add_gem_return = cmake.remove_gem_dependency(
                pathlib.Path("enabled_gems.cmake"), gem_name=gem_name
            )
...
class TestResolveGemDependencyPaths:
                    ...
                    "gemB": [
                        {
                            "gem_name": "gemB",
                            "version": "0.0.0",
                            "path": pathlib.Path("gemB0Path"),
                        },
                        {
                            "gem_name": "gemB",
                            "version": "2.0.0",
                            "path": pathlib.Path("gemB2Path"),
                        },
                        {
                            "gem_name": "gemB",
                            "version": "1.0.0",
                            "path": pathlib.Path("gemB1Path"),
                        },
                    ],
                },
                "gemA;gemAPath;gemB;gemB2Path",
                0,
            ),
            # When a specific version is requested expect it is found
            pytest.param(
                [],
                ["gemA==1.2.3", "gemB==2.3.4"],
                {
                    "gemA": [
                        {
                            "gem_name": "gemA",
                            "version": "1.2.3",
                            "path": pathlib.Path("gemA1Path"),
                        },
                        {
                            "gem_name": "gemA",
                            "version": "2.3.4",
                            "path": pathlib.Path("gemA2Path"),
                        },
                    ],
                    "gemB": [
                        {
    ...
    def test_resolve_gem_dependency_paths(
        self,
        engine_gem_names,
        project_gem_names,
        all_gems_json_data,
        ...
        def get_engine_json_data(
            ...
            return {"engine_name": "o3de", "gem_names": engine_gem_names}
        ...
        with (
            ...
            patch(
                "o3de.manifest.get_this_engine_path", side_effect=get_this_engine_path
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                "2.0.0",
                "",
                cmake.UpdatePresetResult.Error,
            ),
            ...
            pytest.param(
                ...
                "",
                string.Template(
                    cmake.TEMPLATE_CMAKE_PRESETS_INCLUDE_JSON
                ).safe_substitute(
                    CMakePresetsInclude="<other-engine-root>/CMakePresets.json"
                ),
                cmake.UpdatePresetResult.EnginePathAdded,
            ),
            # This should update an existing cmake-presets file
            pytest.param(
                ...
                string.Template(
                    ...
                    CMakePresetsInclude="<engine-root>/CMakePresets.json"
                ),
                cmake.UpdatePresetResult.EnginePathAlreadyIncluded,
            ),
        ],
    )
    def test_update_cmake_presets_for_project(
        self,
        preset_path,
        engine_name,
        engine_version,
        ...
        with (
            patch(
                "pathlib.Path.open", side_effect=preset_json_open
            ) as pathlib_open_mock,
            patch("pathlib.Path.mkdir", return_value=True) as pathlib_mkdir_mock,
            patch(
                "o3de.manifest.get_registered", side_effect=get_registered
            ) as get_registered_mock,
        ):
            result = cmake.update_cmake_presets_for_project(
