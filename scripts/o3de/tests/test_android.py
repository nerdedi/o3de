...
def test_validate_android_config_happy_path(tmpdir):
    ...
    test_android_support_validate_gradle_result = (
        "/home/gradle-8.4/",
        test_gradle_version,
    )
    ...
    with (
        patch(
            "o3de.android_support.validate_java_environment"
        ) as mock_validate_java_environment,
        ...
        patch(
            "o3de.android_support.get_android_gradle_plugin_requirements"
        ) as mock_get_android_gradle_plugin_requirements,
        patch("o3de.android_support.AndroidSDKManager") as mock_get_AndroidSDKManager,
        patch("o3de.android.logger.warn") as mock_warn,
    ):
        mock_validate_java_environment.return_value = (
            test_validate_java_environment_result
        )
        ...
        mock_get_android_gradle_plugin_requirements.return_value = (
            mock_gradle_requirements
        )
        ...
        # Validation
        mock_gradle_requirements.validate_gradle_version.assert_called_with(
            test_gradle_version
        )
        mock_gradle_requirements.validate_java_version.assert_called_with(
            test_validate_java_environment_result
        )
...
def test_validate_android_config_bad_keystore_path(tmpdir):
    ...
    with (
        ...
        mock_validate_java_environment.return_value = (
            mock_validate_java_environment_result
        )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            "",
            "",
            android.VALIDATION_WARNING_SIGNCONFIG_NOT_SET,
            id="no_signing_configs",
        ),
        pytest.param(
            "test.keystore",
            "",
            "",
            "",
            android.VALIDATION_WARNING_SIGNCONFIG_INCOMPLETE,
            id="only_keystore",
        ),
        pytest.param(
            "",
        ...
        pytest.param(
            ...
            "test_key_alias",
            "",
            android.VALIDATION_MISSING_PASSWORD,
            id="no_passwords",
        ),
        pytest.param(
            "test.keystore",
        ...
        pytest.param(
            ...
            "1234",
            android.VALIDATION_MISSING_PASSWORD,
            id="key_passwords_only",
        ),
    ],
)
def test_validate_android_signing_config_warnings(
    tmpdir,
    test_sc_store_file,
    test_store_pw,
    test_key_alias,
...
def test_list_android_config_local():
    ...
    with patch("builtins.print") as mock_print:
        ...
        # Validation
        assert (
            mock_print.call_count == len(all_settings) + 2
        )  # the extra two are for the header and the non-global addendum


def test_list_android_config_global():
    # Test Data
    all_settings = [("one", "uno", None), ("two", "dos", None), ("three", "tres", None)]
    ...
    with patch("builtins.print") as mock_print:
        ...
        # Validation
        assert (
            mock_print.call_count == len(all_settings) + 1
        )  # the extra one is for the header
...
def test_get_android_config_from_args_global():
    ...
    # Mocks
    with (
        patch("o3de.android_support.get_android_config") as mock_get_android_config,
...
def test_get_android_config_no_project_name_no_project_detected():
    ...
    with (
        ...
        patch(
            "o3de.command_utils.resolve_project_name_and_path", new_callable=MagicMock
        ) as mock_resolve,
    ):
        ...
        assert (
            re.search(r"(based on the global settings)", mock_info.call_args.args[0])
            is not None
        )
        assert mock_get_android_config.call_count == 1
...
def test_get_android_config_no_project_name_project_detected():
    ...
    with (
        ...
        assert (
            re.search(
                f"(based on the currently detected project \\({detected_project}\\))",
        ...
        assert "project_path" in mock_get_android_config.call_args_list[0].kwargs
        assert (
            mock_get_android_config.call_args_list[0].kwargs["project_path"]
            == detected_path
        )
...
def test_get_android_config_project_path(tmpdir):
    ...
    with (
        ...
        assert (
            re.search(
                f"(will be based on project \\({test_project}\\))",
                mock_info.call_args.args[0],
            )
            is not None
        )
...
def test_configure_android_options_list():
    ...
    with (
        patch(
            "o3de.android.get_android_config_from_args"
        ) as mock_get_android_config_from_args,
...
def test_configure_android_options_set_value():
    ...
    with (
        ...
        mock_android_config.set_config_value_from_expression.assert_called_once_with(
            test_value
        )
...
def test_configure_android_options_clear_value():
    ...
    with (
        ...
        # Validation
        assert mock_get_android_config_from_args.call_count == 1
        mock_android_config.set_config_value.assert_called_once_with(
            key=test_value, value="", validate_value=False
        )
...
def test_configure_android_options_validate_error():
    ...
    with (
        ...
        mock_android_config = Mock()
        mock_get_android_config_from_args.return_value = (mock_android_config, "foo")
        mock_validate_android_config.side_effect = android_support.AndroidToolError(
            "BAD"
        )
...
def test_generate_android_project_success(tmpdir):
    ...
    with (
        ...
        patch(
            "o3de.android_support.read_android_settings_for_project"
        ) as mock_read_android_settings_for_project,
        patch("o3de.android.validate_android_config") as mock_validate_android_config,
        patch(
            "o3de.android_support.AndroidProjectGenerator"
        ) as mock_get_android_project_generator,
        ...
        mock_get_android_config_from_args.return_value = (
            mock_android_config,
            test_project_name,
        )
        ...
        mock_read_android_settings_for_project.return_value = (
            mock_project_settings,
            mock_android_settings,
        )

        mock_android_gradle_plugin_requirements = Mock()
        setattr(
            mock_android_gradle_plugin_requirements,
            "sdk_build_tools_version",
            test_sdk_build_tools_version,
        )
        ...
        test_android_env = android.ValidatedEnv(
            java_version=test_java_version,
        ...
        # Validation
        mock_manifest_get_registered.assert_called_once_with(
            project_name=test_project_name
        )

        mock_read_android_settings_for_project.assert_called_once_with(
            test_project_root
        )
        ...
        mock_get_signing_config.assert_called_once_with(
            store_file=pathlib.Path(test_key_store_path),
            store_password=test_store_pw,
            key_alias=test_sc_key_alias,
            key_password=test_key_password,
        )

        mock_get_android_project_generator.assert_called_once_with(
            engine_root=android.ENGINE_PATH,
