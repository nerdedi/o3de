...
from scene_api.scene_data import (
    PrimitiveShape,
    DecompositionMode,
    ColorChannel,
    TangentSpaceSource,
...
def add_material_component(entity_id):
    ...
    result = azlmbr.entity.EntityUtilityBus(
        azlmbr.bus.Broadcast,
        "UpdateComponentForEntity",
        entity_id,
        editor_material_component,
        json_update,
    )

    if not result:
        raise RuntimeError(
            "UpdateComponentForEntity for editor_material_component failed"
        )


def add_physx_meshes(
    ...
    physx_box_group = scene_manifest.add_physx_primitive_mesh_group(
        source_file_name + "_box", PrimitiveShape.BOX, 0.0, None
    )
    # Give a unique ID the PhysX mesh group
    physx_box_group["id"] = (
        "{" + str(uuid.uuid5(uuid.NAMESPACE_DNS, source_file_name + "_box")) + "}"
    )
    scene_manifest.physx_mesh_group_add_comment(
        physx_box_group, "This is a box primitive"
    )
    ...
    physx_convex_mesh_group = scene_manifest.add_physx_convex_mesh_group(
        source_file_name + "_convex",
        area_test_epsilon=0.08,
        plane_tolerance=0.0004,
    ...
    # Give a unique ID the PhysX mesh group
    physx_convex_mesh_group["id"] = (
        "{" + str(uuid.uuid5(uuid.NAMESPACE_DNS, source_file_name + "_convex")) + "}"
    )
    scene_manifest.physx_mesh_group_add_comment(
        physx_convex_mesh_group, "This is a convex mesh"
    )
    # Select/Unselect nodes using lists
    all_except_first_mesh = [x for x in all_node_paths if x != first_mesh]
    scene_manifest.physx_mesh_group_add_selected_unselected_nodes(
        physx_convex_mesh_group, [first_mesh], all_except_first_mesh
    )

    # Configure mesh decomposition for this mesh
    scene_manifest.physx_mesh_group_decompose_meshes(
        physx_convex_mesh_group,
        512,
        32,
        0.002,
        100100,
        DecompositionMode.TETRAHEDRON,
        0.06,
        0.055,
        0.00015,
        3,
    ...
    # Add a Triangle mesh
    physx_triangle_mesh_group = scene_manifest.add_physx_triangle_mesh_group(
        source_file_name + "_triangle",
        merge_meshes=False,
        weld_vertices=True,
    ...
    # Give a unique ID the PhysX mesh group
    physx_triangle_mesh_group["id"] = (
        "{" + str(uuid.uuid5(uuid.NAMESPACE_DNS, source_file_name + "_triangle")) + "}"
    )
    scene_manifest.physx_mesh_group_add_selected_unselected_nodes(
        physx_triangle_mesh_group, [first_mesh], all_except_first_mesh
    )
...
def update_manifest(scene):
    ...
    source_relative_path = os.path.dirname(
        os.path.relpath(clean_filename, source_basepath)
    )
    ...
    add_physx_meshes(
        scene_manifest, source_filename_only, mesh_name_list, all_node_paths
    )
    ...
    for activeMeshIndex in range(len(mesh_name_list)):
        ...
        mesh_group["id"] = (
            "{"
            + str(uuid.uuid5(uuid.NAMESPACE_DNS, source_filename_only + mesh_path))
            + "}"
        )
        ...
        scene_manifest.mesh_group_add_cloth_rule(
            mesh_group,
            mesh_path,
            "Col0",
            ColorChannel.GREEN,
        ...
        scene_manifest.mesh_group_add_advanced_mesh_rule(
            mesh_group, True, False, True, "Col0"
        )
        scene_manifest.mesh_group_add_skin_rule(mesh_group, 3, 0.002)
        scene_manifest.mesh_group_add_tangent_rule(
            mesh_group,
            TangentSpaceSource.MIKKT_GENERATION,
            TangentSpaceMethod.TSPACE_BASIC,
        )

        # Create an editor entity
        entity_id = azlmbr.entity.EntityUtilityBus(
            azlmbr.bus.Broadcast, "CreateEditorReadyEntity", mesh_group_name
        )
        # Add an EditorMeshComponent to the entity
        editor_mesh_component = azlmbr.entity.EntityUtilityBus(
            azlmbr.bus.Broadcast,
            "GetOrAddComponentByTypeName",
            entity_id,
        ...
        json_update = json.dumps(
            {
                "Controller": {
                    "Configuration": {
                        "ModelAsset": {
                            "assetHint": os.path.join(
                                source_relative_path, mesh_group_name
                            )
                            + ".fbx.azmodel"
                        }
                    }
        ...
        if previous_entity_id is None:
            physx_mesh_component = azlmbr.entity.EntityUtilityBus(
                azlmbr.bus.Broadcast,
                "GetOrAddComponentByTypeName",
                entity_id,
                "{20382794-0E74-4860-9C35-A19F22DC80D4} EditorMeshColliderComponent",
            )
                        ...
                        "PhysicsAsset": {
                            "Asset": {
                                "assetHint": os.path.join(
                                    ...
                                    source_filename_only + "_triangle.fbx.pxmesh",
                                )
                            }
                        }
                    }
            ...
            result = azlmbr.entity.EntityUtilityBus(
                ...
                "UpdateComponentForEntity",
                entity_id,
                physx_mesh_component,
                json_update,
            )

            if not result:
                raise RuntimeError(
                    "UpdateComponentForEntity failed for PhysX mesh component"
                )
        ...
        transform_component = azlmbr.entity.EntityUtilityBus(
            azlmbr.bus.Broadcast,
            "GetOrAddComponentByTypeName",
            entity_id,
            "27F1E1A1-8D9D-4C3B-BD3A-AFB9762449C0",
        )
        ...
        if previous_entity_id is not None:
            transform_json = json.dumps({"Parent Entity": previous_entity_id.to_json()})
            ...
            if not result:
                raise RuntimeError(
                    "UpdateComponentForEntity failed for Transform component"
                )
