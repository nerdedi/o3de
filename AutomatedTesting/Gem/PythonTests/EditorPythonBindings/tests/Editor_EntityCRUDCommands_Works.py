...
class Editor_EntityCRUDCommands_Works(BaseClass):
    ...
    def test():
        ...
        parentEntityId = editor.ToolsApplicationRequestBus(
            bus.Broadcast, "CreateNewEntity", EntityId()
        )
        childEntityId = editor.ToolsApplicationRequestBus(
            bus.Broadcast, "CreateNewEntity", EntityId()
        )
        levelEntityId = editor.ToolsApplicationRequestBus(
            bus.Broadcast, "GetCurrentLevelEntityId"
        )

        # Test SetParent/GetParent

        queryParent = editor.EditorEntityInfoRequestBus(
            bus.Event, "GetParent", childEntityId
        )
        check_result(
            queryParent.ToString() == levelEntityId.ToString(),
            "childEntity is parented to the Level entity",
        )
        ...
        check_result(
            queryParent.ToString() == parentEntityId.ToString(),
            "childEntity is now parented to the parentEntity",
        )

        # Test SetLockState and IsLocked

        queryLock = editor.EditorEntityInfoRequestBus(
            bus.Event, "IsLocked", parentEntityId
        )
        ...
        queryLock = editor.EditorEntityInfoRequestBus(
            bus.Event, "IsLocked", childEntityId
        )
        ...
        queryVisibility = editor.EditorEntityInfoRequestBus(
            bus.Event, "IsVisible", parentEntityId
        )
        check_result(queryVisibility, "parentEntityId is visible")

        queryHidden = editor.EditorEntityInfoRequestBus(
            bus.Event, "IsHidden", parentEntityId
        )
        check_result(not queryHidden, "parentEntityId isn't hidden")

        editor.EditorEntityAPIBus(
            bus.Event, "SetVisibilityState", parentEntityId, False
        )
        ...
        queryVisibility = editor.EditorEntityInfoRequestBus(
            bus.Event, "IsVisible", childEntityId
        )
        check_result(not queryVisibility, "childEntityId is now hidden too")

        # Test EditorOnly

        queryStatus = editor.EditorEntityInfoRequestBus(
            bus.Event, "GetStartStatus", parentEntityId
        )
        ...
        check_result(
            not (
                queryStatus
                == azlmbr.globals.property.EditorEntityStartStatus_EditorOnly
            ),
        ...
        check_result(
            queryStatus == azlmbr.globals.property.EditorEntityStartStatus_EditorOnly,
            "parentEntityId is now set to Editor Only",
        )

        queryStatus = editor.EditorEntityInfoRequestBus(
            bus.Event, "GetStartStatus", childEntityId
        )
        check_result(
            not (
                queryStatus
        ...
        check_result(
            not (
                queryStatus
                == azlmbr.globals.property.EditorEntityStartStatus_StartActive
            ),
        ...
        check_result(
            queryStatus == azlmbr.globals.property.EditorEntityStartStatus_StartActive,
