...
class Editor_EntityCommands_Works(BaseClass):
    ...
    def test():
        ...
        secondChildEntityId = editor.ToolsApplicationRequestBus(
            bus.Broadcast, "CreateNewEntity", rootEntityId
        )
        check_result(
            secondChildEntityId, "Another new entity with valid parent created"
        )
        check_result(
            secondChildEntityId.invoke("Equal", createdEntityIds[2]),
        ...
        thirdChildEntityId = editor.ToolsApplicationRequestBus(
            bus.Broadcast, "CreateNewEntity", secondChildEntityId
        )
        fourthChildEntityId = editor.ToolsApplicationRequestBus(
            bus.Broadcast, "CreateNewEntity", secondChildEntityId
        )
        editor.ToolsApplicationRequestBus(
            bus.Broadcast, "DeleteEntities", [thirdChildEntityId, fourthChildEntityId]
        )

        # Delete the second child Entity we created
        editor.ToolsApplicationRequestBus(
            bus.Broadcast, "DeleteEntityById", secondChildEntityId
        )

        # Delete the root Entity we created and all its children (so this should also delete the firstChildEntityId)
        editor.ToolsApplicationRequestBus(
            bus.Broadcast, "DeleteEntityAndAllDescendants", rootEntityId
        )
        ...
        # Create new Entity
        parentId = editor.ToolsApplicationRequestBus(
            bus.Broadcast, "CreateNewEntity", EntityId()
        )

        # Create new Entity with parentId as parent
        childId = editor.ToolsApplicationRequestBus(
        ...
        # Search by name
        searchEntityIdList = azlmbr.entity.SearchBus(
            bus.Broadcast, "SearchEntities", searchFilter
        )

        for entityId in searchEntityIdList:
            entityName = editor.EditorEntityInfoRequestBus(
                bus.Event, "GetName", entityId
            )
