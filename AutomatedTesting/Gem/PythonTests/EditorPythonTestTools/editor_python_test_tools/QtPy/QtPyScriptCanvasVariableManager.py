...
class QtPyScriptCanvasVariableManager:
    ...
    def __init__(self, sc_editor):
        self.variable_manager = sc_editor.sc_editor.findChild(
            QtWidgets.QDockWidget, VARIABLE_MANAGER_QT
        )
        self.variable_types = self.get_basic_variable_types()

    def create_new_variable(
        self, variable_name: str, new_variable_type: str
    ...
    def __validate_new_variable(self, new_variable_type: str) -> None:
        ...
        returns: None
        """
        assert type(new_variable_type) is str, (
            "Wrong parameter type provided. new_variable_type was not str"
        )
    ...
    def __click_on_add_variable_button(self) -> None:
        ...
        assert table_view_generated, (
            "Variable list not generated after clicking Create Variable button"
        )
    ...
    def __select_and_verify_variable_type(self, new_variable_type: str) -> None:
        ...
        graph_vars_table = self.variable_manager.findChild(
            QtWidgets.QTableView, GRAPH_VARIABLES_QT
        )
        expected_number_of_variables = (
            graph_vars_table.model().rowCount(QtCore.QModelIndex()) + 1
        )
        ...
        variable_entry = pyside_utils.find_child_by_pattern(
            table_view, new_variable_type
        )
        ...
        actual_number_of_variables = graph_vars_table.model().rowCount(
            QtCore.QModelIndex()
        )
        variable_added = helper.wait_for_condition(
            lambda: (actual_number_of_variables == expected_number_of_variables),
            WAIT_TIME_SEC_3,
        )
    ...
    def __set_new_variable_name(self, new_variable_name: str) -> QtWidgets:
        ...
        graph_vars_page = self.variable_manager.findChild(
            QtWidgets.QWidget, GRAPH_VARIABLES_PAGE_QT
        )
        ...
        var_name_changed = helper.wait_for_condition(
            lambda: line_edit.text() == new_variable_name, WAIT_TIME_SEC_3
        )
    ...
    def __get_variable_selection_index(
        self, graph_vars_page: QtWidgets.QWidget
    ) -> QtCore.QItemSelection:
        """
        ...
        has_selection = helper.wait_for_condition(
            lambda: selection_model.hasSelection() is False, WAIT_TIME_SEC_3
        )
    ...
    def select_variable_from_table(self, item_selection: QtCore.QItemSelection) -> None:
        ...
        has_selection = helper.wait_for_condition(
            lambda: selection_model.hasSelection(), WAIT_TIME_SEC_3
        )
    ...
    def delete_variable(self, variable_name: str) -> None:
        ...
        variable_table_view = self.variable_manager.findChild(
            QtWidgets.QTableView, GRAPH_VARIABLES_QT
        )
        ...
        for row in range(row_count):
            ...
            if data == variable_name:
                ...
                helper.wait_for_condition(
                    lambda: self.variable_manager.isActiveWindow(), WAIT_TIME_SEC_3
                )
                QtTest.QTest.keyClick(
                    self.variable_manager, Qt.Key_Delete, Qt.NoModifier
                )
                break
    ...
    def get_variable_count(self) -> int:
        ...
        """
        graph_variables = self.variable_manager.findChild(
            QtWidgets.QTableView, GRAPH_VARIABLES_QT
        )
    ...
    def validate_variable_count(self, expected) -> None:
        ...
        result = helper.wait_for_condition(
            lambda: expected == row_count, WAIT_TIME_SEC_3
        )
    ...
    def toggle_pin_variable_button(self, variable_type: str) -> None:
        ...
        returns None
        """
        variable_table_view, pinned_toggle = self.__find_pinned_toggle_control(
            variable_type
        )

        assert pinned_toggle is not None, "Failed to find variable pinning qt widget"

        toggle_state_is_different = self.__toggle_the_pinned_control(
            variable_table_view, pinned_toggle
        )
    ...
    def __find_pinned_toggle_control(self, variable_type: str) -> tuple:
        ...
        variable_object = pyside_utils.find_child_by_pattern(
            variable_table_view, variable_type
        )
    ...
    def __toggle_the_pinned_control(self, variable_table_view, pinned_toggle) -> bool:
        ...
        pinned_toggle_state_after = helper.wait_for_condition(
            lambda: pinned_toggle.data(Qt.DecorationRole) is None, WAIT_TIME_SEC_3
        )
