...
class QtPyScriptCanvasNodeInspector:
    def __init__(self, sc_editor):
        self.node_inspector = sc_editor.sc_editor.findChild(
            QtWidgets.QDockWidget, NODE_INSPECTOR_QT
        )

    def change_variable_initial_value(
        self, variable_name: str, variable_type: str, new_value
    ) -> None:
        """
        ...
        variable_qobject = self.node_inspector.findChild(
            QtWidgets.QFrame, f"{variable_name}"
        )

        assert variable_qobject is not None, (
            f"Variable by name {variable_name} was not found."
        )

        match variable_type:
            case "Boolean":
                assert type(new_value) == CheckBoxStates, (
                    f"New value: {new_value} is invalid for this variable type:{variable_type}"
                )
    ...
    def __change_variable_value_Boolean(
        self, variable_qobject: QtWidgets.QFrame, new_value: IntEnum
    ) -> None:
        """
    ...
    def __change_variable_value_string(
        self, variable_qobject: QtWidgets.QFrame, new_value: str
    ) -> None:
        """
    ...
    def change_variable_initial_value_source(self, value_source: IntEnum) -> None:
        ...
        variable_property_editor_frame = self.node_inspector.findChild(
            QtWidgets.QFrame, PROPERTY_EDITOR_QT
        )
        variable_initial_value_source_frame = variable_property_editor_frame.findChild(
            QtWidgets.QFrame, INITIAL_VALUE_SOURCE_QT
        )
        value_initial_source_combo_box = variable_initial_value_source_frame.findChild(
            QtWidgets.QComboBox, ""
        )
