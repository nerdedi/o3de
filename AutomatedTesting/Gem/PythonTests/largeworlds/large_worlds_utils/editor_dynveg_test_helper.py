...
def create_temp_mesh_prefab(model_asset_path, prefab_filename):
    ...
    assert root.has_component("Mesh") and mesh_component.is_enabled(), (
        "Failed to add/activate Mesh component"
    )
    # Assign the specified model asset
    model_asset = asset.AssetCatalogRequestBus(
        bus.Broadcast, "GetAssetIdByPath", model_asset_path, math.Uuid(), False
    )
    mesh_component.set_component_property_value(
        "Controller|Configuration|Model Asset", model_asset
    )
    assert (
...
def create_temp_physx_mesh_collider(physx_mesh_id, prefab_filename):
    ...
    assert (
        root.has_component(PHYSX_MESH_COLLIDER)
        and collider_component.is_enabled()
        and static_rigid_body_component.is_enabled()
    ), "Failed to add/activate PhysX Mesh Collider component"
    # Assign the specified PhysX Mesh asset to PhysX Mesh Collider
    collider_component.set_component_property_value(
        "Shape Configuration|Asset|PhysX Mesh", physx_mesh_id
    )
    assert (
        collider_component.get_component_property_value(
...
def create_surface_entity(name, center_point, box_size_x, box_size_y, box_size_z):
    ...
    surface_entity.create_entity(
        center_point, ["Box Shape", "Shape Surface Tag Emitter"]
    )
    if surface_entity.id.IsValid():
        print(f"'{surface_entity.name}' created")
    surface_entity.get_set_test(
        0,
...
def create_mesh_surface_entity_with_slopes(name, center_point, uniform_scale):
    ...
    hydra.get_set_test(
        surface_entity, 0, "Controller|Configuration|Mesh Asset", mesh_asset
    )
    PrefabWaiter.wait_for_propagation()
    components.TransformBus(
        bus.Event, "SetLocalUniformScale", surface_entity.id, uniform_scale
    )
    return surface_entity


def create_temp_prefab_vegetation_area(
    name, center_point, box_size_x, box_size_y, box_size_z, target_prefab
):
    ...
    spawner_entity.create_entity(
        center_point, ["Vegetation Layer Spawner", "Box Shape", "Vegetation Asset List"]
    )
    ...
    spawnable_asset_id = prefab.PrefabPublicRequestBus(
        bus.Broadcast, "GetInMemorySpawnableAssetId", spawnable_name
    )

    # Create the in-memory spawnable asset from given prefab if the spawnable does not exist
    if not spawnable_asset_id.is_valid():
        create_spawnable_result = prefab.PrefabPublicRequestBus(
            bus.Broadcast,
    ...
    descriptor = hydra.get_component_property_value(
        spawner_entity.components[2], "Configuration|Embedded Assets|[0]"
    )
...
def create_prefab_vegetation_area(
    name, center_point, box_size_x, box_size_y, box_size_z, prefab_path
):
    ...
    prefab_asset_id = asset.AssetCatalogRequestBus(
        bus.Broadcast, "GetAssetIdByPath", prefab_path, math.Uuid(), False
    )
    spawner_entity = hydra.Entity(name)
    spawner_entity.create_entity(
        center_point, ["Vegetation Layer Spawner", "Box Shape", "Vegetation Asset List"]
...
def create_blocker_area(name, center_point, box_size_x, box_size_y, box_size_z):
    ...
    blocker_entity.create_entity(
        center_point, ["Vegetation Layer Blocker", "Box Shape"]
    )
...
def create_empty_vegetation_area(
    ...
    descriptor = spawner_entity.components[2].get_component_property_value(
        "Configuration|Embedded Assets|[0]"
    )
    descriptor.spawner = empty_spawner
    spawner_entity.components[2].set_component_property_value(
        "Configuration|Embedded Assets|[0]", descriptor
    )
    return spawner_entity
...
def validate_instance_count(center, radius, num_expected):
    ...
    num_found = areasystem.AreaSystemRequestBus(
        bus.Broadcast, "GetInstanceCountInAabb", box
    )
...
def validate_instance_count_in_entity_shape(entity_id, num_expected):
    ...
    num_found = vegetation.VegetationSpawnerRequestBus(
        bus.Event, "GetAreaProductCount", entity_id
    )
