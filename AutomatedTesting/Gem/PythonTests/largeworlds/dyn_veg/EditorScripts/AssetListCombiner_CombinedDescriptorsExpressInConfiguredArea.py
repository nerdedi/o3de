...
def AssetListCombiner_CombinedDescriptorsExpressInConfiguredArea():
    ...
    def create_asset_list_entity(name, center, target_prefab):
        ...
        asset_list_entity.create_entity(center, ["Vegetation Asset List"])
        ...
        descriptor = hydra.get_component_property_value(
            asset_list_entity.components[0], "Configuration|Embedded Assets|[0]"
        )
        ...
        asset_list_entity.get_set_test(
            0, "Configuration|Embedded Assets|[0]", descriptor
        )
    ...
    pink_flower_prefab = dynveg.create_temp_mesh_prefab(
        pink_flower_asset_path, "AssetList_PinkFlower"
    )[0]
    purple_flower_asset_path = os.path.join(
        "assets", "objects", "foliage", "grass_flower_pink.fbx.azmodel"
    )
    purple_flower_prefab = dynveg.create_temp_mesh_prefab(
        purple_flower_asset_path, "AssetList_PurpleFlower"
    )[0]
    asset_list_entity = create_asset_list_entity(
        "Asset List 1", center_point, pink_flower_prefab
    )
    asset_list_entity2 = create_asset_list_entity("Asset List 2", center_point, None)
    asset_list_entity3 = create_asset_list_entity(
        "Asset List 3", center_point, purple_flower_prefab
    )
    ...
    # 4) Create a spawner using a Vegetation Asset List Combiner component and a Weight Selector, and disallow
    # spawning empty assets
    spawner_entity = dynveg.create_prefab_vegetation_area(
        "Spawner Entity", center_point, 16.0, 16.0, 16.0, None
    )
    ...
    spawner_entity.get_set_test(
        2, "Configuration|Descriptor Providers", asset_list_entities
    )
    ...
    constant_gradient_entity.create_entity(
        center_point, components_to_add, parent_id=spawner_entity.id
    )
    ...
    dither_gradient_entity.create_entity(
        center_point, components_to_add, parent_id=constant_gradient_entity.id
    )
    dither_gradient_entity.get_set_test(
        0, "Configuration|Gradient|Gradient Entity Id", constant_gradient_entity.id
    )

    # 7) Pin the Dither Gradient Entity to the Asset Weight Selector of the spawner entity
    spawner_entity.get_set_test(
        3, "Configuration|Gradient|Gradient Entity Id", dither_gradient_entity.id
    )
    ...
    spawner_entity.get_set_test(
        2, "Configuration|Descriptor Providers|[1]", asset_list_entity3.id
    )
    num_expected = 20 * 20
    success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
    ...
    editor.EditorComponentAPIBus(
        bus.Broadcast, "DisableComponents", [spawner_entity.components[2]]
    )
    editor.EditorComponentAPIBus(
        bus.Broadcast, "EnableComponents", [spawner_entity.components[2]]
    )
    num_expected = 0
    success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
