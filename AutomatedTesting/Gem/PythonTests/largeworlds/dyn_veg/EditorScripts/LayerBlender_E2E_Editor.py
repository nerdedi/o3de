...
class Tests:
    level_created = ("Successfully created level", "Failed to create level")
...
def LayerBlender_E2E_Editor():
    ...
    spawner_entity_1 = dynveg.create_prefab_vegetation_area(
        "Purple Spawner", purple_position, 16.0, 16.0, 1.0, purple_flower_prefab_path
    )

    pink_position = math.Vector3(520.0, 512.0, 32.0)
    pink_flower_prefab_path = os.path.join("assets", "prefabs", "PinkFlower.spawnable")
    spawner_entity_2 = dynveg.create_prefab_vegetation_area(
        "Pink Spawner", pink_position, 16.0, 16.0, 1.0, pink_flower_prefab_path
    )

    base_position = math.Vector3(512.0, 512.0, 32.0)
    dynveg.create_surface_entity("Surface Entity", base_position, 16.0, 16.0, 1.0)
    ...
    blender_entity.create_entity(
        base_position, ["Box Shape", "Vegetation Layer Blender", "Lua Script"]
    )
    Report.result(Tests.blender_entity_created, blender_entity.id.IsValid())

    blender_entity.get_set_test(
        0, "Box Shape|Box Configuration|Dimensions", math.Vector3(16.0, 16.0, 1.0)
    )
    blender_entity.get_set_test(
        1, "Configuration|Vegetation Areas", [spawner_entity_1.id, spawner_entity_2.id]
    )
    instance_counter_path = os.path.join("luascripts", "instance_counter_blender.lua")
    instance_counter_script = asset.AssetCatalogRequestBus(
        bus.Broadcast, "GetAssetIdByPath", instance_counter_path, math.Uuid(), False
    )
    ...
    for instance in instances:
        ...
        if instance.descriptor.spawner.GetPrefabAssetPath() == pink_flower_prefab_path:
            ...
            instance.descriptor.spawner.GetPrefabAssetPath()
            == purple_flower_prefab_path
        ):
            purple_count += 1
    Report.result(
        Tests.instances_blended,
        pink_count == purple_count and (pink_count + purple_count == num_expected),
    )
    ...
    azlmbr.components.TransformBus(
        bus.Event, "SetLocalRotation", search_entity_ids[0], cam_rot_degrees_vector
    )
