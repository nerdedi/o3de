...
def PhysXColliderSurfaceTagEmitter_E2E_Editor():
    ...
    def validate_behavior_context():
        ...
        behavior_context_test_success = behavior_context_test_success and (
            test_component is not None
        )
        behavior_context_test_success = behavior_context_test_success and (
            test_component.typename == "SurfaceDataColliderComponent"
        )
        ...
        behavior_context_test_success = (
            behavior_context_test_success
            and hydra.get_set_property_test(
                test_component, "providerTags", [provider_tag1, provider_tag2]
            )
        )
        behavior_context_test_success = (
            behavior_context_test_success
            and hydra.get_set_property_test(
                test_component, "modifierTags", [modifier_tag1, modifier_tag2]
            )
        )
        Report.info(
            f"SurfaceDataColliderComponent() BehaviorContext test: {behavior_context_test_success}"
        )
    ...
    dynveg.create_surface_entity(
        "Baseline Surface", entity_center_point, 32.0, 32.0, 1.0
    )
    ...
    pink_flower_prefab = dynveg.create_temp_mesh_prefab(
        pink_flower_asset_path, "PhysXCollider_PinkFlower"
    )[0]
    ...
    spawner_entity.get_set_test(
        3, "Configuration|Inclusion|Surface Tags", [surface_tag]
    )
    ...
    initial_success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count(entity_center_point, 16.0, 0), 5.0
    )
    ...
    hydra.get_set_test(
        collider_entity,
        component_index=0,
        path="Shape Configuration|Sphere|Radius",
        value=collider_radius,
    )
    hydra.get_set_test(
        collider_entity,
        component_index=0,
        path="Shape Configuration|Box|Dimensions",
        value=math.Vector3(collider_diameter, collider_diameter, collider_diameter),
    )
    hydra.get_set_test(
        collider_entity,
        component_index=0,
        path="Shape Configuration|Capsule|Height",
        value=collider_diameter,
    )
    ...
    for collider_shape in range(0, 3):
        ...
        hydra.get_set_test(
            collider_entity,
            component_index=0,
            path="Shape Configuration|Shape",
            value=collider_shape,
        )
        ...
        top_point = math.Vector3(
            entity_center_point.x,
            entity_center_point.y,
            entity_center_point.z + collider_radius,
        )
        baseline_surface_point = math.Vector3(
            entity_center_point.x,
            entity_center_point.y,
            entity_center_point.z + (baseline_surface_height / 2.0),
        )
        top_point_success = helper.wait_for_condition(
            lambda: dynveg.validate_instance_count(top_point, 0.25, 1), 5.0
        )
        Report.result(on_collider_top_point_count, top_point_success)
        baseline_success = helper.wait_for_condition(
            lambda: dynveg.validate_instance_count(baseline_surface_point, 0.25, 0), 5.0
        )
        ...
        top_point_success = helper.wait_for_condition(
            lambda: dynveg.validate_instance_count(top_point, 0.25, 0), 5.0
        )
        Report.result(on_collider_top_point_count, top_point_success)
        baseline_success = helper.wait_for_condition(
            lambda: dynveg.validate_instance_count(baseline_surface_point, 0.25, 1), 5.0
        )
        Report.result(on_collider_baseline_count, baseline_success)
    ...
    test_physx_mesh_asset_id = asset.AssetCatalogRequestBus(
        bus.Broadcast,
        "GetAssetIdByPath",
        os.path.join("assets", "objects", "pxmesh", "test.fbx.pxmesh"),
        math.Uuid(),
        False,
    )
    ...
    helper.wait_for_condition(
        lambda: editor.EditorComponentAPIBus(
            bus.Broadcast, "IsComponentEnabled", collider_entity.components[1]
        ),
        5.0,
    )
    hydra.get_set_test(
        collider_entity,
        component_index=2,
        path="Shape Configuration|Asset|PhysX Mesh",
        value=test_physx_mesh_asset_id,
    )

    # Set the asset scale to match the test heights of the shapes tested
    asset_scale = math.Vector3(1.0, 1.0, 9.0)
    collider_entity.get_set_test(
        2, "Shape Configuration|Asset|Configuration|Asset Scale", asset_scale
    )
