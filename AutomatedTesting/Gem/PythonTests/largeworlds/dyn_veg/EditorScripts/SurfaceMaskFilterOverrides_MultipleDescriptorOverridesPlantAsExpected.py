...
def SurfaceMaskFilterOverrides_MultipleDescriptorOverridesPlantAsExpected():
    ...
    surface_tag_list = [
        surface_data.SurfaceTag("test_tag"),
    ...
    pink_flower_prefab = dynveg.create_temp_mesh_prefab(
        pink_flower_asset_path, "SurfaceMaskOverrides_PinkFlower"
    )[0]
    ...
    desc_asset = hydra.get_component_property_value(
        asset_list_component, "Configuration|Embedded Assets"
    )[0]
    ...
    surface_entity_a = dynveg.create_surface_entity(
        "Surface Entity A", math.Vector3(512.0, 512.0, 32.0), 16.0, 16.0, 1.0
    )
    num_expected_surface_a = 20 * 20  # 20x20 instances on a 16x16 meter surface
    surface_entity_b = dynveg.create_surface_entity(
        "Surface Entity B", math.Vector3(512.0, 512.0, 35.0), 12.0, 12.0, 1.0
    )
    num_expected_surface_b = 15 * 15  # 15x15 instances on a 12x12 meter surface
    surface_entity_c = dynveg.create_surface_entity(
        "Surface Entity C", math.Vector3(512.0, 512.0, 38.0), 8.0, 8.0, 1.0
    )
    num_expected_surface_c = 10 * 10  # 10x10 instances on a 8x8 meter surface

    # Set each surface to emit a different tag
    surface_entity_a.get_set_test(
        1, "Configuration|Generated Tags", [surface_tag_list[0]]
    )
    surface_entity_b.get_set_test(
        1, "Configuration|Generated Tags", [surface_tag_list[1]]
    )
    surface_entity_c.get_set_test(
        1, "Configuration|Generated Tags", [surface_tag_list[2]]
    )

    # 4) Initial Validation: Validate instance count in the spawner area. Instances should plant on all surfaces
    num_expected = (
        num_expected_surface_a + num_expected_surface_b + num_expected_surface_c
    )
    ...
    for index in range(3):
        spawner_entity.get_set_test(
            2,
            f"Configuration|Embedded Assets|[{index}]|Display Per-Item Overrides",
            True,
        )
        spawner_entity.get_set_test(
            2,
            f"Configuration|Embedded Assets|[{index}]|Surface Mask Filter|Override Mode",
            1,
        )
    ...
    test_1_success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
            spawner_entity.id, num_expected_surface_a
        ),
        5.0,
    )
    ...
    test_2_success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
            spawner_entity.id, num_expected_surface_b
        ),
        5.0,
    )
    ...
    test_3_success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
            spawner_entity.id, num_expected_surface_c
        ),
        5.0,
    )
...
if __name__ == "__main__":
    ...
    Report.start_test(
        SurfaceMaskFilterOverrides_MultipleDescriptorOverridesPlantAsExpected
    )
