...
def ScaleModifierOverrides_InstancesProperlyScale():
    ...
    def set_and_validate_scale(entity, min_scale, max_scale):
        # Set Range Min/Max
        entity.get_set_test(
            2, "Configuration|Embedded Assets|[0]|Scale Modifier|Min", min_scale
        )
        entity.get_set_test(
            2, "Configuration|Embedded Assets|[0]|Scale Modifier|Max", max_scale
        )
    ...
    pink_flower_prefab = dynveg.create_temp_mesh_prefab(
        pink_flower_asset_path, "ScaleMod_PinkFlower2"
    )[0]
    ...
    spawner_entity.get_set_test(
        2, "Configuration|Embedded Assets|[0]|Scale Modifier|Override Enabled", True
    )
    scale_min = float(
        format(
            (
                hydra.get_component_property_value(
                    spawner_entity.components[2],
                    "Configuration|Embedded Assets|[0]|Scale Modifier|Min",
                )
            ),
            ".1f",
        )
    ...
    scale_max = float(
        format(
            (
                hydra.get_component_property_value(
                    spawner_entity.components[2],
                    "Configuration|Embedded Assets|[0]|Scale Modifier|Max",
                )
            ),
            ".1f",
        )
    ...
    Report.result(
        Tests.scale_values_set,
        ((scale_max - 1.0) < CLOSE_ENOUGH_THRESHOLD)
        and ((scale_min - 0.1) < CLOSE_ENOUGH_THRESHOLD),
    )
    ...
    Report.result(
        ...
        set_and_validate_scale(spawner_entity, 0.1, 1.0),
    )
    Report.result(
        Tests.instances_properly_scaled,
        set_and_validate_scale(spawner_entity, 2.0, 2.5),
    )
    Report.result(
        Tests.instances_properly_scaled,
        set_and_validate_scale(spawner_entity, 1.0, 5.0),
    )
