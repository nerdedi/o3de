...
def SurfaceMaskFilter_InclusionList():
    ...
    def update_surface_tag_inclusion_list(Entity, component_index, surface_tag):
        ...
        hydra.get_set_test(
            Entity, component_index, "Configuration|Inclusion|Surface Tags", tag_list
        )
        ...
        if new_value == surface_tag:
            Report.info(
                "Inclusive surface mask filter of terrainHole is added successfully"
            )
        else:
    ...
    pink_flower_prefab = dynveg.create_temp_mesh_prefab(
        pink_flower_asset_path, "SurfaceMaskTagInclusion_PinkFlower"
    )[0]
