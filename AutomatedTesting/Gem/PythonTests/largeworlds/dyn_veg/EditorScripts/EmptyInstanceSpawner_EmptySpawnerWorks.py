...
class PropertyTreeTests:
    ...
    spawner_type_set = ("Successfully set spawner type", "Failed to set spawner type")
...
def EmptyInstanceSpawner_EmptySpawnerWorks():
    ...
    empty_spawner_uuid = azlmbr.math.Uuid_CreateString(
        "{23C40FD4-A55F-4BD3-BE5B-DC5423F217C2}"
    )
    ...
    behavior_context_test_success = behavior_context_test_success and (
        empty_spawner is not None
    )
    behavior_context_test_success = behavior_context_test_success and (
        empty_spawner.typename == "EmptyInstanceSpawner"
    )
    Report.critical_result(
        BehaviorContextTests.spawner_initialized, behavior_context_test_success
    )
    Report.info(
        f"EmptyInstanceSpawner() BehaviorContext test: {behavior_context_test_success}"
    )
    ...
    spawner_type_test_success = (
        spawner_type_test_success
        and hydra.get_set_property_test(descriptor, "spawnerType", empty_spawner_uuid)
    )
    spawner_type_test_success = spawner_type_test_success and (
        descriptor.spawner.typename == "EmptyInstanceSpawner"
    )
    Report.result(
        BehaviorContextTests.desc_spawnertype_sets_spawner, spawner_type_test_success
    )
    Report.info(
        f"Descriptor() BehaviorContext spawnerType test: {spawner_type_test_success}"
    )
    ...
    spawner_test_success = spawner_test_success and (
        descriptor.spawnerType.Equal(empty_spawner_uuid)
    )
    spawner_test_success = spawner_test_success and (
        descriptor.spawner.typename == "EmptyInstanceSpawner"
    )
    Report.result(
        BehaviorContextTests.desc_spawner_sets_spawnertype, spawner_test_success
    )
    ...
    Report.critical_result(
        PropertyTreeTests.entity_created, spawner_entity.id.IsValid()
    )
    ...
    dynveg.create_surface_entity(
        "Surface Entity", math.Vector3(512.0, 512.0, 32.0), 1024.0, 1024.0, 1.0
    )
    ...
    property_tree_success = property_tree_success and spawner_entity.get_set_test(
        2, "Configuration|Embedded Assets|[0]|Instance Spawner", empty_spawner_uuid
    )
    ...
    property_tree_success = property_tree_success and helper.wait_for_condition(
    ...
    Report.result(
        PropertyTreeTests.empty_instance_count_validation, property_tree_success
    )
    ...
    Report.result(
        PropertyTreeTests.not_affected_by_allow_empty_assets, allow_empty_assets_success
    )
