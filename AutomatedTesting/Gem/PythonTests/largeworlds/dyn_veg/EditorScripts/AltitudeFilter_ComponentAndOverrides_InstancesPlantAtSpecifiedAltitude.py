...
def AltitudeFilter_ComponentAndOverrides_InstancesPlantAtSpecifiedAltitude():
    ...
    flower_asset_path = os.path.join(
        "assets", "objects", "foliage", "grass_flower_pink.fbx.azmodel"
    )
    flower_prefab = dynveg.create_temp_mesh_prefab(
        flower_asset_path, "AltFilter_PinkFlower"
    )[0]

    spawner_entity = dynveg.create_temp_prefab_vegetation_area(
        "Instance Spawner", center_point, 32.0, 32.0, 32.0, flower_prefab
    )
    ...
    dynveg.create_surface_entity(
        "Planting Surface Elevated", elevated_surface_center_point, 32.0, 32.0, 1.0
    )
    ...
    altitude_min_max_success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
            spawner_entity.id, num_expected
        ),
        5.0,
    ...
    spawner_entity.get_set_test(
        2, "Configuration|Embedded Assets|[0]|Altitude Filter|Override Enabled", True
    )
    spawner_entity.get_set_test(
        2, "Configuration|Embedded Assets|[0]|Altitude Filter|Min", 35
    )
    spawner_entity.get_set_test(
        2, "Configuration|Embedded Assets|[0]|Altitude Filter|Max", 37
    )

    # 8) Validate expected instances at specified elevations
    num_expected = 20 * 20  # 20 instances per 16m per side
    overrides_success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
            spawner_entity.id, num_expected
        ),
        5.0,
