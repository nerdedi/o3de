...
def ImageGradient_ModifiesSurfaces():
    ...
    spawner_entity = dynveg.create_prefab_vegetation_area(
        "Instance Spawner", entity_position, 50.0, 50.0, 10.0, purple_flower_prefab_path
    )
    ...
    image_gradient_entity.components[2].set_component_property_value(
        "Configuration|Shape Entity Id", spawner_entity.id
    )
    ...
    hydra.get_set_test(
        spawner_entity, 3, "Configuration|Inclusion|Surface Tags", tag_list
    )
    ...
    grad_surf_tag_emitter_component.add_container_item(
        "Configuration|Extended Tags", 0, tag_list[0]
    )
    ...
    test_img_gradient_path = os.path.join(
        "Assets", "ImageGradients", "image_grad_test_gsi.png.streamingimage"
    )
    ...
    vegetation.ImageGradientRequestBus(
        bus.Event, "SetImageAssetPath", image_gradient_entity.id, ""
    )
    num_expected_instances_after_clear = 0
    success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
            spawner_entity.id, num_expected_instances_after_clear
        ),
        5.0,
    )
    Report.result(Tests.instance_validation, success)
