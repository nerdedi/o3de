...
def ImageGradient_BilinearFiltering():
    ...
    veg_settings_component = EditorLevelEntity.add_component(
        "Vegetation System Settings"
    )
    veg_settings_component.set_component_property_value(
        "Configuration|Area System Settings|Sector Size In Meters", 20.0
    )
    veg_settings_component.set_component_property_value(
    ...
    image_gradient_entity = EditorEntity.create_editor_entity_at(
        entity_position, "Image Gradient"
    )
    Report.critical_result(
        Tests.image_gradient_entity_created, image_gradient_entity.id.IsValid()
    )
    ...
    image_gradient_entity.components[2].set_component_property_value(
        "Box Shape|Box Configuration|Dimensions", image_gradient_box_size
    )
    image_gradient_entity.components[1].set_component_property_value(
        "Configuration|Wrapping Type", 1
    )

    # Assign the image asset to the image gradient and validate the expected asset was set
    test_img_gradient_path = os.path.join(
        "assets", "imagegradients", "black_white_gsi.png.streamingimage"
    )
    asset = Asset.find_asset_by_path(test_img_gradient_path)
    vegetation.ImageGradientRequestBus(
        bus.Event, "SetImageAssetPath", image_gradient_entity.id, test_img_gradient_path
    )
    compare_asset_path = vegetation.ImageGradientRequestBus(
        bus.Event, "GetImageAssetPath", image_gradient_entity.id
    )
    ...
    purple_flower_prefab_path = os.path.join(
        "assets", "prefabs", "PurpleFlower.spawnable"
    )
    spawner_entity = dynveg.create_prefab_vegetation_area(
        "Instance Spawner", entity_position, 64.0, 1.0, 10.0, purple_flower_prefab_path
    )
    ...
    success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
            spawner_entity.id, num_expected_instances_point
        ),
        5.0,
    )
    ...
    image_gradient_entity.components[0].set_component_property_value(
        "Configuration|Sampling Type", 1
    )
    num_expected_instances_bilinear = 48
    success = helper.wait_for_condition(
        lambda: dynveg.validate_instance_count_in_entity_shape(
            spawner_entity.id, num_expected_instances_bilinear
        ),
        5.0,
    )
