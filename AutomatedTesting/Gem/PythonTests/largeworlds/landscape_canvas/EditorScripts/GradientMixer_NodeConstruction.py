...
def GradientMixer_NodeConstruction():
    ...
    boxShapeNode = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast, "CreateNodeForTypeName", newGraph, "BoxShapeNode"
    )
    ...
    perlinNoiseNode = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast, "CreateNodeForTypeName", newGraph, "PerlinNoiseGradientNode"
    )
    graph.GraphControllerRequestBus(
        bus.Event,
        "AddNode",
    ...
    fastNoiseNode = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast, "CreateNodeForTypeName", newGraph, "FastNoiseGradientNode"
    )
    graph.GraphControllerRequestBus(
        bus.Event,
        "AddNode",
    ...
    gradientMixerNode = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast, "CreateNodeForTypeName", newGraph, "GradientMixerNode"
    )
    graph.GraphControllerRequestBus(
        bus.Event,
        "AddNode",
    ...
    inboundGradientSlotId2 = graph.GraphControllerRequestBus(
        bus.Event, "ExtendSlot", newGraphId, gradientMixerNode, "InboundGradient"
    )
    ...
    graph.GraphControllerRequestBus(
        ...
        newGraphId,
        perlinNoiseNode,
        outboundGradientSlotId,
        gradientMixerNode,
        inboundGradientSlotId,
    )
    graph.GraphControllerRequestBus(
        bus.Event,
        "AddConnectionBySlotId",
    ...
    gradientMixerOutcome = editor.EditorComponentAPIBus(
        bus.Broadcast, "GetComponentOfType", gradientMixerEntityId, gradientMixerTypeId
    )
    gradientMixerComponent = gradientMixerOutcome.GetValue()
    perlinNoiseOutcome = editor.EditorComponentAPIBus(
        bus.Broadcast, "GetComponentOfType", perlinNoiseEntityId, perlinNoiseTypeId
    )
    ...
    previewEntityId = hydra.get_component_property_value(
        perlinNoiseComponent, "Previewer|Preview Settings|Pin Preview to Shape"
    )
    Report.result(
        Tests.preview_entity_set,
        previewEntityId and boxShapeEntityId.invoke("Equal", previewEntityId),
    )

    # Verify the 1st Inbound Gradient EntityId property on our Gradient Mixer component has been set to our Perlin Noise
    # Gradient's EntityId
    inboundGradientEntityId = hydra.get_component_property_value(
        gradientMixerComponent, "Configuration|Layers|[0]|Gradient|Gradient Entity Id"
    )
    Report.result(
        Tests.mixer_inbound_gradient_set_a,
        inboundGradientEntityId
        and perlinNoiseEntityId.invoke("Equal", inboundGradientEntityId),
    )
    ...
    inboundGradientEntityId2 = hydra.get_component_property_value(
        gradientMixerComponent, "Configuration|Layers|[1]|Gradient|Gradient Entity Id"
    )
    Report.result(
        Tests.mixer_inbound_gradient_set_b,
        inboundGradientEntityId2
        and fastNoiseEntityId.invoke("Equal", inboundGradientEntityId2),
    )

    # Verify that Gradient Mixer Layer Operations are properly set
    mixer_operation_a = hydra.get_component_property_value(
        gradientMixerComponent, "Configuration|Layers|[0]|Operation"
    )
    mixer_operation_b = hydra.get_component_property_value(
        gradientMixerComponent, "Configuration|Layers|[1]|Operation"
    )
