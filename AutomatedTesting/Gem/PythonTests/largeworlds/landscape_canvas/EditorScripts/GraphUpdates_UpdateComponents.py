...
def GraphUpdates_UpdateComponent():
    ...
    helper.wait_for_condition(
        lambda: len(entity.SearchBus(bus.Broadcast, "SearchEntities", search_filter))
        > 0,
        5.0,
    )
    prefab_lc_root = entity.SearchBus(bus.Broadcast, "SearchEntities", search_filter)[0]
    azlmbr.prefab.PrefabFocusPublicRequestBus(
        bus.Broadcast, "FocusOnOwningPrefab", prefab_lc_root
    )
    ...
    open_graph_id = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast, "OnGraphEntity", prefab_lc_root
    )
    ...
    rotation_modifier_node = lc.find_nodes_matching_entity_component(
        "Vegetation Rotation Modifier", bush_spawner_id
    )

    # Remove the Rotation Modifier node
    graph.GraphControllerRequestBus(
        bus.Event, "RemoveNode", open_graph_id, rotation_modifier_node[0]
    )
    ...
    has_rotation_modifier = editor.EditorComponentAPIBus(
        bus.Broadcast,
        "HasComponentOfType",
        bush_spawner_id,
    ...
    layer_spawner_node = lc.find_nodes_matching_entity_component(
        "Vegetation Layer Spawner", bush_spawner_id
    )

    # Remove the Vegetation Layer Spawner node and verify the corresponding entity is deleted
    graph.GraphControllerRequestBus(
        bus.Event, "RemoveNode", open_graph_id, layer_spawner_node[0]
    )
    general.idle_wait_frames(2)
    layer_spawner_node = lc.find_nodes_matching_entity_component(
        "Vegetation Layer Spawner", bush_spawner_id
    ...
    rotation_modifier_node = lc.find_nodes_matching_entity_component(
        "Vegetation Rotation Modifier", flower_spawner_id
    )
    invert_node = lc.find_nodes_matching_entity_component(
        "Invert Gradient Modifier", inverted_perlin_noise_id
    )
    ...
    graph.GraphControllerRequestBus(
        ...
        open_graph_id,
        invert_node[0],
        outbound_gradient_slot,
        rotation_modifier_node[0],
        inbound_gradient_z_slot,
    )

    general.idle_wait_frames(
        2
    )  # Add a small wait to ensure component property has time to update
    ...
    rotation_outcome = editor.EditorComponentAPIBus(
        bus.Broadcast, "GetComponentOfType", flower_spawner_id, rotation_type_id
    )
    rotation_component = rotation_outcome.GetValue()
    gradient_reference = hydra.get_component_property_value(
        rotation_component, "Configuration|Rotation Z|Gradient|Gradient Entity Id"
    )
    Report.result(
        Tests.entity_reference_updated, gradient_reference == inverted_perlin_noise_id
    )
