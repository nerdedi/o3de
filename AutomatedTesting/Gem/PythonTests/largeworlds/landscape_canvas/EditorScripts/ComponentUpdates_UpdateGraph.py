...
def ComponentUpdates_UpdateGraph():
    ...
    bush_spawner_entity = EditorEntity.find_editor_entity(
        "BushSpawner", must_be_unique=True
    )
    ...
    has_distribution_filter = bush_spawner_entity.has_component(
    ...
    spawner_dist_filter_component = bush_spawner_entity.get_components_of_type(
        ["Vegetation Distribution Filter"]
    )[0]
    spawner_dist_filter_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast,
        "GetAllNodesMatchingEntityComponent",
        spawner_dist_filter_component.id,
    )
    Report.critical_result(
        Tests.dist_filter_node_found, len(spawner_dist_filter_nodes) > 0
    )

    # Add a Vegetation Altitude Filter component to the BushSpawner entity, and verify the node is added to the graph
    spawner_alt_filter_component = bush_spawner_entity.add_component(
        "Vegetation Altitude Filter"
    )
    has_altitude_filter = bush_spawner_entity.has_component(
        "Vegetation Altitude Filter"
    )
    altitude_filter_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast,
        "GetAllNodesMatchingEntityComponent",
    ...
    has_dist_filter = bush_spawner_entity.has_component(
        "Vegetation Distribution Filter"
    )
    ...
    box_entity = EditorEntity.create_editor_entity(
        "Box Child", parent_id=bush_spawner_entity.id
    )
    Report.result(
        Tests.child_entity_added, box_entity.get_parent_id() == bush_spawner_entity.id
    )
    ...
    box_shape_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast, "GetAllNodesMatchingEntityComponent", box_shape_component.id
    )
