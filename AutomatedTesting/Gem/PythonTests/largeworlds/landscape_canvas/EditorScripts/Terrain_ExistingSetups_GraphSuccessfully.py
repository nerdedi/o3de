...
def Terrain_ExistingSetups_GraphSuccessfully():
    ...
    test_prefab_path = os.path.join(
        "Assets", "Prefabs", "LandscapeCanvasTerrain.prefab"
    )
    prefab_result = prefab.PrefabPublicRequestBus(
        bus.Broadcast,
        "InstantiatePrefab",
        test_prefab_path,
        entity.EntityId(),
    ...
    prefab_root_entity = EditorEntity.find_editor_entity(
        "LandscapeCanvas", must_be_unique=True
    )
    Report.critical_result(Tests.lc_entity_found, prefab_root_entity.id.IsValid())
    terrain_spawner_entity = EditorEntity.find_editor_entity(
        "TerrainSpawner", must_be_unique=True
    )
    Report.critical_result(
        Tests.spawner_entity_found, terrain_spawner_entity.id.IsValid()
    )
    heightmap_entity = EditorEntity.find_editor_entity(
        "HeightmapGradient", must_be_unique=True
    )
    Report.critical_result(Tests.heightmap_entity_found, heightmap_entity.id.IsValid())

    terrain_spawner_component = terrain_spawner_entity.get_components_of_type(
        ["Terrain Layer Spawner"]
    )[0]
    aabb_component = terrain_spawner_entity.get_components_of_type(
        ["Axis Aligned Box Shape"]
    )[0]
    shape_ref_component = heightmap_entity.get_components_of_type(["Shape Reference"])[
        0
    ]
    gradient_component = heightmap_entity.get_components_of_type(
        ["Perlin Noise Gradient"]
    )[0]
    surf_grad_list_component = terrain_spawner_entity.get_components_of_type(
        ["Terrain Surface Gradient List"]
    )[0]
    height_grad_list_component = terrain_spawner_entity.get_components_of_type(
        ["Terrain Height Gradient List"]
    )[0]

    # Open the existing Landscape Canvas graph
    general.open_pane("Landscape Canvas")
    open_graph = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast, "OnGraphEntity", prefab_root_entity.id
    )
    Report.critical_result(Tests.existing_graph_opened, open_graph.IsValid())

    # Validate the proper nodes are present
    spawner_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast,
        "GetAllNodesMatchingEntityComponent",
        terrain_spawner_component.id,
    )
    aabb_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast, "GetAllNodesMatchingEntityComponent", aabb_component.id
    )
    shape_ref_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast, "GetAllNodesMatchingEntityComponent", shape_ref_component.id
    )
    gradient_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast, "GetAllNodesMatchingEntityComponent", gradient_component.id
    )
    surf_grad_list_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast, "GetAllNodesMatchingEntityComponent", surf_grad_list_component.id
    )
    height_grad_list_nodes = landscapecanvas.LandscapeCanvasRequestBus(
        bus.Broadcast,
        "GetAllNodesMatchingEntityComponent",
    ...
    for node_name in nodes.keys():
        node_found = (f"{node_name} found", f"Failed to find {node_name}")
    ...
    def are_slots_connected(node_1, node_1_slot, node_2, node_2_slot):
        connected = graph.GraphControllerRequestBus(
            bus.Event,
            "AreSlotsConnected",
            open_graph,
            nodes[node_1][0],
    ...
    are_slots_connected(
        "AxisAlignedBoxShapeNode", bounds_slot, "ReferenceShapeNode", inbound_shape_slot
    )
