...
def SlotConnections_UpdateComponentReferences():
    ...
    def getEntityIdFromComponentProperty(
        targetEntityId, componentTypeName, propertyPath
    ):
        ...
        propertyOutcome = editor.EditorComponentAPIBus(
            bus.Broadcast, "GetComponentProperty", component, propertyPath
        )
    ...
    randomNoiseNode = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast, "CreateNodeForTypeName", newGraph, "RandomNoiseGradientNode"
    )
    graph.GraphControllerRequestBus(
        bus.Event,
        "AddNode",
    ...
    ditherNode = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast, "CreateNodeForTypeName", newGraph, "DitherGradientModifierNode"
    )
    graph.GraphControllerRequestBus(
        bus.Event, "AddNode", newGraphId, ditherNode, math.Vector2(positionX, positionY)
    )
    ...
    # EntityId
    previewEntityId = getEntityIdFromComponentProperty(
        randomNoiseEntityId,
        "Random Noise Gradient",
        "Previewer|Preview Settings|Pin Preview to Shape",
    )
    random_gradient_success = previewEntityId and boxShapeEntityId.invoke(
        "Equal", previewEntityId
    )
    ...
    inboundGradientEntityId = getEntityIdFromComponentProperty(
        ditherEntityId,
        "Dither Gradient Modifier",
        "Configuration|Gradient|Gradient Entity Id",
    )
    dither_gradient_success = inboundGradientEntityId and randomNoiseEntityId.invoke(
        "Equal", inboundGradientEntityId
    )
    ...
    inboundGradientMixerEntityId = getEntityIdFromComponentProperty(
        gradientMixerEntityId,
        "Gradient Mixer",
        "Configuration|Layers|[0]|Gradient|Gradient Entity Id",
    )
    gradient_mixer_success = inboundGradientMixerEntityId and ditherEntityId.invoke(
        "Equal", inboundGradientMixerEntityId
    )
