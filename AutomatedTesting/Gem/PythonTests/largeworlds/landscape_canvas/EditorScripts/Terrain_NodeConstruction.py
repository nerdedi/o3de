...
def Terrain_NodeConstruction():
    ...
    graph_registered = graph.AssetEditorRequestBus(
        bus.Event, "ContainsGraph", editor_id, new_graph_id
    )
    ...
    layer_spawner_node = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast, "CreateNodeForTypeName", new_graph, "TerrainLayerSpawnerNode"
    )
    graph.GraphControllerRequestBus(
        bus.Event, "AddNode", new_graph_id, layer_spawner_node, node_position
    )
    ...
    aabb_component_outcome = editor.EditorComponentAPIBus(
        bus.Broadcast, "GetComponentOfType", created_entity_id, aabb_type_id
    )
    assert aabb_component_outcome.IsSuccess(), (
        "Failed to get Axis Aligned Box Shape component info"
    )
    ...
    aabb_outcome = editor.EditorComponentAPIBus(
        bus.Broadcast,
    ...
    constant_gradient_node = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast, "CreateNodeForTypeName", new_graph, "ConstantGradientNode"
    )
    graph.GraphControllerRequestBus(
        bus.Event, "AddNode", new_graph_id, constant_gradient_node, node_position
    )

    # Add Terrain Height Gradient List and Terrain Surface Gradient List wrapped nodes to the Terrain Spawner
    height_grad_list_node = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast,
        "CreateNodeForTypeName",
        new_graph,
        "TerrainHeightGradientListNode",
    )
    surface_grad_list_node = landscapecanvas.LandscapeCanvasNodeFactoryRequestBus(
        bus.Broadcast,
        "CreateNodeForTypeName",
        new_graph,
    ...
    graph.GraphControllerRequestBus(
        bus.Event, "WrapNode", new_graph_id, layer_spawner_node, height_grad_list_node
    )
    graph.GraphControllerRequestBus(
        bus.Event, "WrapNode", new_graph_id, layer_spawner_node, surface_grad_list_node
    )
    ...
    graph.GraphControllerRequestBus(
        ...
        new_graph_id,
        constant_gradient_node,
        outbound_grad_slot,
        height_grad_list_node,
        inbound_grad_slot,
    )
    graph.GraphControllerRequestBus(
        bus.Event,
        ...
        constant_gradient_node,
        outbound_grad_slot,
        surface_grad_list_node,
        inbound_grad_slot,
    )

    # Query 2 points we expect terrain to exist and one we don"t and validate
    terrain_exists = not terrain.TerrainDataRequestBus(
        bus.Broadcast, "GetIsHole", math.Vector3(128.0, 128.0, 0.0), 2
    )
    terrain_exists2 = not terrain.TerrainDataRequestBus(
        bus.Broadcast, "GetIsHole", math.Vector3(-64.0, -64.0, 0.0), 2
    )
    terrain_exists3 = not terrain.TerrainDataRequestBus(
        bus.Broadcast, "GetIsHole", math.Vector3(512.0, 512.0, 0.0), 2
    )

    Report.result(
        Tests.terrain_found, terrain_exists and terrain_exists2 and not terrain_exists3
    )
