...
def Terrain_World_ConfigurationWorks():
    ...
    with Tracer() as section_tracer:
        ...
        hydra.set_component_property_value(
            terrain_world_component, "Configuration|Min Height", world_height_min
        )
        hydra.set_component_property_value(
            terrain_world_component, "Configuration|Max Height", world_height_max
        )
        hydra.set_component_property_value(
            terrain_world_component,
            "Configuration|Height Query Resolution (m)",
            height_query_resolution,
        )
        world_min = hydra.get_component_property_value(
            terrain_world_component, "Configuration|Min Height"
        )
        world_max = hydra.get_component_property_value(
            terrain_world_component, "Configuration|Max Height"
        )
        world_query = hydra.get_component_property_value(
            terrain_world_component, "Configuration|Height Query Resolution (m)"
        )
        ...
        Report.result(
            Tests.height_query_changed, world_query == height_query_resolution
        )
        ...
        # 11) Check terrain exists at a known position in the world
        terrainExists = not terrain.TerrainDataRequestBus(
            bus.Broadcast, "GetIsHole", azmath.Vector3(10.0, 10.0, 0.0), CLAMP
        )
        Report.result(Tests.terrain_exists, terrainExists)

        terrainExists = not terrain.TerrainDataRequestBus(
            bus.Broadcast, "GetIsHole", azmath.Vector3(1023.0, 1023.0, 0.0), CLAMP
        )
        Report.result(Tests.terrain_exists, terrainExists)

        # 12) Check terrain does not exist at a known position outside the world
        terrainDoesNotExist = terrain.TerrainDataRequestBus(
            bus.Broadcast, "GetIsHole", azmath.Vector3(1025.0, 1025.0, 0.0), CLAMP
        )
        Report.result(Tests.terrain_does_not_exist, terrainDoesNotExist)

        terrainDoesNotExist = terrain.TerrainDataRequestBus(
            bus.Broadcast, "GetIsHole", azmath.Vector3(-1.0, -1.0, 0.0), CLAMP
        )
        Report.result(Tests.terrain_does_not_exist, terrainDoesNotExist)

        # 13) Check height value is the expected one when query resolution is changed
        height1, exists1 = terrain.TerrainDataRequestBus(
            bus.Broadcast, "GetHeight", azmath.Vector3(10.5, 10.5, 0.0), CLAMP
        )
        ...
        general.idle_wait_frames(1)
        height2, exists2 = terrain.TerrainDataRequestBus(
            bus.Broadcast, "GetHeight", azmath.Vector3(10.5, 10.5, 0.0), CLAMP
        )
        Report.result(
            Tests.values_not_the_same,
            not math.isclose(height1, height2, abs_tol=0.000000001),
        )
