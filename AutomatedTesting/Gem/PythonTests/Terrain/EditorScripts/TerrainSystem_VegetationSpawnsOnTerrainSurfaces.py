...
def TerrainSystem_VegetationSpawnsOnTerrainSurfaces():
    ...
    def FindHighestAndLowestZValuesInArea(aabb):
        vegetation_items = areasystem.AreaSystemRequestBus(
            bus.Broadcast, "GetInstancesInAabb", aabb
        )
    ...
    terrain_entity_1 = create_entity_at(
        "Terrain1",
        [
    ...
    terrain_entity_1.get_set_test(
        1, "Axis Aligned Box Shape|Box Configuration|Dimensions", box_dimensions
    )

    terrain_entity_2 = create_entity_at(
        "Terrain2",
        [
            "Terrain Layer Spawner",
            "Axis Aligned Box Shape",
    ...
    gradient_entity_1 = create_entity_at(
        "Gradient1", ["Constant Gradient"], 0.0, 0.0, 0.0
    )
    gradient_entity_1.get_set_test(0, "Configuration|Value", gradient_value_1)

    gradient_entity_2 = create_entity_at(
        "Gradient2", ["Constant Gradient"], 0.0, 0.0, 0.0
    )
    ...
    vegetation_entity = create_entity_at(
        "Vegetation",
        [
    ...
    vegetation_entity.get_set_test(
        1, "Axis Aligned Box Shape|Box Configuration|Dimensions", box_dimensions
    )
    ...
    descriptor = hydra.get_component_property_value(
        vegetation_entity.components[2], "Configuration|Embedded Assets|[0]"
    )
    ...
    terrain_entity_1.get_set_test(
        2, "Configuration|Gradient Entities", [gradient_entity_1.id]
    )
    terrain_entity_2.get_set_test(
        2, "Configuration|Gradient Entities", [gradient_entity_2.id]
    )
    ...
    helper.wait_for_condition(
        lambda: vegetation.VegetationSpawnerRequestBus(
            bus.Event, "GetAreaProductCount", vegetation_entity.id
        )
        == expected_no_filtering_item_count,
        5.0,
    )

    # Check the spawn count is correct.
    item_count = vegetation.VegetationSpawnerRequestBus(
        bus.Event, "GetAreaProductCount", vegetation_entity.id
    )
    Report.result(
        VegetationTests.unfiltered_vegetation_count_correct,
        item_count == expected_no_filtering_item_count,
    )

    test_aabb = math.Aabb_CreateFromMinMax(
        math.Vector3(-10.0, -10.0, 0.0), math.Vector3(30.0, 10.0, box_height)
    )
    ...
    Report.result(
        VegetationTests.vegetation_on_gradient_1,
        sys_math.isclose(lowest_z, box_height * gradient_value_1, abs_tol=0.01),
    )
    Report.result(
        VegetationTests.vegetation_on_gradient_2,
        sys_math.isclose(highest_z, box_height * gradient_value_2, abs_tol=0.01),
    )

    # Assign SurfaceTags to the SurfaceGradientLists
    terrain_entity_1.get_set_test(
        3,
        "Configuration|Gradient to Surface Mappings|[0]|Surface Tag",
        surface_data.SurfaceTag("test_tag2"),
    )
    terrain_entity_2.get_set_test(
        3,
        "Configuration|Gradient to Surface Mappings|[0]|Surface Tag",
    ...
    vegetation_entity.get_set_test(
        3, "Configuration|Inclusion|Surface Tags", [surface_data.SurfaceTag()]
    )
    vegetation_entity.get_set_test(
        3,
        "Configuration|Inclusion|Surface Tags|[0]",
        surface_data.SurfaceTag("test_tag3"),
    )
    ...
    helper.wait_for_condition(
        ...
        == expected_tag3_included_item_count,
        5.0,
    )
    ...
    Report.result(
        VegetationTests.testTag2_excluded_vegetation_count_correct,
        item_count == expected_tag3_included_item_count,
    )
    ...
    Report.result(
        VegetationTests.testTag2_excluded_vegetation_z_correct,
        lowest_z > box_height * gradient_value_1,
    )
    ...
    vegetation_entity.get_set_test(
        3,
        "Configuration|Inclusion|Surface Tags|[0]",
        surface_data.SurfaceTag("terrain"),
    )
    helper.wait_for_condition(
        lambda: vegetation.VegetationSpawnerRequestBus(
            bus.Event, "GetAreaProductCount", vegetation_entity.id
        )
        == expected_terrain_included_item_count,
        5.0,
    )
    ...
    Report.result(
        VegetationTests.cleared_exclusion_vegetation_count_correct,
        item_count == expected_terrain_included_item_count,
    )
    ...
    helper.wait_for_condition(
        ...
        == expected_tag2_included_item_count,
        5.0,
    )
    ...
    Report.result(
        VegetationTests.testTag3_excluded_vegetation_count_correct,
        item_count == expected_tag2_included_item_count,
    )
    ...
    Report.result(
        VegetationTests.testTag3_excluded_vegetation_z_correct,
        highest_z < box_height * gradient_value_2,
    )
