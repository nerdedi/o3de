...
def TerrainPhysicsCollider_ChangesSizeWithAxisAlignedBoxShapeChanges():
    ...
    # 2) Create test entity
    test_entity = EditorEntity.create_editor_entity_at(
        azmath.Vector3(0.0, 0.0, 0.0), "TestEntity"
    )
    ...
    with Tracer() as section_tracer:
        ...
        Report.result(
            Tests.add_axis_aligned_box_shape,
            test_entity.has_component("Axis Aligned Box Shape"),
        )
        test_entity.add_component("Terrain Physics Heightfield Collider")
        Report.result(
            Tests.add_terrain_collider,
            test_entity.has_component("Terrain Physics Heightfield Collider"),
        )

        # 5) Change the Axis Aligned Box Shape dimensions
        aaBoxShape_component.set_component_property_value(
            "Axis Aligned Box Shape|Box Configuration|Dimensions",
            azmath.Vector3(SET_BOX_X_SIZE, SET_BOX_Y_SIZE, 1.0),
        )
        add_check = aaBoxShape_component.get_component_property_value(
            "Axis Aligned Box Shape|Box Configuration|Dimensions"
        ) == azmath.Vector3(SET_BOX_X_SIZE, SET_BOX_Y_SIZE, 1.0)
        Report.result(Tests.box_dimensions_changed, add_check)

        # 6) Check the Heightfield provider is returning the correct size
        columns = physics.HeightfieldProviderRequestsBus(
            bus.Broadcast, "GetHeightfieldGridColumns"
        )
        rows = physics.HeightfieldProviderRequestsBus(
            bus.Broadcast, "GetHeightfieldGridRows"
        )
        Report.result(
            Tests.configuration_changed,
