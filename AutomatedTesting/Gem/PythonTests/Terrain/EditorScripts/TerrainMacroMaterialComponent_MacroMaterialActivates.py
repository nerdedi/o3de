...
class MacroMaterialTests:
    setup_test = ("Setup successful", "Setup failed")
    ...
    material_created = ("MaterialCreated called successfully", "MaterialCreated failed")
    material_destroyed = (
...
def TerrainMacroMaterialComponent_MacroMaterialActivates():
    ...
    handler.add_callback(
        "OnTerrainMacroMaterialRegionChanged", on_macro_material_region_changed
    )
    handler.add_callback("OnTerrainMacroMaterialDestroyed", on_macro_material_destroyed)

    macro_material_entity = create_entity_at(
        "macro", ["Terrain Macro Material", "Axis Aligned Box Shape"], 0.0, 0.0, 0.0
    )

    # Check that no macro material callbacks happened. It should be "inactive" as it has no assets assigned.
    setup_success = (
        not material_created_called
        and not material_changed_called
        and not material_region_changed_called
        and not material_destroyed_called
    )
    Report.result(MacroMaterialTests.setup_test, setup_success)

    # Find the aabb component.
    aabb_component_type_id_type = azlmbr.editor.EditorComponentAPIBus(
        azlmbr.bus.Broadcast,
        "FindComponentTypeIdsByEntityType",
        ["Axis Aligned Box Shape"],
        0,
    )[0]
    aabb_component_id = azlmbr.editor.EditorComponentAPIBus(
        azlmbr.bus.Broadcast,
        "GetComponentOfType",
        macro_material_entity.id,
        aabb_component_type_id_type,
    ).GetValue()
    ...
    general.idle_wait_frames(1)
    Report.result(
        MacroMaterialTests.material_changed_not_called_when_inactive,
        not material_region_changed_called,
    )

    # Find the macro material component.
    macro_material_id_type = azlmbr.editor.EditorComponentAPIBus(
        azlmbr.bus.Broadcast,
        "FindComponentTypeIdsByEntityType",
        ["Terrain Macro Material"],
        0,
    )[0]
    macro_material_component_id = azlmbr.editor.EditorComponentAPIBus(
        azlmbr.bus.Broadcast,
        "GetComponentOfType",
        macro_material_entity.id,
        macro_material_id_type,
    ).GetValue()

    # Find a color image asset.
    color_image_path = os.path.join("assets", "textures", "image.png.streamingimage")
    color_image_asset = asset.AssetCatalogRequestBus(
        bus.Broadcast, "GetAssetIdByPath", color_image_path, math.Uuid(), False
    )
    ...
    normal_image_asset = asset.AssetCatalogRequestBus(
        bus.Broadcast, "GetAssetIdByPath", normal_image_path, math.Uuid(), False
    )
    ...
    destroyed_call_result = helper.wait_for_condition(
        lambda: material_destroyed_called, 2.0
    )
    Report.result(MacroMaterialTests.material_destroyed, destroyed_call_result)

    recreated_call_result = helper.wait_for_condition(
        lambda: material_created_called, 2.0
    )
    ...
    region_changed_call_result = helper.wait_for_condition(
        lambda: material_region_changed_called, 2.0
    )
    Report.result(
        MacroMaterialTests.material_changed_call_on_aabb_change,
        region_changed_call_result,
    )
