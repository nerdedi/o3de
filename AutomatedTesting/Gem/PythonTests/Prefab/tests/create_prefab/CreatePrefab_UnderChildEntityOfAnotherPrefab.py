...
def CreatePrefab_UnderChildEntityOfAnotherPrefab():
    ...
    assert parent_entity_on_outer_instance.has_component(
    ...
    _, inner_instance = Prefab.create_prefab(
        [child_entity_on_outer_instance], INNER_PREFAB_FILE_NAME
    )

    # Test undo/redo on prefab creation
    prefab_test_utils.validate_undo_redo_on_prefab_creation(
        inner_instance, parent_entity_on_outer_instance.id
    )
    ...
    assert (
        ...
        == inner_instance.container_entity.id
    )
    assert (
        inner_instance.container_entity.get_parent_id()
        == parent_entity_on_outer_instance.id
    )
    assert (
        parent_entity_on_outer_instance.get_parent_id()
        == outer_instance.container_entity.id
    )
