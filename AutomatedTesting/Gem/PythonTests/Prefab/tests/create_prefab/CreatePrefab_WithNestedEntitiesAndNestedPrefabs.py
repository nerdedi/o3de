...
def CreatePrefab_WithNestedEntitiesAndNestedPrefabs():
    ...
    FILE_NAME_OF_PREFAB_WITH_NESTED_ENTITIES_AND_NESTED_PREFABS = (
        Path(__file__).stem + "_" + "new_prefab"
    )
    ...
    prefab_test_utils.validate_linear_nested_entities(
        nested_entities_root, NUM_NESTED_ENTITIES_LEVELS, CREATION_POSITION
    )
    nested_entities_root_name = nested_entities_root.get_name()

    # Creates a new nested prefabs at the root level
    # Asserts if creation didn't succeed
    entity = EditorEntity.create_editor_entity_at(
        CREATION_POSITION, name=NESTED_PREFABS_TEST_ENTITY_NAME
    )
    assert entity.id.IsValid(), "Couldn't create TestEntity"
    entity.add_component(PHYSX_PRIMITIVE_COLLIDER_NAME)
    assert entity.has_component(PHYSX_PRIMITIVE_COLLIDER_NAME), (
        f"Failed to add a {PHYSX_PRIMITIVE_COLLIDER_NAME}"
    )
    _, nested_prefab_instances = prefab_test_utils.create_linear_nested_prefabs(
        [entity],
        NESTED_PREFABS_FILE_NAME_PREFIX,
        NESTED_PREFABS_NAME_PREFIX,
        NUM_NESTED_PREFABS_LEVELS,
    )
    prefab_test_utils.validate_linear_nested_prefab_instances_hierarchy(
        nested_prefab_instances
    )
    ...
    nested_prefabs_root_container_entity_on_instance = (
        new_prefab.get_direct_child_entities()[1]
    )
    if (
        new_prefab.get_direct_child_entities()[0].get_name()
        != nested_entities_root_name
    ):
        (
            nested_entities_root_on_instance,
            nested_prefabs_root_container_entity_on_instance,
        ) = (
            nested_prefabs_root_container_entity_on_instance,
            nested_entities_root_on_instance,
    ...
    assert (
        nested_entities_root_on_instance.get_name() == nested_entities_root_name
        and nested_entities_root_on_instance.get_parent_id()
        == new_prefab_container_entity.id
    ), (
    ...
    prefab_test_utils.validate_linear_nested_entities(
        nested_entities_root_on_instance, NUM_NESTED_ENTITIES_LEVELS, CREATION_POSITION
    )

    # Test undo/redo on prefab creation
    prefab_test_utils.validate_undo_redo_on_prefab_creation(
        new_prefab, nested_entities_root_parent
    )
    ...
    for current_level in range(0, NUM_NESTED_PREFABS_LEVELS):
        assert (
            child_entity_on_inner_instance.get_name()
            == prefab_test_utils.get_linear_nested_items_name(
                NESTED_PREFABS_NAME_PREFIX, current_level
            )
        ), (
        ...
        assert (
            child_entity_on_inner_instance.get_parent_id()
            == parent_prefab_container_entity_on_instance.id
        ), (
        ...
        child_entity_on_inner_instance = child_entity_on_inner_instance.get_children()[
            0
        ]
    ...
    assert (
        child_entity_on_inner_instance.get_name() == NESTED_PREFABS_TEST_ENTITY_NAME
    ), (
        f"The name of the entity inside the nested prefabs should be '{NESTED_PREFABS_TEST_ENTITY_NAME}', "
        f"not '{child_entity_on_inner_instance.get_name()}'"
    )
    assert child_entity_on_inner_instance.has_component(
        PHYSX_PRIMITIVE_COLLIDER_NAME
