...
def AddEntity_UnderUnfocusedInstanceAsOverride():
    ...
    wheel_prefab, left_wheel_instance = Prefab.create_prefab(
        [wheel_entity],
        WHEEL_PREFAB_FILE_NAME,
        prefab_instance_name=LEFT_WHEEL_INSTANCE_NAME,
    )
    ...
    _, axle_instance = Prefab.create_prefab(
        [left_wheel_instance.container_entity, right_wheel_instance.container_entity],
        AXLE_PREFAB_FILE_NAME,
        prefab_instance_name=AXLE_INSTANCE_NAME,
    )

    CAR_INSTANCE_NAME = "CAR"
    _, car_instance = Prefab.create_prefab(
        [axle_instance.container_entity],
        CAR_PREFAB_FILE_NAME,
        prefab_instance_name=CAR_INSTANCE_NAME,
    )
    ...
    left_wheel_instance_container_entity = EditorEntity.find_editor_entity(
        entity_name=LEFT_WHEEL_INSTANCE_NAME, must_be_unique=True
    )
    assert left_wheel_instance_container_entity.id.IsValid(), (
        f"Couldn't find valid entity '{LEFT_WHEEL_INSTANCE_NAME}'"
    )

    TIRE_ENTITY_NAME = "TireEntity"
    tire_entity = EditorEntity.create_editor_entity(
        parent_id=left_wheel_instance_container_entity.id, name=TIRE_ENTITY_NAME
    )
    # Wait till prefab propagation finishes before validation.
    PrefabWaiter.wait_for_propagation()

    # Check if 'TireEntity' is added under 'LeftWheel' only correctly.
    assert tire_entity.id.IsValid(), (
        f"Couldn't create entity '{TIRE_ENTITY_NAME}'' under prefab instance '{LEFT_WHEEL_INSTANCE_NAME}'"
    )
    assert tire_entity.get_name() == TIRE_ENTITY_NAME, (
        f"Entity '{tire_entity.get_name()}''s name should be {TIRE_ENTITY_NAME}"
    )
    assert tire_entity.get_parent_id() == left_wheel_instance_container_entity.id, (
        f"Entity '{LEFT_WHEEL_INSTANCE_NAME}' should be the parent of entity '{TIRE_ENTITY_NAME}'"
    )
    ...
    assert len(child_entity_ids) == 2, (
        f"{len(child_entity_ids)} child entities found under entity '{LEFT_WHEEL_INSTANCE_NAME}'"
        f" after add-entity operation, when there should have been 2 child entities"
    )

    right_wheel_instance_container_entity = EditorEntity.find_editor_entity(
        entity_name=RIGHT_WHEEL_INSTANCE_NAME, must_be_unique=True
    )
    assert right_wheel_instance_container_entity.id.IsValid(), (
        f"Couldn't find valid entity '{RIGHT_WHEEL_INSTANCE_NAME}'"
    )

    child_entity_ids = right_wheel_instance_container_entity.get_children()
    assert len(child_entity_ids) == 1, (
        f"{len(child_entity_ids)} child entities found under entity '{RIGHT_WHEEL_INSTANCE_NAME}'"
    ...
    assert len(tire_entities) == 0, (
        f"{len(tire_entities)} '{TIRE_ENTITY_NAME}' entities exist"
    ...
    tire_entity = EditorEntity.find_editor_entity(
        entity_name=TIRE_ENTITY_NAME, must_be_unique=True
    )
    ...
    tire_entities = EditorEntity.find_editor_entities(entity_names=[TIRE_ENTITY_NAME])
    assert len(tire_entities) == 0, (
        f"Expected 0 '{TIRE_ENTITY_NAME}' entities after Revert Overrides operation, but "
