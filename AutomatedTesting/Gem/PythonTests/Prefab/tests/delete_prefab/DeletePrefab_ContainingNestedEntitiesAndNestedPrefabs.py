...
def DeletePrefab_ContainingNestedEntitiesAndNestedPrefabs():
    ...
    # Asserts if creation didn't succeed
    entity_to_nest = EditorEntity.create_editor_entity_at(
    ...
    assert entity_to_nest.has_component(PHYSX_PRIMITIVE_COLLIDER_NAME), (
    ...
    for entity_id_removed in entity_ids_removed:
        if entity_id_removed in entity_ids_after_delete:
            assert False, (
                "Redo Failed: Not all entities and descendants in target prefabs are deleted."
            )
