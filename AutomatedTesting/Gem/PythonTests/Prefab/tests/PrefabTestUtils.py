...
def create_linear_nested_entities(
    nested_entities_name_prefix, level_count, pos, parent_id=None
):
    """
    ...
    current_entity = EditorEntity.create_editor_entity_at(
        pos,
        name=get_linear_nested_items_name(nested_entities_name_prefix, 0),
        parent_id=parent_id,
    )
    ...
    for current_level in range(1, level_count):
        current_entity = EditorEntity.create_editor_entity(
            ...
            name=get_linear_nested_items_name(
                nested_entities_name_prefix, current_level
            ),
        )
...
def validate_linear_nested_entities(
    nested_entities_root, expected_level_count, expected_pos
):
    """
    ...
    """
    assert expected_level_count > 0, (
        "Can't validate nested entities with less than one layer"
    )
    ...
    level_count = 1
    while True:
        assert current_entity.id.IsValid(), (
            f"Entity '{current_entity.get_name()}' is not valid"
        )
...
def create_linear_nested_prefabs(
    entities,
    nested_prefabs_file_name_prefix,
    nested_prefabs_instance_name_prefix,
    level_count,
):
    """
    ...
    for current_level in range(0, level_count):
        current_prefab, current_prefab_instance = Prefab.create_prefab(
            entities,
            get_linear_nested_items_name(
                nested_prefabs_file_name_prefix, current_level
            ),
            prefab_instance_name=get_linear_nested_items_name(
                nested_prefabs_instance_name_prefix, current_level
            ),
        )
    ...
    parent_entity = EditorEntity(
        created_prefab_instances[0].container_entity.get_parent_id()
    )
...
def validate_linear_nested_prefab_instances_hierarchy(nested_prefab_instances):
    ...
    for current_level in range(0, len(nested_prefab_instances) - 1):
        ...
        current_prefab_instance_container_entity = (
            current_prefab_instance.container_entity
        )
        ...
        assert (
            inner_prefab_instance_container_entity.get_parent_id()
            == current_prefab_instance_container_entity.id
        ), (
    ...
    most_inner_prefab_instance_container_entity = (
        most_inner_prefab_instance.container_entity
    )
    ...
    for entity in direct_child_entities:
        assert (
            entity.get_parent_id() == most_inner_prefab_instance_container_entity.id
        ), (
...
def check_entity_children_count(entity_id, expected_children_count):
    ...
    if not entity_children_count_matched:
        Report.info(
            f"Entity '{entity_id.ToString()}' actual children count: {len(children_entity_ids)}. Expected children count: {expected_children_count}"
        )
...
def validate_child_count_for_named_editor_entity(entity_name, expected_child_count):
    ...
    for entity in entities:
        ...
        assert len(child_entities) == expected_child_count, (
            f"{len(child_entities)} children found. "
...
def validate_undo_redo_on_prefab_creation(prefab_instance, original_parent_id):
    ...
    is_prefab = editor.EditorComponentAPIBus(
        bus.Broadcast,
        "HasComponentOfType",
        prefab_instance.container_entity.id,
        globals.property.EditorPrefabComponentTypeId,
    )
    assert not is_prefab, (
        "Undo operation failed. Entity is still recognized as a prefab."
    )
    ...
    for child_entity in entities_found:
        assert (
            EditorEntity(child_entity).get_parent_id()
            == prefab_instance.container_entity.id
...
def validate_spawned_entity_rotation(entity, expected_rotation):
    ...
    x_rotation_success = math.isclose(
        spawned_entity_rotation.x, expected_rotation.x, rel_tol=1e-5
    )
    y_rotation_success = math.isclose(
        spawned_entity_rotation.y, expected_rotation.y, rel_tol=1e-5
    )
    z_rotation_success = math.isclose(
        spawned_entity_rotation.z, expected_rotation.z, rel_tol=1e-5
    )
    Report.info(
        f"Spawned Entity Rotation: Found {spawned_entity_rotation}, expected {expected_rotation}"
    )
...
def validate_spawned_entity_scale(entity, expected_scale):
    ...
    Report.info(
        f"Spawned Entity Scale: Found {spawned_entity_scale}, expected {expected_scale}"
    )
...
def validate_spawned_entity_translation(entity, expected_position):
    ...
    Report.info(
        f"Spawned Entity Translation: Found {spawned_entity_position}, expected {expected_position}"
    )
    return position_success


def validate_spawned_entity_transform(
    entity, expected_position, expected_rotation, expected_scale
):
    """
    ...
    position_success = helper.wait_for_condition(
        lambda: validate_spawned_entity_translation(entity, expected_position), 5.0
    )
    rotation_success = helper.wait_for_condition(
        lambda: validate_spawned_entity_rotation(entity, expected_rotation), 5.0
    )
    scale_success = helper.wait_for_condition(
        lambda: validate_spawned_entity_scale(entity, expected_scale), 5.0
    )
...
def validate_expected_override_status(
    entity: EditorEntity, expected_override_status: bool
) -> None:
    """
...
def validate_expected_components(
    ...
    unexpected_components: list = None,
) -> None:
    """
