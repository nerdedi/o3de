...
def InstantiatePrefab_WithNestedEntitiesAndNestedPrefabs():
    ...
    FILE_NAME_OF_PREFAB_WITH_NESTED_ENTITIES_AND_NESTED_PREFABS = (
        Path(__file__).stem + "_new_prefab"
    )
    ...
    assert entity_to_nest.id.IsValid(), (
        f"Couldn't create {NESTED_PREFABS_TEST_ENTITY_NAME}"
    )
    ...
    nested_entities_root_on_instance = (
        new_prefab_instance.get_direct_child_entity_by_name(nested_entities_root_name)
    )
    ...
    common_parent_children_ids_after_instantiate_undo = set(
        [child_id.ToString() for child_id in common_parent.get_children_ids()]
    )
    assert (
        common_parent_children_ids_before_new_instance
        == common_parent_children_ids_after_instantiate_undo
    ...
    Prefab.validate_duplicated_prefab(
        [new_prefab_instance],
        common_parent_children_ids_before_new_instance,
