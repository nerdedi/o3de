...
def DuplicateEntity_WithNestedEntitiesAndNestedPrefabs():
    ...
    def validate_duplicated_hierarchy():
        ...
        assert len(root_entities) == 2, (
            "Failed to find expected duplicated entity hierarchy root"
        )
        for root_entity in root_entities:
            expected_children_names = [
                f"{NESTED_PREFABS_NAME_PREFIX}0",
                f"{NESTED_ENTITIES_NAME_PREFIX}0",
            ]
            child_entities = root_entity.get_children()
            for child in child_entities:
                assert child.get_name() in expected_children_names, (
                    "Failed to find expected children of root entity"
                )
                ...
                if child.get_name() == f"{NESTED_ENTITIES_NAME_PREFIX}0":
                    prefab_test_utils.validate_linear_nested_entities(
                        child, NUM_NESTED_ENTITIES_LEVELS, CHILDREN_CREATION_POSITION
                    )
                # Validate the duplicated child nested prefab hierarchy
                    ...
                    assert (
                        first_child_prefab.get_name()
                        == f"{NESTED_PREFABS_NAME_PREFIX}1"
                    ), (
                    ...
                    assert (
                        second_child_prefab.get_name()
                        == f"{NESTED_PREFABS_NAME_PREFIX}2"
                    ), (
    ...
    parent_entity = EditorEntity.create_editor_entity_at(
        PARENT_CREATION_POSITION, "Parent"
    )
    ...
    nested_entities_root = prefab_test_utils.create_linear_nested_entities(
        NESTED_ENTITIES_NAME_PREFIX,
        NUM_NESTED_ENTITIES_LEVELS,
        CHILDREN_CREATION_POSITION,
        parent_id=parent_entity.id,
    )
    prefab_test_utils.validate_linear_nested_entities(
        nested_entities_root, NUM_NESTED_ENTITIES_LEVELS, CHILDREN_CREATION_POSITION
    )

    # Creates new nested prefabs at the root level
    # Asserts if creation didn't succeed
    nested_prefab_root = EditorEntity.create_editor_entity_at(
        CHILDREN_CREATION_POSITION,
        name=NESTED_PREFABS_TEST_ENTITY_NAME,
        parent_id=parent_entity.id,
    )
    ...
    assert nested_prefab_root.has_component(PHYSX_PRIMITIVE_COLLIDER_NAME), (
        f"Failed to add a {PHYSX_PRIMITIVE_COLLIDER_NAME}"
    )
    nested_prefabs, nested_prefab_instances = (
        prefab_test_utils.create_linear_nested_prefabs(
            [nested_prefab_root],
    ...
    duplicate_outcome = prefab.PrefabPublicRequestBus(
        bus.Broadcast, "DuplicateEntitiesInInstance", [parent_entity.id]
    )
    ...
    assert len(root_entities) == 1, (
        "Undo Failed: Found unexpected duplicated entity hierarchy root after Undo"
    )
    prefab_test_utils.validate_linear_nested_entities(
        nested_entities_root, NUM_NESTED_ENTITIES_LEVELS, CHILDREN_CREATION_POSITION
    )
