...
from ly_remote_console.remote_console_commands import (
    capture_screenshot_command as capture_screenshot_command,
)
from ly_remote_console.remote_console_commands import (
...
def get_next_screenshot_at_path(screenshot_path, prefix="screenshot", num_digits=4):
    ...
    raise AssertionError(
        "All possible screenshot names at directory {} are taken".format(
            screenshot_path
        )
    )
...
def take_screenshot(remote_console_instance, workspace, screenshot_name):
    ...
    screenshot_path = os.path.join(
        workspace.paths.platform_cache(), "user", "screenshots"
    )
    expected_screenshot_name = get_next_screenshot_at_path(screenshot_path)
    capture_screenshot_command(remote_console_instance)
    wait_for(
        lambda: os.path.exists(os.path.join(screenshot_path, expected_screenshot_name)),
        timeout=10,
        exc=AssertionError(
            "Screenshot at path:{} and with name:{} not found.".format(
                screenshot_path, expected_screenshot_name
            )
        ),
    )
    wait_for(
        ...
        timeout=10,
        exc=AssertionError(
            "Screenshot at path:{} and with name:{} is still in use.".format(
...
def compare_golden_image(
    ...
    mean_similarity = compare_screenshots(
        (os.path.join(screenshot_path, screenshot)),
        (os.path.join(golden_image_path, golden_image_name)),
    )
    assert mean_similarity > similarity_threshold, (
        "{} screenshot comparison failed! Mean similarity value is: {}".format(
            screenshot, mean_similarity
        )
    )
...
def download_qa_golden_images(project_name, destination_dir, platform):
    ...
    if not s3_utils.key_exists_in_bucket(bucket_name, path):
        raise s3_utils.KeyDoesNotExistError(
            "Key '{}' does not exist in S3 bucket {}".format(path, bucket_name)
        )
    for image in s3_utils.s3.Bucket(bucket_name).objects.filter(Prefix=path):
        file_name = string.replace(image.key, path, "")
        if file_name != "":
            s3_utils.download_from_bucket(
                bucket_name, image.key, destination_dir, file_name
            )
