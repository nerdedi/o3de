...
class TestBenchmarkAssetLoads(object):
    ...
    def setup_teardown(self, request, workspace, project):
        def teardown():
            ...
            if self.remove_benchmark_assets(
                self.get_temp_benchmark_asset_directory(workspace)
            ):
    ...
    def run_ap_batch(self, asset_processor):
        ...
        """
        process_assets_success = asset_processor.batch_process(
            timeout=600, fastscan=True
        )
        assert process_assets_success, "Assets did not process correctly"

    def get_temp_benchmark_asset_directory(self, workspace):
        return os.path.join(
            workspace.paths.engine_root(), workspace.project, "benchmark"
        )
        ...
        for benchmark in benchmarks:
            if benchmark.num_copies > 1:
                for asset_num in range(0, benchmark.num_copies):
                    shutil.copy(
                        os.path.join(
                            source_directory, f"{benchmark.base_name}.benchmarksettings"
                        ),
                        os.path.join(
                            dest_directory,
                            f"{benchmark.base_name}_{asset_num}.benchmarksettings",
                        ),
                    )
            else:
                shutil.copy(
    ...
    def run_benchmark(self, remote_console_instance, num_trials, benchmark):
        ...
        for trial in range(0, num_trials):
            ...
            self.set_cvar(
                remote_console_instance,
                "benchmarkLoadAssetLogLabel",
                f"{benchmark.base_name}_{trial}",
            )
            ...
            send_command_and_expect_response(
                ...
                f"({benchmark.base_name}_{trial}) - Benchmark asset list cleared.",
                timeout=remote_command_timeout,
            )
            ...
            if benchmark.num_copies > 1:
                ...
                for asset_num in range(0, benchmark.num_copies):
                    benchmark_command += (
                        f" benchmark/{benchmark.base_name}_{asset_num}{name_suffix}"
                    )
                    if (
                        asset_num % max_params_per_line == (max_params_per_line - 1)
                    ) or (asset_num == benchmark.num_copies - 1):
                        send_command_and_expect_response(
                            remote_console_instance,
                            benchmark_command,
                            f"({benchmark.base_name}_{trial}) - All requested assets added.",
                            timeout=remote_command_timeout,
                        )
                        benchmark_command = "benchmarkAddAssetsToList"
            else:
            ...
            send_command_and_expect_response(
                ...
                f"({benchmark.base_name}_{trial}) - Benchmark run complete.",
                timeout=remote_command_timeout,
            )
    ...
    def clear_windows_file_cache(self):
        ...
        luid = LUID()
        if result > 0:
            result = windll.advapi32.LookupPrivilegeValueW(
                None, "SeProfileSingleProcessPrivilege", byref(luid)
            )
            if result == 0:
        ...
        if result > 0:
            ...
            result = windll.advapi32.AdjustTokenPrivileges(
                token_handle, False, byref(tp), 0, None, None
            )
            if result > 0:
                ...
                if errCode == ERROR_NOT_ALL_ASSIGNED:
                    ...
                    logger.info(
                        "Windows file cache will not be cleared, insufficient privileges."
                    )
                    logger.info(
                        "If you wish to clear the cache when running this benchmark, run with Administrator privileges."
                    )
                    return False
                elif errCode > 0:
        ...
        if result > 0:
            ...
            status = c_int32(
                windll.ntdll.NtSetSystemInformation(
                    SystemMemoryListInformation, byref(command), sizeof(command)
                )
            )
        ...
        if result > 0:
            ...
            errCode = windll.kernel32.GetLastError()
            logger.error(
                f"Unexpected error while attempting to clear the Windows file cache: {errCode}"
            )

        windll.kernel32.CloseHandle(token_handle)
        return result > 0

    def base_test_BenchmarkAssetLoads(
        self, workspace, launcher, level, remote_console_instance, use_dev_mode
    ):
        benchmarks = [
        ...
        self.create_benchmark_assets(
            self.get_temp_benchmark_asset_directory(workspace), benchmarks
        )
        self.run_ap_batch(workspace.asset_processor)
        ...
        with launcher.start():
            waiter.wait_for(
                ...
                listener_timeout,
                exc=AssertionError(
                    "Port {} not listening.".format(remote_console_port)
                ),
            )
            ...
            self.set_cvar(
                remote_console_instance, "benchmarkLoadAssetTimeoutMs", "30000"
            )
            self.set_cvar(remote_console_instance, "benchmarkLoadAssetPollMs", "20")
            self.set_cvar(
                remote_console_instance, "benchmarkLoadAssetDisplayProgress", "false"
            )
            ...
            send_command_and_expect_response(
                remote_console_instance,
                "quit",
                "Executing console command 'quit'",
                timeout=remote_command_timeout,
            )
            launcher.stop()
        ...
        with open(results_csv, "w") as csv_file:
            csv_file.write(
                '"Test Name","Num Assets Loaded","First Time(ms)","Last Time(ms)","Max Time(ms)","Min Time (ms)",'
                + '"Avg Time (ms)","Median Time (ms)"\n'
            )
            for benchmark in benchmarks:
                csv_file.write(
                    f"{benchmark.base_name},{benchmark.num_copies},{benchmark.load_times[0]},{benchmark.load_times[-1]},"
    ...
    def test_BenchmarkAssetLoads_optMode(
        self, workspace, launcher, level, remote_console_instance
    ):
        self.base_test_BenchmarkAssetLoads(
            workspace, launcher, level, remote_console_instance, use_dev_mode=False
        )

    @pytest.mark.SUITE_benchmark
    def test_BenchmarkAssetLoads_devMode(
        self, workspace, launcher, level, remote_console_instance
    ):
        self.base_test_BenchmarkAssetLoads(
            workspace, launcher, level, remote_console_instance, use_dev_mode=True
        )
