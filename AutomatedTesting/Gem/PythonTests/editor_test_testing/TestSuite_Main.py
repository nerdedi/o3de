...
@mock.patch(
    "ly_test_tools._internal.pytest_plugin.terminal_report._add_ownership",
    mock.MagicMock(),
)
...
class TestEditorTest:
            ...
            for arg in TestEditorTest.args:
                if arg.startswith("--build-directory"):
                    ...
                    TestEditorTest.args[build_dir_arg_index] = (
                        f"{first}={os.path.abspath(second)}"
                    )
                    break
                build_dir_arg_index += 1
        if build_dir_arg_index == len(TestEditorTest.args):
            raise ValueError(
                f"Must pass --build-directory argument in order to run this test. Found args: {TestEditorTest.args}"
            )
    ...
    def _run_single_test(cls, pytester, workspace, module_name):
        ...
        if cls._asset_processor is None:
            if not process_utils.process_exists(
                "AssetProcessor", ignore_extensions=True
            ):
        ...
        extracted_results = EditorTestSuite._get_results_using_output(
            [get_class(module_name)], output, output
        )
    ...
    def test_single_pass_test(self, request, workspace, launcher_platform, pytester):
        (extracted_result, result) = TestEditorTest._run_single_test(
            pytester, workspace, "EditorTest_That_Passes"
        )
    ...
    def test_single_fail_test(self, request, workspace, launcher_platform, pytester):
        (extracted_result, result) = TestEditorTest._run_single_test(
            pytester, workspace, "EditorTest_That_Fails"
        )
    ...
    def test_single_crash_test(self, request, workspace, launcher_platform, pytester):
        (extracted_result, result) = TestEditorTest._run_single_test(
            pytester, workspace, "EditorTest_That_Crashes"
        )
        result.assert_outcomes(failed=1)
        ...
        assert isinstance(extracted_result, Result.Unknown) or isinstance(
            extracted_result, Result.Fail
        )

    @classmethod
    def _run_shared_test(
        cls, pytester, workspace, module_class_code, extra_cmd_line=None
    ):
    ...
    @pytest.mark.skip(reason="Skipped for test efficiency, but keeping for reference.")
    def test_batched_1_pass_1_fail(
        self, request, workspace, launcher_platform, pytester
    ):
        result = self._run_shared_test(
            pytester,
            workspace,
    ...
    @pytest.mark.skipif(
        sys.platform == "linux",
        reason="Skipping crash tests on Linux due to inconsistency between AR and local tests, but keeping for reference.",
    )
    def test_batched_1_pass_1_fail_1_crash(
        self, request, workspace, launcher_platform, pytester
    ):
        result = self._run_shared_test(
            pytester,
    ...
    def test_parallel_1_pass_1_fail_1_crash(
        self, request, workspace, launcher_platform, pytester
    ):
        result = self._run_shared_test(
            pytester,
    ...
    @pytest.mark.skip(reason="Skipped for test efficiency, but keeping for reference.")
    def test_parallel_batched_2_pass(
        self, request, workspace, launcher_platform, pytester
    ):
        result = self._run_shared_test(
            pytester,
    ...
    def test_parallel_batched_1_pass_1_fail_1_crash(
        self, request, workspace, launcher_platform, pytester
    ):
        result = self._run_shared_test(
            pytester,
    ...
    def test_selection_2_deselected_1_selected(
        self, request, workspace, launcher_platform, pytester
    ):
        result = self._run_shared_test(
            pytester,
