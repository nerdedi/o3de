...
def Multiplayer_AutoComponent_RPC():
    ...
    with Tracer() as section_tracer:
        ...
        Report.critical_result(
            TestSuccessFailTuples.find_network_player, player_id.IsValid()
        )
        ...
        helper.succeed_if_log_line_found(
            "EditorServer",
            "Script: AutoComponent_RPC: Sending client PlayerNumber 1",
            section_tracer.prints,
            PLAYERID_RPC_WAIT_TIME_SECONDS,
        )
        helper.succeed_if_log_line_found(
            "Script",
            "AutoComponent_RPC: I'm Player #1",
            section_tracer.prints,
            PLAYERID_RPC_WAIT_TIME_SECONDS,
        )

        # Authority->Client RPC
        PLAYFX_RPC_WAIT_TIME_SECONDS = (
            1.1  # The server will send an RPC to play an fx on the client every second.
        )
        helper.succeed_if_log_line_found(
            "EditorServer",
            "Script: AutoComponent_RPC_NetLevelEntity Activated on entity: NetLevelEntity",
            section_tracer.prints,
            PLAYFX_RPC_WAIT_TIME_SECONDS,
        )
        helper.succeed_if_log_line_found(
            "EditorServer",
            "Script: AutoComponent_RPC_NetLevelEntity: Authority sending RPC to play some fx.",
            section_tracer.prints,
            PLAYFX_RPC_WAIT_TIME_SECONDS,
        )
        helper.succeed_if_log_line_found(
        ...
        helper.succeed_if_log_line_found(
            "Script",
            "AutoComponent_RPC: Sending AutonomousToAuthorityNoParam RPC.",
            section_tracer.prints,
            AUTONOMOUS_TO_AUTHORITY_RPC_WAIT_TIME_SECONDS,
        )
        helper.succeed_if_log_line_found(
            "Script",
            "AutoComponent_RPC: Sending AutonomousToAuthority RPC (with float param).",
            section_tracer.prints,
            AUTONOMOUS_TO_AUTHORITY_RPC_WAIT_TIME_SECONDS,
        )
        helper.succeed_if_log_line_found(
            "EditorServer",
            "Script: AutoComponent_RPC: Successfully received AutonomousToAuthorityNoParams RPC.",
            section_tracer.prints,
            AUTONOMOUS_TO_AUTHORITY_RPC_WAIT_TIME_SECONDS,
        )
        helper.succeed_if_log_line_found(
            "EditorServer",
            "Script: AutoComponent_RPC: Successfully received AutonomousToAuthority RPC (with expected float param).",
            section_tracer.prints,
            AUTONOMOUS_TO_AUTHORITY_RPC_WAIT_TIME_SECONDS,
        )
        ...
        helper.succeed_if_log_line_found(
            "EditorServer",
            "Script: AutoComponent_RPC_NetLevelEntity: Send ServerToAuthority RPC.",
            section_tracer.prints,
            SERVER_TO_AUTHORITY_RPC_WAIT_TIME_SECONDS,
        )
        helper.succeed_if_log_line_found(
            "EditorServer",
            "Script: AutoComponent_RPC: Received ServerToAuthority RPC. Damage=42.",
            section_tracer.prints,
            SERVER_TO_AUTHORITY_RPC_WAIT_TIME_SECONDS,
        )
