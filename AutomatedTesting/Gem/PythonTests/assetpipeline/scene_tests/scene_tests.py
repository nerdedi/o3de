...
blackbox_scene_tests, blackbox_test_ids = test_builder.parametrize_blackbox_scene_test(
    test_dir_path
)
...
@pytest.fixture
def local_resources(
    request: any, workspace: AbstractWorkspaceManager, ap_setup_fixture: dict
) -> None:
...
class TestScene_AllPlatforms(object):
    ...
    @pytest.mark.parametrize(
        "blackbox_param", blackbox_scene_tests, ids=blackbox_test_ids
    )
    def test_SceneBlackboxTest_SourceFiles_Processed_ResultInExpectedProducts(
        self,
        workspace: AbstractWorkspaceManager,
        ap_setup_fixture: dict,
        asset_processor: object,
        project: any,
        blackbox_param: ParameterSet,
    ) -> None:
        """
    ...
    def populate_asset_info(
        workspace: AbstractWorkspaceManager, project: any, assets: iter
    ) -> None:
    ...
    def run_scene_test(
        ...
        expected_product_list = parse_and_compare.populate_expected_product_assets(
            assets_to_validate
        )
        ...
        db_path = os.path.join(
            asset_processor.temp_asset_root(), "Cache", "assetdb.sqlite"
        )
        ...
        actual_scene_debug_file = os.path.join(
            get_cache_folder(asset_processor), blackbox_params.scene_debug_file
        )
        ...
        if blackbox_params.scene_debug_file:
            expected_hashes_to_skip, actual_hashes_to_skip = (
                parse_and_compare.compare_scene_debug_file(
                    asset_processor, expected_scene_debug_file, actual_scene_debug_file
                )
            )
