...
class TestsPythonAssetProcessing_APBatch(object):
    ...
    def test_ProcessAssetWithoutScriptAfterAssetWithScript_ScriptOnlyRunsOnExpectedAsset(
        ...
        asset_processor.prepare_test_environment(
            ...
            "TwoSceneFiles_OneWithPythonOneWithout_PythonOnlyRunsOnFirstScene",
        )

        result, _ = asset_processor.batch_process(
            extra_params=self.asset_processor_extra_params
        )
        ...
        missing_assets, _ = utils.compare_assets_with_cache(
            expected_product_list, asset_processor.project_test_cache_folder()
        )
        ...
        assert not os.path.exists(unexpected_path), (
            f"Found unexpected output test asset {unexpected_path}"
        )
    ...
    def compute_udp_asset_debug_file(
        self, workspace, asset_processor, debug_filename, ap_setup_fixture
    ):
        asset_folder_name = "UserDefinedProperties"
        return self.compute_asset_debug_file(
            workspace,
            asset_processor,
            debug_filename,
            asset_folder_name,
    ...
    def compute_asset_debug_file(
        self, workspace, asset_processor, debug_filename, folder_name, ap_setup_fixture
    ):
        # computes the file name of the debug file for a UserDefinedProperties test file

        asset_processor.prepare_test_environment(
            ap_setup_fixture["tests_dir"], folder_name
        )
        result, _ = asset_processor.batch_process(
            extra_params=self.asset_processor_extra_params
        )
    ...
    def test_ProcessSceneWithMetadata_SupportedMayaDataTypes_Work(
        self, workspace, ap_setup_fixture, asset_processor
    ):
        ...
        dbgsg_file = self.compute_udp_asset_debug_file(
            workspace, asset_processor, asset_dbgsg, ap_setup_fixture
        )
        assert self.find_user_defined_property(dbgsg_file, "o3de_atom_lod: false"), (
            "Malformed o3de_atom_lod value"
        )
        assert self.find_user_defined_property(dbgsg_file, "o3de_atom_material: 0"), (
            "Malformed o3de_atom_material value"
        )
        assert self.find_user_defined_property(
            dbgsg_file, "o3de_default_lod: 0.000000"
        ), "Malformed o3de_default_lod value"
        assert self.find_user_defined_property(
            dbgsg_file,
            "o3de_default_material: gem/sponza/assets/objects/sponza_mat_bricks.azmaterial",
        ), "Malformed o3de_default_material value"

    def test_ProcessSceneWithMetadata_SupportedMaxDataTypes_Work(
        self, workspace, ap_setup_fixture, asset_processor
    ):
        ...
        assert self.find_user_defined_property(
            dbgsg_file, "o3de_phyx_lodY: 0.000000"
        ), "Malformed o3de_phyx_lodY value"
        assert self.find_user_defined_property(
            dbgsg_file, "o3de_default_lod: 0.000000"
        ), "Malformed o3de_default_lod value"
        assert self.find_user_defined_property(dbgsg_file, "o3de_atom_lod: false"), (
    ...
    def test_ProcessSceneWithMetadata_SupportedBlenderDataTypes_Work(
        self, workspace, ap_setup_fixture, asset_processor
    ):
        ...
        assert self.find_user_defined_property(dbgsg_file, "o3de_atom_lod: 0"), (
            "Malformed o3de_atom_lod value"
        )

    def test_ProcessSceneWithMetadata_DebugSceneManifest_Work(
        self, workspace, ap_setup_fixture, asset_processor
    ):
        # This test detects the debug .assetinfo file in the cahce folder

        assetinfo_dbg = "blender_with_attributes.fbx.assetinfo.dbg"
        assetinfo_dbg_file = self.compute_udp_asset_debug_file(
            workspace, asset_processor, assetinfo_dbg, ap_setup_fixture
        )
        assert assetinfo_dbg_file, "The debug assetinfo file is missing"

    def test_ProcessSceneWithCommonParent_DefaultMeshGroups_Work(
        self, workspace, ap_setup_fixture, asset_processor
    ):
        ...
        for assetinfo_dbg in assetinfo_dbg_list:
            ...
            assetinfo_dbg_file = self.compute_asset_debug_file(
                workspace,
                asset_processor,
                assetinfo_dbg,
                asset_folder_name,
            ...
            for group in groups:
                if "MeshGroup" in group.get("$type", str()) and group.get(
                    "name", str()
                ).startswith("default_"):
                    selection_lists = group.get("nodeSelectionList")
                    selected_node_list = selection_lists.get("selectedNodes", [])
                    assert len(selected_node_list) == 1, (
                        "Selected node list does not contain exactly one mesh"
                    )
                    ...
                    assert current_set == set(unselected_node_list), (
                        "Unselected node list does not contain the correct meshes"
                    )
                    found_mesh_nodes.add(selected_node_list[0])
            assert found_mesh_nodes == expected_mesh_nodes, (
                "Not all expected mesh groups found"
            )

    def test_ProcessAssetScript_SimpleFbx_HasExpectedProducts(
        self, workspace, ap_setup_fixture, asset_processor
    ):
        ...
        asset_processor.prepare_test_environment(
            ap_setup_fixture["tests_dir"], "script_basics"
        )
        ...
        expected_product_list = ["simple.fbx.fake_asset"]
