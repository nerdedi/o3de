...
from ..ap_fixtures.ap_config_backup_fixture import (
    ap_config_backup_fixture as ap_config_backup_fixture,
)
from ..ap_fixtures.ap_config_default_platform_fixture import (
    ap_config_default_platform_fixture as ap_config_default_platform_fixture,
)
...
class TestsFBX_AllPlatforms(object):
    ...
    def test_FBXBlackboxTest_SourceFiles_Processed_ResultInExpectedProducts(
        ...
        if blackbox_param is None:
            return
        self.run_fbx_test(
            workspace, ap_setup_fixture, asset_processor, project, blackbox_param
        )
    ...
    def test_FBXBlackboxTest_AssetInfoModified_AssetReprocessed_ResultInExpectedProducts(
        ...
        self.run_fbx_test(
            workspace, ap_setup_fixture, asset_processor, project, blackbox_param, True
        )
    ...
    def populate_asset_info(workspace, project, assets):
        ...
        for expected_source in assets:
            for job in expected_source.jobs:
                job.platform = ASSET_PROCESSOR_PLATFORM_MAP[
    ...
    @staticmethod
    def trim_floating_point_values_from_same_length_lists(
        ...
        assert initial_diff_actual_len == initial_diff_expected_len, (
            "Scene mismatch - different line counts"
        )
        ...
        diff_actual, diff_expected = utils.get_differences_between_lists(
        ...
        if (diff_actual is None and diff_expected is None) or (
            len(diff_actual) == 0 and len(diff_actual) == 0
        ):
            warnings.warn(
                f"Floating point drift detected between {expected_file_path} and {actual_file_path}."
            )
        ...
        assert len(diff_actual) == len(diff_expected), (
            "Scene mismatch - different line counts after truncation"
        )
        ...
        if (
            len(diff_actual) != initial_diff_actual_len
            or len(diff_expected) != initial_diff_expected_len
        ):
            warnings.warn(
                f"Floating point drift detected between {expected_file_path} and {actual_file_path}."
            )
    ...
    @staticmethod
    def scan_scene_debug_xml_file_for_issues(
        diff_actual: List[str],
        diff_expected: List[str],
        actual_hashes_to_skip: List[str],
    ...
    @staticmethod
    def scan_scene_debug_scene_graph_file_differences_for_issues(
        diff_actual: List[str],
        diff_expected: List[str],
        actual_file_path: str,
        expected_file_path: str,
        ...
        hashes_removed_diffs_identical = utils.compare_lists(
            diff_actual_hashes_removed, diff_expected_hashes_removed
        )
    ...
    def compare_scene_debug_file(
        ...
        debug_graph_path = os.path.join(
            get_cache_folder(asset_processor), actual_file_path
        )
        expected_debug_graph_path = os.path.join(
            asset_processor.project_test_source_folder(),
            "SceneDebug",
            expected_file_path,
        )
        ...
        if is_xml_dbgsg:
            return TestsFBX_AllPlatforms.scan_scene_debug_xml_file_for_issues(
                diff_actual,
                diff_expected,
            ...
            return TestsFBX_AllPlatforms.scan_scene_debug_scene_graph_file_differences_for_issues(
                diff_actual, diff_expected, actual_file_path, expected_file_path
            )
    ...
    def run_fbx_test(
        ...
        test_assets_folder = (
            blackbox_params.override_asset_folder
            if overrideAsset
            else blackbox_params.asset_folder
        )
        logger.info(
            f"{blackbox_params.test_name}: Processing assets in folder '"
        ...
        if not overrideAsset:
            asset_processor.prepare_test_environment(
                ap_setup_fixture["tests_dir"], test_assets_folder
            )
        else:
            asset_processor.prepare_test_environment(
        ...
        assetsToValidate = (
            blackbox_params.override_assets if overrideAsset else blackbox_params.assets
        )
        ...
        missing_assets, _ = utils.compare_assets_with_cache(
            expected_product_list, cache_folder
        )

        # If the test is going to fail, print information to help track down the cause of failure.
        if missing_assets:
            logger.info(
                f"The following assets were missing from cache: {pformat(missing_assets)}"
            )
            in_cache = os.listdir(cache_folder)
            logger.info(
                f"The cache {cache_folder} contains this content: {pformat(in_cache)}"
            )
        ...
        if blackbox_params.scene_debug_file:
            scene_debug_file = (
                blackbox_params.override_scene_debug_file
                if overrideAsset
    ...
    def test_FBX_ModifiedFBXFile_ConsistentProductOutput_OutputSucceeds(
        self, workspace, ap_setup_fixture, asset_processor
    ):
        """
        ...
        scene_debug_actual = os.path.join(
            asset_processor.temp_project_cache(
                asset_platform=ASSET_PROCESSOR_PLATFORM_MAP[
                    workspace.asset_processor_platform
                ]
            ),
        ...
        expected_hashes_to_skip, actual_hashes_to_skip = self.compare_scene_debug_file(
        ...
        filestoremove = [
            os.path.join(
                asset_processor.project_test_source_folder(), "ModifiedFBXFile.fbx"
            ),
    ...
    def test_FBX_MixedCaseFileExtension_OutputSucceeds(
        self, workspace, ap_setup_fixture, asset_processor
    ):
        """
        ...
        for extension, expected_debug_name in zip(extensionlist, expected_debug_list):
            asset_processor.prepare_test_environment(
                ap_setup_fixture["tests_dir"], "OneMeshOneMaterial"
            )
            rename_src = os.path.join(
                asset_processor.project_test_source_folder(), original_extension
            )
            rename_dst = os.path.join(
                asset_processor.project_test_source_folder(), extension
            )
            ...
            missing_assets, _ = utils.compare_assets_with_cache(
                expectedassets, get_cache_folder(asset_processor)
            )
