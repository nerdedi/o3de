...
def ap_missing_dependency_fixture(request, workspace, ap_setup_fixture) -> Any:
    class MissingDependencyHelper:
        def __init__(self):
            ...
            self.DB_NO_MISSING_DEP = (
                "No missing dependencies found",
                "{00000000-0000-0000-0000-000000000000}:0",
            )

            self.log_file = workspace.paths.ap_batch_log()
            self.ap_batch = os.path.join(
                workspace.paths.build_directory(), "AssetProcessorBatch"
            )
            self.asset_db = workspace.paths.asset_db()
            self.backup_dir = os.path.join(
                tempfile.gettempdir(), "MissingDependencyBackup"
            )

        def extract_missing_dependencies_from_output(
            self, log_output: List[str or List[str]]
            ...
            re_content = re.compile(
                r'Missing dependency: String "(.*)" matches asset: (.*)\Z'
            )

            result = {}  # The dictionary to return
            missing_dependencies = (
                None  # Temp list to hold current_products missing dependency tuples
            )
            current_product = None  # Current product in the log file

            for line in log_output:
                next_file = re_file_path.search(
                    line, re.I
                )  # Attempt pattern search for next 'product'
                if next_file:
                    ...
                    assert current_product, (
                        "REGEX 'current_product' parsing error-- Did the log format change?"
                    )
                else:
        ...
        def extract_missing_dependencies_from_log(
            self,
        ) -> Dict[str, List[Tuple[str, str]]]:
            """
            ...
            Returns a dictionary, where:
                ...
                each value: a list of tuples where:
                    ...
                    each tuple[1] - Missing dependency asset id
            """
            return self.extract_missing_dependencies_from_output(
                APLogParser(self.log_file).get_lines(run=-1)
            )

        def extract_missing_dependencies_from_database(
            self, product: str, check_platforms: List[str] = None
            ...
            Returns a dictionary, where:
                ...
                each value: a set of tuples where:
                    ...
                    each tuple[1] - Missing dependency asset id
            """
            platforms = check_platforms or db_utils.get_active_platforms_from_db(
                self.asset_db
            )
            logger.info(
                f"Searching for product {product} in active platforms: {platforms}"
            )
            product_names = [  # Get product name in database (<platform_cache>/<product>)
                f"{str(platform)}/{product}".replace("\\", "/")
            ...
            for product_name in product_names:
                ...
                missing_dependencies[product_name] = (
                    set()
                ...
                assert product_id, (
                    f"Expected to find exactly one DB match for {product_name}, "
                    f"instead found {len(product_id)}"
                )

                db_dependencies = db_utils.get_missing_dependencies(
                    self.asset_db, product_id
                )
                for db_dep in db_dependencies:
                    ...
                    uuid = hex(
                        int.from_bytes(
                            db_dep[self.UUID_INDEX], byteorder="big", signed=False
                        )
                    )
                    ...
                    if (
                        file_str != self.DB_NO_MISSING_DEP[0]
                        and asset_id != self.DB_NO_MISSING_DEP[1]
                    ):
                        logger.info(
                            f"Found missing dependency '{file_str}' with asset ID '{asset_id}'"
                        )
        ...
        def uuid_format(self, hex_string: str) -> str:
            ...
            hex_string = hex_string.split(":")[
                0
            ]  # Anything after ':' is the sub id; remove
        ...
        def validate_expected_dependencies(
            ...
            """
            logger.info(
                f"Searching output for expected dependencies for product {product}"
            )
            ...
            for (
                product_name,
                missing_deps,
            ) in self.extract_missing_dependencies_from_output(log_output).items():
            ...
            for (
                product_name,
                missing_deps,
            ) in self.extract_missing_dependencies_from_database(
                product, platforms
            ).items():
