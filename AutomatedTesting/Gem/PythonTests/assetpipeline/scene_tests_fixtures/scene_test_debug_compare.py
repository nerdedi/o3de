...
def populate_expected_product_assets(
    assets_to_validate: List[asset_db_utils.DBSourceAsset],
) -> List[str]:
    """
...
def compare_scene_debug_file(
    ...
    expected_debug_graph_path = os.path.join(
        asset_processor.project_test_source_folder(), "SceneDebug", expected_file_path
    )
    ...
    if is_xml_dbgsg:
        return scan_scene_debug_xml_file_for_issues(
            diff_actual, diff_expected, actual_hashes_to_skip, expected_hashes_to_skip
        )
    else:
        return scan_scene_debug_scene_graph_file_differences_for_issues(
            diff_actual, diff_expected, actual_file_path, expected_file_path
        )
