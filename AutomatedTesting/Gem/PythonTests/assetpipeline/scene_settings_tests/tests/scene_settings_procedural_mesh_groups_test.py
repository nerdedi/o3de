    ...
    async def run_test():
        ...
        path_to_manifest, widget_main_window, reflected_property_root = (
            scene_test_helpers.prepare_scene_ui_for_test(
                test_file_name="auto_test_fbx.fbx",
                manifest_should_exist=False,
        ...
        for repeat_num in range(TEST_REPEAT_COUNT):
            ...
            name_mesh_labels = widget_main_window.findChildren(
                QtWidgets.QFrame, "Name mesh"
            )
            for name_mesh_label in name_mesh_labels:
                ...
                if mesh_name == editable_mesh_group_name:
                    Report.critical_result(
                        tm.Test_Messages.scene_settings_base_mesh_group_enabled,
                        mesh_name_line_edit.isEnabled(),
                    )
                else:
                    # All other mesh groups should be read-only
                    Report.critical_result(
                        (
                            f"Good {repeat_num}: {mesh_name}",
                            f"{repeat_num}: Group {mesh_name} is enabled and should be disabled",
                        ),
                        not mesh_name_line_edit.isEnabled(),
                    )

            expected_diff, found_diff = utils.get_differences_between_lists(
                expected_mesh_names, found_mesh_names
            )

            Report.critical_result(
                tm.Test_Messages.scene_settings_expected_mesh_groups_found,
                len(expected_diff) == 0 and len(found_diff) == 0,
            )
            ...
            ui_headers = widget_main_window.findChildren(
                QtWidgets.QWidget, "AZ__SceneAPI__UI__HeaderWidget"
            )
            for ui_header in ui_headers:
                name_label = ui_header.findChild(QtWidgets.QLabel, "m_nameLabel")
                if name_label.text() == "Prefab group":
                    delete_button = ui_header.findChild(
                        QtWidgets.QToolButton, "m_deleteButton"
                    )
                    delete_button.click()
                    break
            ...
            Report.critical_result(
                tm.Test_Messages.scene_settings_expected_mesh_groups_removed,
                len(new_found_mesh_names) == 1,
            )

            Report.critical_result(
                tm.Test_Messages.scene_settings_expected_mesh_group_found,
                editable_mesh_group_name == new_found_mesh_names[0],
            )

            tab_bar.setCurrentIndex(PREFAB_TAB_INDEX)
            general.idle_wait_frames(PAUSE_TIME_IN_FRAMES)
            add_buttons = widget_main_window.findChildren(
                QtWidgets.QPushButton, "m_addButton"
            )
        ...
        clear_unsaved_changes_action = widget_main_window.findChild(
            QtWidgets.QAction, "m_actionClearUnsavedChanges"
        )
