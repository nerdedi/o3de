...
def prepare_scene_ui_for_test(
    test_file_name, manifest_should_exist, should_create_manifest
):
    ...
    project_root_folder = editor.EditorToolsApplicationRequestBus(
    ...
    Report.critical_result(
        tm.Test_Messages.scene_settings_test_asset_exists,
        os.path.exists(path_to_test_asset),
    )

    # Make sure the scene settings file doesn't yet exist
    manifest_expected_error_message = (
        tm.Test_Messages.scene_settings_scene_settings_not_created_yet
    )
    if manifest_should_exist:
        manifest_expected_error_message = (
            tm.Test_Messages.scene_settings_test_asset_manifest_exists
        )
    Report.critical_result(
        manifest_expected_error_message,
        os.path.exists(path_to_manifest) == manifest_should_exist,
    )
    window_id = azlmbr.qt.SceneSettingsAssetImporterForScriptRequestBus(
        azlmbr.bus.Broadcast, "EditImportSettings", path_to_test_asset
    )
    ...
    if should_create_manifest:
        has_unsaved_changes = azlmbr.qt.SceneSettingsRootDisplayScriptRequestBus(
            azlmbr.bus.Broadcast, "HasUnsavedChanges"
        )
        Report.critical_result(
            tm.Test_Messages.scene_settings_update_disabled_on_launch,
            not has_unsaved_changes,
        )

        card_layout_area = widget_main_window.findChild(
            QtWidgets.QWidget, "m_cardAreaLayoutWidget"
        )
        ...
        first_card_push_buttons = card_layout_area.findChildren(
            QtWidgets.QPushButton, ""
        )
        Report.critical_result(
            tm.Test_Messages.scene_settings_only_one_card_in_layout,
            len(first_card_push_buttons) == 1,
        )
        Report.critical_result(
            tm.Test_Messages.scene_settings_status_card_can_be_clicked,
            first_card_push_buttons[0].isEnabled(),
        )
        ...
        Report.critical_result(
            tm.Test_Messages.scene_setting_card_dismissed_on_click,
            len(first_card_push_buttons) == 0,
        )

    reflected_property_root = widget_main_window.findChild(
        QtWidgets.QWidget, "m_rootWidget"
    )
...
def save_and_verify_manifest(path_to_manifest, widget_main_window):
    ...
    while not saving_completed and time_out_frames_on_scene_settings_updated > 0:
        ...
        if len(first_card_push_buttons) > 0:
            ...
            if first_card_push_buttons[-1].isEnabled():
                ...
                break
        time_out_frames_on_scene_settings_updated = (
            time_out_frames_on_scene_settings_updated - frames_to_wait_between_checks
        )
    ...
    Report.critical_result(
        tm.Test_Messages.scene_settings_saved_successfully, saving_completed
    )
    ...
    Report.critical_result(
        tm.Test_Messages.scene_settings_file_update_disabled_after_save,
        not has_unsaved_changes,
    )

    # Make sure the manifest was actually created
    Report.critical_result(
        tm.Test_Messages.scene_settings_file_created, os.path.exists(path_to_manifest)
    )
