...
class TestsAssetProcessorBatch_PeriodicSerial_AllPlatforms(object):
    ...
    @pytest.mark.assetpipeline
    def test_reprocessFileList_processesInOrder(
        ...
        asset_processor.prepare_test_environment(
            ap_setup_fixture["tests_dir"], "test_reprocessFileList_processesInOrder"
        )
        ...
        result, output = asset_processor.batch_process(
            capture_output=True, fastscan=False, extra_params=extra_params
        )
        assert result, "AP Batch failed"
        num_processed_assets = asset_processor_utils.get_num_processed_assets(output)
            ...
            if expected_processed_assets_list is None:
                ...
                if match_result:
                    ...
                    for reprocess_file in reprocess_file_list:
                        ...
                        expected_processed_assets_list.append(
                            f"Builder with ID {builder_uuid} processed {reprocess_file_path_fix}"
                        )
            else:
        ...
        assert not expected_processed_assets_list, (
            f"Did not find expected processed output message(s) {expected_processed_assets_list}"
        )
