...
from ly_test_tools.o3de.asset_processor import (
    ...
    ASSET_PROCESSOR_SETTINGS_ROOT_KEY,
)
...
class TestsAssetProcessorGUI_WindowsAndMac(object):
    ...
    @pytest.mark.BAT
    @pytest.mark.assetpipeline
    def test_WindowsAndMacPlatforms_GUIFastScanNoSettingSet_FastScanSettingCreated(
        self, asset_processor, ap_fast_scan_setting_backup_fixture
    ):
        """
        ...
        assert not fast_scan_setting.entry_exists(), (
            "Registry Key's Value was not deleted"
        )
        ...
        assert key_value.lower() == "true", (
            f"The fast scan setting found was {key_value}"
        )
    ...
    @pytest.mark.BAT
    @pytest.mark.assetpipeline
    def test_WindowsMacPlatforms_GUIFastScanEnabled_GameLauncherWorksWithAP(
        self, asset_processor, workspace, ap_fast_scan_setting_backup_fixture
    ):
        """
        ...
        launcher = launcher_helper.create_game_launcher(
            workspace, args=["-NullRenderer"]
        )
        launcher.start()
...
class TestsAssetProcessorGUI_AllPlatforms(object):
    ...
    def test_AllSupportedPlatforms_AddScanFolder_AssetsProcessed(
        ...
        test_assets_folder, cache_folder = asset_processor.prepare_test_environment(
            env["tests_dir"], "C4874115", relative_asset_root="", add_scan_folder=False
        )
        ...
        assert os.path.exists(test_assets_folder), (
            f"Test assets folder was not found {test_assets_folder}"
        )
        ...
        assert not test_assets_added_to_cache(), (
            "Test assets are present in cache before adding scan folder"
        )
        ...
        test_scan_folder_root_key = (
            f"{ASSET_PROCESSOR_SETTINGS_ROOT_KEY}/ScanFolder C4874115"
        )
        test_scan_folder_params.append(
            f'--regset="{test_scan_folder_root_key}/watch=@ROOT@/C4874115"'
        )
        test_scan_folder_params.append(
            f'--regset="{test_scan_folder_root_key}/recursive=1"'
        )
        test_scan_folder_params.append(
            f'--regset="{test_scan_folder_root_key}/order=5000"'
        )
        ...
        assert test_assets_added_to_cache(), (
            "Test assets are missing from cache after adding scan folder"
        )

    @pytest.mark.test_case_id("C4874114")
    @pytest.mark.BAT
    @pytest.mark.assetpipeline
    def test_AllSupportedPlatforms_InvalidAddress_AssetsProcessed(
        self, workspace, request, asset_processor
    ):
        """
        ...
        extra_params = []
        extra_params.append(
            f'--regset="/Amazon/AzCore/Bootstrap/remote_ip={test_ip_address}"'
        )
    ...
    @pytest.mark.BAT
    @pytest.mark.assetpipeline
    def test_AllSupportedPlatforms_ModifyAssetInfo_AssetsReprocessed(
        ...
        assert utils.compare_lists(test_assets_list, expected_test_assets), (
            "Test assets are not as expected"
        )

        # Run AP Gui
        result, _ = asset_processor.gui_process(
            run_until_idle=True,
            extra_params='--regset="/O3DE/SceneAPI/AssetImporter/SkipAtomOutput=true"',
        )
        assert result, "AP GUI failed"
        ...
        timestamps = [
            os.stat(os.path.join(cache_folder, asset)).st_mtime
            for asset in cache_assets_list
        ]

        # Append new line to contents of *.assetinfo file in project folder
        asset_info_list = [
            item for item in test_assets_list if Path(item).suffix == ".assetinfo"
        ]
        assetinfo_paths_list = [
            os.path.join(test_assets_folder, item) for item in asset_info_list
        ]
