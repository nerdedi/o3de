...
def Collider_PxMeshConvexMeshCollides():
    ...
    # Constants
    MESH_ASSET_PATH = os.path.join(
        "assets",
        "Physics",
        "Collider_PxMeshConvexMeshCollides",
        "spherebot",
        "r0-b_body.fbx.pxmesh",
    )
    TIMEOUT = 2.0
    ...
    Report.result(
        Tests.physx_collider_added, collider.has_component(PHYSX_MESH_COLLIDER)
    )

    collider.add_component("PhysX Dynamic Rigid Body")
    Report.result(
        Tests.physx_rigid_body_added, collider.has_component("PhysX Dynamic Rigid Body")
    )
    ...
    collider_component.set_component_property_value(
        "Shape Configuration|Asset|PhysX Mesh", mesh_asset.id
    )
    mesh_asset.id = collider_component.get_component_property_value(
        "Shape Configuration|Asset|PhysX Mesh"
    )
    Report.result(
        Tests.assign_fbx_mesh,
        mesh_asset.get_path().lower() == MESH_ASSET_PATH.replace(os.sep, "/").lower(),
    )
    ...
    box_shape_component.set_component_property_value(
        "Box Shape|Box Configuration|Dimensions", math.Vector3(1024.0, 1024.0, 1.0)
    )
    # 6) Enter game mode
