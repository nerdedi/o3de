...
class TestUtils(TestAutomationBase):
    def test_UtilTest_Tracer_PicksErrorsAndWarnings(
        self, request, workspace, launcher_platform, editor
    ):
    ...
    def test_FileManagement_FindingFiles(self, workspace, launcher_platform):
        ...
        search_dir = os.path.join(
            workspace.paths.project(), "Levels", "Utils", "Managed_Files"
        )
        find_me_path = search_dir
        first_also_find_me_path = os.path.join(search_dir, "some_folder")
        second_also_find_me_path = os.path.join(
            search_dir, "some_folder", "another_folder", "one_more_folder"
        )
        ...
        try:
            ...
            fm._find_files(
                ["FindMe.txt", "AlsoFindMe.txt", "FindMeToo.txt"],
                search_dir,
                search_subdirs=True,
            )
        ...
        try:
            ...
            call_failed = True
        assert call_failed, (
            "_find_files() DID NOT fail when looking for a non-existent file"
        )

        # Expected successful usages
        found_me = fm._find_files(["FindMe.txt"], search_dir)
        also_found_me_1 = fm._find_files(
            ["AlsoFindMe.txt"], os.path.join(search_dir, "some_folder")
        )
        ...
        find_a_few = fm._find_files(
            ["FindMe.txt", "FindMeToo.txt"], search_dir, search_subdirs=True
        )

        # Test results
        assert found_me["FindMe.txt"] == find_me_path, (
            "FindMe.txt path was not as expected. Expected: {} Found: {} ".format(
                find_me_path, found_me["FindMe.txt"]
            )
        )
        assert also_found_me_1["AlsoFindMe.txt"] == first_also_find_me_path, (
        ...
        assert also_found_me_2["AlsoFindMe.txt"] == second_also_find_me_path, (
        ...
        for file_name, file_path in find_a_few.items():
            assert file_name == "FindMe.txt" or file_name == "FindMeToo.txt", (
                "{} was found on accident".format(file_name)
            )
        assert "FindMe.txt" in find_a_few and find_a_few["FindMe.txt"] is not None, (
            "Finding multiple files did not find 'FindMe.txt'"
        )
        assert find_a_few["FindMe.txt"] == find_me_path, (
            "FindMe.txt path was not as expected. Expected: {} Found: {} ".format(
                find_me_path, found_me["FindMe.txt"]
            )
        )
        ...
        assert find_a_few["FindMeToo.txt"] == find_me_too_path, (
    ...
    def test_FileManagement_FileBackup(self, workspace, launcher_platform):
        ...
        assert os.path.exists(target_file_path), (
            "Target file: {} does not exist".format(target_file_path)
        )
        ...
        assert target_file_path in file_map.keys(), (
            "File path {} was not added to FileManagement file_map"
        )
        backup_file_path = os.path.join(backup_folder, file_map[target_file_path])

        # Ensure back up worked as expected
        assert os.path.exists(target_file_path), (
            "Original file got destroyed: {}".format(target_file_path)
        )
        for i, line in enumerate(get_contents(target_file_path)):
            assert original_contents[i] == line, (
    ...
    def test_FileManagement_FileRestoration(self, workspace, launcher_platform):
        ...
        file_map = fm._load_file_map()
        assert target_file_path in file_map.keys(), (
            "File path {} was not added to FileManagement file_map".format(
                target_file_path
            )
        )
        backup_file_path = os.path.join(backup_folder, file_map[target_file_path])
        ...
        assert os.path.exists(backup_file_path), (
            "Back up file {} was not created".format(backup_file_path)
        )
        ...
        assert changed_lines[0] == text_to_write, (
            "The file {} did not get changed while writing in test".format(
                target_file_path
            )
        )
        assert changed_lines[0] != original_contents[0], (
            "The file {} did not get changed while writing in test".format(
                target_file_path
            )
        )
        ...
        assert not os.path.exists(backup_file_path), (
            "Backup file {} WAS NOT deleted on restoration".format(backup_file_path)
        )

        # Ensure saved file map was updated
        file_map = fm._load_file_map()
        assert target_file_path not in file_map.keys(), (
            "File path {} was not cleared from file map after restoration".format(
                target_file_path
            )
        )
        ...
        assert restored_lines[0] != text_to_write, (
            "File restore for {} did not work".format(target_file_path)
        )
        for restored, original, i in zip(
            restored_lines, original_contents, range(len(original_contents))
        ):
            assert restored == original, (
                "Restored file varies from original. Line [{}]: original: {}, restored: {}".format(
                    i, original, restored
                )
            )
