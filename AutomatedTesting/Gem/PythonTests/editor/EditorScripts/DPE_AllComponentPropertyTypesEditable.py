...
class Tests:
    dpe_enabled = ("DPE is enabled", "DPE is not enabled")
...
def DPE_AllComponentPropertyTypesEditable():
    ...
    def edit_component_property_value(component, property_name, value_to_set):
        Report.info(
            f"Starting edits on {component.get_component_name()}'s {property_name} property."
        )
    ...
    with Tracer() as error_tracer:
        ...
        Report.critical_result(
            Tests.dpe_enabled, general.get_cvar("ed_enableDPEInspector") == "true"
        )
        ...
        container_entity = EditorEntity.create_editor_entity(
            "ContainerComponentTestEntity"
        )
        ...
        edit_component_property_value(
            dither_component,
            "Previewer|Preview Settings|Preview Size",
            math.Vector3(2.0, 2.0, 2.0),
        )

        # Float Property Type
        edit_component_property_value(
            dither_component, "Configuration|Gradient|Opacity", 0.5
        )

        # Dropdown/Combo Box Property Type
        edit_component_property_value(
            light_component, "Controller|Configuration|Light type", 1
        )

        # Toggle Property Type
        edit_component_property_value(
            light_component, "Controller|Configuration|Shadows|Enable shadow", True
        )

        # Slider (Float) Property Type
        edit_component_property_value(
            light_component, "Controller|Configuration|Intensity", 250.0
        )
        ...
        edit_component_property_value(
            ...
            math.Color(0.5, 0.5, 0.5, 0.5),
        )

        # Asset Picker
        asset_to_pick_path = os.path.join(
            "assets", "objects", "foliage", "grass_flower_pink.fbx.azmodel"
        )
        asset_to_pick = asset.AssetCatalogRequestBus(
            bus.Broadcast, "GetAssetIdByPath", asset_to_pick_path, math.Uuid(), False
        )
        edit_component_property_value(
            mesh_component, "Controller|Configuration|Model Asset", asset_to_pick
        )
