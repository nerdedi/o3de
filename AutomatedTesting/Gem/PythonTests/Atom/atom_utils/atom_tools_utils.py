...
from Atom.atom_utils.atom_constants import (
    AtomToolsDocumentRequestBusEvents,
    AtomToolsDocumentSystemRequestBusEvents,
    AtomToolsMainWindowRequestBusEvents,
    EntityPreviewViewportSettingsRequestBusEvents,
)

MATERIAL_TYPES_PATH = pathlib.PurePath(
    azlmbr.paths.resolve_path("@gemroot:Atom_Feature_Common@/Assets/Materials/Types")
)
MATERIALCANVAS_GRAPH_PATH = pathlib.PurePath(
    azlmbr.paths.resolve_path("@gemroot:MaterialCanvas@/Assets/MaterialCanvas/TestData")
)
TEST_DATA_MATERIALS_PATH = pathlib.PurePath(
    azlmbr.paths.resolve_path("@gemroot:Atom_TestData@/Assets/TestData/Materials")
)
VIEWPORT_LIGHTING_PRESETS_PATH = pathlib.PurePath(
    azlmbr.paths.resolve_path(
        "@gemroot:MaterialEditor@/Assets/MaterialEditor/LightingPresets"
    )
)
VIEWPORT_MODELS_PRESETS_PATH = pathlib.PurePath(
    azlmbr.paths.resolve_path(
...
def is_close(
    actual: int or float or object,
    expected: int or float or object,
    buffer: float = sys.float_info.min,
) -> bool:
    """
...
def compare_colors(
    color1: azlmbr.math.Color, color2: azlmbr.math.Color, buffer: float = 0.00001
) -> bool:
    """
...
def verify_one_material_document_opened(
    material_document_ids_list: [azlmbr.math.Uuid], opened_document_index: int
) -> bool:
    """
...
def is_document_open(document_id: azlmbr.math.Uuid) -> bool:
    return azlmbr.atomtools.AtomToolsDocumentSystemRequestBus(
        bus.Broadcast,
        AtomToolsDocumentSystemRequestBusEvents.IS_DOCUMENT_OPEN,
        document_id,
    )
...
def save_document(document_id: azlmbr.math.Uuid) -> bool:
    return azlmbr.atomtools.AtomToolsDocumentSystemRequestBus(
        bus.Broadcast,
        AtomToolsDocumentSystemRequestBusEvents.SAVE_DOCUMENT,
        document_id,
    )
...
def save_document_as_copy(document_id: azlmbr.math.Uuid, target_path: str) -> bool:
    return azlmbr.atomtools.AtomToolsDocumentSystemRequestBus(
        bus.Broadcast,
        AtomToolsDocumentSystemRequestBusEvents.SAVE_DOCUMENT_AS_COPY,
        document_id,
        target_path,
    )
...
def save_document_as_child(document_id: azlmbr.math.Uuid, target_path: str) -> bool:
    return azlmbr.atomtools.AtomToolsDocumentSystemRequestBus(
        bus.Broadcast,
        AtomToolsDocumentSystemRequestBusEvents.SAVE_DOCUMENT_AS_CHILD,
        document_id,
        target_path,
    )
...
def close_document(document_id: azlmbr.math.Uuid):
    return azlmbr.atomtools.AtomToolsDocumentSystemRequestBus(
        bus.Broadcast,
        AtomToolsDocumentSystemRequestBusEvents.CLOSE_DOCUMENT,
        document_id,
    )
...
def close_all_except_selected(document_id: azlmbr.math.Uuid) -> bool:
    return azlmbr.atomtools.AtomToolsDocumentSystemRequestBus(
        bus.Broadcast,
        AtomToolsDocumentSystemRequestBusEvents.CLOSE_ALL_DOCUMENTS_EXCEPT,
        document_id,
    )
...
def is_pane_visible(pane_name: str) -> bool:
    return azlmbr.atomtools.AtomToolsMainWindowRequestBus(
        bus.Broadcast,
        AtomToolsMainWindowRequestBusEvents.IS_DOCK_WIDGET_VISIBLE,
        pane_name,
    )
...
def set_pane_visibility(pane_name: str, value: bool) -> None:
    azlmbr.atomtools.AtomToolsMainWindowRequestBus(
        bus.Broadcast,
        AtomToolsMainWindowRequestBusEvents.SET_DOCK_WIDGET_VISIBLE,
        pane_name,
        value,
    )
...
def load_lighting_preset_by_asset_id(asset_id: azlmbr.math.Uuid) -> bool:
    ...
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        ...
        EntityPreviewViewportSettingsRequestBusEvents.LOAD_LIGHTING_PRESET_BY_ASSET_ID,
        asset_id,
    )
...
def load_lighting_preset_by_path(asset_path: str) -> bool:
    ...
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.LOAD_LIGHTING_PRESET,
        asset_path,
    )
...
def get_last_lighting_preset_asset_id() -> azlmbr.math.Uuid:
    ...
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.GET_LAST_LIGHTING_PRESET_ASSET_ID,
    )
...
def get_last_model_preset_path() -> str:
    ...
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.GET_LAST_MODEL_PRESET_PATH_WITHOUT_ALIAS,
    )
...
def get_last_model_preset_asset_id() -> azlmbr.math.Uuid:
    ...
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.GET_LAST_MODEL_PRESET_ASSET_ID,
    )
...
def set_grid_enabled(value: bool) -> None:
    azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.SET_GRID_ENABLED,
        value,
    )
...
def get_grid_enabled() -> bool:
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.GET_GRID_ENABLED,
    )
...
def set_shadow_catcher_enabled(value: bool) -> None:
    azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.SET_SHADOW_CATCHER_ENABLED,
        value,
    )
...
def get_shadow_catcher_enabled() -> bool:
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.GET_SHADOW_CATCHER_ENABLED,
    )
...
def load_model_preset_by_asset_id(asset_id: azlmbr.math.Uuid) -> bool:
    ...
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.LOAD_MODEL_PRESET_BY_ASSET_ID,
        asset_id,
    )
...
def load_model_preset_by_path(asset_path: str) -> bool:
    ...
    return azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast,
        EntityPreviewViewportSettingsRequestBusEvents.LOAD_MODEL_PRESET,
        asset_path,
    )
...
def undo(document_id: azlmbr.math.Uuid) -> bool:
    return azlmbr.atomtools.AtomToolsDocumentRequestBus(
        bus.Event, AtomToolsDocumentRequestBusEvents.UNDO, document_id
    )


def redo(document_id: azlmbr.math.Uuid) -> bool:
    return azlmbr.atomtools.AtomToolsDocumentRequestBus(
        bus.Event, AtomToolsDocumentRequestBusEvents.REDO, document_id
    )
...
def disable_document_message_box_settings() -> None:
    ...
    :return: None
    """
    azlmbr.atomtools.util.SetSettingsValue_bool(
...
def disable_graph_compiler_settings() -> None:
    ...
    """
    azlmbr.atomtools.util.SetSettingsValue_bool(
        "/O3DE/AtomToolsFramework/GraphCompiler/CompileOnOpen", False
    )
    azlmbr.atomtools.util.SetSettingsValue_bool(
        "/O3DE/AtomToolsFramework/GraphCompiler/CompileOnSave", False
    )
    azlmbr.atomtools.util.SetSettingsValue_bool(
        "/O3DE/AtomToolsFramework/GraphCompiler/CompileOnEdit", False
    )


def wait_for_condition(
    function: (), timeout_in_seconds: float = 1.0
) -> bool or TypeError:
    """
...
class ShaderAssetTestHelper:
    ...
    def copy_tmp_files_in_order(
        src_directory, file_list, dst_directory, wait_time_in_between=0.0
    ):
        ...
        for filename in file_list:
            ...
            ShaderAssetTestHelper.copy_file(
                src_name, src_directory, filename, dst_directory
            )
    ...
    def asset_exists(cache_relative_path):
        asset_id = azasset.AssetCatalogRequestBus(
            bus.Broadcast, "GetAssetIdByPath", cache_relative_path, azmath.Uuid(), False
        )
...
class ScreenshotHelper(object):
    ...
    def capture_screenshot_blocking(self, filename: str) -> bool:
        ...
        outcome = azlmbr.atom.FrameCaptureRequestBus(
            azlmbr.bus.Broadcast, "CaptureScreenshot", filename
        )
...
def capture_screenshot(file_path: str) -> bool:
    return ScreenshotHelper(
        azlmbr.atomtools.general.idle_wait_frames
    ).capture_screenshot_blocking(file_path)
