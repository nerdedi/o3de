...
def create_screenshots_archive(screenshot_path):
    ...
    formatted_timestamp = datetime.datetime.utcfromtimestamp(timestamp).strftime(
        "%Y-%m-%d_%H-%M-%S"
    )
    screenshots_zip_file = os.path.join(
        screenshot_path, f"screenshots_{formatted_timestamp}.zip"
    )

    # Write all of the valid .png and .ppm files to the archive file.
    with zipfile.ZipFile(
        screenshots_zip_file, "w", compression=zipfile.ZIP_DEFLATED, allowZip64=True
    ) as zip_archive:
...
def compare_screenshot_to_golden_image(
    screenshot_directory, test_screenshot, golden_image
):
    """
    ...
    return compare_screenshots(
        f"{screenshot_directory}/{test_screenshot}",
        f"{golden_images_dir}/{golden_image}",
    )
...
def enter_exit_game_mode_take_screenshot(
    screenshot_name, enter_game_tuple, exit_game_tuple, timeout_in_seconds=4
):
    """
    ...
    TestHelper.wait_for_condition(
        function=lambda: general.is_in_game_mode(),
    ...
    Report.result(
        ("Screenshot taken", "Screenshot failed to be taken"), success_screenshot
    )
    TestHelper.exit_game_mode(exit_game_tuple)
    TestHelper.wait_for_condition(
        function=lambda: not general.is_in_game_mode(),
        timeout_in_seconds=timeout_in_seconds,
    )
...
def create_basic_atom_rendering_scene():
    ...
    ground_plane_mesh_asset_path = os.path.join(
        "testdata", "objects", "plane.fbx.azmodel"
    )
    ground_plane_mesh_asset = Asset.find_asset_by_path(
        ground_plane_mesh_asset_path, False
    )
