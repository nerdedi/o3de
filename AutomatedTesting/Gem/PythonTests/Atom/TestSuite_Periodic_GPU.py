...
expected_lines = ["screenshot compare passed. Diff score is"]
...
def run_test(
    ...
    game_launcher.start()
    waiter.wait_for(
        lambda: process_utils.process_exists(launcher_name, ignore_extensions=True)
    )

    game_launcher_log_file = os.path.join(
        game_launcher.workspace.paths.project_log(), "Game.log"
    )
    ...
    try:
        game_launcher_log_monitor.monitor_log_for_lines(
            expected_lines, unexpected_lines, halt_on_unexpected=False, timeout=400
        )
    finally:
...
class TestPeriodicSuite_DX12_GPU(object):
    @pytest.mark.parametrize(
        "test_name, screenshot_name, test_script, level_path, compare_tolerance, camera_name",
        atom_feature_test_list,
    )
...
@pytest.mark.REQUIRES_gpu
@pytest.mark.skipif(
    not WINDOWS and not LINUX,
    reason="Vulkan is only supported on windows, linux, & android",
)
@pytest.mark.parametrize("project", ["AutomatedTesting"])
