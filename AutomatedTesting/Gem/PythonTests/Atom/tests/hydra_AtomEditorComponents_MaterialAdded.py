...
class Tests:
    ...
    model_asset = ("Model Asset set", "P1: Model Asset not set")
    default_material = (
...
def AtomEditorComponents_Material_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        Report.result(
            ...
            material_entity.has_component(AtomComponentProperties.actor()),
        )
        ...
        Report.result(
            Tests.actor_undo,
            not material_entity.has_component(AtomComponentProperties.actor()),
        )
        ...
        Report.result(
            Tests.mesh_component,
            material_entity.has_component(AtomComponentProperties.mesh()),
        )
        ...
        # 13. Wait for Model Materials to indicate count 5 terminate early if container fails to reflect correct count
        Report.critical_result(
            Tests.model_material_count,
            TestHelper.wait_for_condition(
                lambda: (
                    material_component.get_container_count(
                        AtomComponentProperties.material("Model Materials")
                    )
                    == 5
                ),
        ...
        item = material_component.get_container_item(
            AtomComponentProperties.material("Model Materials"), key=0
        )
        ...
        Report.result(
            ...
            default_asset_path.startswith(
                "testdata/objects/modelhotreload/sphere_5lods_lambert0_"
            ),
        )

        # 15. Enable the use of LOD materials
        material_component.set_component_property_value(
            AtomComponentProperties.material("Enable LOD Materials"), True
        )
        Report.result(
            Tests.enable_lod_materials,
            material_component.get_component_property_value(
                AtomComponentProperties.material("Enable LOD Materials")
            )
            is True,
        )
        ...
        # 16. Wait for LOD Materials to indicate count 5 terminate early if container fails to reflect correct count
        Report.critical_result(
            Tests.lod_material_count,
            TestHelper.wait_for_condition(
                lambda: (
                    material_component.get_container_count(
                        AtomComponentProperties.material("LOD Materials")
                    )
                    == 5
                ),
                timeout_in_seconds=5,
        ...
        # 17. Get the slot zero list from LOD Material
        item = material_component.get_container_item(
            AtomComponentProperties.material("LOD Materials"), key=0
        )  # list of EditorMaterialComponentSlot
        active_asset = Asset(item[0].GetActiveAssetId())
        active_asset_path = active_asset.get_path()
        Report.result(
            Tests.lod_material_asset_path,
            active_asset_path.startswith(
                "testdata/objects/modelhotreload/sphere_5lods_lambert0_"
            ),
        )
        ...
        Report.result(
            Tests.lod_material_set, item[0].GetActiveAssetId() == metal_gold.id
        )
        ...
        Report.result(
            Tests.lod_material_set,
            render.MaterialComponentRequestBus(
                bus.Event, "GetMaterialAssetId", material_entity.id, assignment_id
            )
            == metal_gold.id,
        )
        ...
        # 21. Clear the assignment of a default material using MaterialComponentRequestBus and confirm null asset
        render.MaterialComponentRequestBus(
            bus.Event, "ClearMaterialAssetIdOnDefaultSlot", material_entity.id
        )
        default_material_asset_id = render.MaterialComponentRequestBus(
            bus.Event, "GetMaterialAssetIdOnDefaultSlot", material_entity.id
        )
        Report.result(
            Tests.clear_default_material,
            default_material_asset_id == azlmbr.asset.AssetId(),
        )
        ...
        Report.result(
            Tests.default_material, default_material_asset_id == metal_gold.id
        )
