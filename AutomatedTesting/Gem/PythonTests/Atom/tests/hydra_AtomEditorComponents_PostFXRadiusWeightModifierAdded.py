...
class Tests:
    ...
    radius = ("Radius property set successfully", "P1: Radius property failed to set")
...
def AtomEditorComponents_PostFXRadiusWeightModifier_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # Test steps begin.
        # 1. Create a Post FX Radius Weight Modifier entity with no components.
        postfx_radius_weight_entity = EditorEntity.create_editor_entity(
            AtomComponentProperties.postfx_radius()
        )
        Report.critical_result(
            Tests.postfx_radius_weight_creation, postfx_radius_weight_entity.exists()
        )

        # 2. Add Post FX Radius Weight Modifier component to Post FX Radius Weight Modifier entity.
        postfx_radius_component = postfx_radius_weight_entity.add_component(
            AtomComponentProperties.postfx_radius()
        )
        Report.critical_result(
            Tests.postfx_radius_weight_component,
            postfx_radius_weight_entity.has_component(
                AtomComponentProperties.postfx_radius()
            ),
        )
        ...
        Report.result(
            ...
            not postfx_radius_weight_entity.has_component(
                AtomComponentProperties.postfx_radius()
            ),
        )
        ...
        # 5. Verify PostFX Radius Weight Modifier component not enabled.
        Report.result(
            Tests.postfx_radius_weight_disabled,
            not postfx_radius_component.is_enabled(),
        )

        # 6. Add PostFX Layer component since it is required by the PostFX Radius Weight Modifier component.
        postfx_radius_weight_entity.add_component(
        ...
        # 7. Verify PostFX Radius Weight Modifier component is enabled.
        Report.result(
            Tests.postfx_radius_weight_enabled, postfx_radius_component.is_enabled()
        )

        # 8. Set Radius property
        postfx_radius_component.set_component_property_value(
            AtomComponentProperties.postfx_radius("Radius"), 100.0
        )
        Report.result(
            Tests.radius,
            postfx_radius_component.get_component_property_value(
                AtomComponentProperties.postfx_radius("Radius")
            )
            == 100.0,
        )
        ...
        general.idle_wait_frames(1)
        Report.result(
            Tests.is_visible, postfx_radius_weight_entity.is_visible() is True
        )
