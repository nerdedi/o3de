...
class Tests:
    ...
    deletion_undo = ("UNDO Entity deletion success", "P0: UNDO Entity deletion failed")
    deletion_redo = ("REDO Entity deletion success", "P0: REDO Entity deletion failed")
...
def AtomEditorComponents_CubeMapCapture_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # Test steps begin.
        # 1. Create a Cubemap_Capture entity with no components.
        cubemap_capture_entity = EditorEntity.create_editor_entity(
            AtomComponentProperties.cube_map_capture()
        )
        Report.critical_result(
            Tests.cubemap_capture_creation, cubemap_capture_entity.exists()
        )

        # 2. Add Cubemap_Capture component to Cubemap_Capture entity.
        cubemap_capture_component = cubemap_capture_entity.add_component(
            AtomComponentProperties.cube_map_capture()
        )
        Report.critical_result(
            Tests.cubemap_capture_component,
            cubemap_capture_entity.has_component(
                AtomComponentProperties.cube_map_capture()
            ),
        )
        ...
        Report.critical_result(
            Tests.cubemap_capture_component_removal,
            not cubemap_capture_entity.has_component(
                AtomComponentProperties.cube_map_capture()
            ),
        )
        ...
        for quality in SPECULAR_IBL_QUALITY:
            ...
            cubemap_capture_component.set_component_property_value(
                AtomComponentProperties.cube_map_capture(
                    "Specular IBL CubeMap Quality"
                ),
                SPECULAR_IBL_QUALITY[quality],
            )
            Report.result(
                test_quality,
                cubemap_capture_component.get_component_property_value(
                    AtomComponentProperties.cube_map_capture(
                        "Specular IBL CubeMap Quality"
                    )
                )
        ...
        cubemap_capture_component.set_component_property_value(
            AtomComponentProperties.cube_map_capture("Exposure"), 16.0
        )
        get_exposure_property = cubemap_capture_component.get_component_property_value(
            AtomComponentProperties.cube_map_capture("Exposure")
        )
        ...
        Report.result(
            ...
            == CUBEMAP_CAPTURE_TYPE["Diffuse ILB"],
        )
