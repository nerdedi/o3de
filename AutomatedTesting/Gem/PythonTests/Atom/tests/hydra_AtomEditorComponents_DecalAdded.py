...
def AtomEditorComponents_Decal_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # Test steps begin.
        # 1. Create a Decal entity with no components.
        decal_entity = EditorEntity.create_editor_entity(
            AtomComponentProperties.decal()
        )
        ...
        Report.critical_result(
            Tests.decal_component,
            decal_entity.has_component(AtomComponentProperties.decal()),
        )
        ...
        # 5. Set Material property on Decal component.
        decal_material_asset_path = os.path.join(
            "materials", "decal", "airship_symbol_decal.azmaterial"
        )
        decal_material_asset = Asset.find_asset_by_path(
            decal_material_asset_path, False
        )
        decal_component.set_component_property_value(
            AtomComponentProperties.decal("Material"), decal_material_asset.id
        )
        get_material_property = decal_component.get_component_property_value(
            AtomComponentProperties.decal("Material")
        )
        Report.result(
            Tests.material_property_set,
            get_material_property == decal_material_asset.id,
        )

        # 6. Set Attenuation Angle property on Decal component
        decal_component.set_component_property_value(
            AtomComponentProperties.decal("Attenuation Angle"), value=0.75
        )
        ...
        # 7. Set Opacity property on Decal component
        decal_component.set_component_property_value(
            AtomComponentProperties.decal("Opacity"), value=0.5
        )
        get_opacity_property = decal_component.get_component_property_value(
            AtomComponentProperties.decal("Opacity")
        )
        Report.result(Tests.opacity_property_set, get_opacity_property == 0.5)

        # 8. Set Normal Map Opacity property on Decal component
        decal_component.set_component_property_value(
            AtomComponentProperties.decal("Normal Map Opacity"), value=0.5
        )
        get_normal_map_opacity_property = decal_component.get_component_property_value(
            AtomComponentProperties.decal("Normal Map Opacity")
        )
        Report.result(
            Tests.normal_map_opacity_property_set,
            get_normal_map_opacity_property == 0.5,
        )

        # 9. Set Sort Key property on Decal component
        decal_component.set_component_property_value(
            AtomComponentProperties.decal("Sort Key"), value=255.0
        )
        get_sort_key_property = decal_component.get_component_property_value(
            AtomComponentProperties.decal("Sort Key")
        )
        Report.result(Tests.sort_key_property_set, get_sort_key_property == 255.0)
        decal_component.set_component_property_value(
            AtomComponentProperties.decal("Sort Key"), value=0
        )
        get_sort_key_property = decal_component.get_component_property_value(
            AtomComponentProperties.decal("Sort Key")
        )
        ...
        general.idle_wait_frames(1)
        Report.result(
            Tests.decal_component_removed,
            not decal_entity.has_component(AtomComponentProperties.decal()),
        )
        general.undo()
        general.idle_wait_frames(1)
        Report.result(
