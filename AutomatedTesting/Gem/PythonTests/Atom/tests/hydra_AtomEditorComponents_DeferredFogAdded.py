...
class Tests:
    ...
    edit_fog_mode = ("Fog mode updated", "P1: Failed to set the fog mode")
    fog_density_min = (
...
def AtomEditorComponents_DeferredFog_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # Test steps begin.
        # 1. Create an Deferred Fog entity with no components.
        deferred_fog_entity = EditorEntity.create_editor_entity(
            AtomComponentProperties.deferred_fog()
        )
        Report.critical_result(
            Tests.deferred_fog_creation, deferred_fog_entity.exists()
        )
        ...
        Report.critical_result(
            Tests.deferred_fog_component_removal,
            not deferred_fog_entity.has_component(
                AtomComponentProperties.deferred_fog()
            ),
        )
        ...
        # 5. Verify Deferred Fog component not enabled.
        Report.result(
            Tests.deferred_fog_disabled, not deferred_fog_component.is_enabled()
        )
        ...
        Report.result(
            Tests.enable_deferred_fog_parameter_enabled,
            deferred_fog_component.get_component_property_value(
                AtomComponentProperties.deferred_fog("Enable Deferred Fog")
            )
            is True,
        )
        ...
        Report.result(
            Tests.enable_deferred_fog_parameter_disabled,
            deferred_fog_component.get_component_property_value(
                AtomComponentProperties.deferred_fog("Enable Deferred Fog")
            )
            is False,
        ...
        for fog_mode in FOG_MODES.keys():
            ...
            Report.result(
                Tests.edit_fog_mode,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Fog Mode")
                )
                == FOG_MODES[fog_mode],
            )
            ...
            deferred_fog_component.set_component_property_value(
                AtomComponentProperties.deferred_fog("Enable Turbulence Properties"),
                True,
            )
            Report.result(
                Tests.enable_turbulence_properties_parameter_enabled,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Enable Turbulence Properties")
                )
                is True,
            ...
            Report.result(
                Tests.enable_turbulence_properties_parameter_disabled,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Enable Turbulence Properties")
                )
                is False,
            ...
            Report.result(
                Tests.enable_fog_layer_parameter_enabled,
                deferred_fog_component.get_component_property_value(
            ...
            Report.result(
                Tests.enable_fog_layer_parameter_disabled,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Enable Fog Layer")
                )
                is False,
            ...
            Report.result(
                Tests.edit_fog_color, fog_color_value.IsClose(violet_color_value)
            )
            ...
            Report.result(
                Tests.fog_start_min,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Fog Start Distance")
                )
                == 0.0,
            )
            ...
            if FOG_MODES[fog_mode] == FOG_MODES["Linear"]:
                ...
                Report.result(
                    Tests.fog_end_max,
                    deferred_fog_component.get_component_property_value(
                        AtomComponentProperties.deferred_fog("Fog End Distance")
                    )
                    == 5000.0,
                )
            ...
            Report.result(
                Tests.fog_bottom_height_max,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Fog Bottom Height")
                )
                == 5000.0,
            )
            ...
            Report.result(
                Tests.fog_max_height_min,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Fog Max Height")
                )
                == -5000.0,
            )
            ...
            Report.result(
                Tests.fog_density_clamp_max,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Fog Density Clamp")
                )
                == 1.0,
            )
            ...
            if FOG_MODES[fog_mode] in (
                FOG_MODES["Exponential"],
                FOG_MODES["ExponentialSquared"],
            ):
                ...
                Report.result(
                    Tests.fog_density_max,
                    deferred_fog_component.get_component_property_value(
                        AtomComponentProperties.deferred_fog("Fog Density")
                    )
                    == 1.0,
                )
            ...
            deferred_fog_component.set_component_property_value(
                AtomComponentProperties.deferred_fog(
                    "Noise Texture First Octave Scale"
                ),
                set_octave,
            )
            get_first_octave_scale = (
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog(
                        "Noise Texture First Octave Scale"
                    )
            ...
            Report.result(
                Tests.first_octave_scale, get_first_octave_scale.IsClose(set_octave)
            )

            # 18. Update the Noise Texture First Octave Velocity parameter to low/high values.
            deferred_fog_component.set_component_property_value(
                AtomComponentProperties.deferred_fog(
                    "Noise Texture First Octave Velocity"
                ),
                set_octave,
            )
            get_first_octave_velocity = (
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog(
                        "Noise Texture First Octave Velocity"
                    )
            ...
            Report.result(
                Tests.first_octave_velocity,
                get_first_octave_velocity.IsClose(set_octave),
            )

            # 19. Update the Noise Texture Second Octave Scale parameter to low/high values.
            deferred_fog_component.set_component_property_value(
                AtomComponentProperties.deferred_fog(
                    "Noise Texture Second Octave Scale"
                ),
                set_octave,
            )
            get_second_octave_scale = (
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog(
                        "Noise Texture Second Octave Scale"
                    )
            ...
            Report.result(
                Tests.second_octave_scale, get_second_octave_scale.IsClose(set_octave)
            )

            # 20. Update the Noise Texture Second Octave Velocity parameter to low/high values.
            deferred_fog_component.set_component_property_value(
                AtomComponentProperties.deferred_fog(
                    "Noise Texture Second Octave Velocity"
                ),
                set_octave,
            )
            get_second_octave_velocity = (
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog(
                        "Noise Texture Second Octave Velocity"
                    )
            ...
            Report.result(
                Tests.second_octave_velocity,
                get_second_octave_velocity.IsClose(set_octave),
            )
            ...
            Report.result(
                Tests.octaves_blend_factor_min,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Octaves Blend Factor")
                )
                == 0.0,
            )
            ...
            Report.result(
                Tests.octaves_blend_factor_max,
                deferred_fog_component.get_component_property_value(
                    AtomComponentProperties.deferred_fog("Octaves Blend Factor")
                )
                == 1.0,
