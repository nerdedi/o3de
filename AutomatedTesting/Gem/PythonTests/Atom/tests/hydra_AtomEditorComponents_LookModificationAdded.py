...
def AtomEditorComponents_LookModification_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # Test steps begin.
        # 1. Create an Look Modification entity with no components.
        look_modification_entity = EditorEntity.create_editor_entity(
            AtomComponentProperties.look_modification()
        )
        Report.critical_result(
            Tests.look_modification_creation, look_modification_entity.exists()
        )
        ...
        Report.critical_result(
            ...
            look_modification_entity.has_component(
                AtomComponentProperties.look_modification()
            ),
        )
        ...
        Report.result(
            Tests.look_modification_component_removal,
            not look_modification_entity.has_component(
                AtomComponentProperties.look_modification()
            ),
        )
        ...
        # 5. Verify Look Modification component not enabled.
        Report.result(
            Tests.look_modification_disabled,
            not look_modification_component.is_enabled(),
        )
        ...
        # 7. Verify Look Modification component is enabled.
        Report.result(
            Tests.look_modification_enabled, look_modification_component.is_enabled()
        )
        ...
        Report.result(
            ...
            look_modification_component.get_component_property_value(
                AtomComponentProperties.look_modification("Enable look modification")
            )
            is True,
        )
        ...
        color_grading_lut_path = os.path.join(
            "colorgrading",
            "testdata",
            "photoshop",
            "inv-Log2-48nits",
        ...
        color_grading_lut_asset = Asset.find_asset_by_path(
            color_grading_lut_path, False
        )
        look_modification_component.set_component_property_value(
            AtomComponentProperties.look_modification("Color Grading LUT"),
            color_grading_lut_asset.id,
        )
        Report.result(
        ...
        for shaper_type in SHAPER_TYPE.keys():
            # 10. Set the Shaper Type parameter (from atom_constants.py SHAPER_TYPE, default none)
            look_modification_component.set_component_property_value(
                AtomComponentProperties.look_modification("Shaper Type"),
        ...
        Report.result(
            Tests.lut_intensity_min_value,
            look_modification_component.get_component_property_value(
                AtomComponentProperties.look_modification("LUT Intensity")
            )
            == 0.0,
        )
        ...
        Report.result(
            Tests.lut_override_min_value,
            look_modification_component.get_component_property_value(
                AtomComponentProperties.look_modification("LUT Override")
            )
            == 0.0,
        )
