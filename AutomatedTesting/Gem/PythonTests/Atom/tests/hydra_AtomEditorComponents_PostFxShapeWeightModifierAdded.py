...
def AtomEditorComponents_postfx_shape_weight_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # Test steps begin.
        # 1. Create a PostFx Shape Weight Modifier entity with no components.
        postfx_shape_weight_entity = EditorEntity.create_editor_entity(
            AtomComponentProperties.postfx_shape()
        )
        Report.critical_result(
            Tests.postfx_shape_weight_creation, postfx_shape_weight_entity.exists()
        )

        # 2. Add a PostFx Shape Weight Modifier component to PostFx Shape Weight Modifier entity.
        postfx_shape_weight_component = postfx_shape_weight_entity.add_component(
            AtomComponentProperties.postfx_shape()
        )
        Report.critical_result(
            Tests.postfx_shape_weight_component,
            postfx_shape_weight_entity.has_component(
                AtomComponentProperties.postfx_shape()
            ),
        )
        ...
        Report.critical_result(
            ...
            not postfx_shape_weight_entity.has_component(
                AtomComponentProperties.postfx_shape()
            ),
        )
        ...
        # 5. Verify PostFx Shape Weight Modifier component not enabled.
        Report.result(
            Tests.postfx_shape_weight_disabled,
            not postfx_shape_weight_component.is_enabled(),
        )
        ...
        for shape in AtomComponentProperties.postfx_shape("shapes"):
            ...
            Report.result(
                Tests.postfx_shape_weight_enabled,
                postfx_shape_weight_component.is_enabled(),
            )

            # 9. Undo to remove each added shape and verify PostFX Shape Weight Modifier is not enabled.
            general.undo()
            TestHelper.wait_for_condition(
                lambda: not postfx_shape_weight_entity.has_component(shape), 1.0
            )
            Report.result(
                Tests.postfx_shape_weight_disabled,
                not postfx_shape_weight_component.is_enabled(),
        ...
        Report.result(
            Tests.tube_shape_component,
            postfx_shape_weight_entity.has_component("Tube Shape"),
        )
        Report.result(
            Tests.postfx_shape_weight_enabled,
            postfx_shape_weight_component.is_enabled(),
        ...
        Report.result(
            ...
            postfx_shape_weight_component.get_component_property_value(
                AtomComponentProperties.postfx_shape("Fall-off Distance")
            )
            == 100.0,
        )
