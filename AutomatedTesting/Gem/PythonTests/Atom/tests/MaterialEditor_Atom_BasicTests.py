...
class Tests:
    ...
    grid_disabled = ("Grid disabled successfully.", "P0: Grid failed to disable.")
    grid_enabled = ("Grid enabled successfully.", "P0: Grid failed to enable.")
...
def MaterialEditor_BasicFunctionalityChecks_AllChecksPass():
    ...
    with Tracer() as error_tracer:
        ...
        standard_pbr_material_type = os.path.join(
            atom_tools_utils.MATERIAL_TYPES_PATH, "StandardPBR.materialtype"
        )
        standard_pbr_test_cases = os.path.join(
            atom_tools_utils.TEST_DATA_MATERIALS_PATH, "StandardPbrTestCases"
        )
        test_material_1 = os.path.join(
            standard_pbr_test_cases, "001_DefaultWhite.material"
        )
        test_material_2 = os.path.join(
            standard_pbr_test_cases, "002_BaseColorLerp.material"
        )
        test_material_3 = os.path.join(
            standard_pbr_test_cases, "003_MetalMatte.material"
        )
        test_material_4 = os.path.join(standard_pbr_test_cases, "004_MetalMap.material")
        test_material_5 = os.path.join(
            standard_pbr_test_cases, "005_RoughnessMap.material"
        )
        test_material_6 = os.path.join(
            standard_pbr_test_cases, "006_SpecularF0Map.material"
        )
        test_materials = [
            test_material_1,
            test_material_2,
        ...
        # 1. Open an existing material document.
        material_document_id = atom_tools_utils.open_document(
            standard_pbr_material_type
        )
        Report.result(
        ...
        Report.result(
            ...
            atom_tools_utils.verify_one_material_document_opened(
                material_document_ids, 0
            )
            is True,
        )
        ...
        initial_color = material_editor_utils.get_property(
            document_id, base_color_property_name
        )
        ...
        material_editor_utils.set_property(
            document_id, base_color_property_name, expected_color
        )
        atom_tools_utils.end_edit(document_id)
        Report.result(
            Tests.changed_material_asset_color,
            material_editor_utils.get_property(document_id, base_color_property_name)
            == expected_color,
        )
        ...
        neutral_urban_background_loaded = atom_tools_utils.load_lighting_preset_by_path(
            neutral_urban_background_path
        )
        Report.result(
            Tests.viewport_background_selected, neutral_urban_background_loaded is True
        )

        # 11. Verify the lighting background asset is the expected value.
        Report.result(
            Tests.viewport_background_has_expected_asset,
            atom_tools_utils.get_last_lighting_preset_path().replace(os.sep, "/")
            == neutral_urban_background_path,
        )
        ...
        lythwood_room_asset_loaded = atom_tools_utils.load_lighting_preset_by_path(
            lythwood_room_background_path
        )
        Report.result(
            Tests.viewport_background_changed, lythwood_room_asset_loaded is True
        )
        ...
        beveled_cone_model_asset_path = os.path.join(
            atom_tools_utils.VIEWPORT_MODELS_PRESETS_PATH,
            "BeveledCone.modelpreset.azasset",
        ).replace(os.sep, "/")
        beveled_cone_model_asset = atom_tools_utils.load_model_preset_by_path(
            beveled_cone_model_asset_path
        )
        ...
        Report.result(
            ...
            atom_tools_utils.get_last_model_preset_path().replace(os.sep, "/")
            == beveled_cone_model_asset_path,
        )

        # 16. Change the model asset and verify the change succeeded.
        cone_model_asset_path = os.path.join(
            atom_tools_utils.VIEWPORT_MODELS_PRESETS_PATH, "Cone.modelpreset.azasset"
        ).replace(os.sep, "/")
        cone_model_asset = atom_tools_utils.load_model_preset_by_path(
            cone_model_asset_path
        )
        ...
        Report.result(
            Tests.shadow_catcher_disabled,
            atom_tools_utils.get_shadow_catcher_enabled() is False,
        )

        # 21. Enable the Shadow Catcher.
        atom_tools_utils.set_shadow_catcher_enabled(True)
        Report.result(
            Tests.shadow_catcher_enabled,
            atom_tools_utils.get_shadow_catcher_enabled() is True,
        )
