...
def AtomGPU_BasicLevelSetup_SetsUpLevel():
    ...
    with Tracer() as error_tracer:
        ...
        all_entities = azlmbr.entity.SearchBus(
            azlmbr.bus.Broadcast, "SearchEntities", search_filter
        )
        azlmbr.editor.ToolsApplicationRequestBus(
            azlmbr.bus.Broadcast, "DeleteEntities", all_entities
        )

        # Setup: Define the screenshot names and threshold pairs
        screenshot_thresholds = {"AtomBasicLevelSetup.png": 0.02}
        ...
        # 3. Create Grid Entity as a child entity of the Default Level Entity.
        grid_entity = EditorEntity.create_editor_entity(
            AtomComponentProperties.grid(), default_level_entity.id
        )
        ...
        grid_component.set_component_property_value(
            AtomComponentProperties.grid("Secondary Grid Spacing"),
            secondary_grid_spacing_value,
        )
        secondary_grid_spacing_set = (
        ...
        # 6. Add HDRi Skybox component to the Global Skylight (IBL) Entity.
        hdri_skybox_component = global_skylight_entity.add_component(
            AtomComponentProperties.hdri_skybox()
        )
        Report.result(
            Tests.hdri_skybox_component_added,
            global_skylight_entity.has_component(AtomComponentProperties.hdri_skybox()),
        )

        # 7. Add Global Skylight (IBL) component to the Global Skylight (IBL) Entity.
        global_skylight_component = global_skylight_entity.add_component(
            AtomComponentProperties.global_skylight()
        )
        Report.result(
            Tests.global_skylight_component_added,
            global_skylight_entity.has_component(
                AtomComponentProperties.global_skylight()
            ),
        )
        ...
        global_skylight_image_asset = Asset.find_asset_by_path(
            global_skylight_image_asset_path, False
        )
        hdri_skybox_component.set_component_property_value(
            AtomComponentProperties.hdri_skybox("Cubemap Texture"),
            global_skylight_image_asset.id,
        )
        Report.result(
        ...
        global_skylight_diffuse_image_asset = Asset.find_asset_by_path(
            global_skylight_diffuse_image_asset_path, False
        )
        global_skylight_component.set_component_property_value(
            AtomComponentProperties.global_skylight("Diffuse Image"),
            global_skylight_diffuse_image_asset.id,
        )
        Report.result(
        ...
        global_skylight_component.set_component_property_value(
            ...
            global_skylight_specular_image_asset.id,
        )
        global_skylight_specular_image_set = (
            global_skylight_component.get_component_property_value(
                AtomComponentProperties.global_skylight("Specular Image")
        ...
        ground_plane_entity = EditorEntity.create_editor_entity(
            ground_plane_name, default_level_entity.id
        )
        ground_plane_material_component = ground_plane_entity.add_component(
        ...
        ground_plane_material_asset = Asset.find_asset_by_path(
            ground_plane_material_asset_path, False
        )
        ground_plane_material_component.set_component_property_value(
            AtomComponentProperties.material("Material Asset"),
            ground_plane_material_asset.id,
        )
        Report.result(
        ...
        # 13. Add the Mesh component to the Ground Plane Entity and set the Mesh component Model Asset property.
        ground_plane_mesh_component = ground_plane_entity.add_component(
            AtomComponentProperties.mesh()
        )
        Report.result(
            Tests.mesh_component_added,
            ground_plane_entity.has_component(AtomComponentProperties.mesh()),
        )
        ground_plane_model_asset_path = os.path.join(
            "testdata", "objects", "plane.fbx.azmodel"
        )
        ground_plane_model_asset = Asset.find_asset_by_path(
            ground_plane_model_asset_path, False
        )
        ...
        Report.result(
            ...
            == ground_plane_model_asset.id,
        )

        # 14. Create a Directional Light Entity as a child entity of the Default Level Entity.
        directional_light_entity = EditorEntity.create_editor_entity_at(
            math.Vector3(0.0, 0.0, 10.0),
            AtomComponentProperties.directional_light(),
            default_level_entity.id,
        )

        # 15. Add Directional Light component to Directional Light Entity and set entity rotation.
        directional_light_entity.add_component(
            AtomComponentProperties.directional_light()
        )
        directional_light_entity_rotation = math.Vector3(
            DEGREE_RADIAN_FACTOR * -90.0, 0.0, 0.0
        )
        directional_light_entity.set_local_rotation(directional_light_entity_rotation)
        Report.result(
            Tests.directional_light_component_added,
            directional_light_entity.has_component(
                AtomComponentProperties.directional_light()
            ),
        )
        ...
        sphere_material_component = sphere_entity.add_component(
            AtomComponentProperties.material()
        )
        Report.result(
            Tests.sphere_material_component_added,
            sphere_entity.has_component(AtomComponentProperties.material()),
        )

        # 17. Set the Material Asset property of the Material component for the Sphere Entity.
        sphere_material_asset_path = os.path.join(
            "materials", "presets", "pbr", "metal_brass_polished.azmaterial"
        )
        sphere_material_asset = Asset.find_asset_by_path(
            sphere_material_asset_path, False
        )
        ...
        Report.result(
            Tests.sphere_material_set,
            sphere_material_component.get_component_property_value(
                AtomComponentProperties.material("Material Asset")
            )
            == sphere_material_asset.id,
        )

        # 18. Add Mesh component to Sphere Entity and set the Model Asset property for the Mesh component.
        sphere_mesh_component = sphere_entity.add_component(
            AtomComponentProperties.mesh()
        )
        ...
        Report.result(
            Tests.sphere_model_asset_set,
            sphere_mesh_component.get_component_property_value(
                AtomComponentProperties.mesh("Model Asset")
            )
            == sphere_model_asset.id,
        )

        # 19. Create a Camera Entity as a child entity of the Default Level Entity then add a Camera component.
        camera_entity = EditorEntity.create_editor_entity_at(
            math.Vector3(5.5, -12.0, 9.0),
            AtomComponentProperties.camera(),
            default_level_entity.id,
        )
        camera_component = camera_entity.add_component(AtomComponentProperties.camera())
        Report.result(
            Tests.camera_component_added,
            camera_entity.has_component(AtomComponentProperties.camera()),
        )

        # 20. Set the Camera Entity rotation value and set the Camera component Field of View value.
        camera_entity_rotation = math.Vector3(
            DEGREE_RADIAN_FACTOR * -27.0,
            DEGREE_RADIAN_FACTOR * -12.0,
            DEGREE_RADIAN_FACTOR * 25.0,
        )
        camera_entity.set_local_rotation(camera_entity_rotation)
        camera_fov_value = 60.0
        camera_component.set_component_property_value(
            AtomComponentProperties.camera("Field of view"), camera_fov_value
        )
        azlmbr.camera.EditorCameraViewRequestBus(
            azlmbr.bus.Event, "ToggleCameraAsActiveView", camera_entity.id
        )
        Report.result(
            Tests.camera_fov_set,
            camera_component.get_component_property_value(
                AtomComponentProperties.camera("Field of view")
            )
            == camera_fov_value,
        )
        ...
        # 21. Enter/Exit game mode taking screenshot.
        enter_exit_game_mode_take_screenshot(
            "AtomBasicLevelSetup.png", Tests.enter_game_mode, Tests.exit_game_mode
        )

        # 22. Compare the screenshots to golden images.
        for screenshot_name, screenshot_threshold in screenshot_thresholds.items():
            image_diff_outcome = compare_screenshot_to_golden_image(
                FOLDER_PATH, screenshot_name, screenshot_name
            )
            ...
            Report.result(screenshot_compare_execution, image_diff_outcome.IsSuccess())
