...
def MaterialCanvas_BasicFunctionalityChecks_AllChecksPass():
    ...
    with Tracer() as error_tracer:
        ...
        test_1_material_graph_file = os.path.join(
            atom_tools_utils.MATERIALCANVAS_GRAPH_PATH, "test1.materialgraph"
        )
        test_2_material_graph_file = os.path.join(
            atom_tools_utils.MATERIALCANVAS_GRAPH_PATH, "test2.materialgraph"
        )
        test_3_material_graph_file = os.path.join(
            atom_tools_utils.MATERIALCANVAS_GRAPH_PATH, "test3.materialgraph"
        )
        test_4_material_graph_file = os.path.join(
            atom_tools_utils.MATERIALCANVAS_GRAPH_PATH, "test4.materialgraph"
        )
        test_5_material_graph_file = os.path.join(
            atom_tools_utils.MATERIALCANVAS_GRAPH_PATH, "test5.materialgraph"
        )

        # 1. Open an existing material graph document.
        test_1_material_graph = Graph(
            atom_tools_utils.open_document(test_1_material_graph_file)
        )
        Report.result(
            Tests.open_existing_material_graph,
            atom_tools_utils.is_document_open(test_1_material_graph.document_id)
            is True,
        )
        ...
        material_graph_document_files = [
            test_1_material_graph_file,
            test_2_material_graph_file,
            test_3_material_graph_file,
            test_4_material_graph_file,
            test_5_material_graph_file,
        ]
        ...
        for material_graph_document_file in material_graph_document_files:
            test_material_graph = Graph(
                atom_tools_utils.open_document(material_graph_document_file)
            )
            Report.result(
                Tests.verify_all_material_graphs_are_opened,
                atom_tools_utils.is_document_open(test_material_graph.document_id)
                is True,
            )
        ...
        Report.result(
            ...
            and atom_tools_utils.is_document_open(test_material_graphs[1].document_id)
            is False
            and atom_tools_utils.is_document_open(test_material_graphs[2].document_id)
            is False
            and atom_tools_utils.is_document_open(test_material_graphs[3].document_id)
            is False
            and atom_tools_utils.is_document_open(test_material_graphs[4].document_id)
            is False,
        )
        ...
        world_position_node = test_1_material_graph.add_node(
            created_world_position_node, math.Vector2(-200.0, 10.0)
        )
        Report.result(
        ...
        standard_pbr_node = test_1_material_graph.add_node(
            created_standard_pbr_node, math.Vector2(10.0, 220.0)
        )
        Report.result(
        ...
        test_1_material_graph.add_connection_by_slot_id(
            world_position_node,
            world_position_node_slots["outPosition"],
            standard_pbr_node,
            standard_pbr_node_slots["inPositionOffset"],
        )
