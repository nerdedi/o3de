...
class Tests:
    ...
    gamma = ("Gamma property set", "P1: Gamma property failed to set expected value")
    display_mapper_type = (
...
def AtomEditorComponents_DisplayMapper_AddedToEntity():
    ...
    with Tracer() as error_tracer:
        ...
        # Test steps begin.
        # 1. Create a Display Mapper entity with no components.
        display_mapper_entity = EditorEntity.create_editor_entity(
            AtomComponentProperties.display_mapper()
        )
        Report.critical_result(
            Tests.display_mapper_creation, display_mapper_entity.exists()
        )

        # 2. Add Display Mapper component to Display Mapper entity.
        display_mapper_component = display_mapper_entity.add_component(
            AtomComponentProperties.display_mapper()
        )
        Report.critical_result(
            Tests.display_mapper_component,
            display_mapper_entity.has_component(
                AtomComponentProperties.display_mapper()
            ),
        )
        ...
        display_mapper_asset = Asset.find_asset_by_path(
            display_mapper_asset_path, False
        )
        display_mapper_component.set_component_property_value(
            AtomComponentProperties.display_mapper("LDR color Grading LUT"),
            display_mapper_asset.id,
        )
        Report.result(
        ...
        for operation_type in DISPLAY_MAPPER_OPERATION_TYPE.keys():
            # 6. Set the Display Mapper Operation Type enumerated in DISPLAY_MAPPER_OPERATION_TYPE for each type
            display_mapper_component.set_component_property_value(
                AtomComponentProperties.display_mapper("Type"),
                DISPLAY_MAPPER_OPERATION_TYPE[operation_type],
            )
            general.idle_wait_frames(3)
            set_type = render.DisplayMapperComponentRequestBus(
                bus.Broadcast, "GetDisplayMapperOperationType"
            )
            Report.info(f"DiplayMapperOperationType: {set_type}")
            Report.result(
                Tests.display_mapper_type,
                set_type == DISPLAY_MAPPER_OPERATION_TYPE[operation_type],
            )

            # 7. Set Enable LDR color grading LUT property True
            display_mapper_component.set_component_property_value(
                AtomComponentProperties.display_mapper("Enable LDR color grading LUT"),
                True,
            )
            Report.result(
            ...
            Report.result(
                ...
                display_mapper_component.get_component_property_value(
                    AtomComponentProperties.display_mapper("Override Defaults")
                )
                is False,
            )
            ...
            Report.result(
                ...
                display_mapper_component.get_component_property_value(
                    AtomComponentProperties.display_mapper("Alter Surround")
                )
                is True,
            )
            ...
            Report.result(
                ...
                display_mapper_component.get_component_property_value(
                    AtomComponentProperties.display_mapper("Alter Desaturation")
                )
                is True,
            )
            ...
            Report.result(
                Tests.alter_cat,
                display_mapper_component.get_component_property_value(
                    AtomComponentProperties.display_mapper("Alter CAT D60 to D65")
                )
                is True,
            )
            ...
            # set high value
            display_mapper_component.set_component_property_value(
                AtomComponentProperties.display_mapper("Mid Point (luminance)"),
                value=1005.0,
            )
            Report.result(
                Tests.luminance_level_mid,
                Math_IsClose(
                    display_mapper_component.get_component_property_value(
                        AtomComponentProperties.display_mapper("Mid Point (luminance)")
                    ),
            ...
            # set a high value
            display_mapper_component.set_component_property_value(
                AtomComponentProperties.display_mapper("Max Point (luminance)"),
                value=4000.0,
            )
            Report.result(
                Tests.luminance_level_max,
                Math_IsClose(
                    display_mapper_component.get_component_property_value(
                        AtomComponentProperties.display_mapper("Max Point (luminance)")
                    ),
            ...
            Report.result(
                Tests.surround_gamma,
                Math_IsClose(
                    display_mapper_component.get_component_property_value(
                        AtomComponentProperties.display_mapper("Surround Gamma")
                    ),
                    1.2,
                    TOLERANCE,
                ),
            ...
            Report.result(
                ...
                Math_IsClose(
                    display_mapper_component.get_component_property_value(
                        AtomComponentProperties.display_mapper("Gamma")
                    ),
                    4.0,
                    TOLERANCE,
                ),
        ...
        for preset in DISPLAY_MAPPER_PRESET.keys():
            ...
            test_preset = (
                f"Preset {preset} loaded expected value",
            ...
            Report.result(
                ...
                Math_IsClose(
                    ...
                    cinema_limit_white_presets[DISPLAY_MAPPER_PRESET[preset]],
                    TOLERANCE,
                ),
            )
