...
class NodeCategory_ExpandOnClick:
    ...
    def left_click_expander_button(self, node_palette_node_tree, category_index):
        ...
        QtTest.QTest.mouseClick(
            node_palette_node_tree.viewport(),
    ...
    def wait_and_verify_category_expanded_state(
        self,
        test_case,
        node_palette_node_tree,
        node_palette_math_category,
        expanded=True,
    ):
        category_has_expanded = helper.wait_for_condition(
            lambda: node_palette_node_tree.isExpanded(node_palette_math_category),
            WAIT_TIME_3,
        )
    ...
    async def run_test(self):
        ...
        helper.wait_for_condition(
            lambda: general.is_pane_visible(SCRIPT_CANVAS_UI) is None, WAIT_TIME_3
        )

        # 1) Open Script Canvas pane
        general.open_pane(SCRIPT_CANVAS_UI)
        Report.critical_result(
            Tests.pane_open, general.is_pane_visible(SCRIPT_CANVAS_UI)
        )
        ...
        helper.wait_for_condition(
            lambda: self.sc_editor.findChild(QtWidgets.QDockWidget, NODE_PALETTE_QT)
            is not None,
            WAIT_TIME_3,
        )
        node_palette_node_tree = scripting_tools.get_node_palette_node_tree_qt_object(
            self
        )
        node_palette_math_category = (
            scripting_tools.get_node_palette_category_qt_object(
                self, NODE_CATEGORY_MATH
            )
        )
        ...
        # 4) Left-Click on a node category arrow to expand it and verify it's expanded
        self.left_click_expander_button(
            node_palette_node_tree, node_palette_math_category
        )
        self.wait_and_verify_category_expanded_state(
            Tests.click_expand, node_palette_node_tree, node_palette_math_category
        )
        ...
        # 6) Double-Click on a node category to expand it then verify it's expanded
        self.double_click_node_category(
            node_palette_node_tree, node_palette_math_category
        )
        self.wait_and_verify_category_expanded_state(
            Tests.dClick_expand, node_palette_node_tree, node_palette_math_category
        )
