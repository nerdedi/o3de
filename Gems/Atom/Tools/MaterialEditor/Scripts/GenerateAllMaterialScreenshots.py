...
g_materialTestFolder = os.path.join(
    g_engroot,
    "Gems",
    "Atom",
    "TestData",
...
def OpenMaterial(filename):
    documentId = azlmbr.atomtools.AtomToolsDocumentSystemRequestBus(
        ...
        "OpenDocument",
        os.path.join(g_materialTestFolder, filename),
    )
    return documentId


def CloseMaterial(documentId):
    azlmbr.atomtools.AtomToolsDocumentSystemRequestBus(
        azlmbr.bus.Broadcast, "CloseDocument", documentId
    )


def LoadLightingPreset(path):
    assetId = azlmbr.asset.AssetCatalogRequestBus(
        azlmbr.bus.Broadcast, "GetAssetIdByPath", path, azlmbr.math.Uuid(), False
    )
    azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast, "LoadLightingPresetByAssetId", assetId
    )
...
def LoadModelPreset(path):
    ...
    azlmbr.atomtools.EntityPreviewViewportSettingsRequestBus(
        azlmbr.bus.Broadcast, "LoadModelPresetByAssetId", assetId
    )


def SetCameraDistance(distance):
    azlmbr.render.ArcBallControllerRequestBus(
        azlmbr.bus.Broadcast, "SetDistance", distance
    )


def SetCameraHeading(heading):
    azlmbr.render.ArcBallControllerRequestBus(
        azlmbr.bus.Broadcast, "SetHeading", heading
    )
...
def CaptureScreenshot(screenshotOutputPath):
    ...
    ).capture_screenshot_blocking(screenshotOutputPath)
...
def ResizeViewport(width, height):
    ...
    azlmbr.atomtools.AtomToolsMainWindowRequestBus(
        azlmbr.bus.Broadcast, "LockViewportRenderTargetSize", width, height
    )
    ...
    azlmbr.atomtools.AtomToolsMainWindowRequestBus(
        azlmbr.bus.Broadcast, "ResizeViewportRenderTarget", width, height
    )


def ReleaseViewportResolutionLock():
    azlmbr.atomtools.AtomToolsMainWindowRequestBus(
        azlmbr.bus.Broadcast, "UnlockViewportRenderTargetSize"
    )


def GenerateMaterialScreenshot(
    materialName,
    uniqueSuffix="",
    cameraHeading=-30.0,
    cameraPitch=20.0,
    ...
    print(
        "GenerateMaterialScreenshot('{}', '{}')...".format(materialName, uniqueSuffix)
    )
    ...
    IdleFrames(
        g_defaultDelayFrames
    )  # The UI needs to time to process the changed scene data
    ...
    expectedScreenshotPath = os.path.join(
        screenshotsFolder, uniqueFileName + ".expected.ppm"
    )
    actualScreenshotPath = os.path.join(
        screenshotsFolder, uniqueFileName + ".actual.ppm"
    )
...
def GenerateAllMaterialScreenshots():
    ...
    IdleFrames(
        g_defaultDelayFrames
    )  # Allows the UI to refresh before continuing; otherwise the viewport will appear stretched while the user waits a second for the screen capture.
    ...
    GenerateMaterialScreenshot(
        "009_Opacity_Blended",
        lighting="materialeditor/lightingpresets/neutral_urban.lightingpreset.azasset",
        model="materialeditor/viewportmodels/beveledcube.modelpreset.azasset",
    )
    GenerateMaterialScreenshot(
        "009_Opacity_Cutout_PackedAlpha_DoubleSided",
        lighting="materialeditor/lightingpresets/neutral_urban.lightingpreset.azasset",
        model="materialeditor/viewportmodels/beveledcube.modelpreset.azasset",
    )
    GenerateMaterialScreenshot(
        "009_Opacity_Cutout_SplitAlpha_DoubleSided",
        lighting="materialeditor/lightingpresets/neutral_urban.lightingpreset.azasset",
        model="materialeditor/viewportmodels/beveledcube.modelpreset.azasset",
    )
    GenerateMaterialScreenshot(
        "009_Opacity_Cutout_SplitAlpha_SingleSided",
        lighting="materialeditor/lightingpresets/neutral_urban.lightingpreset.azasset",
        model="materialeditor/viewportmodels/beveledcube.modelpreset.azasset",
    )
    ...
    GenerateMaterialScreenshot(
        "012_Parallax_POM",
        model="materialeditor/viewportmodels/cube.modelpreset.azasset",
        cameraHeading=-35.0,
        cameraPitch=35.0,
    )
    GenerateMaterialScreenshot(
        "013_SpecularAA_Off", lighting="testdata/test.lightingpreset.azasset"
    )
    GenerateMaterialScreenshot(
        "013_SpecularAA_On", lighting="testdata/test.lightingpreset.azasset"
    )
    ...
    GenerateMaterialScreenshot(
        "100_UvTiling_Normal_Dome_Rotate20",
        model="materialeditor/viewportmodels/cube.modelpreset.azasset",
        lighting="testdata/test.lightingpreset.azasset",
        cameraHeading=225.0,
    )
    GenerateMaterialScreenshot(
        "100_UvTiling_Normal_Dome_Rotate90",
        model="materialeditor/viewportmodels/cube.modelpreset.azasset",
        lighting="testdata/test.lightingpreset.azasset",
        cameraHeading=225.0,
    )
    GenerateMaterialScreenshot(
        "100_UvTiling_Normal_Dome_ScaleOnlyU",
        model="materialeditor/viewportmodels/cube.modelpreset.azasset",
        lighting="testdata/test.lightingpreset.azasset",
        cameraHeading=225.0,
    )
    GenerateMaterialScreenshot(
        "100_UvTiling_Normal_Dome_ScaleOnlyV",
        model="materialeditor/viewportmodels/cube.modelpreset.azasset",
        lighting="testdata/test.lightingpreset.azasset",
        cameraHeading=225.0,
    )
    GenerateMaterialScreenshot(
        "100_UvTiling_Normal_Dome_ScaleUniform",
        model="materialeditor/viewportmodels/cube.modelpreset.azasset",
        lighting="testdata/test.lightingpreset.azasset",
        cameraHeading=225.0,
    )
    GenerateMaterialScreenshot(
        "100_UvTiling_Normal_Dome_TransformAll",
        model="materialeditor/viewportmodels/cube.modelpreset.azasset",
        lighting="testdata/test.lightingpreset.azasset",
        cameraHeading=225.0,
    )
    GenerateMaterialScreenshot(
        "100_UvTiling_Opacity",
        lighting="materialeditor/lightingpresets/neutral_urban.lightingpreset.azasset",
    )
    GenerateMaterialScreenshot(
        "100_UvTiling_Parallax_A",
        uniqueSuffix="Angle1",
        model="materialeditor/viewportmodels/cube.modelpreset.azasset",
        cameraHeading=35.0,
        cameraPitch=35.0,
