...
class Material_Component_Converter(object):
    ...
    def create_material_asset_string_from_assetid(self, assetId):
        ...
        return "".join(
            (
                "id=",
                assetId,
                ",type={522C7BE0-501D-463E-92C6-15184A2B7AD8},hint={",
                hint_path,
                "},loadBehavior=1",
            )
        )
    ...
    def create_material_assignment_id(self, slotAssetId, isMapAssignment):
        ...
        #           </Class>
        #       </Class>
        materialAssignmentId = create_xml_element_from_string(
            "".join(
                (
                    '<Class name="AZ::Render::MaterialAssignmentId" field="',
                    field,
                    '" version="1" type="{EB603581-4654-4C17-B6DE-AE61E79EDA97}">',
                )
            )
        )
        ...
        materialAssignmentId_AssetId_Uuid = create_xml_element_from_string(
            "".join(
                (
                    '<Class name="AZ::Uuid" field="guid" value="',
                    uuid,
                    '" type="{E152C105-A133-4D03-BBF8-3D4B2FBA3E2A}"/>',
                )
            )
        )
        subId = get_subid_from_assetId(slotAssetId)
        materialAssignmentId_AssetId_subid = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "unsigned int",
                "field": "subId",
    ...
    def create_editor_material_assignment_slot(
        self, slotAssetId, materialAssetId, isDefaultSlot
    ):
        field = ""
        if isDefaultSlot:
        ...
        #       <Class name="AZStd::unordered_map" field="propertyOverrides" type="{6E6962E1-04C9-56F9-89C4-361031CC1384}"/>
        #   </Class>
        materialComponentSlot = create_xml_element_from_string(
            "".join(
                (
                    '<Class name="EditorMaterialComponentSlot" field="',
                    field,
                    '" version="4" type="{344066EB-7C3D-4E92-B53D-3C9EBD546488}">',
                )
            )
        )
        isMapAssignment = False
        materialAssignmentId = self.create_material_assignment_id(
            slotAssetId, isMapAssignment
        )
    ...
    def create_material_asset(self, materialAssetId):
        materialAssetString = self.create_material_asset_string_from_assetid(
            materialAssetId
        )
        materialAsset = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "Asset",
                "field": "materialAsset",
                "value": materialAssetString,
                "version": "2",
                "type": "{77A19D40-8731-4D3C-9041-1B43047366A4}",
            },
        )
    ...
    def create_material_component_with_material_assignments(
        self, atomMaterialInDefaultSlotAssetId, materialAssignmentList
    ):
        ...
        editorMaterialComponent = create_xml_element_from_string(
            '<Class name="EditorMaterialComponent" field="element" version="5" type="{02B60E9D-470B-447D-A6EE-7D635B154183}">'
        )
        # can't use create_xml_element_from_string here because of the spaces in the class name
        editorRenderComponentAdapter = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "EditorRenderComponentAdapter<MaterialComponentController MaterialComponent MaterialComponentConfig >",
                "field": "BaseClass1",
                "type": "{DF046B40-536D-5D59-96EF-7A40DA6191B2}",
            },
        )
        editorComponentAdapter = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "EditorComponentAdapter<MaterialComponentController MaterialComponent MaterialComponentConfig >",
                "field": "BaseClass1",
                "version": "1",
                "type": "{6C4D4557-3728-56F8-A0FF-A309C3AAB853}",
            },
        )
        ...
        for atomMaterial in materialAssignmentList:
            if (
                atomMaterial.assignmentAssetId
                and atomMaterial.assignmentAssetId != defaultSlot
            ):
                materialMapElement = self.create_material_map_entry(
                    atomMaterial.slotAssetId, atomMaterial.assignmentAssetId
                )
        ...
        isDefaultSlot = True
        defaultMaterialComponentSlot = self.create_editor_material_assignment_slot(
            defaultSlot, atomMaterialInDefaultSlotAssetId, isDefaultSlot
        )

        #   <Class name="AZStd::vector" field="materialSlots" type="{7FDDDE36-46C8-5DBC-8566-E792AA358BD9}">
        #        ... (slots)
        isDefaultSlot = False
        materialsSlots = create_xml_element_from_string(
            'Class name="AZStd::vector" field="materialSlots" type="{7FDDDE36-46C8-5DBC-8566-E792AA358BD9}"'
        )
        for atomMaterial in materialAssignmentList:
            if atomMaterial.assignmentAssetId:
                materialSlotElement = self.create_editor_material_assignment_slot(
                    atomMaterial.slotAssetId,
                    atomMaterial.assignmentAssetId,
                    isDefaultSlot,
                )
                ...
                materialSlotElement = self.create_editor_material_assignment_slot(
                    atomMaterial.slotAssetId,
                    self.get_default_material_assetid(),
                    isDefaultSlot,
                )
                materialsSlots.append(materialSlotElement)
    ...
    def convert_legacy_mtl_relative_path_to_atom_material_assetid(
        self,
        normalizedProjectDir,
        oldMaterialRelativePath,
        oldFbxRelativePathWithoutExtension,
    ):
        if len(oldMaterialRelativePath) == 0:
            ...
            cacheMaterialRelativePath = "".join(
                (oldFbxRelativePathWithoutExtension, ".azmaterial")
            )
            if (
                cacheMaterialRelativePath
                in self.assetCatalogHelper.relativePathToAssetIdDict
            ):
                assetId = self.assetCatalogHelper.relativePathToAssetIdDict[
                    cacheMaterialRelativePath
                ]
                return assetId
            else:
        ...
        assetId = self.assetCatalogHelper.get_asset_id_from_relative_path(
            atomRelativePath
        )
        if assetId:
            return assetId
    ...
    def convert_legacy_mtl_relative_path_to_atom_material_list(
        self,
        normalizedProjectDir,
        oldMaterialRelativePath,
        oldFbxRelativePathWithoutExtension,
        ...
        if cacheFbxPath in self.assetCatalogHelper.relativePathToAssetIdDict:
            ...
            for productAssetId in fbxProductList:
                relativePath = self.assetCatalogHelper.assetIdToRelativePathDict[
                    productAssetId
                ]
                if relativePath.endswith(".azmaterial"):
                    ...
                    convertedRelativePath = "".join(
                        (relativePath[:extraCharactersIndex], ".azmaterial")
                    )
                    ...
                    if (
                        ...
                        assignment = self.assetCatalogHelper.relativePathToAssetIdDict[
                            convertedRelativePath
                        ]
                    elif (
                        cacheFbxPath.replace(".azmodel", ".azmaterial")
                        in self.assetCatalogHelper.relativePathToAssetIdDict
                    ):
                        ...
                        print(
                            "Could not match {0} to a corresponding source atom material".format(
                                convertedRelativePath
                            )
                        )
