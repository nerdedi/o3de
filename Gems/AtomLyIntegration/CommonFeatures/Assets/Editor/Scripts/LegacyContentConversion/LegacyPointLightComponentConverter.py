...
class Point_Light_Component_Converter(Component_Converter):
    ...
    def convert_legacy_light_intensity_to_atom_intensity(
        self, diffuse_multiplier: str
    ) -> str:
    ...
    def is_this_the_component_im_looking_for(self, xmlElement, parent):
        if (
            ...
            and xmlElement.get("name") == "EditorPointLightComponent"
        ):
            return True
        return False
    ...
    def gather_info_for_conversion(self, xmlElement, _):
        ...
        for editorPointLightComponentChild in xmlElement:
            ...
            for editorLightComponentChild in editorPointLightComponentChild:
                ...
                if (
                    "name" in editorLightComponentChild.keys()
                    and editorLightComponentChild.get("name")
                    == "EditorLightConfiguration"
                ):
                    ...
                    for editorLightConfigurationChild in editorLightComponentChild:
                        for lightConfigurationChild in editorLightConfigurationChild:
                            ...
                            if (
                                "field" in lightConfigurationChild.keys()
                                and lightConfigurationChild.get("field") == "Color"
                            ):
                                self.color = lightConfigurationChild.get("value")
                            elif (
                                "field" in lightConfigurationChild.keys()
                                and lightConfigurationChild.get("field")
                                == "DiffuseMultiplier"
                            ):
                                self.diffuse_multiplier = lightConfigurationChild.get(
                                    "value"
                                )
                            elif (
                                "field" in lightConfigurationChild.keys()
                                ...
                                self.point_max_distance = lightConfigurationChild.get(
                                    "value"
                                )
    ...
    def __create_adapter_xml(self, color, intensity, attenuationRadius):
        ...
        editorRenderComponentAdapter = xml.etree.ElementTree.Element(
            "Class",
            {
                ...
                "field": "BaseClass1",
                "type": "{B09B7A31-789F-5996-AD50-EF71942A5271}",
            },
        )
        editorComponentAdapter = xml.etree.ElementTree.Element(
            "Class",
            {
                ...
                "field": "BaseClass1",
                "version": "1",
                "type": "{DC9066D5-4557-52C7-B901-4B5626C4F35A}",
            },
        )
        ...
        pointLightComponentConfig = xml.etree.ElementTree.Element(
            "Class",
            {
        ...
        componentConfig = xml.etree.ElementTree.Element(
            "Class",
            {
        ...
        colorConfig = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "Color",
                "field": "Color",
        ...
        colorIntesntiyModeConfig = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "char",
        ...
        inensityConfig = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "float",
                "field": "Intensity",
        ...
        attenuationRadiusModeConfig = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "unsigned char",
        ...
        attenuationRadiusConfig = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "float",
        ...
        bulbRadiusConfig = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "float",
                "field": "BulbRadius",
    ...
    def convert(self, xmlElement, _):
        ...
        xmlElement.append(
            self.__create_adapter_xml(
                self.color,
                self.convert_legacy_light_intensity_to_atom_intensity(
                    self.diffuse_multiplier
                ),
                self.point_max_distance,
            )
