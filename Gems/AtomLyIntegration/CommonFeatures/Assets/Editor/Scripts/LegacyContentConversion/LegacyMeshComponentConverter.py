...
class Mesh_Component_Converter(Component_Converter):
    ...
    def is_this_the_component_im_looking_for(self, xmlElement, parent):
        if (
            "name" in xmlElement.keys()
            and xmlElement.get("name") == "EditorMeshComponent"
        ):
            return True
        return False
    ...
    def gather_info_for_conversion(self, xmlElement, parent):
        ...
        for editorMeshComponentChild in xmlElement:
            if (
                "name" in editorMeshComponentChild.keys()
                and editorMeshComponentChild.get("name") == "MeshComponentRenderNode"
            ):
                for meshComponentRenderNodeChild in editorMeshComponentChild:
                    if (
                        "name" in meshComponentRenderNodeChild.keys()
                        and meshComponentRenderNodeChild.get("name") == "Asset"
                    ):
                        ...
                        self.oldFbxRelativePathWithoutExtension = assetId[
                            meshPathStartIndex:meshPathEndIndex
                        ]

                        # swap the sub-id for the atom model sub-id
                        atomModelRelativePath = "{0}.fbx.azmodel".format(
                            self.oldFbxRelativePathWithoutExtension
                        )
                        if (
                            atomModelRelativePath
                            in self.assetCatalogHelper.relativePathToAssetIdDict
                        ):
                            atomAssetId = (
                                self.assetCatalogHelper.relativePathToAssetIdDict[
                                    atomModelRelativePath
                                ]
                            )
                            ...
                            self.newAssetId = "".join(
                                (
                                    assetId[:subIdStartIndex],
                                    subIdReplacement,
                                    assetId[subIdEndIndex:typeStartIndex],
                                    typeReplacement,
                                    assetId[typeEndIndex:extensionStartIndex],
                                    extensionReplacement,
                                    assetId[extensionEndIndex:],
                                )
                            )
                        else:
                            ...
                            print(
                                "Could not find {0} in the asset catalog. Make sure the corresponding source file ends in .fbx not .cgf, and that the asset has finished processing with no errors.".format(
                                    atomModelRelativePath
                                )
                            )
                    elif (
                        "field" in meshComponentRenderNodeChild.keys()
                        and meshComponentRenderNodeChild.get("field")
                        == "Material Override"
                    ):
                        for simpleAssetReferenceChild in meshComponentRenderNodeChild:
                            if (
                                "name" in simpleAssetReferenceChild.keys()
                                and simpleAssetReferenceChild.get("name")
                                == "SimpleAssetReferenceBase"
                            ):
                                for (
                                    simpleAssetReferenceBaseChild
                                ) in simpleAssetReferenceChild:
                                    if (
                                        "field" in simpleAssetReferenceBaseChild.keys()
                                        and simpleAssetReferenceBaseChild.get("field")
                                        == "AssetPath"
                                        and "value"
                                        in simpleAssetReferenceBaseChild.keys()
                                    ):
                                        # We've found the material override
                                        self.oldMaterialRelativePath = (
                                            simpleAssetReferenceBaseChild.get("value")
                                        )
    ...
    def create_adapter_with_new_model_assetId(self, assetIdValue):
        ...
        editorRenderComponentAdapter = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "EditorRenderComponentAdapter<AZ::Render::MeshComponentController AZ::Render::MeshComponent AZ::Render::MeshComponentConfig >",
                "field": "BaseClass1",
                "type": "{3D614286-9164-53B5-833B-4F98D2820BA7}",
            },
        )
        ...
        editorComponentAdapter = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "EditorComponentAdapter<AZ::Render::MeshComponentController AZ::Render::MeshComponent AZ::Render::MeshComponentConfig >",
                "field": "BaseClass1",
                "version": "1",
                "type": "{52DFE044-18C1-5861-BA2A-EDB61107FEE9}",
            },
        )

        editorComponentBase = xml.etree.ElementTree.Element(
            "Class",
            {
        ...
        component = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "AZ::Component",
                "field": "BaseClass1",
        ...
        u64 = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "AZ::u64",
                "field": "Id",
                "value": "4528867579411318958",
        ...
        meshComponentController = create_xml_element_from_string(
            '<Class name="AZ::Render::MeshComponentController" field="Controller" type="{D0F35FAC-4194-4C89-9487-D000DDB8B272}">'
        )

        meshComponentConfig = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "AZ::Render::MeshComponentConfig",
                "field": "Configuration",
                "type": "{63737345-51B1-472B-9355-98F99993909B}",
            },
        )
        modelAsset = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "Asset",
                "field": "ModelAsset",
        ...
        excludeFromReflectionCubeMaps = xml.etree.ElementTree.Element(
            "Class",
            {
                "name": "bool",
                "field": "ExcludeFromReflectionCubeMaps",
                "value": "false",
    ...
    def convert(self, xmlElement, parent):
        ...
        if (
            len(self.oldMaterialRelativePath) == 0
            or self.oldMaterialRelativePath[
                0 : self.oldMaterialRelativePath.find(".mtl")
            ]
            == self.oldFbxRelativePathWithoutExtension
        ):
        ...
        atomMaterialInDefaultSlot = self.materialComponentConverter.convert_legacy_mtl_relative_path_to_atom_material_assetid(
            self.normalizedProjectDir,
            self.oldMaterialRelativePath,
            self.oldFbxRelativePathWithoutExtension,
        )
        isActor = False
        atomMaterialList = self.materialComponentConverter.convert_legacy_mtl_relative_path_to_atom_material_list(
            self.normalizedProjectDir,
            self.oldMaterialRelativePath,
            self.oldFbxRelativePathWithoutExtension,
            isActor,
        ...
        parent.append(
            self.materialComponentConverter.create_material_component_with_material_assignments(
                atomMaterialInDefaultSlot, atomMaterialList
            )
        )
