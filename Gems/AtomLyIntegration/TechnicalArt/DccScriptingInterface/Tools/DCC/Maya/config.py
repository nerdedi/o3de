...
PATH_DCCSI_TOOLS_DCC_MAYA_SETTINGS = PATH_DCCSI_TOOLS_DCC_MAYA.joinpath(
    SETTINGS_FILE_SLUG
).resolve()

from DccScriptingInterface import LOCAL_SETTINGS_FILE_SLUG

PATH_DCCSI_TOOLS_DCC_MAYA_LOCAL_SETTINGS = PATH_DCCSI_TOOLS_DCC_MAYA.joinpath(
    LOCAL_SETTINGS_FILE_SLUG
).resolve()
# -------------------------------------------------------------------------
...
class MayaConfig(ConfigClass):
    ...
    def maya_version(
        ...
        # store setting
        self.add_setting(envar, value, check_envar=check_envar, set_envar=set_envar)

        # update the maya location
        self.maya_location = Path(
            f"{PATH_PROGRAMFILES_X64}\\Autodesk\\Maya{self._maya_version}"
        )
...
maya_config.add_setting(
    ENVAR_PATH_DCCSI_TOOLS_DCC_MAYA, Path(PATH_DCCSI_TOOLS_DCC_MAYA)
)
...
maya_config.add_setting(
    ENVAR_DCCSI_MAYA_SCRIPT_PATH, Path(DCCSI_MAYA_SCRIPT_PATH), set_pythonpath=True
)
...
maya_config.add_path_to_envar(
    maya_config.settings.DCCSI_MAYA_PLUG_IN_PATH, ENVAR_MAYA_PLUG_IN_PATH
)
maya_config.add_path_to_envar(
    maya_config.settings.DCCSI_MAYA_SHELF_PATH, ENVAR_MAYA_SHELF_PATH
)
maya_config.add_path_to_envar(
    maya_config.settings.DCCSI_MAYA_XBMLANGPATH, ENVAR_XBMLANGPATH
)
maya_config.add_path_to_envar(
    maya_config.settings.DCCSI_MAYA_SCRIPT_PATH, ENVAR_MAYA_SCRIPT_PATH
)

# these are specifically ours and would not potentially be multi-path lists
maya_config.add_setting(
    ENVAR_DCCSI_MAYA_SCRIPT_MEL_PATH, Path(DCCSI_MAYA_SCRIPT_MEL_PATH)
)
maya_config.add_setting(
    ENVAR_DCCSI_MAYA_SCRIPT_PY_PATH, Path(DCCSI_MAYA_SCRIPT_PY_PATH)
)
...
if __name__ == "__main__":
    ...
    _LOGGER.info(
        f"Exporting local settings: {PATH_DCCSI_TOOLS_DCC_MAYA_LOCAL_SETTINGS}"
    )
