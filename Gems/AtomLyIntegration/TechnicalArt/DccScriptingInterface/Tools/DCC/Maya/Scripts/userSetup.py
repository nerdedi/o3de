...
__all__ = ["config", "constants", "setup", "start", "scripts"]
...
_logging.basicConfig(level=_logging.DEBUG, format=FRMT_LOG_LONG, datefmt="%m-%d %H:%M")
...
_LOGGER.debug(
    f"{ENVAR_DCCSI_CONFIG_DCC_MAYA}: {maya_config.settings.DCCSI_CONFIG_DCC_MAYA}"
)
_LOGGER.debug(
    f"{ENVAR_PATH_DCCSI_TOOLS_DCC_MAYA}: {maya_config.settings.PATH_DCCSI_TOOLS_DCC_MAYA}"
)
...
def post_startup():
    ...
    if _G_LOAD_CALLBACKS:
        try:
            ...
            from DccScriptingInterface.Tools.DCC.Maya.Scripts.set_callbacks import (
                _G_CALLBACKS,
            )
    ...
    _project_workspace = os.path.join(
        maya_config.settings.MAYA_PROJECT, SLUG_MAYA_WORKSPACE
    )
