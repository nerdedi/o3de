...
_settings_core = dccsi_core_config.get_config_settings(
...
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_PATH_DCCSI_TOOLS_DCC_BLENDER,
)
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    PATH_DCCSI_TOOLS_DCC_BLENDER,
)

# blender settings.json
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    PATH_DCCSI_TOOLS_DCC_BLENDER_SETTINGS,
)

# blender settings.local.json
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    PATH_DCCSI_TOOLS_DCC_BLENDER_LOCAL_SETTINGS,
)
# -------------------------------------------------------------------------
...
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_DCCSI_CONFIG_DCC_BLENDER,
)
...
blender_config.add_setting(ENVAR_PATH_DCCSI_TOOLS, PATH_DCCSI_TOOLS.as_posix())


PATH_DCCSI_TOOLS_DCC_BLENDER = Path(PATH_DCCSI_TOOLS_DCC_BLENDER).resolve()
blender_config.add_setting(
    ENVAR_PATH_DCCSI_TOOLS_DCC_BLENDER, PATH_DCCSI_TOOLS_DCC_BLENDER.as_posix()
)

from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_PATH_DCCSI_TOOLS_DCC_BLENDER_SCRIPTS,
)
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    PATH_DCCSI_TOOLS_DCC_BLENDER_SCRIPTS,
)

PATH_DCCSI_TOOLS_DCC_BLENDER_SCRIPTS = Path(
    PATH_DCCSI_TOOLS_DCC_BLENDER_SCRIPTS
).resolve()
blender_config.add_setting(
    ENVAR_PATH_DCCSI_TOOLS_DCC_BLENDER_SCRIPTS,
...
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_DCCSI_BLENDER_VERSION,
)

blender_config.add_setting(ENVAR_DCCSI_BLENDER_VERSION, SLUG_DCCSI_BLENDER_VERSION)

try:
    PATH_DCCSI_BLENDER_LOCATION = settings.PATH_DCCSI_BLENDER_LOCATION
except:
    from DccScriptingInterface.Tools.DCC.Blender.constants import (
        PATH_DCCSI_BLENDER_ROOT,
    )

    PATH_DCCSI_BLENDER_LOCATION = (
        f"{PATH_DCCSI_BLENDER_ROOT} {SLUG_DCCSI_BLENDER_VERSION}"
    )
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_DCCSI_BLENDER_LOCATION,
)
...
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_PATH_DCCSI_BLENDER_EXE,
)

PATH_DCCSI_BLENDER_EXE = Path(PATH_DCCSI_BLENDER_EXE).resolve()
blender_config.add_setting(
    ENVAR_PATH_DCCSI_BLENDER_EXE, PATH_DCCSI_BLENDER_EXE.as_posix()
)
...
try:
    ...
    from DccScriptingInterface.Tools.DCC.Blender.constants import (
        SLUG_BLENDER_LAUNCHER_EXE,
    )

    PATH_DCCSI_BLENDER_LAUNCHER_EXE = (
        f"{PATH_DCCSI_BLENDER_LOCATION}\\{SLUG_BLENDER_LAUNCHER_EXE}"
    )
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_DCCSI_BLENDER_LAUNCHER_EXE,
)

PATH_DCCSI_BLENDER_LAUNCHER_EXE = Path(PATH_DCCSI_BLENDER_LAUNCHER_EXE).resolve()
blender_config.add_setting(
    ENVAR_DCCSI_BLENDER_LAUNCHER_EXE, PATH_DCCSI_BLENDER_LAUNCHER_EXE.as_posix()
)

try:
    PATH_DCCSI_BLENDER_PYTHON_LOC = settings.PATH_DCCSI_BLENDER_PYTHON_LOC
except:
    PATH_DCCSI_BLENDER_PYTHON_LOC = (
        f"{PATH_DCCSI_BLENDER_LOCATION}\\{SLUG_DCCSI_BLENDER_VERSION}/python"
    )
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_DCCSI_BLENDER_PYTHON_LOC,
)
...
blender_config.add_setting(
    ENVAR_DCCSI_BLENDER_PY_EXE, PATH_DCCSI_BLENDER_PY_EXE.as_posix()
)

from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_PATH_DCCSI_BLENDER_BOOTSTRAP,
)
from DccScriptingInterface.Tools.DCC.Blender.constants import (
    PATH_DCCSI_BLENDER_BOOTSTRAP,
)

PATH_DCCSI_BLENDER_BOOTSTRAP = Path(PATH_DCCSI_BLENDER_BOOTSTRAP).resolve()
blender_config.add_setting(
    ENVAR_PATH_DCCSI_BLENDER_BOOTSTRAP, PATH_DCCSI_BLENDER_BOOTSTRAP.as_posix()
)

from DccScriptingInterface.Tools.DCC.Blender.constants import (
    ENVAR_URL_DCCSI_BLENDER_WIKI,
)
from DccScriptingInterface.Tools.DCC.Blender.constants import URL_DCCSI_BLENDER_WIKI

blender_config.add_setting(ENVAR_URL_DCCSI_BLENDER_WIKI, str(URL_DCCSI_BLENDER_WIKI))
...
if __name__ == "__main__":
    ...
    _LOGGER.info(
        f"Exporting local settings: {PATH_DCCSI_TOOLS_DCC_BLENDER_LOCAL_SETTINGS}"
    )

    try:
        blender_config.export_settings(set_env=True, log_settings=True)
