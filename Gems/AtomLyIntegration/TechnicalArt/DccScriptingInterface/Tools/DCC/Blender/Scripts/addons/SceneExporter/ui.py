...
class ReportCard(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        # Get Selected Stats
        vert_count, edge_count, polygon_count, material_count = (
            utils.check_selected_stats_count()
        )
        # Get filename
        ...
        issues.label(text="Issues Found:")
        ...
        if not transforms_status:
            issues.alert = True
            issues.label(
                text="Warning, Your Selected Object(s)", icon="SEQUENCE_COLOR_03"
            )
            issues.label(text="Transforms have not been Applied (Frozen).")
        ...
        if not selected_uvs_check:
            ...
            issues.label(text="Are missing UV's")
            self.reported_issues_int += 1
        if invalid_bone_names:
            ...
            issues.label(text="Have Invalid Bone Names for O3DE.")
            issues.label(text="Please do not use these types of Characters")
            issues.label(text=r"in Bone names <>[\]~.`")
            self.reported_issues_int += 1
        ...
        mesh_info.label(text="Issues Found:")
        ...
        mesh_info.label(
            text=f"Scene Units are in: {bpy.context.scene.unit_settings.length_unit}"
        )
        mesh_info.label(text=f"Transforms are applied: {transforms_status}")
        mesh_info.label(
            text=f"Export as Triangles: {bpy.types.Scene.convert_mesh_to_triangles}"
        )
        ...
        mesh_info.label(
            text=f"Export to Path: {bpy.types.Scene.selected_o3de_project_path}"
        )
...
class CustomProjectPath(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        if (
            ...
            not in context.scene.o3de_projects_list
        ):
            o3de_utils.save_project_list_json(
                str(bpy.types.Scene.selected_o3de_project_path)
            )
            # Refesh the addon
...
class AddColliderMesh(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.types.Scene.pop_up_confirm_label = (
            "Adding UDP Type: o3de_atom_phys to mesh."
        )
...
class SceneExporterFileMenu(Operator, ExportHelper):
    ...
    export_mesh_as_triangles: BoolProperty(
        name="Export Mesh as Triangles", description="Export Mesh as Triangles"
    )
...
class O3deTools(Panel):
    ...
    def draw(self, context):
        ...
        if engine_is_installed:
            ...
            current_selected_options_row.label(
                text="CURRENT SELECTED", icon="OUTLINER_DATA_GP_LAYER"
            )
            ...
            if len(name_selections) == 0:
                ...
                installed_lable.label(
                    text=f"({len(selected_name)}) Selected: {selected_objects.name}"
                )
            ...
            if bone_are_in_selected:
                ...
                if bpy.types.Scene.animation_export == constants.NO_ANIMATION:
                    animation_action_lable.label(text="Action: ")
                    animation_export_lable.label(
                        text=f"Animation: {bpy.types.Scene.animation_export}"
                    )
                else:
                    animation_action_lable.label(
                        text=f"Action: {action_name} ({action_count})"
                    )
                    animation_export_lable.label(
                        text=f"Animation: {bpy.types.Scene.animation_export}"
                    )
            ...
            if bpy.types.Scene.export_textures_folder:
                texture_export_option_label.label(
                    text="Texture Export: Textures Folder"
                )
            ...
            o3de_projects_panel.operator(
                "wm.projectlist", text="O3DE Projects", icon="OUTLINER"
            )

            # Let user choose a custom project path
            local_project_path = layout.row()
            local_project_path.operator(
                "project.open_filebrowser",
                text="Add Custom Project Path",
            ...
            create_collider_button.operator(
                "vin.collider", text="Create PhysX Collider", icon="SNAP_VOLUME"
            )
            ...
            texture_export_options_panel.operator(
                "wm.exportoptions", text="Texture Export Options", icon="OUTPUT"
            )

            # This is the UI Animation Export Options List
            animation_export_options_panel = layout.row()
            animation_export_options_panel.operator(
                "wm.animationoptions", text="Animation Export Options", icon="POSE_HLT"
            )

            # If more than one object is selected, let user choose export options
            if len(name_selections) > 1:
                multi_file_export_label = (
                    "Export Multi-Files"
                    if wm.multi_file_export_toggle
                    else "Export a Single File"
                )
                multi_file_export_button = layout.row()
                multi_file_export_button.prop(
                    wm,
                    "multi_file_export_toggle",
                    text=multi_file_export_label,
                    toggle=True,
                )
                if wm.multi_file_export_toggle:
                ...
                bpy.types.Scene.multi_file_export_o3de = False
            # Export mesh options
            export_mesh_triangles_quads_label = (
                "Exporting as Triangles"
                if wm.mesh_triangle_export_toggle
                else "Exporting as Quads"
            )
            export_mesh_triangles_quads_button = layout.row()
            export_mesh_triangles_quads_button.prop(
                wm,
                "mesh_triangle_export_toggle",
                text=export_mesh_triangles_quads_label,
                toggle=True,
            )
            if wm.mesh_triangle_export_toggle:
            ...
            elif not utils.check_if_valid_path(
                bpy.types.Scene.selected_o3de_project_path
            ):
                export_files_row.enabled = False
            ...
            export_files_row.operator(
                "vin.report_card_button", text="EXPORT TO O3DE", icon="BLENDER"
            )
        else:
