...
_SETTINGS = _DCCSI_CORE_CONFIG.get_config_settings(
...
def get_config_settings(
    core_config=_DCCSI_CORE_CONFIG,
    settings=_SETTINGS,
    ...
    os.environ[f"DYNACONF_{ENVAR_DCCSI_SUBSTANCE_SET_CALLBACKS}"] = str(
        DCCSI_SUBSTANCE_SET_CALLBACKS
    )

    os.environ[f"DYNACONF_{ENVAR_DCCSI_TOOLS_SUBSTANCE}"] = (
        PATH_DCCSI_TOOLS_SUBSTANCE.as_posix()
    )
    os.environ[f"DYNACONF_{ENVAR_PATH_DCCSI_TOOLS}"] = PATH_PATH_DCCSI_TOOLS.as_posix()
    os.environ[f"DYNACONF_{ENVAR_DCCSI_SUBSTANCE_RESOURCES}"] = (
        PATH_DCCSI_SUBSTANCE_RESOURCES.as_posix()
    )
    os.environ[f"DYNACONF_{ENVAR_DCCSI_SUBSTANCE_LOCATION}"] = (
        PATH_DCCSI_SUBSTANCE_LOCATION.as_posix()
    )
    os.environ[f"DYNACONF_{ENVAR_DCCSI_SUBSTANCE_EXE}"] = (
        PATH_DCCSI_SUBSTANCE_EXE.as_posix()
    )
    os.environ[f"DYNACONF_{ENVAR_DCCSI_SUBSTANCE_PYTHON}"] = (
        PATH_DCCSI_SUBSTANCE_PYTHON.as_posix()
    )
    os.environ[f"DYNACONF_{ENVAR_DCCSI_SUBSTANCE_PY_EXE}"] = (
        PATH_DCCSI_SUBSTANCE_PY_EXE.as_posix()
    )
    os.environ[f"DYNACONF_{ENVAR_DCCSI_IDE_PY_SUBSTANCE}"] = (
        PATH_DCCSI_IDE_PY_SUBSTANCE.as_posix()
    )
    os.environ[f"DYNACONF_{ENVAR_DCCSI_SUBSTANCE_SCRIPTS}"] = (
        PATH_DCCSI_SUBSTANCE_SCRIPTS.as_posix()
    )
    os.environ[f"DYNACONF_{ENVAR_DCCSI_SUBSTANCE_CFG}"] = (
...
if __name__ == "__main__":
    ...
    _logging.basicConfig(
        level=_DCCSI_LOGLEVEL, format=FRMT_LOG_LONG, datefmt="%m-%d %H:%M"
    )
    ...
    if _DCCSI_GDEBUG or args.cache_local_settings:
        if args.export_settings:
            ...
            _DCCSI_CORE_CONFIG.export_settings(
                settings=settings, settings_filepath=export_settings_path.as_posix()
            )
        else:
            export_settings_path = Path(
                PATH_DCCSI_TOOLS_SUBSTANCE, "settings.local.json"
            ).resolve()
            _DCCSI_CORE_CONFIG.export_settings(
                settings=settings, settings_filepath=export_settings_path.as_posix()
            )
    ...
    if arg_bool(
        args.install_package_dependencies, desc="args.install_package_dependencies"
    ):
        import foundation
