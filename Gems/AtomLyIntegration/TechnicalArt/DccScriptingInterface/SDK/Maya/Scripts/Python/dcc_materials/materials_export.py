...
class MaterialsToLumberyard(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.material_definitions_layout = QtWidgets.QHBoxLayout(
            self.material_definitions_widget
        )
        self.material_definitions_layout.setSpacing(0)
        self.material_definitions_layout.setContentsMargins(0, 0, 0, 0)
        self.material_definitions_frame = QtWidgets.QFrame(
            self.material_definitions_widget
        )
        ...
        self.material_definitions_scroller.setHorizontalScrollBarPolicy(
    ...
    def populate_export_materials_list(self):
        ...
        for count, value in enumerate(self.material_definitions):
            material_definition_node = MaterialNode(
                [value, self.material_definitions[value]], count
            )
    ...
    def get_maya_material_values(self, target_files):
        ...
        script_path = str(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "maya_materials.py"
            )
        )
        ...
        p = subprocess.Popen(
            command, shell=True, env=runtime_env, stdout=subprocess.PIPE
        )
            ...
            for m in maps:
                ...
                for tex in file_connections.keys():
                    if tex.find(texture_attribute) != -1:
                        key = (
                            m
                            if m not in naming_exceptions
                            else naming_exceptions.get(m)
                        )
    ...
    def set_transfer_status(self, transfer_info):
        ...
        for row in range(self.target_files_table.rowCount()):
            if (
                self.target_files_table.item(row, 0).text().strip()
                == transfer_info["file_location"]
            ):
    ...
    def set_export_materials_description(self):
        ...
        for row in range(self.model.rowCount()):
            ...
            self.set_pbr_material_description(
                source_file, name, material_type, file_connections
            )
    ...
    def set_pbr_material_description(
        self, source_file, name, material_type, file_connections
    ):
        ...
        self.material_definitions[
            name
            if name not in self.material_definitions.keys()
