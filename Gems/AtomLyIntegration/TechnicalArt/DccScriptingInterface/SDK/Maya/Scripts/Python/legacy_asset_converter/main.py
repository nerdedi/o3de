...
class LegacyFilesConverter(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.headers = (
            "dirname dirpath fbxname mayafile designer diff ddna spec emi basecolor metallic normal "
            "emissive status synced"
        ).split(" ")
        self.initialization_files = [
            "workspace.mel",
            "Launch_Cmd.bat",
            "Launch_Maya_2020.bat",
            "Launch_WingIDE-7-1.bat",
            "Project_Env.bat",
        ]
        ...
        self.input_path_field = QtWidgets.QLineEdit(
            "E:/LY/spectra_atom/dev/Gems/AtomStarterGame/Environment/Assets/Objects/AAGun"
        )
        ...
        self.set_output_button.clicked.connect(
            partial(self.set_directory_button_clicked, "output")
        )
        ...
        self.create_material_files_checkbox = QtWidgets.QCheckBox(
        ...
        self.skip_white_texture_materials = QtWidgets.QCheckBox(
            "Skip White Texture Materials"
        )
        ...
        self.results_stacked_widget.setSizePolicy(
        ...
        self.combobox_results_layout = QtWidgets.QVBoxLayout(
            self.combobox_results_widget
        )
        ...
        self.directory_combobox.currentIndexChanged.connect(
            self.directory_combobox_changed
        )
        ...
        self.asset_directory_combobox.currentIndexChanged.connect(
            self.asset_directory_combobox_changed
        )
        ...
        self.material_files_combobox.setStyleSheet(
            "QComboBox:item {padding-left:30px;}"
        )
        ...
        self.file_textures_table.setHorizontalHeaderLabels(
            ["Texture Type", "Texture Name"]
        )
        self.file_textures_table.horizontalHeader().setStyleSheet(
            "QHeaderView::section "
    ...
    def process_directory(self, directory_path):
        ...
        for key, values in directory_audit.items():
            ...
            for fbx_file in fbx_files:
                ...
                mtl_info = lumberyard_data.get_material_info(
                    base_directory_path, f"{fbx_file.stem}.mtl"
                )
                asset_information = self.filter_asset_information(
                    fbx_file, directory_audit, mtl_info
                )
                self.set_material_information(
                    asset_information, mtl_info, fbx_file, destination_directory
                )
            self.create_maya_files(
                base_directory_path, destination_directory, fbx_files
            )
    ...
    def process_single_asset(self, fbx_path):
        ...
        self.start_load_sequence(directory_audit)
        destination_directory = self.set_destination_directory(
            directory_audit[0].directoryname
        )
        self.progress_event_fired("fbx_{}".format(fbx_file.name))
        mtl_info = lumberyard_data.get_material_info(
            fbx_file.parent, f"{fbx_file.stem}.mtl"
        )
        asset_information = self.filter_asset_information(
            fbx_file, directory_audit[0], mtl_info
        )
        self.transfer_existing_files(
            fbx_file.parent, destination_directory, fbx_file.name
        )
        self.set_material_information(
            asset_information, mtl_info, fbx_file, destination_directory
        )
    ...
    def compare_boilerplate_settings(
        self, property_values, material_template, export_file_path
    ):
        """
                ...
                for k, v in property_values.items():
                    ...
                    if lower_property_key == lower_template_key:
                        ...
                        if v != value:
                            ...
                            updated = True
                            target_value = self.format_material_value(
                                v, export_file_path
                            )
    ...
    def format_material_value(self, value, export_file_path):
        ...
        if isinstance(value, list):
            ...
            if os.path.isfile(str(value)):
                target_path = self.get_relative_path_from_material(
                    value, export_file_path
                )
            ...
            try:
                for key, values in material_template.items():
                    ...
                    # Values driven by textures and numerical values
                        ...
                        for (
                            material_property
                        ) in constants.SUPPORTED_MATERIAL_PROPERTIES:
                            temp_dict = {}
                            texture_key = None
                            if material_property == "general":
                                ...
                                if "Emittance" in material_values.numericalsettings:
                                    ...
                                    emission_intensity = self.get_emission_intensity(
                                        factor
                                    )
                                    if (
                                        "Emissive" in material_textures.keys()
                                        or emission_intensity != -5
                                    ):
                                ...
                                texture_key = "Opacity"
                                alpha = self.get_alpha_test_value(
                                    material_values.numericalsettings
                                )
                                if alpha > 0.0:
                                    temp_dict["mode"] = "Cutout"
                                    temp_dict["factor"] = 1.0 - alpha
                            elif material_property == "uv":
                                modifications = material_values["modifications"]
                                if modifications:
                                    for modification_key in modifications.keys():
                                        _LOGGER.info(
                                            f"{material_name.lower()} <+++++++> {modification_key.lower()}"
                                        )
                                        if (
                                            material_name.lower()
                                            == modification_key.lower()
                                        ):
                                            for mod_key, mod_value in modifications[
                                                material_name
                                            ].items():
                                                _LOGGER.info(
                                                    f"ModKey: {mod_key}   ModValue: {mod_value}"
                                                )
                                                temp_dict[mod_key] = mod_value
                            else:
                                _LOGGER.info(
                                    f"Unknown Material Property found: {material_property}"
                                )

                            # Add Texture Map
                            if (
                                texture_key
                                and texture_key in material_textures.keys()
                                and material_textures[texture_key]
                            ):
                                if (
                                    material_property not in ["emissive", "general"]
                                    and "factor" not in temp_dict.keys()
                                ):
                                    temp_dict["factor"] = 1.0
                                    temp_dict["useTexture"] = True
                                temp_dict["textureMap"] = os.path.normpath(
                                    material_textures[texture_key]
                                )

                            # Determine if property should be added based on deviation from default settings
                            _LOGGER.info(f"TempDict: {temp_dict}")
                            if temp_dict:
                                comparison_dictionary = material_template["properties"][
                                    material_property
                                ]
                                _LOGGER.info(
                                    f"ComparisonDictionary: {comparison_dictionary}"
                                )
                                custom_settings = self.compare_boilerplate_settings(
                                    temp_dict,
                                ...
                                if custom_settings:
                                    material_description[key][material_property] = (
                                        custom_settings
                                    )
                ...
                if self.create_material_files_checkbox.isChecked():
                    if len(material_description) > 4:
                        _LOGGER.info(
                            ...
                            "+++++++++++++++\n_\n".format(
                                json.dumps(
                                    material_description, indent=4, sort_keys=False
                                )
                            )
                        )
                        lumberyard_data.export_lumberyard_material(
                            os.path.abspath(export_file_path), material_description
                        )
                        return os.path.abspath(export_file_path)
            except Exception as e:
                _LOGGER.info(
                    f"###################### Error processing material file [{file_name}]: {e}"
                )
        else:
            return os.path.abspath(export_file_path)
        return None
    ...
    def get_emission_intensity(self, mtl_setting):
        ...
        if float(mtl_setting) > 0.0:
            return math.log(mtl_setting * 1000.0 / 0.125, 2) / 3.0
        else:
            return -5.0
    ...
    def filter_asset_information(self, target_path, directory_audit, mtl_info):
        ...
        if target_path.is_file():
            ...
            materials = self.get_material_information(
                directory_path, mtl_info, f"{target_path.name}.assetinfo"
            )
            fbx_files = {
                target_path.name: {
                    constants.FBX_MAYA_FILE: "",
                    constants.FBX_MATERIALS: materials,
                }
            }
            asset_dictionary = {
                constants.FBX_DIRECTORY_NAME: directory_name,
                constants.FBX_DIRECTORY_PATH: directory_path,
    ...
    def reformat_materials_db(self, to_maya, updates=False):
        ...
        if to_maya:
            reformatted_database = utilities.convert_box_dict_to_standard(
                temp_dictionary
            )
            for key, value in reformatted_database.items():
                self.materials_db[key] = value
        else:
            reformatted_database = utilities.convert_standard_dict_to_box(
                temp_dictionary
            )
            for key, value in reformatted_database.items():
                self.materials_db[key] = value
            ...
            for k, v in updates.items():
                if k == key:
                    for fbx_file, maya_file_value in v.items():
                        for target_fbx, target_fbx_listing in value[
                            constants.FBX_FILES
                        ].items():
                            if fbx_file == target_fbx:
                                _LOGGER.info(
                                    "Updating Mayafile: {}".format(
                                        maya_file_value["maya_file"]
                                    )
                                )
                                target_fbx_listing[constants.FBX_MAYA_FILE] = (
                                    maya_file_value["maya_file"]
                                )
    ...
    def create_checklist_data(self, directory_index):
        ...
        checklist_data = {}
        texture_types = (
            "basecolor metallic roughness normal emissive scattering opacity".split(" ")
        )
        ...
        if self.create_checklist_checkbox.isChecked():
            ...
            try:
                for key, values in target_dictionary[constants.FBX_FILES].items():
                    fbx_name = key
                    destination_directory = self.set_destination_directory(
                        target_dictionary[constants.FBX_DIRECTORY_NAME]
                    )

                    for material_name, texture_set in values[
                        constants.FBX_MATERIALS
                    ].items():
                        checklist = collections.OrderedDict()
                        checklist["fbxname"] = fbx_name
                        checklist["materialname"] = material_name
                        checklist["dirname"] = target_dictionary[
                            constants.FBX_DIRECTORY_NAME
                        ]
                        checklist["dirpath"] = target_dictionary[
                            constants.FBX_DIRECTORY_PATH
                        ]
                        ...
                        for texture in texture_types:
                            ...
                            try:
                                ...
                                _LOGGER.info(
                                    f"Could not gather image data for index [{directory_index}]: {e}"
                                )
                        checklist["status"] = "Started"
                        checklist["synced"] = "No"
                        checklist_data[material_name] = {
                            key: values for key, values in checklist.items()
                        }
    ...
    def transfer_existing_files(
        self, base_directory, destination_directory, target_prefix=None
    ):
        """
        ...
        for file in base_directory.iterdir():
            if file.suffix in constants.TRANSFER_EXTENSIONS:
                if target_prefix:
                    if str(file).startswith(target_prefix) or str(file).startswith(
                        target_prefix.lower()
                    ):
                        image_conversion.transfer_file(
                            base_directory / file.name,
                            destination_directory / file.name,
                        )
                else:
                    image_conversion.transfer_file(
                        base_directory / file.name, destination_directory / file.name
                    )
    ...
    def get_material_information(self, search_path, mtl_info, assetinfo_filename):
        ...
        for key, values in mtl_info.items():
            ...
            if self.skip_white_texture_materials.isChecked():
                if "engineassets/textures/white.dds" in (
                    str(path.as_posix()).lower() for path in values.textures.values()
                ):
            ...
            if values.attributes.Shader in constants.EXPORT_MATERIAL_TYPES:
                ...
                all_textures = self.get_additional_textures(
                    search_path, legacy_textures
                )
                numerical_settings = self.get_numerical_settings(mtl_info, key)
                uses_alpha = self.get_alpha_test_value(numerical_settings) > 0.0
                fbx_material_dictionary[key][constants.FBX_NUMERICAL_SETTINGS] = (
                    numerical_settings
                )
                fbx_material_dictionary[key][constants.FBX_TEXTURES] = (
                    self.get_texture_set(search_path, all_textures, uses_alpha)
                    if legacy_textures
                    else ""
                )
                fbx_material_dictionary[key][constants.FBX_TEXTURE_MODIFICATIONS] = (
                    values.modifications if values.modifications else ""
                )
                fbx_material_dictionary[key][constants.FBX_MATERIAL_FILE] = ""
                fbx_material_dictionary[key][constants.FBX_ASSIGNED_GEO] = (
                    lumberyard_data.get_asset_info(search_path, assetinfo_filename, key)
                )
            elif values.attributes.Shader == "Nodraw":
                _LOGGER.info("Found No-draw shader... skipping")
            else:
                _LOGGER.info(
                    f"Shader {values.attributes.Shader} not supported... skipping"
                )
                pass
        _LOGGER.info(
    ...
    def get_additional_textures(self, search_path, texture_set):
        ...
        if texture_set:
            ...
            for key in texture_keys:
                try:
                    texture_basename = image_conversion.get_texture_basename(
                        texture_set[key]
                    )
                    if texture_basename:
    ...
    def get_numerical_settings(self, mtl_info, material):
        ...
        for material_property in property_list:
            try:
                numerical_settings[material_property] = mtl_info[material].attributes[
                    material_property
                ]
            except Exception:
                pass
        ...
        for key, value in mtl_textures.items():
            for file in search_path.iterdir():
                if file.stem.lower() == value.stem:
                    ...
                    break
        return image_conversion.get_pbr_textures(
            mtl_textures,
    ...
    def get_mayapy_path(self):
        ...
        maya_versions = [
            ...
            if x.name.startswith("Maya") and x.name[-1].isdigit()
        ]
        if maya_versions:
            target_version = f"Maya{max(maya_versions)}"
            self.create_maya_files_checkbox.setChecked(True)
            return self.autodesk_directory / target_version / r"bin\mayapy.exe"
        else:
            ...
            if constants.FBX_MATERIALS in v.keys():
                for k, v in v[constants.FBX_MATERIALS].items():
                    for mat_key, mat_val in v.items():
                        ...
                        elif mat_key == constants.FBX_TEXTURES and isinstance(
                            mat_val, dict
                        ):
                            if mat_val.keys():
                                for texture_key, texture_path in mat_val.items():
                                    try:
                                        ...
                                        _LOGGER.info(
                                            f"Could not add TextureKey/Path in GET_SECTION_AUDIT: {e}"
                                        )
                        else:
                            pass
    ...
    def set_material_information(
        self, asset_information, mtl_info, fbx_file, destination_directory
    ):
        """
            ...
            for k, v in mtl_info.items():
                if key is v.attributes.Name:
                    if v.attributes.Shader in constants.EXPORT_MATERIAL_TYPES:
                        ...
                        material_file = self.export_material_file(
                            f"{fbx_file.stem}_{key}.material".lower(),
                            key,
                            material_values,
    ...
    def set_asset_window(self, section_data=None):
        ...
        try:
            ...
            index = self.asset_directory_combobox.findText(
                current_directory, QtCore.Qt.MatchFixedString
            )
    ...
    def set_data_window(self, target_section="All"):
        ...
        if target_section == "All":
            count_totals = Box(
                {"materials": 0, "textures": 0, "fbxfiles": 0, "mayafiles": 0}
            )
                    ...
                    try:
                        ...
                        for k, v in values.fbxfiles.items():
                            ...
                            if v[constants.FBX_MATERIALS]:
                                for material_name, material_values in v[
                                    constants.FBX_MATERIALS
                                ].items():
    ...
    def set_destination_directory(self, target_directory_name):
        ...
        """
        destination_directory = (
            self.output_directory / r"Assets\Objects" / target_directory_name
        )
    ...
    def set_asset_directory_combobox(self):
        ...
        """
        directories_list = [
            x[constants.FBX_DIRECTORY_NAME] for x in self.materials_db.values()
        ]
    ...
    def start_load_sequence(self, audit_info):
        ...
        for key, listing in audit_info.items():
            for fbx in listing["files"][".fbx"]:
                if self.create_maya_files_checkbox.isChecked():
                    self.load_events[1] += (
                        constants.LOAD_WEIGHTS["maya"] + constants.LOAD_WEIGHTS["fbx"]
                    )
                else:
        ...
        if self.use_directory_radio_button.isChecked():
            ...
            if file_path != "":
                self.input_path_field.setText(
                    file_path
                ) if target == "input" else self.output_path_field.setText(file_path)
        else:
            if target == "input":
                file_browser = QtWidgets.QFileDialog()
                file_path = file_browser.getOpenFileName(
                    self, "Set Target File", self.desktop_location, "FBX Files (*.fbx)"
                )
                if file_path != "":
    ...
    def directory_combobox_changed(self):
        target_data = (
            "All"
            if self.directory_combobox.currentText() == ""
            else self.directory_combobox.currentText()
        )
        self.set_data_window(target_data)

    def asset_directory_combobox_changed(self):
        self.set_asset_window(
            self.materials_db[str(self.asset_directory_combobox.currentIndex())]
        )
            ...
            if values[constants.FBX_DIRECTORY_NAME] == current_directory:
                ...
                for val in fbx_values:
                    ...
                    for material_key, material_values in material_list.items():
                        for texture_key, texture_path in material_values[
                            constants.FBX_TEXTURES
                        ].items():
                            if texture_type:
                                if os.path.exists(texture_path):
                                    try:
                                        subprocess.Popen(
                                            r'explorer /select, "{}"'.format(
                                                texture_path
                                            )
                                        )
                                    except Exception as e:
                                        _LOGGER.info(
                                            f"Couldnt open image file directory: {e}"
                                        )
                            return
    ...
    def open_material_clicked(self):
        if self.material_files_combobox.currentText() != "Choose":
            ...
            file_path = os.path.join(
                base_path, self.material_files_combobox.currentText()
            )
