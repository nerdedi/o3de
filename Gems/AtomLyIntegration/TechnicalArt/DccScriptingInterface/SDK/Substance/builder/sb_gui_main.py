...
_LOGGER = azpy.initialize_logger(
    name=_MODULENAME, log_to_file=True, default_log_level=_log_level
)
...
_LOGGER.debug(
    "QT_QPA_PLATFORM_PLUGIN_PATH: {}".format(settings.QT_QPA_PLATFORM_PLUGIN_PATH)
)
# -------------------------------------------------------------------------
...
_GEM_MATPLAY_PATH = Path(
    _O3DE_DEV, "Gems", "AtomContent", "AtomMaterialPlayground"
).resolve()
_GEM_ROYALTYFREE = Path(_O3DE_DEV, "Gems", "AtomContent", "RoyaltyFreeAssets").resolve()
_GEM_SUBSOURCELIBRARY = Path(
    _O3DE_DEV, "Gems", "AtomContent", "SubstanceSourceLibrary"
).resolve()
_SUB_LIBRARY_PATH = Path(
    _GEM_SUBSOURCELIBRARY, "Assets", "SubstanceSource", "Library"
).resolve()
...
_WATCHER_SCRIPT_PATH = Path(
    _DCCSI_SDK_PATH, "substance", "builder", "watchdog", "__init__.py"
).resolve()
...
class Window(QtWidgets.QDialog):
    def __init__(self, parent=None, project_path=None, default_material_path=None):
        ...
        if default_material_path is None:
            self._default_material_path = Path(
                _PATH_DCCSIG,
                "sdk",
                "substance",
                "resources",
        ...
        self.atomMatNameComboBox = self.createComboBox(
            "Setup Material name here(default:sbsar name)"
        )
        ...
        self.directoryComboBox = self.createComboBox(
            self.sbsarDirectory.canonicalPath()
        )
        self.builderDirComboBox = self.createComboBox(
            self.sbsarDirectory.canonicalPath()
        )
    ...
    def browse(self):
        self.root_directory = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Find watcher folder", str(_SUB_LIBRARY_PATH)
        )
        if self.root_directory:
            if (
                self.directoryComboBox.findText(self.root_directory)
                or self.builderDirComboBox.findText(self.root_directory) == -1
            ):
            ...
            self.builderDirComboBox.setCurrentIndex(
                self.builderDirComboBox.findText(self.root_directory)
            )
    ...
    def browseRenderPath(self):
        self.texRenderpath = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Setup textures output path", str(_TEX_RNDR_PATH)
        )
        if self.texRenderpath:
            ...
            self.texRenderPathComboBox.setCurrentIndex(
                self.texRenderPathComboBox.findText(self.texRenderpath)
            )
        self.texRenderPathComboBox.setCurrentText(self.texRenderpath)
        return self.texRenderpath

    def browseMaterialPath(self):
        self.material_path = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Setup Atom Material output path", str(_MAT_OUTPUT_PATH)
        )
        if self.material_path:
            ...
            self.matOutputPathComboBox.setCurrentIndex(
                self.matOutputPathComboBox.findText(self.material_path)
            )
    ...
    def find_files(self, files, text):
        ...
        for i in range(len(files)):
            ...
            progressDialog.setLabelText(
                "Searching file number %d of %d..." % (i, len(files))
            )
    ...
    def show_files(self, files):
        for fn in files:
            ...
            self.filesTable.setStyleSheet(
                "alternate-background-color: #444444; background-color: #4d4d4d;"
            )
        self.filesFoundLabel.setText(
            "%d file(s) found (Double click on a SBSAR to cook, SBS file to load.)"
            % len(files)
        )
    ...
    def create_files_table(self):
        ...
        self.filesTable.horizontalHeaderItem(0).setTextAlignment(
            QtCore.Qt.AlignVCenter | QtCore.Qt.AlignLeft
        )
        self.filesTable.horizontalHeaderItem(1).setTextAlignment(
    ...
    def create_sbsar_tex(self):
        ...
        self.sbsar_tex_outputs = sbsar_utils.output_info(
            self.path, self.sbsarName, "tex_maps"
        )
    ...
    def create_sbsar_presets(self):
        ...
        self.sbsar_presets = sbsar_utils.output_info(
            self.path, self.item.text().replace(".sbsar", ""), "presets"
        )
    ...
    def preset_selected(self, index):
        ...
        if self.preset in self.sbsar_presets:
            self.atomMatNameComboBox.setCurrentText(
    ...
    def atom_material(
        self, preset=None, material_type=None, tex_ext=".tif", mat_ext=".material"
    ):
        """To Do"""
        if preset is None:
            preset = self.preset
        ...
        for tex_index in range(num_index):
            ...
            # posiz / slash
            material.set_map(
                material.tex[tex_index], str(rel_texture_map_path.as_posix())
            )
    ...
    def create_atom_mat(self, item_path=None):
        self.atom_matPath_button = self.create_button(
            "&Browse", self.browseMaterialPath
        )
        self.atom_material_button = self.create_button(
            "&Create Single", self.create_single
        )
        self.atom_material_output = self.create_button(
            "&Create All", self.create_all_material
        )
        self.renderPathbutton = self.create_button(
            "&Output Path", self.browseRenderPath
        )
    ...
    def open_file_of_item(self, row, column):
        ...
        # Cook sbsar file and trigger watcher to generate textures.
            ...
            sbsar_utils.cook_sbsar(
                self.path + "/" + self.item.text(),
                sbsar_utils._PYSBS_CONTEXT,
                self.path,
