...
_LOGGER = _logging.getLogger(
    "DCCsi.azpy.o3de.renderer.materials.o3de_material_generator"
)
...
class MaterialGenerator:
    ...
    def __init__(
        self, input, output, mesh, attributes, base_path, options, master_paths
    ):
        ...
        self.material_properties_dict = (
            material_utilities.get_all_properties_description(output)
        )
    ...
    def get_transferable_properties(self):
        ...
        if self.dcc_application:
            return material_utilities.get_transferable_properties(
                self.dcc_application, self.input_material
            )
        return None
    ...
    def get_default_material_settings(self):
        ...
        """
        self.default_dcc_material_settings = (
            material_utilities.get_default_material_settings(
                self.dcc_application, self.input_material
            )
        )
            ...
            for key, value in self.transferable_properties.items():
                if value == material_property:
                    ...
                    if property_components[1] == "textureMap":
                        key_match = self.get_matching_key(
                            [key, property_components[0]], self.texture_list
                        )
                        if key_match:
                            try:
    ...
    def get_default_value(self, key):
        ...
        """
        if (
            self.default_dcc_material_settings["settings"][key]
            != self.dcc_material_settings[key]
        ):
    ...
    def get_relative_path(self, asset_path):
        ...
        base_path_key = (
            "Assets" if "Assets" in path_list else Path(self.output_base_path).name
        )
    ...
    def set_properties_formatting(properties_dictionary):
        ...
        for key, value in properties_dictionary.items():
            ...
            if attribute_parts[0] not in formatted_dictionary.keys():
                ...
                formatted_dictionary[attribute_parts[0]].update(
                    {attribute_parts[-1]: value}
                )
    ...
    def export_json(self):
        ...
        results_data = (
            Path(os.environ["USERPROFILE"]) / "Desktop/dcc_material_data.json"
        )
        helpers.set_json_data(results_data, self.material_attributes)
        # "All properties" data for target material type ---------->>
        all_properties_data = (
            Path(os.environ["USERPROFILE"]) / "Desktop/all_properties_data.json"
        )
    ...
    def export_mesh(self):
        if "Preserve Grouped Objects" not in self.export_options:
            material_utilities.export_mesh(
                self.dcc_application,
                self.mesh_name,
                self.mesh_export_location,
                self.export_options,
            )
        ...
        for texture_type, texture_info in self.texture_list.items():
            if texture_info:
                if texture_info.get("path"):
                    ...
                    self.texture_list[texture_type]["path"] = self.get_relative_path(
                        str(dst)
                    )
    ...
    def texture_transferred(self, src, master_paths):
        ...
        if path in master_paths.keys():
            for target_object in master_paths[path]["objects"]:
                search_path = (
                    Path(base_directory) / target_object / "textures" / Path(path).name
                )
...
class MayaMaterialGenerator(MaterialGenerator):
    def __init__(
        ...
        super().__init__(
            input, output, mesh, attributes, base_path, options, master_paths
        )
    ...
    def generate_material(self):
        ...
        for key, values in self.material_properties_dict.items():
            self.material_definition[key] = (
                values if key != "propertyValues" else self.get_property_values(values)
            )
        ...
        try:
            ...
            return {
                self.mesh_export_location: {
                    self.material_name: self.material_definition
                }
            }
...
def export_grouped_objects(material_info, output_path, dcc_application, export_options):
    ...
    for key, values in exported_groups.items():
        ...
        material_utilities.export_mesh(
            dcc_application, values["children"], output_path, export_options
        )
...
def export_standard_pbr_materials(material_info, base_path, options, master_paths):
    ...
    for scene_name, translation_values in material_info.items():
        if "Preserve Grouped Objects" in options:
            export_grouped_objects(
                translation_values, base_path, dcc_application, options
            )
        ...
        for object_key, material_data in translation_values.items():
            ...
            for material_name, material_values in material_data.items():
                ...
                _LOGGER.info(
                    f"MaterialDefinition: {material_definition}\n\n\n\n\n\n\n\n\n"
                )
