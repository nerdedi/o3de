...
_spec_dccsi_config = importlib.util.spec_from_file_location(
    "dccsi.config", Path(_PATH_DCCSIG, "config.py")
)
...
_PATH_DCCSI_PYTHON_LIB = os.getenv(ENVAR_PATH_DCCSI_PYTHON_LIB, PATH_DCCSI_PYTHON_LIB)
...
_logging.basicConfig(level=_logging.INFO, format=FRMT_LOG_LONG, datefmt="%m-%d %H:%M")
_LOGGER = _logging.getLogger(_PACKAGENAME)
    ...
    if _dir_to_last_file is None:
        ...
        while 1:
            ...
            if len(tail) == 0:
                ...
                if _DCCSI_GDEBUG:
                    _LOGGER.debug(
                        "~Not able to find the path to that file "
...
def resolve_envar_path(
    ...
    r"""This is meant to resolve a '\dev' path for LY
    ...
    try:
        ...
        _LOGGER.info(r"Not a lumbertard \dev path: {0}".format(e))
        raise EnvironmentError
...
def stash_env(_SYNTH_ENV_DICT=OrderedDict()):
    ...
    _G_COMPANY_NAME = os.getenv(ENVAR_COMPANY, TAG_DEFAULT_COMPANY)
    ...
    _O3DE_DEV = resolve_envar_path(
        ENVAR_O3DE_DEV,  # envar
    ...
    _O3DE_BUILD_DIR_NAME = os.getenv(
        ENVAR_O3DE_BUILD_DIR_NAME, TAG_DIR_O3DE_BUILD_FOLDER
    )
    ...
    # -- envar --
    _PATH_O3DE_BUILD = Path(os.getenv(ENVAR_PATH_O3DE_BUILD, PATH_O3DE_BUILD))
    _SYNTH_ENV_DICT[ENVAR_PATH_O3DE_BUILD] = _PATH_O3DE_BUILD.as_posix()

    # -- envar --
    _PATH_O3DE_BIN = Path(os.getenv(ENVAR_PATH_O3DE_BIN, PATH_O3DE_BIN))
    ...
    _PATH_O3DE_PROJECT = Path(
        os.getenv(ENVAR_PATH_O3DE_PROJECT, Path(_O3DE_DEV, _O3DE_PROJECT))
    )
    _SYNTH_ENV_DICT[ENVAR_PATH_O3DE_PROJECT] = _PATH_O3DE_PROJECT.as_posix()

    # -- envar --
    _PATH_DCCSIG = resolve_envar_path(
        ENVAR_PATH_DCCSIG,  # envar
        _THIS_MODULE_PATH,  # search path
    ...
    # -- envar --
    _AZPY_PATH = Path(
        os.getenv(ENVAR_DCCSI_AZPY_PATH, Path(_PATH_DCCSIG, TAG_DIR_DCCSI_AZPY))
    )
    _SYNTH_ENV_DICT[ENVAR_DCCSI_AZPY_PATH] = _AZPY_PATH.as_posix()

    # -- envar --
    _PATH_DCCSI_TOOLS = Path(
        os.getenv(ENVAR_PATH_DCCSI_TOOLS, Path(_PATH_DCCSIG, TAG_DIR_DCCSI_TOOLS))
    )
    _SYNTH_ENV_DICT[ENVAR_PATH_DCCSI_TOOLS] = _PATH_DCCSI_TOOLS.as_posix()

    # -- envar --
    # external dccsi site-packages
    _PATH_DCCSI_PYTHON_LIB = Path(
        os.getenv(ENVAR_PATH_DCCSI_PYTHON_LIB, PATH_DCCSI_PYTHON_LIB)
    )
    ...
    _DEFAULT_PY_PATH = Path(os.getenv(ENVAR_DCCSI_PY_DEFAULT, _DEFAULT_PY_PATH))
    _SYNTH_ENV_DICT[ENVAR_DCCSI_PY_DEFAULT] = _DEFAULT_PY_PATH.as_posix()

    # -- envar --
    # wing ide vars
    _WINGHOME_DEFAULT_PATH = Path(os.getenv(ENVAR_WINGHOME, PATH_DEFAULT_WINGHOME))
...
def init_ly_pyside(env_dict=_SYNTH_ENV_DICT):
    ...
    # -- envar --
    _QTFORPYTHON_PATH = Path(os.getenv(ENVAR_QTFORPYTHON_PATH, PATH_QTFORPYTHON_PATH))
    # some of these need hard checks
    if not _QTFORPYTHON_PATH.exists():
        raise Exception(
            "QTFORPYTHON_PATH does NOT exist: {0}".format(_QTFORPYTHON_PATH)
        )
    else:
    ...
    # -- envar --
    _QT_PLUGIN_PATH = Path(os.getenv(ENVAR_QT_PLUGIN_PATH, PATH_QT_PLUGIN_PATH))
    # some of these need hard checks
    if not _QT_PLUGIN_PATH.exists():
        ...
        os.environ["PATH"] = (
            _QT_PLUGIN_PATH.as_posix() + os.pathsep + os.environ["PATH"]
        )

    QTFORPYTHON_PATH = Path.joinpath(
        O3DE_DEV, "Gems", "QtForPython", "3rdParty", "pyside2", "windows", "release"
    ).resolve()
    ...
    PATH_O3DE_BIN = Path.joinpath(O3DE_DEV, "windows", "bin", "profile").resolve()
    ...
    os.environ["DYNACONF_QT_QPA_PLATFORM_PLUGIN_PATH"] = str(
    ...
    if sys.platform.startswith("win"):
        ...
        newPath += (
            str(Path.joinpath(QTFORPYTHON_PATH, "shiboken2").resolve()) + os.pathsep
        )
        newPath += (
            str(Path.joinpath(QTFORPYTHON_PATH, "PySide2").resolve()) + os.pathsep
        )
        newPath += path
...
def test_Qt():
    ...
    try:
        buttonFlags = QtWidgets.QMessageBox.information(
            QtWidgets.QApplication.activeWindow(), "title", "ok"
        )
        qapp.instance().quit
        qapp.exit()
    except Exception as e:
    ...
    for opt, arg in opts:
        if opt == "-h":
            ...
            try:
                ...
                _LOGGER.info(str(env_dict_object.to_json(sort_keys=False, indent=4)))
            except Exception as e:
                _LOGGER.error("ERROR: {0}".format(e))
                raise e
...
if __name__ == "__main__":
    ...
    if _DCCSI_GDEBUG:
        ...
        # -- BOX STORE ------
        _LOGGER.info(str(_SYNTH_ENV_DICT.to_json(sort_keys=False, indent=4)))
        try:
            # _SYNTH_ENV_DICT.to_json(filename=None, encoding='utf-8', errors='strict')
            # from os import fspath
            # tempBoxJsonFilePath = fspath(tempBoxJsonFilePath)
            _SYNTH_ENV_DICT.to_json(
                filename=tempBoxJsonFilePath.as_posix(), sort_keys=False, indent=4
            )
        ...
        parseJsonBox = Box.from_json(
            filename=tempBoxJsonFilePath,
            encoding="utf-8",
            errors="strict",
        ...
        _LOGGER.info(
            json.dumps(parseJsonBox, indent=4, sort_keys=False, ensure_ascii=False)
        )
