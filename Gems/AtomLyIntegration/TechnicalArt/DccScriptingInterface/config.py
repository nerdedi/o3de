...
try:
    ...
    _LOGGER.warning("Could not import dynaconf")
    _LOGGER.warning(
        "Most likely the application loading this module does not have DCCsi pkg dependancies installed. Use foundation.py to install pkgs for "
    )
...
def add_path_list_to_envar(path_list=_DCCSI_SYS_PATH, envar="PATH"):
    """!
...
# An improvement would be to move this to azpy.config_utils
# and be refactored to utilize py3+ fstrings
def add_path_list_to_addsitedir(path_list=_DCCSI_PYTHONPATH, envar="PYTHONPATH"):
    """ "!
...
def init_o3de_pyside2(dccsi_sys_path=_DCCSI_SYS_PATH):
    ...
    _DCCSI_LOCAL_SETTINGS["QT_QPA_PLATFORM_PLUGIN_PATH"] = (
        QT_QPA_PLATFORM_PLUGIN_PATH.as_posix()
    )
...
def init_o3de_pyside2_tools():
    ...
    if _DCCSI_PYSIDE2_TOOLS.exists():
        ...
        os.environ["PATH"] = (
            _DCCSI_PYSIDE2_TOOLS.as_posix() + os.pathsep + os.environ["PATH"]
        )
...
def init_o3de_core(
    ...
    os.environ["DYNACONF_PATH_O3DE_PYTHON_SCRIPTS"] = (
        PATH_O3DE_PYTHON_SCRIPTS.as_posix()
    )
    ...
    if engine_bin_path:
        ...
        _LOGGER.warning(
            "The engine binary folder not set/found, "
            "method input var: engine_bin_path, "
            "the ENVAR is PATH_O3DE_BIN"
        )
    ...
    _DCCSI_LOG_PATH = Path(
        PATH_DCCSI_LOG_PATH.format(
            PATH_O3DE_PROJECT=project_path, TAG_DCCSI_NICKNAME=TAG_DCCSI_NICKNAME
        )
    )
...
def init_o3de_python(
    ...
    _PATH_DCCSI_PYTHON_LIB = (
        DccScriptingInterface.azpy.config_utils.bootstrap_dccsi_py_libs(_PATH_DCCSIG)
    )
    os.environ["PATH_DCCSI_PYTHON_LIB"] = str(_PATH_DCCSI_PYTHON_LIB.as_posix())
    _DCCSI_LOCAL_SETTINGS["PATH_DCCSI_PYTHON_LIB"] = str(
        _PATH_DCCSI_PYTHON_LIB.as_posix()
    )
    ...
    _LOGGER.info(
        "~   O3DE_PYTHONHOME - is now the folder containing O3DE python executable"
    )

    # this will always be the O3DE pythin interpreter
    _PATH_O3DE_PYTHON_INSTALL = Path(_O3DE_DEV, "python")
    os.environ["DYNACONF_PATH_O3DE_PYTHON_INSTALL"] = str(
        _PATH_O3DE_PYTHON_INSTALL.as_posix()
    )
...
def get_config_settings(
    ...
    dccsi_sys_path, settings = init_o3de_core(
        dccsi_sys_path=_DCCSI_SYS_PATH, dccsi_pythonpath=_DCCSI_PYTHONPATH
    )
...
def export_settings(
    ...
    if not use_dynabox:
        ...
        if log_settings:
            _LOGGER.info(str(_settings_box.to_json(sort_keys=True, indent=4)))

        # writes settings box
        _settings_box.to_json(
            filename=_settings_file.as_posix(), sort_keys=True, indent=4
        )
...
settings = get_config_settings(
    enable_o3de_python=False, enable_o3de_pyside2=False, set_env=True
)
...
if __name__ == "__main__":
    ...
    _logging.basicConfig(
        level=DCCSI_LOGLEVEL, format=FRMT_LOG_LONG, datefmt="%m-%d %H:%M"
    )
    ...
    if args.set_debugger:
        _LOGGER.info(
            "Setting and switching debugger type not implemented (default=WING)"
        )
    ...
    settings = get_config_settings(
        enable_o3de_python=args.enable_python, enable_o3de_pyside2=args.enable_qt
    )
    ...
    if args.enable_python:
        _LOGGER.info(STR_CROSSBAR)
        _LOGGER.info("PATH_DCCSI_PYTHON")
        _LOGGER.info(
            "PATH_DCCSI_PYTHON_LIB: {}".format(
                _DCCSI_LOCAL_SETTINGS["PATH_DCCSI_PYTHON_LIB"]
            )
        )
        ...
        _LOGGER.info(
            "Tip: add arg --enable-python (-py) to extend the environment with O3DE python access"
        )
    ...
    if args.enable_qt:
        ...
        # _LOGGER.info('QTFORPYTHON_PATH: {}'.format(settings.QTFORPYTHON_PATH))
        _LOGGER.info(
            "QT_PLUGIN_PATH: {}".format(_DCCSI_LOCAL_SETTINGS["QT_PLUGIN_PATH"])
        )
        _LOGGER.info(
            "QT_QPA_PLATFORM_PLUGIN_PATH: {}".format(
                _DCCSI_LOCAL_SETTINGS["QT_QPA_PLATFORM_PLUGIN_PATH"]
            )
        )
        try:
        ...
        _LOGGER.info(
            "Tip: add arg --enable-qt (-qt) to extend the environment with O3DE Qt/PySide2 support"
        )
        _LOGGER.info(
            "Tip: add arg --test-pyside2 (-tp) to test the O3DE Qt/PySide2 support"
        )
    ...
    if DCCSI_GDEBUG or args.cache_local_settings:
        ...
        if args.export_settings:
            ...
            export_settings(settings=settings, log_settings=log_settings)
            # this should be set if there are local settings!?
            _LOGGER.debug(
                "DCCSI_LOCAL_SETTINGS: {}".format(settings.DCCSI_LOCAL_SETTINGS)
            )
    ...
    if DCCSI_GDEBUG or args.test_pyside2:
        ...
        try:
            ...
            _LOGGER.warning(
                "Refer to: '{}/3rdParty/Python/README.txt'".format(settings.PATH_DCCSIG)
            )
            _LOGGER.error(e)
