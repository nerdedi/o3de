...
STR_CROSSBAR = f"{('-' * 74)}"
FRMT_LOG_LONG = (
    "[%(name)s][%(levelname)s] >> %(message)s (%(asctime)s; %(filename)s:%(lineno)d)"
)
...
_logging.basicConfig(level=DCCSI_LOGLEVEL, format=FRMT_LOG_LONG, datefmt="%m-%d %H:%M")
...
STR_DCCSI_PYTHON_LIB = (
    f"{PATH_DCCSIG.as_posix()}"
    + "\\3rdParty\\Python\\Lib"
...
PATH_DCCSI_PYTHON_LIB = Path(
    os.getenv(ENVAR_PATH_DCCSI_PYTHON_LIB, str(PATH_DCCSI_PYTHON_LIB))
)
...
try:
    ...
    _LOGGER.debug(
        f"{ENVAR_PATH_DCCSI_PYTHON_LIB} is: {PATH_DCCSI_PYTHON_LIB.as_posix()}"
    )
except NotADirectoryError as e:
    _LOGGER.warning(
        f"{ENVAR_PATH_DCCSI_PYTHON_LIB} does not exist:"
        + f"{PATH_DCCSI_PYTHON_LIB.as_Posix()}"
    )
    _LOGGER.warning("Pkg dependencies may not be available for import")
    _LOGGER.warning(
        "Try using foundation.py to install pkg dependencies for the target python runtime"
    )
    _LOGGER.info('>.\\python foundation.py -py="c:\\path\to\\som\\python.exe"')
...
PATH_DCCSI_SETTINGS_LOCAL = Path.joinpath(
    PATH_DCCSIG, DCCSI_SETTINGS_LOCAL_FILENAME
).resolve()
...
O3DE_MANIFEST_PATH = Path(USER_HOME, TAG_USER_O3DE, SLUG_MANIFEST_FILENAME).resolve()
...
if not PATH_O3DE_3RDPARTY.exists():
    _LOGGER.warning(
        f"Default o3de 3rdparty does not exist: {PATH_O3DE_3RDPARTY.as_posix()}"
    )
    _LOGGER.warning(
        "The engine may not be installed, or you need to run it. The o3de user home needs to be initialized (start o3de.exe)."
    )
...
try:
    ...
    _LOGGER.warning(f"{e}")
    _LOGGER.warning("Not running o3do, no: azlmbr.paths")
    _LOGGER.warning("Using fallbacks ...")
#     if DCCSI_STRICT:
#         _LOGGER.exception(f'{e} , traceback =', exc_info=True)
#         raise e
# -------------------------------------------------------------------------
...
if not PATH_DCCSI_SETTINGS_LOCAL.exists():
    ...
    _LOGGER.info("Open a CMD at root of DccScriptingInterface, then run:")
    _LOGGER.info(r">.\python config.py")
...
if DCCSI_SETTINGS_DATA:
    _LOGGER.debug(
        f"O3DE {DCCSI_SETTINGS_LOCAL_FILENAME} found: {PATH_DCCSI_SETTINGS_LOCAL}"
    )
...
try:
    ...
    _LOGGER.warning("Make sure the engine is installed, and run O3DE.exe")
    _LOGGER.warning("Or build from source and and register the engine ")
    _LOGGER.warning("CMD > c:\\path\\to\\o3de\\scripts\\o3de register --this-engine")
...
if not O3DE_DEV:
    ...
    if O3DE_MANIFEST_DATA:
        ...
        if ENGINES:
            if len(ENGINES) < 1:
                _LOGGER("no engines in o3de manifest")
            ...
            # what if there are multiple engines? We don't know which to use
                ...
                _LOGGER.warning(
                    f"Manifest defines more then one engine: {O3DE_DEV.as_posix()}"
                )
                _LOGGER.warning("Not sure which to use? We suggest...")
                _LOGGER.warning(
                    f"Put 'set {ENVAR_O3DE_DEV}=c:\\path\\to\\o3de' in: {PATH_ENV_DEV}"
                )
                _LOGGER.warning(
                    f"And '{ENVAR_O3DE_DEV}:c:\\path\\to\\o3de' in: {PATH_DCCSI_SETTINGS_LOCAL}"
                )

            try:
                O3DE_DEV.resolve(strict=True)  # make sure the found engine exists
                _LOGGER.debug(
                    f"O3DE Manifest {ENVAR_O3DE_DEV} is: {O3DE_DEV.as_posix()}"
                )
            except NotADirectoryError as e:
                _LOGGER.warning(
                    f"Manifest engine does not exist: {O3DE_DEV.as_posix()}"
                )
                _LOGGER.warning("Make sure the engine is installed, and run O3DE.exe")
                _LOGGER.warning("Or build from source and and register the engine ")
                _LOGGER.warning(
                    "CMD > c:\\path\\to\\o3de\\scripts\\o3de register --this-engine"
                )
                _LOGGER.error(f"{e} , traceback =", exc_info=True)
                O3DE_DEV = None
...
if not PATH_O3DE_PROJECT:
    ...
    if O3DE_MANIFEST_DATA:
        ...
        if len(PROJECTS) == 1:  # there can only be one
            ...
            _LOGGER.warning("Manifest defines more then one project")
            _LOGGER.warning("Not sure which to use? We suggest...")
            _LOGGER.warning(
                f'Put "set {ENVAR_PATH_O3DE_PROJECT}=C:\\path\\to\\o3de\\project" in: {PATH_ENV_DEV}'
            )
            _LOGGER.warning(
                f'And "{ENVAR_PATH_O3DE_PROJECT}":"C:\\path\\to\\o3de\\project" in: {PATH_DCCSI_SETTINGS_LOCAL}'
            )
...
if not PATH_O3DE_PROJECT:
    ...
    if O3DE_BOOTSTRAP_DATA:
        try:
            PATH_O3DE_PROJECT = Path(
                O3DE_BOOTSTRAP_DATA["Amazon"]["AzCore"]["Bootstrap"]["project_path"]
            )
...
if PATH_O3DE_PROJECT:  # could still end up None?
    ...
    try:
        ...
        _LOGGER.warning(f"{e}")
        _LOGGER.warning(
            f"envar {ENVAR_O3DE_PROJECT} may not exist: {ENVAR_O3DE_PROJECT}"
        )
        _LOGGER.warning(
            f'Put "set {ENVAR_O3DE_PROJECT}=C:\\path\\to\\o3de" in : {PATH_ENV_DEV}'
        )
        _LOGGER.warning(
            f'And "{ENVAR_O3DE_PROJECT}":"C:\\path\\to\\o3de" in: {PATH_DCCSI_SETTINGS_LOCAL}'
        )
...
O3DE_PROJECT = str(os.getenv(ENVAR_O3DE_PROJECT, PATH_O3DE_PROJECT.name))
...
if PATH_O3DE_BIN:
    ...
    try:
        ...
        _LOGGER.warning(
            f"{ENVAR_PATH_O3DE_BIN} may not exist: {PATH_O3DE_BIN.as_posix()}"
        )
        _LOGGER.error(f"{e} , traceback =", exc_info=True)
        PATH_O3DE_BIN = None
        if DCCSI_STRICT:
    ...
    try:
        ...
        _LOGGER.warning(
            f'Put "set {ENVAR_PATH_O3DE_BIN}=C:\\path\\to\\o3de" in: {PATH_ENV_DEV}'
        )
        _LOGGER.warning(
            f'And "{ENVAR_PATH_O3DE_BIN}":"C:\\path\\to\\o3de" in: {PATH_DCCSI_SETTINGS_LOCAL}'
        )
        _LOGGER.error(f"{e} , traceback =", exc_info=True)
        PATH_O3DE_BIN = None
        if DCCSI_STRICT:
            ...
            _LOGGER.warning(
                f"some modules functionality may fail if no {ENVAR_PATH_O3DE_BIN} is defined"
            )
...
if not PATH_O3DE_3RDPARTY:
    ...
    try:
        PATH_O3DE_3RDPARTY = USER_HOME.joinpath(f"{TAG_USER_O3DE}", "3rdparty").resolve(
            strict=True
        )
    except NotADirectoryError as e:
        _LOGGER.warning("The engine may not be built yet ...")
        _LOGGER.warning("Or User is not using an installer build ...")
        _LOGGER.error(f"{e} , traceback =", exc_info=True)
    ...
    try:
        ...
        _LOGGER.warning(
            f"{ENVAR_PATH_O3DE_3RDPARTY} will not resolve: {PATH_O3DE_3RDPARTY}"
        )
        _LOGGER.warning(
            f'Put "set {ENVAR_PATH_O3DE_3RDPARTY}=C:\\path\\to\\o3de" in: {PATH_ENV_DEV}'
        )
        _LOGGER.warning(
            f'And "{ENVAR_PATH_O3DE_3RDPARTY}":"C:\\path\\to\\o3de" in: {PATH_DCCSI_SETTINGS_LOCAL}'
        )
        _LOGGER.error(f"{e} , traceback =", exc_info=True)
        if DCCSI_STRICT:
            _LOGGER.exception(f"{e} , traceback =", exc_info=True)
...
try:
    ...
    _LOGGER.error("Could not import dynaconf")
    _LOGGER.info(
        "Most likely python package dependencies are not installed for target runtime"
    )
    _LOGGER.info(f"Py EXE  is:  {sys.executable}")
    _LOGGER.info(
        f"The Python version running: {sys.version_info[0]}.{sys.version_info[1]}"
    )
    _LOGGER.info(f"{ENVAR_PATH_DCCSI_PYTHON_LIB} location is: {PATH_DCCSI_PYTHON_LIB}")
    _LOGGER.info("Follow these steps then re-start the DCC app (or other target):")
    _LOGGER.info("1. open a cmd prompt")
    _LOGGER.info(f"2. change directory to: {PATH_DCCSIG}")
    _LOGGER.info("3. run this command...")
    _LOGGER.info(rf'4. >.\python foundation.py -py="{sys.executable}"')
    _LOGGER.error(f"{e} , traceback =", exc_info=True)
    pass  # be forgiving
