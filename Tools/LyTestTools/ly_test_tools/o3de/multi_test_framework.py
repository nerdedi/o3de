...
from ly_test_tools._internal.exceptions import (
    EditorToolsFrameworkException,
    TestResultException,
)
...
class Result(object):
    ...
    class Pass(ResultType):
        def __init__(
            self, test_spec: type(AbstractTestBase), output: str, log_output: str
        ):
            """
...
class MultiTestSuite(object):
    ...
    class MultiTestCollector(pytest.Class):
            ...
            def single_run(
                ...
                if is_single_test:
                    ...
                    if not isinstance(wrap, types.GeneratorType):
                        raise EditorToolsFrameworkException(
                            "wrap_run must return a generator, did you forget 'yield'?"
                        )
                    next(wrap, None)
                    # Setup step
                    inner_test_spec.setup(self, request, workspace)
                # Run
                self._run_single_test(
                    request, workspace, collected_test_data, inner_test_spec
                )
                if is_single_test:
                    # Teardown
        ...
        @staticmethod
        def _create_runner(
            runner_name: str, function: typing.Callable, tests: list, cls
        ) -> MultiTestSuite.Runner:
            """
            ...
            def make_shared_run():
                @MultiTestSuite.MultiTestCollector._set_marks(
                    {"runner": target_runner, "run_type": "run_shared"}
                )
                def shared_run(
        ...
        def collect(self):
            ...
            batched_tests = cls.filter_shared_tests(
                shared_tests, is_parallelizable=False, is_batchable=True
            )
            parallel_tests = cls.filter_shared_tests(
                shared_tests, is_parallelizable=True, is_batchable=False
            )
            parallel_batched_tests = cls.filter_shared_tests(
                shared_tests, is_parallelizable=True, is_batchable=True
            )
            ...
            for test_spec in single_tests:
                ...
                single_run_test = MultiTestSuite.MultiTestCollector._make_single_run(
                    test_spec
                )
            ...
            runners.append(
                self._create_runner(
                    "run_batched_tests", cls._run_batched_tests, batched_tests, cls
                )
            )
            runners.append(
                self._create_runner(
                    "run_parallel_tests", cls._run_parallel_tests, parallel_tests, cls
                )
            )
            runners.append(
                self._create_runner(
                    "run_parallel_batched_tests",
                    cls._run_parallel_batched_tests,
                    parallel_batched_tests,
                    cls,
                )
            )
            ...
            collected_run_pytestfuncs = [
                ...
                getattr(item.obj, "marks", {}).setdefault("run_type", None)
                == "run_shared"
            ]
            collected_result_pytestfuncs = [
                item
                for item in collection
                if getattr(item.obj, "marks", {}).setdefault("run_type", None)
                == "result"
            ]
    ...
    def _get_number_parallel_executables(
        self, request: _pytest.fixtures.FixtureRequest
    ) -> int:
        """
        ...
        """
        parallel_executables_value = request.config.getoption(
            "--parallel-executables", None
        )
        ...
        for runner in cls._runners:
            ...
            if len(runner.tests) > 0:
                ...
                for result_pytestfunc in runner.result_pytestfuncs:
                    found_test = next(
                        (item for item in items if item == result_pytestfunc), None
                    )
                    if found_test:
    ...
    def collected_test_data(
        self, request: _pytest.fixtures.FixtureRequest
    ) -> MultiTestSuite.TestData:
        """
    ...
    def get_single_tests(cls) -> list[_single_test_class]:
        ...
        single_tests = [
            c[1]
            for c in cls.__dict__.items()
            if inspect.isclass(c[1]) and issubclass(c[1], cls._single_test_class)
        ]
        return single_tests
    ...
    def get_shared_tests(cls) -> list[_shared_test_class]:
        ...
        shared_tests = [
            c[1]
            for c in cls.__dict__.items()
            if inspect.isclass(c[1]) and issubclass(c[1], cls._shared_test_class)
        ]
        return shared_tests

    @classmethod
    def get_session_shared_tests(
        cls, session: _pytest.main.Session
    ) -> list[_shared_test_class]:
        """
    ...
    def filter_session_shared_tests(
        session_items: list[_pytest.python.Function(_shared_test_class)],
        shared_tests: list[_shared_test_class],
    ) -> list[_shared_test_class]:
        """
        ...
        selected_shared_tests = [
            ...
            if test.__name__ in session_items_by_name.keys()
            and will_run(session_items_by_name[test.__name__])
        ]
        return selected_shared_tests

    @staticmethod
    def filter_shared_tests(
        shared_tests: list[_shared_test_class],
        ...
        return [
            ...
            if (
                ...
                and getattr(test, "is_parallelizable", None) is is_parallelizable
            )
        ]
    ...
    def _setup_test(self, workspace, collected_test_data):
        ...
        if self.atom_tools_executable_name:  # Atom Tools test.
            self.executable = launcher_helper.create_atom_tools_launcher(
                workspace, self.atom_tools_executable_name
            )
        else:  # Editor test.
        ...
        editor_utils.prepare_asset_processor(
            workspace,
            collected_test_data,
            extra_params=self.global_assetprocessor_extra_cmdline_args,
        )
        editor_utils.kill_all_ly_processes(include_asset_processor=False)
    ...
    def _setup_cmdline_args(self, cmdline_args, executable, test_spec_list, workspace):
        ...
        if type(executable) in [
            WinEditor,
            LinuxEditor,
        ]:  # Handle Editor CLI args since we need workspace context to populate them.
            test_cmdline_args += [
    ...
    def _run_single_test(
        ...
        if result is None:
            logger.error(
                f"Unexpectedly found no test run in the {self.log_name} during {test_spec}"
            )
            result = {
                "Unknown": Result.Unknown(
                    test_spec=test_spec,
    ...
    def _exec_single_test(
        ...
        """
        test_cmdline_args = self._setup_cmdline_args(
            cmdline_args, executable, [test_spec], workspace
        )
        ...
        if type(executable) in [WinEditor, LinuxEditor]:
            ...
            cmdline = [
                ...
                f"@log@/{log_name}",
                "-project-log-path",
                log_path_function(run_id, workspace),
            ] + test_cmdline_args
            ...
            cmdline = [
                ...
                test_filename,
                "-logfile",
                os.path.join(log_path_function(run_id, workspace), log_name),
            ] + test_cmdline_args
        ...
        try:
            ...
            if return_code == 0:
                ...
                if has_crashed:
                    crash_output = editor_utils.retrieve_crash_output(
                        run_id, workspace, self._timeout_crash_log
                    )
                    test_result = Result.Crash(
                        test_spec, output, return_code, crash_output, None
                    )
                    ...
                    if os.path.exists(crash_file_name):
                        ...
                        logger.warning(
                            f"Crash occurred, but could not find log {crash_file_name}"
                        )
                else:
            ...
            test_result = Result.Timeout(
                test_spec, output, test_spec.timeout, executable_log_content
            )

        executable_log_content = log_content_function(run_id, log_name, workspace)
        results = self._get_results_using_output(
            [test_spec], output, executable_log_content
        )
    ...
    def _run_batched_tests(
        self,
        request: _pytest.fixtures.FixtureRequest,
        workspace: AbstractWorkspaceManager,
        collected_test_data: MultiTestSuite.TestData,
        test_spec_list: list[BatchedTest],
        extra_cmdline_args: list[str] = None,
    ) -> None:
        """
    ...
    def _run_parallel_tests(
        ...
        if not parallel_executables > 0:
            logger.warning(
                "Expected 1 or more parallel_executables, found 0. Setting to 1."
            )
        ...
        for iteration in range(num_iterations):
            tests_for_iteration = test_spec_list[
                iteration * parallel_executables : (iteration + 1)
            ...
            for i in range(total_threads):
                ...
                def make_parallel_test_func(test_spec, index, current_executable):
                    def run(request, workspace, extra_cmdline_args):
                        try:
                            ...
                            logger.warning(
                                f"Found exception trying to run Editor tests. Current log name is "
                                f"{self.log_name} and test is {str(test_spec)}"
                            )
                            raise e
                        if not results:
                            raise EditorToolsFrameworkException(
                                f"Results not found. Current log name is "
                ...
                current_executable = self.executable.__class__(
                    workspace, self.executable.args.copy()
                )
                parallel_test_function = make_parallel_test_func(
                    tests_for_iteration[i], i, current_executable
                )
                parallel_test_thread = threading.Thread(
                    target=parallel_test_function,
                    args=(request, workspace, extra_cmdline_args),
                )
            ...
            self._test_reporting(
                collected_test_data, results_per_thread, workspace, ParallelTest
            )
    ...
    def _run_parallel_batched_tests(
        self,
        request: _pytest.fixtures.FixtureRequest,
        workspace: AbstractWorkspaceManager,
        collected_test_data: MultiTestSuite.TestData,
        ...
        for iteration in range(total_threads):
            tests_for_thread = test_spec_list[
                iteration * tests_per_executable : (iteration + 1)
                * tests_per_executable
            ]

            def make_shared_test_function(
                test_spec_list_for_executable, index, current_executable
            ):
                def run(request, workspace, extra_cmdline_args):
                    results = None
            ...
            shared_test_function = make_shared_test_function(
                tests_for_thread, iteration, current_executable
            )
            shared_test_thread = threading.Thread(
                target=shared_test_function,
                args=(request, workspace, extra_cmdline_args),
            )
        ...
        self._test_reporting(
            collected_test_data, results_per_thread, workspace, SharedTest
        )
    ...
    def _exec_multitest(
        ...
        """
        test_cmdline_args = self._setup_cmdline_args(
            cmdline_args, executable, test_spec_list, workspace
        )
        ...
        if type(executable) in [WinEditor, LinuxEditor]:
            ...
            for test_spec in test_spec_list:
                # Test script
                test_script_list += (
                    editor_utils.get_testcase_module_filepath(test_spec.test_module)
                    + ";"
                )
            ...
            temp_batched_script_file = tempfile.NamedTemporaryFile(
            ...
            temp_batched_case_file = tempfile.NamedTemporaryFile(
                mode="w", delete=False, suffix=".txt"
            )
            ...
            cmdline = [
                "-runpythontest",
                temp_batched_script_file.name,
                "-pythontestcase",
                temp_batched_case_file.name,
                "-logfile",
                f"@log@/{log_name}",
                "-project-log-path",
                log_path_function(run_id, workspace),
        ...
        try:
            ...
            try:
                ...
                path_to_artifact = os.path.join(
                    log_path_function(run_id, workspace), log_name
                )
                workspace.artifact_manager.save_artifact(
                    path_to_artifact, full_log_name
                )
            ...
            if return_code == 0:
                ...
                results = self._get_results_using_output(
                    test_spec_list, output, executable_log_content
                )
                if not len(results) == len(test_spec_list):
                    ...
                    raise EditorToolsFrameworkException(
                        "Error when retrieving test results, the number of results "
                        ...
                        if isinstance(result, Result.Unknown):
                            if not crashed_result:
                                ...
                                results[test_spec_name].extra_info = (
                                    f"This test has unknown result,"
            ...
            for test_spec_name, result in results.items():
                if isinstance(result, Result.Unknown):
                    if not timed_out_result:
                        ...
                        # update with info about the offender
                        results[test_spec_name].extra_info = (
                            f"This test has unknown result, test "
    ...
    def _get_results_using_output(
        test_spec_list: list[AbstractTestBase], output: str, log_output: str
    ) -> dict[any, [Result.Unknown, Result.Pass, Result.Fail]]:
        """
        ...
        for m in out_matches:
            try:
                ...
                Exception
            ):  # Intentionally broad to avoid failing if the output data is corrupt
        ...
        for m in log_matches:
            try:
                ...
                Exception
            ):  # Intentionally broad, to avoid failing if the log data is corrupt
                logger.warning("Error reading result JSON", exc_info=True)
                continue
