...
class TestSetTestName(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_artifact_manager = (
            ly_test_tools._internal.managers.artifact_manager.ArtifactManager(
                self.mock_root
            )
        )

    @mock.patch(
        "ly_test_tools._internal.managers.artifact_manager.ArtifactManager.set_dest_path"
    )
    ...
    def test_SetDestPath_NoPathExists_CreatesPathSetsAttributes(self):
        ...
        self.mock_artifact_manager.dest_path = (
            None  # Reset the value to test the function
        )
    ...
    @mock.patch("os.path.exists", mock.MagicMock(return_value=False))
    @mock.patch(
        "ly_test_tools._internal.managers.artifact_manager.ArtifactManager._get_collision_handled_filename"
    )
    def test_SetDestPathAmount_IsCalled_CallsGetCollision(self, mock_get_collision):
        ...
        mock_get_collision.assert_called_once_with(
            os.path.join(self.mock_root, test_name), create_amount
        )

    @mock.patch("os.makedirs", mock.MagicMock())
    @mock.patch("os.path.exists", mock.MagicMock(return_value=True))
    def test_SetDestPathAmount_ValidFilePathExists_NoFilePathCreatedSetsAttributes(
        self,
    ):
        test_name = "dummy"
        expected_path = os.path.join(
            self.mock_artifact_manager.artifact_path, test_name
        )
...
class TestSaveArtifact(unittest.TestCase):
    ...
    def test_SaveArtifact_ArtifactNameIsNotNone_CopyTreeCalledCorrectly(
        self, mock_reducer, mock_path_isdir, mock_copy_tree
    ):
        test_name = "dummy"
        ...
        mock_copy_tree.assert_called_once_with(
            updated_path,
            os.path.join(updated_path, mock_artifact_name[:-5]),
            dirs_exist_ok=True,
        )
        mock_reducer.assert_called_once_with(
            file_name=mock_artifact_name, max_length=25
        )
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("ly_test_tools.environment.file_system.reduce_file_name_length")
    def test_SaveArtifact_ArtifactNameIsNone_CopyTreeCalledCorrectly(
        self, mock_reducer, mock_path_isdir, mock_copy_tree
    ):
        ...
        self.mock_artifact_manager.save_artifact(
            self.mock_artifact_manager.artifact_path, mock_artifact_name
        )
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("ly_test_tools.environment.file_system.reduce_file_name_length")
    def test_SaveArtifact_ArtifactNameIsNotNone_CopyCalledCorrectly(
        self, mock_reducer, mock_path_isdir, mock_copy
    ):
        mock_artifact_name = "mock_artifact_name"
        ...
        mock_copy.assert_called_once_with(
            self.mock_artifact_manager.artifact_path,
            os.path.join(
                self.mock_artifact_manager.artifact_path, mock_artifact_name[:-5]
            ),
        )
        mock_reducer.assert_called_once_with(
            file_name=mock_artifact_name, max_length=25
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("ly_test_tools.environment.file_system.reduce_file_name_length")
    def test_SaveArtifact_ArtifactNameIsNone_CopyCalledCorrectly(
        self, mock_reducer, mock_path_isdir, mock_copy
    ):
        mock_artifact_name = None
        mock_path_isdir.return_value = False
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("ly_test_tools.environment.file_system.reduce_file_name_length")
    def test_SaveArtifact_ArtifactNameIsNotNone_ChmodCalledCorrectly(
        self, mock_reducer, mock_path_isdir, mock_chmod
    ):
        mock_artifact_name = "mock_artifact_name"
        mock_path_isdir.return_value = False
        mock_reducer.return_value = mock_artifact_name[:-5]
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("ly_test_tools.environment.file_system.reduce_file_name_length")
    def test_SaveArtifact_ArtifactNameIsNone_ChmodCalledCorrectly(
        self, mock_reducer, mock_path_isdir, mock_chmod
    ):
        mock_artifact_name = None
        mock_path_isdir.return_value = False
    ...
    def test_SaveArtifact_DestinationCollides_CallsGetCollisionHandledFilename(
        self, under_test
    ):
        self.mock_artifact_manager.save_artifact(
            self.mock_artifact_manager.artifact_path
...
class TestGenerateArtifactFileName(unittest.TestCase):
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("ly_test_tools.environment.file_system.reduce_file_name_length")
    def test_GenerateFileName_HasArtifactName_ReturnsFilePath(
        self, mock_reducer, mock_path_isdir
    ):
        mock_artifact_name = "mock_artifact_name"
        mock_path_isdir.return_value = True

        mock_artifact_file = self.mock_artifact_manager.generate_artifact_file_name(
            mock_artifact_name
        )
        ...
        assert mock_artifact_file == os.path.join(
            self.mock_artifact_manager.artifact_path, mock_artifact_name
        ), (
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("ly_test_tools.environment.file_system.reduce_file_name_length")
    def test_GenerateFileName_ArtifactNameIsNone_ReturnsFilePath(
        self, mock_reducer, mock_path_isdir
    ):
        mock_artifact_name = None
        mock_path_isdir.return_value = True
...
class TestGatherArtifacts(unittest.TestCase):
    ...
    def test_TestGatherArtifacts_TestNameIsNone_MakeArchiveCalled(
        self, mock_make_archive
    ):
        ...
        mock_make_archive.assert_called_once_with(
            mock_destination, "zip", self.mock_artifact_manager.artifact_path
        )

    @mock.patch("ly_test_tools.environment.file_system.sanitize_file_name")
    @mock.patch("shutil.make_archive")
    def test_TestGatherArtifacts_TestNameIsNotNone_MakeArchiveCalled(
        self, mock_make_archive, mock_sanitize_file_name
    ):
        test_name = "dummy"
        mock_destination = "mock_destination"
...
class TestGetCollisionHandledFilename(unittest.TestCase):
    ...
    def test_GetCollisionHandledFilename_FileNameExists_FileNameAppendsNumber(self):
        ...
        under_test = self.mock_artifact_manager._get_collision_handled_filename(
            test_name, create_amount
        )
    ...
    def test_GetCollisionHandledFilename_HasLargeAmountAndNotPathExists_EndsLoopEarly(
    ...
    @mock.patch("os.path.exists")
    def test_GetCollisionHandledFilename_HasLargeAmountAndCollides_ReachesMaxAmount(
        self, mock_path_exists
    ):
        mock_file_path = "foo"
        amount = 99
