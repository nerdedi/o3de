...
class TestCheckFreeSpace(unittest.TestCase):
    def setUp(self):
        ...
        self.disk_usage = psutil._common.sdiskusage(
            100 * file_system.ONE_GIB, 0, 100 * file_system.ONE_GIB, 100
        )
    ...
    def test_CheckFreeSpace_NoSpace_RaisesIOError(self, mock_disk_usage):
        ...
        mock_disk_usage.return_value = psutil._common.sdiskusage(
            total, used, total - used, used / total
        )
...
class TestSafeMakedirs(unittest.TestCase):
    ...
    @mock.patch("os.makedirs")
    def test_SafeMakedirs_RaisedOSErrorErrnoEEXIST_DoesNotPropagate(
        self, mock_makedirs
    ):
        error = OSError()
...
class TestGetNewestFileInDir(unittest.TestCase):
    ...
    @mock.patch("glob.iglob")
    def test_GetNewestFileInDir_ThreeResultsTwoExts_CtimeCalledSixTimes(
        self, mock_glob, mock_ctime
    ):
...
class TestRemovePathAndExtension(unittest.TestCase):
    def test_GetNewestFileInDir_ValidPath_ReturnsStripped(self):
        result = file_system.remove_path_and_extension(
            os.path.join("C", "packages", "lumberyard-XXXX.zip")
        )
...
class TestUnZip(unittest.TestCase):
    ...
    def test_Unzip_NotEnoughSpaceInDestination_FailsWithErrorMessage(
        self, mock_join, mock_check_free, mock_exists
    ):
        ...
        mock_check_free.assert_called_once_with(
            self.dest_path,
    ...
    @mock.patch("ly_test_tools.environment.file_system.check_free_space")
    @mock.patch("os.path.join")
    def test_Unzip_BuildDirExistsForceAndAllowExistsNotSet_CRITICALLogged(
        ...
        mock_log.log.assert_called_with(
            level, "Found existing {}.  Will not overwrite.".format(path)
        )

    @mock.patch("ly_test_tools.environment.file_system.logger")
    @mock.patch("os.path.exists")
    @mock.patch(
        "ly_test_tools.environment.file_system.check_free_space", mock.MagicMock()
    )
...
class TestUnTgz(unittest.TestCase):
    ...
    @mock.patch("os.path.join")
    @mock.patch("os.stat")
    def test_Untgz_ReleaseBuild_ReturnsCorrectPath(
        self, mock_stat, mock_join, mock_check_free
    ):
        build_name = "lumberyard-1.2.0.3-54321-pc-1234"
        expected_path = self.dest_path + "\\" + build_name
        mock_join.return_value = expected_path
    ...
    @mock.patch("os.path.join", mock.MagicMock())
    @mock.patch("os.stat")
    def test_Untgz_BuildDirExistsForceAndAllowExistsNotSet_CRITICALLogged(
        self, mock_stat, mock_exists, mock_log
    ):
        force = False
    ...
    def test_Untgz_AllowExiststSet_INFOLogged(self, mock_stat, mock_exists, mock_log):
        ...
        with mock.patch(self.decomp_obj_name, self.mock_decomp):
            path = self.call_decomp(
                self.dest_path, self.src_path, False, force, allow_exists
            )
    ...
    @mock.patch("os.path.join")
    @mock.patch("os.stat")
    def test_Untgz_BuildDirExistsForceSetTrue_INFOLogged(
        self, mock_stat, mock_join, mock_check_free, mock_exists, mock_log
    ):
        path = ""
        mock_exists.return_value = True
        mock_stat.return_value = self.src_stat
...
class TestChangePermissions(unittest.TestCase):
    ...
    @mock.patch("os.walk")
    @mock.patch("os.chmod")
    def test_ChangePermissions_DefaultValues_ChmodCalledCorrectly(
        self, mock_chmod, mock_walk
    ):
        os.walk.return_value = self.walk_iter
        file_system.change_permissions(".", 0o777)
        self.assertEqual(
            mock_chmod.mock_calls,
            [
                mock.call(os.path.join(self.root, self.dirs[0]), 0o777),
    ...
    @mock.patch("os.walk")
    @mock.patch("os.chmod")
    def test_ChangePermissions_DefaultValues_ReturnsTrueOnSuccess(
        self, mock_chmod, mock_walk
    ):
        os.walk.return_value = self.walk_iter
...
class TestUnlockFile(unittest.TestCase):
    ...
    def test_UnlockFile_WriteLocked_UnlockFile(
        self, mock_access, mock_chmod, mock_stat
    ):
    ...
    @mock.patch("os.chmod")
    @mock.patch("os.access")
    def test_UnlockFile_AlreadyUnlocked_LogAlreadyUnlocked(
        self, mock_access, mock_chmod, mock_stat
    ):
...
class TestLockFile(unittest.TestCase):
    ...
    @mock.patch("os.chmod")
    @mock.patch("os.access")
    def test_LockFile_UnlockedFile_FileLockedSuccessReturnsTrue(
        self, mock_access, mock_chmod, mock_stat
    ):
        mock_access.return_value = True
        os.stat.return_value = MockStatResult(stat.S_IREAD | stat.S_IWRITE)
    ...
    @mock.patch("os.chmod")
    @mock.patch("os.access")
    def test_LockFile_AlreadyLocked_FileLockedFailedReturnsFalse(
        self, mock_access, mock_chmod, mock_stat
    ):
        mock_access.return_value = False
        os.stat.return_value = MockStatResult(stat.S_IREAD)
...
class TestRemoveSymlinks(unittest.TestCase):
    ...
    @mock.patch("os.walk")
    @mock.patch("os.rmdir")
    def test_RemoveSymlinks_DefaultValues_RmdirCalledCorrectly(
        self, mock_rmdir, mock_walk
    ):
        os.walk.return_value = self.walk_iter
        ...
        self.assertEqual(
            mock_rmdir.mock_calls,
            [
                mock.call(os.path.join(self.root, self.dirs[0])),
    ...
    @mock.patch("os.walk")
    @mock.patch("os.rmdir")
    def test_RemoveSymlinks_OSErrnoEEXISTRaised_RaiseOSError(
        self, mock_rmdir, mock_walk
    ):
        os.walk.return_value = self.walk_iter
        error = OSError()
...
class TestDelete(unittest.TestCase):
    ...
    @mock.patch(
        "ly_test_tools.environment.file_system.change_permissions", mock.MagicMock()
    )
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("os.path.isfile")
    def test_ChangePermissions_OSErrorRaised_ReturnsZero(
        self, mock_isfile, mock_isdir, mock_remove, mock_rmtree, mock_chper
    ):
        mock_rmtree.side_effect = OSError()
        self.assertEqual(
            file_system.delete(self.path_list, del_files=True, del_dirs=True), False
        )
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("os.path.isfile")
    def test_ChangePermissions_DefaultValues_ReturnsLenOfList(
        self, mock_isfile, mock_isdir, mock_remove, mock_rmtree, mock_chper
    ):
        self.assertEqual(
            file_system.delete(self.path_list, del_files=True, del_dirs=True), True
        )
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("os.path.isfile")
    def test_ChangePermissions_DirsFalse_RMTreeNotCalled(
        self, mock_isfile, mock_isdir, mock_remove, mock_rmtree, mock_chper, mock_chmod
    ):
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("os.path.isfile")
    def test_ChangePermissions_NoDirs_RMTreeNotCalled(
        self, mock_isfile, mock_isdir, mock_remove, mock_rmtree, mock_chper
    ):
        mock_isdir.return_value = False
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("os.path.isfile")
    def test_ChangePermissions_FilesFalse_RemoveNotCalled(
        self, mock_isfile, mock_isdir, mock_remove, mock_rmtree, mock_chper
    ):
        file_system.delete(self.path_list, del_files=False, del_dirs=True)
    ...
    @mock.patch("os.path.isdir")
    @mock.patch("os.path.isfile")
    def test_ChangePermissions_NoFiles_RemoveNotCalled(
        self, mock_isfile, mock_isdir, mock_remove, mock_rmtree, mock_chper
    ):
        mock_isfile.return_value = False
        file_system.delete(self.path_list, del_files=True, del_dirs=False)
...
class TestRename(unittest.TestCase):
    ...
    def test_Rename_SourceDoesNotExist_ErrorReported(self, mock_rename):
        with self.assertLogs(
            "ly_test_tools.environment.file_system", "ERROR"
        ) as captured_logs:
...
class TestDeleteOldest(unittest.TestCase):
    ...
    def test_DeleteOldest_DefaultValuesKeepAll_DeleteCalledWithEmptyList(
        self, mock_delete, mock_ctime, mock_glob
    ):
        ...
        file_system.delete_oldest(
            "", len(self.path_list), del_files=True, del_dirs=False
        )
    ...
    @mock.patch("os.path.getctime")
    @mock.patch("ly_test_tools.environment.file_system.delete")
    def test_DeleteOldest_DefaultValuesKeepNone_DeleteCalledWithList(
        self, mock_delete, mock_ctime, mock_glob
    ):
        mock_glob.return_value = self.path_list
        mock_ctime.side_effect = self.age_list
    ...
    @mock.patch("os.path.getctime")
    @mock.patch("ly_test_tools.environment.file_system.delete")
    def test_DeleteOldest_DefaultValuesKeepOne_DeleteCalledWithoutNewest(
        self, mock_delete, mock_ctime, mock_glob
    ):
        mock_glob.return_value = self.path_list
        mock_ctime.side_effect = self.age_list
    ...
    @mock.patch("os.path.getctime")
    @mock.patch("ly_test_tools.environment.file_system.delete")
    def test_DeleteOldest_UnsortedListDeleteOldest_DeleteCalledWithOldest(
        self, mock_delete, mock_ctime, mock_glob
    ):
...
class TestMakeJunction(unittest.TestCase):
    ...
    def test_MakeJunction_Win32_SubprocessFails_RaiseSubprocessError(
        self, mock_isdir, mock_sub_call
    ):
        mock_isdir.return_value = True
    ...
    def test_MakeJunction_Darwin_SubprocessFails_RaiseSubprocessError(
        self, mock_isdir, mock_sub_call
    ):
        mock_isdir.return_value = True
        mock_sub_call.side_effect = subprocess.CalledProcessError(1, "cmd", "output")
...
class TestFileBackup(unittest.TestCase):
    def setUp(self):
        ...
        self._dummy_backup_file = os.path.join(
            self._dummy_dir, "{}.bak".format(self._dummy_file)
        )

    @mock.patch("shutil.copy2")
    @mock.patch("os.path.exists")
    @mock.patch("os.path.isdir")
    def test_BackupSettings_SourceExists_BackupCreated(
        self, mock_path_isdir, mock_backup_exists, mock_copy
    ):
    ...
    @mock.patch("os.path.exists")
    @mock.patch("os.path.isdir")
    def test_BackupSettings_BackupExists_WarningLogged(
        self, mock_path_isdir, mock_backup_exists, mock_copy, mock_logger_warning
    ):
        mock_path_isdir.return_value = True
    ...
    @mock.patch("os.path.exists")
    @mock.patch("os.path.isdir")
    def test_BackupSettings_SourceNotExists_WarningLogged(
        self, mock_path_isdir, mock_backup_exists, mock_copy, mock_logger_warning
    ):
        mock_path_isdir.return_value = True
    ...
    @mock.patch("os.path.exists")
    @mock.patch("os.path.isdir")
    def test_BackupSettings_CannotCopy_WarningLogged(
        self, mock_path_isdir, mock_backup_exists, mock_copy, mock_logger_warning
    ):
        mock_path_isdir.return_value = True
        mock_backup_exists.side_effect = [True, False]
    ...
    @mock.patch("os.path.exists")
    @mock.patch("os.path.isdir")
    def test_BackupSettings_InvalidDir_ErrorLogged(
        self, mock_path_isdir, mock_backup_exists, mock_logger_error
    ):
...
class TestFileBackupRestore(unittest.TestCase):
    ...
    @mock.patch("os.path.exists")
    @mock.patch("os.path.isdir")
    def test_RestoreSettings_BackupRestore_Success(
        self, mock_path_isdir, mock_exists, mock_copy
    ):
        mock_path_isdir.return_value = True
        mock_exists.return_value = True
    ...
    @mock.patch("os.path.exists")
    @mock.patch("os.path.isdir")
    def test_RestoreSettings_CannotCopy_WarningLogged(
        self, mock_path_isdir, mock_exists, mock_copy, mock_logger_warning
    ):
        mock_path_isdir.return_value = True
        mock_exists.return_value = True
        mock_copy.side_effect = Exception("some error")
    ...
    @mock.patch("os.path.exists")
    @mock.patch("os.path.isdir")
    def test_RestoreSettings_BackupNotExists_WarningLogged(
        self, mock_path_isdir, mock_exists, mock_copy, mock_logger_warning
    ):
        mock_path_isdir.return_value = True
    ...
    @mock.patch("ly_test_tools.environment.file_system.logger.error")
    @mock.patch("os.path.isdir")
    def test_RestoreSettings_InvalidDir_ErrorLogged(
        self, mock_path_isdir, mock_logger_error
    ):
        mock_path_isdir.return_value = False
        file_system.restore_backup(self._dummy_file, self._dummy_dir)
...
class TestReduceFileName(unittest.TestCase):
    def test_Reduce_LongString_ReturnsReducedString(self):
        target_name = (
            "really_long_string_that_needs_reduction"  # len(mock_file_name) == 39
        )
        max_length = 25
...
class TestFindAncestorFile(unittest.TestCase):
    @mock.patch(
        "os.path.exists", mock.MagicMock(side_effect=[False, False, True, True])
    )
